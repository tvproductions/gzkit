...
def step_init_project(context) -> None:  # type: ignore[no-untyped-def]
...
def step_vendor_manifest_lite(_context) -> None:  # type: ignore[no-untyped-def]
    """Already set up in step_init_project via _VENDOR_MANIFEST."""


@given('the corpus for "AGENTS.md" contains an invariant entry "{text}"')
def step_corpus_invariant(context, text: str) -> None:  # type: ignore[no-untyped-def]
...
def step_corpus_compressible(context, text: str) -> None:  # type: ignore[no-untyped-def]
...
def step_candidate_with_invariant_and_compressed(context) -> None:  # type: ignore[no-untyped-def]
...
def step_candidate_with_invariant_and_content(context) -> None:  # type: ignore[no-untyped-def]
    text = f"{context.invariant_text}\nsome content"
    context.candidate_file.write_text(text, encoding="utf-8")


@given("a candidate file containing the invariant entry text")
def step_candidate_with_invariant_text(context) -> None:  # type: ignore[no-untyped-def]
    text = f"{context.invariant_text}\nsome other content"
    context.candidate_file.write_text(text, encoding="utf-8")


@given("a candidate file containing the invariant entry text verbatim")
def step_candidate_with_invariant_verbatim(context) -> None:  # type: ignore[no-untyped-def]
    text = f"{context.invariant_text}\nsome other content"
    context.candidate_file.write_text(text, encoding="utf-8")


@given("a valid candidate file containing the invariant entry")
def step_valid_candidate(context) -> None:  # type: ignore[no-untyped-def]
    text = f"{context.invariant_text}\nsome compressed content"
    context.candidate_file.write_text(text, encoding="utf-8")


@given('no corpus store exists for "{surface}"')
def step_no_corpus(context, surface: str) -> None:  # type: ignore[no-untyped-def]
    context.missing_surface = surface


@given("a candidate file with some text")
def step_candidate_any(_context) -> None:  # type: ignore[no-untyped-def]
    Path("candidate.md").write_text("some text", encoding="utf-8")


@given('"{filename}" exists with some content')
def step_surface_exists(_context, filename: str) -> None:  # type: ignore[no-untyped-def]
    Path(filename).write_text(f"# {filename}\nsome content", encoding="utf-8")


@when('I run "gz content compose AGENTS.md --consumer codex --candidate <file>"')
def step_run_compose(context) -> None:  # type: ignore[no-untyped-def]
    code, output = _invoke(
        [
            "content",
            "compose",
...
def step_run_compose_twice(context) -> None:  # type: ignore[no-untyped-def]
    args = [
        "content",
        "compose",
        "AGENTS.md",
...
def step_run_compose_unknown_vendor(context) -> None:  # type: ignore[no-untyped-def]
    code, output = _invoke(
        [
            "content",
            "compose",
...
def step_run_compose_missing_surface(context) -> None:  # type: ignore[no-untyped-def]
...
def step_exits_0(context) -> None:  # type: ignore[no-untyped-def]
...
def step_candidate_exists(_context, path: str) -> None:  # type: ignore[no-untyped-def]
    assert Path(path).exists(), f"Expected candidate at {path!r} but it does not exist"


@then('the output includes "{text}"')
def step_output_includes(context, text: str) -> None:  # type: ignore[no-untyped-def]
...
def step_ledger_has_event(_context, event_type: str, surface: str) -> None:  # type: ignore[no-untyped-def]
...
def step_both_exit_0(context) -> None:  # type: ignore[no-untyped-def]
    assert context.exit_code == 0, f"Second run failed: {context.output}"


@then("the byte evidence output is identical between runs")
def step_byte_evidence_identical(context) -> None:  # type: ignore[no-untyped-def]
...
def step_candidate_contains_invariant(context, path: str) -> None:  # type: ignore[no-untyped-def]
...
def step_no_candidate(context, path: str) -> None:  # type: ignore[no-untyped-def]
...
def step_no_candidate_for_consumer(_context, consumer: str) -> None:  # type: ignore[no-untyped-def]
...
def step_file_unchanged(context, filename: str) -> None:  # type: ignore[no-untyped-def]
    expected = f"# {filename}\nsome content"
