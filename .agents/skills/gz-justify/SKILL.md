...
| You are filling blocks before reading the anchor body the CLI embedded in Section 1. | Section 1 pre-populates anchor-body citations specifically so your reasoning can ground in them. Skipping the read step is the defining move of vibe coding (see `.gzkit/rules/agent-contract.md` § Craftsmanship — DO IT RIGHT, 6b and 6e). |
