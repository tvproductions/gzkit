...
Operate the `gz obpi brief-drift` command surface as a reusable governance
...
3. If drift is real and the amendments are correct, preview with
   `uv run gz obpi brief-drift <OBPI-ID> --apply --attestor "<name>" --dry-run`.
4. Apply with `uv run gz obpi brief-drift <OBPI-ID> --apply --attestor "<name>"`.
