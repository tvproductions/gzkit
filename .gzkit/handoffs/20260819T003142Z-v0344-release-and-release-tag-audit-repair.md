...
adr_id: null
branch: main
timestamp: '2026-08-19T00:31:42Z'
agent: g0
session_id: 10d761ce-7fb4-48e9-b7c4-0038b37483dc
continues_from: .gzkit/handoffs/20260818T084542Z-three-ghis-closed-mis-aimed-instruments.md
---

## Current State Summary

Two distinct bodies of work landed, both complete and pushed. Branch main is level with its remote (git rev-list --left-right --count returns 0 0), tree clean at 3f6e56e3f.

**1. Patch release v0.34.4 shipped.** 21 GHIs are release content. The GitHub latest-release API returns v0.34.4. Version synced across `pyproject.toml`, `src/gzkit/__init__.py`, and the README badge; `RELEASE_NOTES.md` carries the curated narrative and `CHANGELOG.md` the 22-entry exhaustive projection (gz validate --changelog exit 0). Six discovery buckets were adjudicated per the patch-release skill's Steps 1a/1b/1c, and all six adjudications are recorded in the release notes' Gate Evidence section rather than left to memory.

**2. A three-deep repair chain on the release-tag audit.** It began as an incidental rejected-tag line during the release fetch and measured out into a real class. GHI #828 and GHI #829 are both CLOSED with commit SHAs; GHI #830 is OPEN and deliberately unfixed.

## Important Context

**The 2026-04-19 rewrite is the load-bearing fact for anything touching tags.** 33 of 68 documented releases have tags that are NOT reachable from the remote main branch. The boundary is exact — v0.25.11 (2026-04-18 20:12) unreachable, v0.25.12 (2026-04-19 06:37) reachable — and it is the filter-repo rewrite and force-push named in `AGENTS.md` under Local Agent Rules. The tags survived the force-push; their commits did not. Any future check over historical tags MUST account for this or it reports 33 legitimate releases as defects on its first run. The set is carried in `data/release_tag_reachability_grandfather.json`, shrink-only and registered in the waiver ratchet.

**audit_version_release had three independent blind spots, not one.** They compounded: (a) it read only local tags, so an unpushed tag satisfied it; (b) it consulted the in-flight release manifest BEFORE the tag lookup, and manifests are committed and never removed — 35 sit in `docs/releases/` — so the escape was permanent and the tag check was dead code for every version gz patch release ever shipped; (c) it checked only the current pyproject version, never sweeping. Blind spots (a) and (b) closed under GHI #828, (c) under GHI #829.

**GHI #217 explains where the bad tags came from.** It records the operator workaround the chicken-and-egg deadlock forced: creating a local vX.Y.Z tag on the PRIOR commit so the audit sees some tag. That dance manufactures an out-of-band local tag satisfying a local-only audit whether or not it ever ships. Corroborated by the tagger field: of 71 tags only 3 were annotated, and 2 of those 3 carried a stand-in Test User identity rather than a real one.

**The sweep landed under GHI #829 is one-directional by construction.** It reads `RELEASE_NOTES.md` headings as the roster and checks tags against it, so a release absent from the roster is absent from the loop. That is exactly the gap GHI #830 now tracks. Its non-vacuity figure reports 68 swept, which is the wrong denominator if the roster itself is incomplete.

**Network stays out of this audit.** GHI #205 scoped network access out because it runs at pre-commit time. Both arms built this session honor that: reachability is judged against the local remote-tracking ref, and absence of that ref is treated as absence of evidence rather than as a defect, so fresh clones and CI sandboxes stay green.

## Decisions Made

- [operator-ruled] Backfill the runtime label on GHI #806 so it qualifies for the release. Its remedy shape is fix(skills) and its diff lands under `src/gzkit/skills/`, which is wheel-distributed content gz init delivers, so the defect was operator-visible.
- [operator-ruled] Approve the v0.34.4 release notes as drafted (verbatim: "approve as-is, proceed"), triggering the skill's Iron Law run through Steps 4a-4e without pauses.
- [operator-ruled] Close the GHI #828 class with the reachability arm and NO network (verbatim selection: "Reachability arm, no network"), over the remote-consulting arm and over a combined arm carrying a historical sweep.
- [operator-ruled] File AND fix the GHI #829 residual in the same session (verbatim: "file and fix"), rather than filing it for later.
- [operator-ruled] File GHI #830 and stop there (verbatim: "file, commit to handoff") — the backfill archaeology on three historical releases does not happen tonight.
- [agent-chose] Reordered audit_version_release to check the tag FIRST and consult the in-flight manifest only when no tag exists. Without this the new reachability arm would have been dead code, because the permanent manifest short-circuited every released version.
- [agent-chose] Used `RELEASE_NOTES.md` headings as the release roster rather than `docs/releases/` manifests. Only 35 manifests exist because they postdate the ceremony that writes them; the notes carry all 68.
- [agent-chose] Carried the 33 rewrite-orphaned releases in a disclosed shrink-only grandfather file matching the 15 sibling surfaces under `data/`, rather than hardcoding a 2026-04-19 date cutoff inside the predicate.
- [agent-chose] Scoped the grandfather exemption to REACHABILITY only, so a deleted historical tag is still refused. Aimed the negative control at a grandfathered version specifically, because that was the assertion most likely to be silently wrong.
- [agent-chose] Registered the new grandfather file in `data/waiver_ratchet_registry.json` as a shrink-ratchet surface with a measured baseline of 33, rather than listing it under the excluded set.
- [agent-chose] Kept the GHI #829 sweep one-directional and filed the inverse direction as GHI #830 rather than widening scope mid-fix.

## Immediate Next Steps

1. Decide the GHI #830 predicate before writing any code for it. Whether a tag has a published GitHub release is not a local fact, and GHI #205 [settled] keeps network out of this audit at pre-commit time; the local proxy "every tag owes a RELEASE_NOTES entry" would fire on any legitimately unreleased tag. This ruling is the blocker, not the implementation.
2. If GHI #830 proceeds, do the backfill BEFORE the witness. Adding the witness first fails the tree closed on the three known-undocumented releases. v0.25.1 transcribes verbatim from its own GitHub release body (which carries 25 version sections, the GHI #710 [settled] defect); v0.18.1 and v0.24.1 have only 84-byte auto-generated stubs and need their commit ranges read.
3. Consider whether v0.7.0 should stay reachable while its neighbours do not. It was retagged this session at 61f04d138, the surviving post-rewrite commit with an identical tree, which makes it the only pre-rewrite tag pointing into live history. This is strictly more correct than its neighbours but inconsistent with them, and no ruling has been taken on whether the other 33 should be repointed the same way.
4. Nothing else is owed. Every item the operator asked for this session is landed, pushed, and either closed or deliberately left open.

## Pending Work / Open Loops

- **GHI #830 is OPEN and deliberately unfixed** — three published releases (v0.18.1, v0.24.1, v0.25.1) documented nowhere in the repo. Verified OPEN at handoff time. Two halves: a mechanical inverse witness and a judgment-heavy backfill. See Immediate Next Steps for the ordering constraint.
- **GHI #815 and GHI #810 remain open by adjudication, not oversight.** Both had work land in the v0.34.4 range and both are described under Known issues in the release notes rather than counted in Stats, per the patch-release skill's Step 1b Case 2. Do not close either merely to tidy a discovery bucket.
- **The three tags with no RELEASE_NOTES entry are the whole of GHI #830's population** as measured today. If the sweep's denominator of 68 moves, re-measure before assuming the population is still three.

## Verification Checklist

```bash
# Release landed and is current
gh api repos/tvproductions/gzkit/releases/latest --jq '.tag_name'      # expect v0.34.4
...
# Tag surface converged (this is what started the whole chain)
git fetch --tags --prune origin                                        # expect NO rejected-tag line

# The GHI #829 sweep is non-vacuous, not merely clean
uv run python -c "
from pathlib import Path
from gzkit.governance.trust_audits import release as R
print('swept', len(R._documented_versions(Path('.'))))
print('grandfathered', len(R._reachability_grandfather(Path('.'))))
"                                                                      # expect 68 and 33
...
Claim-verification note for the resuming agent: gz obpi status and gz state are NOT relevant here — no OBPI or ADR was touched this session. Every claim above resolves against GitHub, git refs, or a gz validate scope.

## Evidence / Artifacts

Commits, all pushed to the remote main branch:

- 452867817 — release: v0.34.4, enforcement's missing exemption half
- a02c9d8d6 — fix(release-audit): judge a tag by reachability, not by local presence (GHI #828)
- 3f6e56e3f — fix(release-audit): sweep every documented release, not just the current one (GHI #829)

Files:

- `RELEASE_NOTES.md` — v0.34.4 curated narrative with the six-bucket Gate Evidence adjudication record
- `CHANGELOG.md` — v0.34.4 exhaustive block, 22 entries
- `docs/releases/PATCH-v0.34.4.md` — release manifest
- `src/gzkit/governance/trust_audits/release.py` — reachability arm, tag-first ordering, documented-release sweep
- `tests/governance/test_promoted_advisory_audits.py` — 10 new tests across two classes
- `data/release_tag_reachability_grandfather.json` — the 33 rewrite-orphaned releases, shrink-only
- `data/waiver_ratchet_registry.json` — registers the above as a shrink-ratchet surface

ARB receipts, canonical invocations, all exit_status=0:

- arb-ruff-d17d4dcd510844dd8f04d778ab727b7a
- arb-step-typecheck-8188f24d41f0400ba74bb87d3580035c
- arb-step-unittest-ff8bf27d15ce4e6181f7865479ac95f9 (8493 tests)
