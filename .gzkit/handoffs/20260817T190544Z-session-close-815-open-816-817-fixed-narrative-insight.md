...
adr_id: null
branch: main
timestamp: '2026-08-17T19:05:44Z'
agent: claude-code
session_id: 1fc95331-a12d-4cbb-a6b6-f4fe450bf27e
continues_from: .gzkit/handoffs/20260817T184605Z-ghi-815-reopened-817-chore-corpus-routing-fixed.md
---

## Current State Summary

Session close. Supersedes `20260817T184605Z-ghi-815-reopened-817-chore-corpus-routing-fixed.md`, adding one delta: an `improvement` insight booked under scope `agent-narrative-discipline` after the operator had to ask whether a successful GHI filing had failed.

Resumed `20260817T130147Z`, booked `proceed` with advised step 2 only. Six operator rulings carried the session well past that scope.

Seven commits, all synced, `origin/main` `0 0`, tree clean:

- `b20b210e4` — GHI #815 re-prioritisation recorded into `ADR-pool.render-order-truncation-survival` on a live re-measurement
- `01daaf8ae` — the two consumers citing the then-closed #815 as their live record, repaired
- `94997208f` — GHI #816 filed and direct-fixed: `gz git-sync` no longer discards a refusing hook's diagnosis
- `8e378b6ed` — GHI #817 filed and direct-fixed: `instructions-files-diet` routed through the corpus, chore `1.0.0` → `2.0.0`
- `3791b1e7d` / `2bca45fcc` / `1b811d531` — predecessor handoffs

Issue state at authoring: **#815 OPEN** (reopened), **#816 closed fixed**, **#817 closed fixed**.

No ADR or OBPI advanced. The campaign spine has been set aside for two consecutive sessions.

## Important Context

**A fallback-path success reported as a footnote reads as a failure.** GHI #817 was filed and closed, but `gh issue create` had hit a GraphQL 503 and the work went through the REST endpoint instead. The session report stated the filing correctly and then explained the fallback in a trailing operational paragraph — so the operator asked whether the filing had failed at all. Booked as an `improvement` insight: when a step succeeds via a fallback, the outcome and the fallback belong in **one sentence at the point of the claim**, never as an aside after the summary. The same shape would misreport any retried, degraded, or rerouted success.

**#815 was REOPENED, not refiled, and the distinction is a skill rule.** `ghi-author` § Step 0 reopens a ≤30-day close of the same root cause rather than duplicating it into a fresh number; refiling would have split the measurement across two issues. The `superseded` routing to the pool ADR STANDS — the reopen restored a tracker, not a reversal of the ranking.

**A correction that changes the ranking's argument, though not its outcome.** Three surfaces this session said *"shrinking buys headroom; it does not buy delivery, because a smaller surface in the same order still puts rank 11 last."* That holds ONLY while the surface remains over the cap. Compress `AGENTS.md` below 32768 B and every section renders inside it, `architectural-boundaries` included — so shrink IS a delivery fix. The honest axis is **durability, not capability**: reorder is growth-invariant, shrink restores delivery today and decays on the next corpus growth. Reorder stays ranked first because its fix does not decay. Corrected in the pool ADR body; appended as a dated entry to `data/instructions_files_budget.json` rather than edited, per that file's own reading convention.

**The compression work is chore-shaped, and "these are rendered" relocates it rather than blocking it.** The corpus ceremony already carries the right seam: `gz content compose` writes a *candidate*, `gz content commit` promotes it *under attestation*. `instructions-files-diet` v2.0.0 does the full compression labor and **stops at the Gate 5 boundary**, handing the operator a candidate. A chore that ran `gz content commit` itself would land Layer-1 canon unattested — a worse defect than the one it fixes, since Gate 5 is universal.

**The chore contradicted itself, which is worse than being uniformly wrong.** Its § 5 and § Anti-patterns already said never to edit `.claude/rules/**` mirrors, while § 3's Lift table named a mirror as the origin to edit. Two-of-three agreement means a reader who checks one section gets the right answer and a reader who follows the procedure gets the wrong one. Only the `AGENTS.md`/corpus arm was a genuine blind spot — zero mentions of `corpus`, `rendition`, `gz content`, `compose`, or `Gate 5` anywhere in the file.

**The chore ships in the wheel.** Its `registry.json` entry carries no `"projectLocal": true`, so it is canonical class, present at `src/gzkit/chores/`, packaged, and scaffolded into every adopting project by `gz init`. Check the registry class before scoping a chore defect's blast radius.

**GHI #816's defect was one underscore, twice.** `sync.py` bound `git_cmd`'s stdout to `_out_push` and `_out_commit` — Python's convention for *deliberately unused* — and appended only stderr to `blockers`. Hook frameworks write their report to stdout. An underscore prefix is a claim about intent that no checker verifies.

**Nothing global could have caught #816, by design.** `.gzkit/rules/guardrail-feedback-prose.md` § Enforcement posture rejects a global prose-grader by name as *"grading by shape"* and enforces the three-part bar per-surface, in each surface's own covering test. When adding a fail-closed surface or a wrapper around one, the covering test is where the bar lives — there is no scope flag to look for.

## Decisions Made

- [operator-ruled] Author a fresh handoff and sync (verbatim: "write fresh handoff, hit sync"). Given minutes after the predecessor landed; the delta captured is the narrative-discipline insight below, booked in the same turn.
- [operator-ruled] Track the Codex cap with a GHI AND repair the diet chore, as two findings in two vehicles (verbatim selection: "GHI the cap + repair the chore (Recommended)"), chosen from a four-option picker whose alternatives were repair-the-chore-only, GHI-the-cap-only, and repair-then-run-the-compression-this-session.
- [operator-ruled] The cap belongs in a GHI (verbatim: "I wanted the codex cap to be GHI'd"), reversing the same day's decision to decline a successor tracker at the #815 close.
- [operator-ruled] Consider a chore as the vehicle for the compression work, with the rendering problem named as the obstacle (verbatim: "perhaps a chore can do the work of compressing. the problem is that these are rendered."). Both halves proved correct: the chore is the right vehicle and the rendering constraint relocates the work to the corpus rather than blocking it.
- [agent-chose] Booked an `improvement` insight under scope `agent-narrative-discipline` after the operator asked whether the #817 filing had failed. The filing had succeeded via REST after a GraphQL 503; the report stated the outcome correctly but explained the fallback only in a trailing paragraph.
- [agent-chose] Reopened #815 rather than filing a successor GHI, per `ghi-author` § Step 0's ≤30-day same-root-cause rule.
- [agent-chose] Scoped `instructions-files-diet` v2.0.0 to STOP at `gz content compose`, never running `gz content commit`, so Gate 5 stays universal.
- [agent-chose] Corrected the shrink/reorder overstatement in the pool ADR body but APPENDED the correction to `data/instructions_files_budget.json` rather than editing its prior entry.
- [agent-chose] Audited only `instructions-files-diet` and declared its siblings explicitly unaudited in #817's close comment, rather than claiming a class sweep the session did not perform.
- [agent-chose] Filed and closed #817 through the REST endpoint when GraphQL was 503ing, after Step 0's prior-art sweep completed. The `/ghi-author` prohibition protects Step 0, not a particular HTTP path.

## Immediate Next Steps

1. **Sweep the remaining chores for the derived-surface shape.** GHI #817 [settled] fixed `instructions-files-diet` and explicitly did NOT audit its siblings in `.gzkit/chores/registry.json`. Any chore naming `AGENTS.md`, `CLAUDE.md`, `.claude/**`, `.github/**`, or `src/gzkit/{rules,skills,personas,templates}/` as an edit target carries the identical defect, and no mechanical check exists — nothing couples a chore's procedure to the layer model of the surfaces it names. Cheap first pass: grep every `CHORE.md` for those paths and read the hits.
2. **Resume the campaign spine.** `ADR-0.35.0-canon-entry-corpus-landing` is the reachable topmost item, set aside two consecutive sessions. Read its lifecycle and OBPI figures from `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing`; this line deliberately transcribes no count. Its § Decision 3 is the destination for BOTH the budget-stay exit condition and GHI #533's registry-projection migration.
3. **Consider running the repaired chore to close GHI #815.** `instructions-files-diet` v2.0.0 is now safe to run: it authors against the corpus and hands back a candidate for Gate 5 attestation. Bringing `AGENTS.md` under the Codex cap would restore delivery of `architectural-boundaries` immediately, though it decays on the next corpus growth — the durable fix remains the reorder half.
4. **Consider declaring `CLAUDE.md` in the survival declaration.** `data/agents_md_survival_declaration.json` declares only `AGENTS.md`, so any other control surface crossing a vendor cap is silently unmeasured by design. Not live, but it is the one empty cell in #815's cause-to-witness table.
...
- **GHI #815 is OPEN and its breach is live.** `AGENTS.md` renders 34354 B against the 32768 B Codex cap; `architectural-boundaries` begins at byte 33759, wholly past the cap. By the witness's own doctrine a declared-unrecoverable section that is not delivered is not in force. Two remedies now exist and both are usable: the reorder half (`ADR-pool.render-order-truncation-survival`, durable, unpromoted) and the shrink half (`instructions-files-diet` v2.0.0, immediate, decays).
- **The other chores in `registry.json` are UNAUDITED for the #817 [settled] shape.** Declared explicitly in that issue's close comment rather than left implicit. No mechanical check exists for a chore whose procedure names a derived surface.
- **`git pull` still discards stdout** at `src/gzkit/commands/sync.py`, so merge-conflict file lists do not reach the blocker. Deliberately out of GHI #816 [settled]'s scope — no hook fires there — and untracked by any GHI.
- **GHI #533 remains open and its body's blocker is stale.** It names `ADR-0.0.37` as still mid-implementation; that ADR reached a terminal `Validated` lifecycle and the dependency relocated to `ADR-0.35.0` § Decision 3. Read both from `uv run gz adr status <ADR-ID>`. Corrected in a routing comment; the issue body was deliberately not rewritten.
- **The pool ADR's deferral trigger has fired while its parking stands.** "Pays off only once the cap binds" is spent. The parking now rests solely on "only one feature at a time" while `ADR-0.35.0` is in flight; the ADR carries an explicit fence against reading the expired trigger as an implicit promotion.
- **The advisory scorecard still carries one fewer Mechanical row.** Row 17b moved Mechanical to Judgment under the budget stay and reclassifies back only when the stay lifts.
- **`ADR-0.36.0` and `ADR-0.37.0` remain Pending** behind `ADR-0.35.0` in absolute ADR order. Movement B stays TOPMOST while its carrier `ADR-0.37.0` is third of three, so the campaign's topmost item is structurally unreachable for two full ADRs. Intended, not a defect.
- **`OBPI-0.33.0-06` attestation question stays deliberately undecided**: it reached `attested_completed` citing a test that does not exist.
...
uv run -m unittest tests.commands.test_sync_hook_diagnosis tests.governance.test_surface_delivery_witness tests.governance.test_audit_instructions_files_budget
git rev-list --left-right --count origin/main...HEAD
```

Expected, each observed at authoring rather than assumed: bare `validate` reports 13 scopes passed; `--instructions-files-budget` exits 0 while printing two `[advisory] WARNING` lines about the Codex cap (exit 0 WITH warnings is the ruled posture — a non-zero exit there is a regression of the budget stay); `--invariant-coherence` and `--chores-layout` exit 0; the chore renders and exits 0; the three test modules run 31 tests and exit 0; the branch count is `0 0`.

Three witnesses worth reading rather than only running. The delivery advisory must contain "If no OPEN GHI" and must NOT name any issue as the live record. The chore's `acceptance.json` must list `uv run gz validate --invariant-coherence` FIRST — if that criterion disappears, the chore is blind again to the exact drift its procedure could cause. And `tests/commands/test_sync_hook_diagnosis.py` carries three negative controls that passed at RED; if a future change makes all six pass trivially, the controls have gone vacuous.

Piping a verifier is refused by the `verifier-pipe-gate` hook; use `set -o pipefail` or redirect to a file, and read the real exit code.

## Evidence / Artifacts

- `.gzkit/insights/agent-insights.jsonl` — two records this session: a `defect` under scope `handoff-authoring`, and an `improvement` under scope `agent-narrative-discipline`
- `.gzkit/chores/instructions-files-diet/CHORE.md` — v2.0.0; § 3 Lift table gains an "edit this source instead" column, new § 3a routes the `AGENTS.md` arm through the corpus and stops at Gate 5, four new anti-patterns
- `.gzkit/chores/instructions-files-diet/acceptance.json` — `--invariant-coherence` added and leading; five criteria to six
- `src/gzkit/chores/instructions-files-diet/CHORE.md` — wheel copy, synced; the chore is canonical class and ships to adopters
- `src/gzkit/chores/instructions-files-diet/acceptance.json` — wheel copy, byte-identical to canonical after sync
- `docs/design/adr/pool/ADR-pool.render-order-truncation-survival.md` — two dated measurement records with authorities cited; the shrink/reorder correction recorded in the body rather than silently patched
- `data/instructions_files_budget.json` — two appended dated entries this session; budget values unchanged throughout
- `src/gzkit/commands/sync.py` — `_hook_failure_detail`; both hook-bearing call sites keep the refusing hook's stdout
- `tests/commands/test_sync_hook_diagnosis.py` — 6 tests, three of them negative controls that passed at RED
...
- `.gzkit/rules/guardrail-feedback-prose.md` — the invariant GHI #816 violated, and the § Enforcement posture explaining why no global check caught it
- `.gzkit/rules/skill-surface-sync.md` — § Non-negotiable rules #4, the mirror prohibition the chore's § 3 table contradicted
- `.gzkit/handoffs/20260817T184605Z-ghi-815-reopened-817-chore-corpus-routing-fixed.md` — the predecessor this supersedes
