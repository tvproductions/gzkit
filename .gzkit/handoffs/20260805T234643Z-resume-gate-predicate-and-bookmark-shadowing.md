...
adr_id: ADR-0.0.65
branch: main
timestamp: '2026-08-05T23:46:43Z'
agent: claude-code
session_id: 3aa0d86f-af4c-4de0-af4c-f750c8441896
continues_from: .gzkit/handoffs/20260805T212218Z-checkpoint-mode-and-exit-beat.md
---

## Current State Summary

Four commits landed and pushed; tree clean, origin/main in sync (0 0). Started from a triage request and ended in the handoff-lifecycle subsystem, because triage surfaced that the session's own orientation input was corrupt.

GHI #732 (resume-gate read allowlist) closed fixed at 484a61a6d. GHI #758 (floor bookmarks shadowing authored handoffs) filed and closed fixed at 650e302cf plus 0dc811fbd. GHI #759 (lock-handoff-coupling proves disk presence, not repository durability) filed and left OPEN with a blocker naming two operator design points. Insight recorded at 498e8e7fd.

GHI triage ran over all 11 open issues and produced a rank-ordered deliverable; the open queue is still 11 (#732 closed, #758 filed-and-closed, #759 added).

## Important Context

The through-line: three independent newest-first readers exist over .gzkit/handoffs/, and each was fixed separately. find_handoff_for_release (release arm, fixed under GHI #756), newest_handoff (resume gate, fixed here at 650e302cf), and scripts/session_orientation.py collect_handoff (orientation, fixed here at 0dc811fbd). Nothing couples them. A fourth reader would repeat the class; the note lives on GHI #758 so the next author finds it rather than rediscovering it.

Two premises were disproved by probing rather than reasoning, and both would have produced wrong fixes:

1. GHI #732's suggested shape assumed a verb's write surface is reachable only through flags. It is not. 'find . -fprint FILE' and 'sed -n 1,2w FILE' both write, neither through a guarded flag. Adopting the suggestion would have widened a live hole instead of closing it.

2. Filtering the resume arm on frontmatter mode looks like the obvious fix for shadowing and is wrong. A machine floor bookmark and an operator-authored mid-flight checkpoint are BOTH mode CHECKPOINT. The discriminator had to be authorship (agent), single-sourced as gzkit.session_exit.FLOOR_BOOKMARK_AGENT.

Third constraint discovered while implementing: in newest_handoff, deprioritizing a document class and skipping one are not interchangeable. This selector's absence of a result disables the gate entirely, so a skip fails OPEN. The malformed-frontmatter branch still returns its candidate on purpose.

The exit beat structurally cannot commit its own output because SessionEnd fires after the session's last chance to. That is why the operator ruled sensemaking belongs at SessionStart rather than making the hook commit.

## Decisions Made

- [operator-ruled] The bookmark is durable evidence and the hook should land it, not merely write it (verbatim: 'the bookmark is durable evidence — the hook should commit it').
- [operator-ruled] SessionStart owns sensemaking; committing at the exit beat is too much friction, and the operator will normally author a handoff and git-sync manually on the way out (verbatim: 'i think the design is something for session start to pick up for planning since committing is going to be too much friction. this session end is a precuation').
- [operator-ruled] SessionStart must look for bookmarks, offer sensemaking, and flag them for inclusion going forward (verbatim: 'make sure we look for bookmarks as a part of sessionstart - offer the do sensemaking and flag for inclusion moving forward').
- [operator-ruled] Fix the shadowing as a direct fix under a new GHI and file the validator durability gap separately (verbatim: 'yes, ghi it').
- [operator-ruled] Proceed on the resumed handoff at session start (verbatim: 'review handoff, run ghi triage, review'); booked via gz handoff decide, advised step 2 set aside.
- [agent-chose] Removed find from the resume-gate allowlist rather than adding sed under a weaker predicate, after probing showed both verbs write outside the flag guard. This diverges from GHI #732's stated suggested shape; the divergence and its evidence are recorded in the close comment.
- [agent-chose] Filed GHI #758 as a new issue rather than reopening #756, on #756's own exit-edge/entry-edge boundary, after verifying 6e81b85ed does not touch newest_handoff. Cross-link comments posted on both #756 and #757 at authoring time.
- [agent-chose] Left the gz gates allowlist entry in place and moved its rationale out of SKILL.md into the gate module comment, because a skill naming a retired verb fails gz check on the Invariant-1 inversion.
- [agent-chose] Scoped the new orientation section to bookmarks newer than the last authored handoff, and made it render nothing when empty, so it does not become noise that trains the reader to skip it.
- [agent-chose] Corrected the GHI #758 close comment after discovering the third selector, rather than leaving an inaccurate claim that two selectors were correct.

## Immediate Next Steps

1. Rule on GHI #759 design point (a): does staged count as durable, or only committed? This also settles the exit-beat design, because if staged counts the beat can git add and be done.
2. Rule on GHI #759 design point (b): may a trust-audit validator shell to git, or must the tracked-set be injected behind a port per hexagonal-architecture rule 1?
3. Confirm the new orientation section reads correctly at the next session start, when it will list this session's own exit bookmark.
4. Consider whether three independent handoff-corpus readers now justify a shared selection helper; the evidence note is on GHI #758.
5. Re-run gz ghi-triage if planning the next pull; the ranked list from this session is at .gzkit/cache/triage/rank.json and names #581 and #611 as the highest-merit open items.

## Pending Work / Open Loops

- GHI #759 is OPEN with a blocker comment; it is not discharged by 650e302cf, which repaired one dangling reference but not the validator.
- Triage surfaced three open GHIs whose stated preconditions have since cleared and which nobody has re-derived: #581 (its gate #519 is settled), #533 (ADR-0.0.37 is now Validated 15/15), #594 (sibling #585 settled and gz handoff archive now ships). Each is running on a stale premise.
- GHI #670's Open reproducibility gap is stale: codex:rescue is now available in-session as both a skill and an agent type.
- ARB receipts have grown 1875 to 3272 since GHI #594 was filed, a 75 percent increase in two months.
- No coupling exists between the three handoff-corpus selectors; a fourth would repeat the class.
- The campaign's topmost item is untouched by this session: ADR-0.35.0-canon-entry-corpus-landing, 0 of 9 OBPIs.

## Verification Checklist

- git status --short is empty and git rev-list --left-right --count origin/main...main returns 0 0.
- uv run gz check exits 0.
- uv run python -c to call newest_handoff and collect_handoff both select the authored handoff, not a floor bookmark.
- uv run python scripts/session_orientation.py exits 0 and emits the Session-exit bookmarks awaiting sensemaking section.
- gh issue view 732 and gh issue view 758 both report CLOSED; gh issue view 759 reports OPEN.
- uv run -m unittest tests.governance.test_handoff_resume_gate tests.scripts.test_session_orientation exits 0.

## Evidence / Artifacts

- `src/gzkit/handoff_resume_gate.py` — allowlist predicate and the newest_handoff rank rule.
- `src/gzkit/session_exit.py` — FLOOR_BOOKMARK_AGENT, the single-sourced discriminator.
- `scripts/session_orientation.py` — orientation rank fix and the sensemaking section.
- `tests/governance/test_handoff_resume_gate.py` — predicate and shadowing tests.
- `tests/scripts/test_session_orientation.py` — orientation ranking and sensemaking-section tests.
- `.gzkit/skills/gz-session-handoff/SKILL.md` — Claim Verification Gate predicate prose, skill-version 6.25.0.
- `.gzkit/insights/agent-insights.jsonl` — flag-guard-as-membership-predicate discovery.
- `.gzkit/cache/triage/rank.json` — the ranked triage input for this session.
- ARB receipts: arb-step-unittest-3e7841b4e4154da984123fbfb79c0f74, arb-step-unittest-d1154bb98acc40cd945fc3e851af9578, arb-step-unittest-4b60c6ab900149e788aa76b53563b5ce.
- Commits: 484a61a6d, 498e8e7fd, 650e302cf, 0dc811fbd.
