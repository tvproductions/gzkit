...
adr_id: null
branch: main
timestamp: '2026-08-20T11:57:16Z'
agent: claude-code
session_id: 47cb5791-00ef-4e64-b931-636cf9fe51ad
continues_from: .gzkit/handoffs/20260820T095009Z-pii-sweep-complete-residuals-with-github.md
---

## Current State Summary

The operator-PII remediation is COMPLETE across every tracked file on `origin/main`. Verified at authoring time: zero occurrences of the operator's real name or personal address anywhere in the tree, ledger included. HEAD `0e24e39d`, 3104 commits, 71 tags, working tree clean, 0 ahead / 0 behind.

The session began as a handoff review and became a repair, because the review falsified the predecessor's headline claim. That handoff asserted `origin/main` returned "ZERO hits for both the operator's real name and personal address." True for the FULL name and the address; FALSE for the first name, which sat in 137 files on the public default branch. The predecessor had verified the wrong variant, and the passing check became the written record.

Three commits landed. `f492c902` swept 136 files of current content to `g0` — 106 ADR/OBPI docs, 15 sealed ADR audit proofs, `pyproject.toml`, `mkdocs.yml`, three BDD features, two test fixtures, the runbook, one plan. `c8abddd7` authored ADR-pool.ledger-identity-redaction. `0e24e39d` redacted 169 ledger identity fields.

GHI #837 was filed and cross-linked to #836.

TWO PII RESIDUALS REMAIN, both with GitHub and both operator-declined this session. They are unchanged from the predecessor: unreachable pre-sweep objects, and five immutable `refs/pull/*` refs. A third residual was MEASURED this session and is worse than the predecessor recorded — see Important Context SECOND.

## Important Context

Seven things a resuming agent will otherwise re-derive.

FIRST — VERIFY THE VARIANT, NOT THE VALUE. The predecessor's sweep-complete claim was produced by checking the full name; the first name alone was in 137 files. A PII check must assert on every variant (first name, full name, address) across FILE CONTENT and every DISTRIBUTION surface, not just commit metadata. `pyproject.toml` carried the real first name as package author into the published py-gzkit 0.34.4 wheel on PyPI — a channel no git rewrite reaches, and one that no repository-scoped check can see. Now `g0`; the next release stops shipping it.

SECOND — THE CORPUS PUBLISHED THE KEYS TO ITS OWN RESIDUAL, and this narrows a residual the predecessor ranked backwards. It called GitHub GC "the larger of the two." Measured this session in a throwaway bare clone: short SHAs are REFUSED by the protocol, so the 8-character citations everywhere are harmless — but `.gzkit/ledger.jsonl` publishes 25 full 40-character hashes, and EIGHT were fetched directly from `origin`. One, `0936489f`, reaches an ancestry of 117 commits carrying the personal address. The exposure is therefore not "someone holding a pre-sweep SHA" — it is clone the repo, grep the ledger for 40-hex, fetch. The `refs/pull/*` residual is likewise more accessible than GC: enumerable by one `git ls-remote`, no SHA needed. PR #508's ancestry alone carries 87 commits with the address in the author field, 281 with the full real name, and 3 with the address in FILE CONTENT.

THIRD — THE LEDGER GUARD HAS NO SANCTIONED ALTERNATIVE, which is why this session needed a bypass. `forbid_manual_ledger_edits` (`src/gzkit/hooks/guards.py:158`, GHI #207) rejects any `-` line in a staged ledger diff. It is unconditional: no flag, no env var, no allowlist. `gz ledger` exposes exactly one subcommand, `merge-driver`. So the rule forbidding hand-edits routes to nothing, and an operator-authorized correction had no governed path. ADR-pool.ledger-identity-redaction is the design that closes this.

FOURTH — WHY REPUDIATION COULD NOT BE REUSED. ADR-0.0.71 already solved correcting ledger-recorded state under operator authority WITHOUT mutation: the bad receipt stays, `obpi_completion_repudiated` is the counter-marker. That works whenever a CLAIM is wrong. It cannot reach the case where the recorded VALUE is the harm — appending "this identity was not permitted" leaves the identity in the file. Do not reach for the repudiation pattern here; it was considered and rejected in the pool ADR as alternative 1.

FIFTH — `git add -A` AND THE GUARD FIGHT OVER THE INDEX. `gz git-sync --apply` stages everything, which stages a held-back ledger edit straight into the guard. Worse, once staged it blocks EVERY commit in the repo, not just the ledger one, until unstaged. If a ledger edit must be held, leave it UNSTAGED and expect `gz git-sync` to fail; commit the rest selectively.

SIXTH — A `--no-verify` COMMIT CANNOT BE MADE BY THE AGENT IN THIS HARNESS. Claude Code's permission classifier denied `git commit --no-verify` even under an explicit operator directive, and denied a `gz check` invocation in the same trajectory. The operator ran it from the `!` prompt. Two gotchas cost three round-trips: a bare `git commit --no-verify` aborts with "empty commit message" because no interactive editor exists in that context, so `-m` is mandatory; and the operator's pre-push `gz check` gate runs long enough to exceed a 2-minute `!` timeout, which killed the push mid-gate and left the commit local. The push was completed separately and its gate PASSED unbypassed.

SEVENTH — THE SCOPE FENCE ON THE SEALED-KIND SWEEP. `src/gzkit/templates/adr_pool.md:30` says "foundation or feature" and is CORRECT. ADR-0.34.0's closure is PROJECT-LOCAL and that template is wheel-shipped to adopters whose foundation kind stays OPEN. A naive `grep -rl 'foundation'` sweep hits it, and sweeping it would re-introduce GHI #740's framework-wide over-reach in the template layer after it was fixed in the code layer. GHI #837 carries this fence in its body.

## Decisions Made

- [operator-ruled] Decline the GitHub route entirely (verbatim: "i'm not writing gh, what else?"). This set aside advised steps 1 and 2 of the resumed handoff, and also rules out deleting the five PRs, since GitHub does not permit PR deletion outside Support.
- [operator-ruled] Fix `pyproject.toml` and sweep the remaining current-tree occurrences (verbatim: "do 1 and 2"), chosen over a repo-visibility flip, over accepting-and-documenting, and over touching the ledger at that point.
- [operator-ruled] Scrub the ledger too (verbatim: "scrub ledger"), then the substitution target was fixed (verbatim: "swap for g0"). Raised as doctrinally forbidden by AGENTS.md Never #2 before proceeding; the operator ruled and the ruling stands.
- [operator-ruled] Accept that git history retains the prior values (verbatim: "I know the history will find it, it's ok"). A second filter-repo pass over the ledger was NOT authorized and was not run.
- [operator-ruled] Build the governed verb rather than bypass permanently (option 2 of three, chosen over an operator `--no-verify` stopgap alone and over reverting the scrub).
- [operator-ruled] Write the redaction verb to pool first, without promoting (chosen over authoring a feature ADR now, over queueing behind Movement B, and over building it as a direct fix).
- [operator-ruled] File the sealed-kind pool-ADR defect as one GHI via `/ghi-author` (chosen over fixing it inline this session, over an insight-only record, and over leaving it).
- [operator-ruled] Commit the ledger redaction with `--no-verify` (verbatim: "you do it"), overriding AGENTS.md Never #10. The agent could not execute it — the harness permission classifier denied the call — so the operator ran it from the `!` prompt.
- [agent-chose] Ran the full gate suite MANUALLY against the scrubbed tree before the bypass (8523 unittest OK, `gz validate` 5 scopes pass), so the `--no-verify` skipped only the one guard the operator overrode rather than all verification.
- [agent-chose] Pushed WITHOUT `--no-verify`, so the pre-push `gz check` gate ran unbypassed. It passed.
- [agent-chose] Raw-text substitution on the ledger, never a JSON round-trip — re-serializing would have reformatted all 15206 lines. The byte delta of exactly 169 x 4 is the proof nothing else moved.
- [agent-chose] Reworded two prose occurrences in `.gzkit/insights/agent-insights.jsonl` rather than substituting them; a blind swap would have inverted the meaning of the record describing this very correction.
- [agent-chose] Recorded the PII findings via `gz insights remember` rather than `/ghi-author`, because a public GitHub issue describing an operator-PII leak would itself be a disclosure.
- [agent-chose] Named the proposed verb `redact`, not `rewrite`, on the GHI #822 precedent that a governance verb must not be named for the artifact it writes.
- [agent-chose] Fenced the new pool ADR as a SIBLING of ADR-pool.attested-record-edit-doctrine rather than merging them — that ADR governs Layer-1 attested documents, this one Layer-2, and merging would let a Layer-1 semantics test appear to speak to Layer-2 mutability.
- [agent-chose] Corrected an in-flight over-count: the sealed-kind defect is 5 pool ADRs, not the 6 first reported, and the generator template is NOT part of it.

## Immediate Next Steps

1. Decide the four open surface decisions in ADR-pool.ledger-identity-redaction if the redaction verb is to be promoted: digest shape and coverage, where the field allowlist is encoded, whether bulk redaction is scriptable, and whether `gz init` scaffolds the verb for adopters. Promotion is `gz adr promote --kind feature`; foundation is sealed.
2. GHI #837 needs one decision before its sweep can run: `docs/design/adr/pool/ADR-pool.adr-layer-coherence.md:55` names ADR-0.0.37 as its promotion semver candidate, and that ID is a TERMINAL foundation ADR. A feature slot must be picked; the other four files are pure wording swaps. Observe the § Scope fence — `src/gzkit/templates/adr_pool.md` must not be touched.
3. GHI #836 remains open and is the sibling cut of #837 — ADR-0.34.0's residue in the unconditional root contract. Its repair is harder than #837's because `AGENTS.md` is both gzkit's own contract and the one `gz init` scaffolds into adopters, so the surface must express one rendering and two project states.
4. `gz-content-remember` (last_reviewed 2026-06-05) becomes a BLOCKING skill-audit error on 2026-09-03. It needs a real review, then a `skill-version` and `last_reviewed` bump in the same edit — never the date alone. Carried unworked from the predecessor.
...
- GitHub unreachable-object GC not requested and operator-declined this session; 8 pre-sweep commits were demonstrated fetchable from `origin` by full SHA.
- The ledger publishes 25 full 40-character hashes, which are the retrieval keys for the residual above. Removing them would need another history rewrite and was NOT authorized.
- Five immutable `refs/pull/*` refs still resolve to commits carrying the personal address and the full real name. GitHub does not permit PR deletion outside Support.
- Git history retains the operator's real name and address in every pre-redaction commit of `.gzkit/ledger.jsonl`, by explicit operator ruling.
...
- GHI #832 open — 22 repairable grandfathered release tags, possibly unblocked, unverified.
- GHI #813 open — handoff authorship has no Layer-2 event.
- `gz-content-remember` review deadline 2026-09-03, after which the skill audit blocks.
- AGENTS.md is 38480 B against the codex 32768 B cap and two must-survive sections sit past it. Parked by standing ruling.
- ADR-pool.documentation-type-templates, the design-references bibliography pool ADR, and the controlled-language pool ADR all remain unpromoted.

## Verification Checklist

Run these before trusting anything above. A piped verifier reports the filter's exit status, not the verifier's, so use `set -o pipefail` — a PreToolUse hook enforces this.

```bash
git status -sb
git log --all --format='%an <%ae>' | sort -u
git grep -l -F '<operator first name>' -- .          # substitute; never commit the literal
uv run gz check
uv run gz validate --ledger --authorship --receipt-shape --frontmatter --documents
```

Expect: a clean tree; the author list to contain ONLY `g0`, `dependabot[bot]` and `Test User`; the name grep to return nothing; `gz check` to exit 0; and the five validate scopes to pass.

To re-measure the SECOND residual in Important Context without polluting this clone, fetch into a THROWAWAY bare repo and delete it afterwards — never fetch pre-sweep objects into the working repository:

```bash
git init --bare /tmp/probe && cd /tmp/probe
git remote add origin https://github.com/tvproductions/gzkit.git
git fetch origin <full-40-char-sha-from-the-ledger>
```

At authoring time: HEAD `0e24e39d`, 3104 commits, 71 tags, working tree clean, 0 ahead / 0 behind `origin/main`, and `git grep -F` for the operator's first name returned zero across every tracked file including the ledger. GHIs 813, 832, 834, 835, 836 and 837 were all OPEN; re-resolve with `gh issue view` rather than trusting those states.

NOTE there is no rollback for the ledger redaction beyond git history. The pre-scrub backup lived in the session scratchpad, which does not survive the session.

## Evidence / Artifacts

- `docs/design/adr/pool/ADR-pool.ledger-identity-redaction.md` — the design for the governed verb; discriminating principle, four-part verb shape, five rejected alternatives, four open surface decisions
- `docs/design/adr/pool/ADR-pool.attested-record-edit-doctrine.md` — the Layer-1 sibling the new ADR is fenced against
- `.gzkit/ledger.jsonl` — 169 identity fields redacted to `g0` across 166 lines; byte delta exactly 169 x 4
- `pyproject.toml` — package author now `g0`; the PyPI channel closed on the next release
- `mkdocs.yml` — `site_author` now `g0`
- `.gzkit/insights/agent-insights.jsonl` — three insights stamped 2026-08-20: the incomplete-sweep finding, the ledger-scrub doctrine exception, and the two prose occurrences reworded rather than substituted
- `src/gzkit/hooks/guards.py` — `forbid_manual_ledger_edits`, the guard with no sanctioned alternative; the surface ADR-pool.ledger-identity-redaction amends
- `docs/governance/state-doctrine.md` — § Layer 2, the invariant the pool ADR proposes to narrow
- `src/gzkit/templates/adr_pool.md` — the wheel-shipped template that is CORRECT and must not be swept by GHI #837
