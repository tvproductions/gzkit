...
branch: main
timestamp: '2026-08-11T11:39:01Z'
agent: claude-code
session_id: f7c00342-4258-4a04-8dc0-cd7403894a20
continues_from: .gzkit/handoffs/20260811T105251Z-clips-healed-three-critic-briefs-authored.md
---

## Current State Summary

The campaign was amended (`93d10b4ce`): the 2026-08-09 pull-ahead is WITHDRAWN, `ADR-0.35.0-canon-entry-corpus-landing` is the in-flight feature, and `ADR-0.36.0-convergence-moment-cross-family-critic` is next-drawn. Both Movement A boxes were updated to match, since each carried the sequencing claim in its own text.

`ADR-0.35.0` was reviewed for implementation readiness and three ADR-body defects were repaired (`584778396`). It now passes `gz obpi validate --authored` 10/10 with `--req-kind-discipline`, `--brief-reconcile` and `--cli-alignment` each at exit 0. Its status is `Draft`, so a lifecycle step precedes any OBPI work.

THIS HANDOFF SUPERSEDES ITS PREDECESSOR ON TWO POINTS. The predecessor's advised steps 1 and 5 both directed the next session at `ADR-0.36.0` work; the amendment voided both, and a resuming agent reading them would pull the deferred feature. Steps 2, 3 and 4 survive and are carried below. `origin/main` was level at `0 0` at `93d10b4ce`; the tree is clean.

## Important Context

THE AMENDMENT RESTS ON AN INTERACTION THE PRIOR RULINGS NEVER WEIGHED, and that is the only ground it stands on. `2026-08-09 (2)` made the critic's delivery STAGED — OBPI-09's automatic door ships dark, lit only by OBPI-08's pilot. `2026-08-09` pulled the critic ahead on the reasoning that every session without it is unguarded. Both were ratified in the same session. Together they mean completing all nine OBPIs still leaves the guard off, so the trade actually on offer was: pause the feature with ten authored briefs to author nine more for a feature that would still not supply the always-on critic. The amendment states its own admission test in place, because this campaign edition names re-adjudication as the disease — a future amendment restoring the pull-ahead needs new evidence on the same terms, and the pilot lighting the dark door would be it.

THE ADR DRIFTED FROM ITS BRIEFS, NOT THE REVERSE, which is the direction no check covers. `gz validate --brief-reconcile` exited 0 throughout: the ten briefs were correct. The vendor-alignment-codex ADR, formerly carried at feature number 0.44.0, had been returned to pool as `ADR-pool.vendor-alignment-codex`; OBPI-09 was updated to the pool id everywhere including `REQ-0.35.0-09-07 [structural-fence]`, while the ADR body kept the old id at three live sites. One of them was `BI-07` — the fence's ONLY proof channel — so the requirement cited one artifact and its proof cited another. A structural fence whose proof names a different artifact than the requirement is not a weaker fence; it is an absent one. The § Checklist site also broke the 1:1 synchronization mandate, since OBPI-09 quotes its checklist item verbatim.

THE Q&A TRANSCRIPT IS A SEALED DUPLICATE AND FORCED THE EDIT TECHNIQUE. `## Q&A Transcript` carries verbatim copies of § Decision and § Consequences, so § Decision item 8 is byte-identical to its copy at line 367 — a string edit would have been ambiguous and a replace-all would have silently rewritten a primary source. Repairs used line-numbered edits guarded by assertions on both a context token and the old string. The transcript records what was SAID; correcting it to today's identifiers would falsify the archive, so lines 367 and 412 are left stale ON PURPOSE.

THE `[kind]` TAG CASE HAS NO AUTHORITY TO NORMALIZE TOWARD. Repo-wide: 499 uppercase against 299 lowercase across ADR briefs. `.gzkit/rules/tests.md` documents lowercase; `gz-obpi-specify` SKILL.md documents uppercase; neither derives from the other; `gz validate --req-kind-discipline` accepts both. Only ADR-0.35.0's internal split was closed (OBPI-10 held all seven of its uppercase tags against 97 lowercase elsewhere). Rewriting ~800 tags on an agent's preference between two live conventions is the taste-driven cleanup DO IT RIGHT #11 forbids.

ADR-0.35.0'S OWN DEPENDENCY ORDER IS DECLARED AND SHOULD BE READ BEFORE PULLING AN OBPI: 01 -> 02 -> 03 is a chain, and OBPI-09 is independent of it and explicitly NOT cuttable per § Scope Minimization. The ADR also pins the tombstone fold algebra in its own § Decision rather than deferring it, on the stated ground that it is the one genuinely irreversible commitment.

## Decisions Made

- [operator-ruled] Review `ADR-0.35.0` (verbatim: "review 0.35.0"), after challenging the sequencing (verbatim: "not sure i wan't to author 0.36.0 while we haven't finished 0.35.0"; spelling preserved).
- [operator-ruled] Fix the three defects the review surfaced before anything else (verbatim: "yes:" quoting the offer back — the three live stale-ADR-id references, the dangling forced-decisions pointer, and the tag casing).
- [operator-ruled] Amend the campaign to withdraw the critic pull-ahead and restore `ADR-0.35.0` to the in-flight position (verbatim: "yes, amend campaign").
- [operator-ruled] Sync each landing as it completed (verbatim: "git sync", "sync it").
- [agent-chose] Repaired the ADR body with line-numbered, assertion-guarded edits rather than string replacement, because § Decision item 8 is byte-identical to its sealed Q&A transcript copy.
- [agent-chose] Left the Q&A transcript's two stale references unrepaired, on the ground that a transcript records what was said and correcting it would falsify the archive.
- [agent-chose] Normalized `[kind]` tag case within ADR-0.35.0 only and tracked the repo-wide split as a `defect` insight, rather than rewriting ~800 tags toward a convention with no authority behind it.
- [agent-chose] Repointed `(see Forced Decisions)` to the existing `DESIGN_FORCING_FUNCTIONS.md` § Closing items 3 and 4 rather than authoring a new section — the content was never missing, only the pointer.

## Immediate Next Steps

1. RULE THE `ADR-0.35.0` LIFECYCLE STEP. Its status is `Draft` while `ADR-0.36.0` is `Proposed`. Confirm what the in-flight feature's status must be before OBPI work begins, and move it if so. This gates everything below it and is cheap to settle.

2. PULL `OBPI-0.35.0-01-corpus-tombstone-schema-and-fold`. It heads the declared 01 -> 02 -> 03 chain and carries the fold algebra the ADR pinned in § Decision as its one irreversible commitment. Read that algebra from the ADR rather than re-deriving it — single reverse pass, never a fixpoint iteration, and unset tombstone fields MUST be omitted from serialization or `corpus_fingerprint()` re-fingerprints the whole corpus on the landing commit.

3. RULE ON THE THREE ABRIDGED TWINS, carried from the predecessor. They are NOT clip damage: the fuller form is already in the same document, so repair means REMOVING a twin, which is a dedup-policy change on `_dedup_rulings` — a path both composition steps share deliberately. Wants a covering test first and an explicit ruling on the collapse direction, since `_ruling_key` declines that collapse on purpose.

4. RULE ON THE REPO-WIDE `[kind]` TAG CASE, or decline to. Two documenting surfaces disagree and the validator accepts both, so this needs an authority chosen rather than a majority followed. Declining is a legitimate outcome; leaving it unruled is what makes it recur.

5. DECIDE THE MULTI-PARENT LINEAGE GAP — `continues_from` is single-valued, so a genuine merge cannot be expressed and the 2026-08-11 fork collapse had to carry one parent by value. Confirmed unfiled against the six open GHIs. Carried unworked across three handoffs now, which is itself the signal to either file it or drop it.

## Pending Work / Open Loops

`ADR-0.36.0` IS DEFERRED, NOT ABANDONED. Three briefs (01 critic-skill-contract, 02 cross-family-transport, 03 operator-door) are authored and passing at `1b808b18d`; six remain `draft (scaffold)` — 04 agent-door, 05 decision-envelope, 06 risk-tiering, 07 verdict-resolution-transition, 08 pilot-instrumentation, 09 asked-question-gate-dark. Its § Fidelity Assertions table still carries the scaffold's example row, so `--fidelity-presence` passes while the ADR asserts nothing about its own thesis. Its § Forcing Functions are unfilled.

THE THREE ABRIDGED TWINS ARE UNREPAIRED. The clip arm has a mechanical census that enumerates it on every run; the twin arm has none and is counted only in transcribed prose.

THE REPO-WIDE `[kind]` TAG CASE IS SPLIT 499 UPPERCASE / 299 LOWERCASE with two conflicting documenting surfaces and a case-insensitive validator.
...
Confirm the sequencing before acting on it — the campaign governs, and this document only reports it:
...
uv run gz obpi validate --adr ADR-0.35.0-canon-entry-corpus-landing --authored
uv run gz check
```
...
Session state confirmed before authoring: `gz check` exit 0 at `93d10b4ce`; `git rev-list --left-right --count origin/main...main` returned `0 0`; tree clean.

## Evidence / Artifacts

Landed this session:

- `docs/governance/build-to-1.0-campaign-2026-07-18.md` — the 2026-08-11 amendment plus both Movement A boxes
- `docs/design/adr/pre-release/ADR-0.35.0-canon-entry-corpus-landing/ADR-0.35.0-canon-entry-corpus-landing.md` — three live stale-ADR-id references repaired, forced-decisions pointer repointed
- `docs/design/adr/pre-release/ADR-0.35.0-canon-entry-corpus-landing/obpis/OBPI-0.35.0-10-classification-reader-and-ownership.md` — seven `[kind]` tags lowercased

Read to ground the review and the amendment:

- `docs/design/adr/pre-release/ADR-0.35.0-canon-entry-corpus-landing/DESIGN_FORCING_FUNCTIONS.md` — § Closing items 3 and 4 are the real home of the forced decisions
- `docs/design/adr/pool/ADR-pool.vendor-alignment-codex.md` — the demoted ADR the stale references pointed at
- `.gzkit/handoffs/20260811T105251Z-clips-healed-three-critic-briefs-authored.md` — the predecessor, whose steps 1 and 5 this document voids
- `tests/governance/test_settled_ruling_integrity.py` — the clip witness landed earlier this session
