...
adr_id: null
branch: main
timestamp: '2026-08-17T18:46:05Z'
agent: claude-code
session_id: 1fc95331-a12d-4cbb-a6b6-f4fe450bf27e
continues_from: .gzkit/handoffs/20260817T181832Z-ghi-815-closed-816-filed-fixed-git-sync-diagnosis-restored.md
---

## Current State Summary

Supersedes `20260817T181832Z-ghi-815-closed-816-filed-fixed-git-sync-diagnosis-restored.md`, which predates the GHI #815 reopen and the whole #817 arc.

Resumed `20260817T130147Z`, booked `proceed` with advised step 2 only. Five operator rulings then carried the session well past that scope.

Six commits, all synced, `origin/main` `0 0`, tree clean:
...
- `3791b1e7d` / `2bca45fcc` — predecessor handoffs
...
**A correction that changes the ranking's argument, though not its outcome.** Three surfaces this session said *"shrinking buys headroom; it does not buy delivery, because a smaller surface in the same order still puts rank 11 last."* That holds ONLY while the surface remains over the cap. Compress `AGENTS.md` below 32768 B and every section renders inside it, `architectural-boundaries` included — so shrink IS a delivery fix. The honest axis is **durability, not capability**: reorder is growth-invariant, shrink restores delivery today and decays on the next corpus growth. They are complementary. Reorder stays ranked first because its fix does not decay. Corrected in the pool ADR body; appended as a fourth dated entry to `data/instructions_files_budget.json` rather than edited, per that file's own reading convention.

**The compression work is chore-shaped, and "these are rendered" relocates it rather than blocking it.** The corpus ceremony already carries the right seam: `gz content compose` writes a *candidate*, `gz content commit` promotes it *under attestation*. `instructions-files-diet` v2.0.0 therefore does the full compression labor and **stops at the Gate 5 boundary**, handing the operator a candidate. A chore that ran `gz content commit` itself would land Layer-1 canon unattested — a worse defect than the one it fixes, since Gate 5 is universal.

**The chore contradicted itself, which is worse than being uniformly wrong.** Its § 5 and § Anti-patterns already said never to edit `.claude/rules/**` mirrors, while § 3's Lift table named a mirror as the origin to edit. Two-of-three agreement means a reader who checks one section gets the right answer and a reader who follows the procedure gets the wrong one. Only the `AGENTS.md`/corpus arm was a genuine blind spot — zero mentions of `corpus`, `rendition`, `gz content`, `compose`, or `Gate 5` anywhere in the file. GHI #817's body says "no awareness of the layer it is editing"; that is accurate for one arm and overstated for the other, and the close comment records the refinement.

**The chore ships in the wheel.** Its `registry.json` entry carries no `"projectLocal": true`, so it is canonical class, present at `src/gzkit/chores/`, packaged, and scaffolded into every adopting project by `gz init`. A defect in a chore procedure is not gzkit-local by default — check the registry class before scoping blast radius.
...
**GitHub returned sustained 503s mid-session.** `gh issue create` and `gh issue close` use GraphQL and failed repeatedly; the REST endpoints (`gh api --method POST repos/.../issues`) stayed up. Filing via REST is a transport change, not a skill bypass — `/ghi-author`'s Step 0 prior-art sweep ran first, which is what the prohibition actually protects.
...
- [operator-ruled] The cap belongs in a GHI (verbatim: "I wanted the codex cap to be GHI'd"), which reversed the same day's decision to decline a successor tracker at the #815 close.
...
- [agent-chose] Reopened #815 rather than filing a successor GHI, per `ghi-author` § Step 0's ≤30-day same-root-cause rule. Filing #818 for the same physical breach would have fragmented the record and orphaned the measurement already recorded on #815.
- [agent-chose] Scoped `instructions-files-diet` v2.0.0 to STOP at `gz content compose`, never running `gz content commit`. Gate 5 is universal (ADR-0.0.36), so a chore that landed canon would be a worse defect than the one being repaired.
- [agent-chose] Corrected the shrink/reorder overstatement in the pool ADR body but APPENDED the correction to `data/instructions_files_budget.json` rather than editing its prior entry — that file's convention is that a dated entry records what was believed on its date.
- [agent-chose] Audited only `instructions-files-diet` and declared the other chores explicitly unaudited in #817's close comment, rather than claiming a class sweep the session did not perform.
...
4. **Consider declaring `CLAUDE.md` in the survival declaration.** `data/agents_md_survival_declaration.json` declares only `AGENTS.md`, so any other control surface crossing a vendor cap is silently unmeasured by design. Not live — `CLAUDE.md` is small against the same cap — but it is the one empty cell in #815's cause-to-witness table.
...
- `.gzkit/handoffs/20260817T181832Z-ghi-815-closed-816-filed-fixed-git-sync-diagnosis-restored.md` — the predecessor this refreshes
- `.gzkit/insights/agent-insights.jsonl` — one `defect` insight under scope `handoff-authoring`
