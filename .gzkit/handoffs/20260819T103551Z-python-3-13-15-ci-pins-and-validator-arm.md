...
adr_id: null
branch: main
timestamp: '2026-08-19T10:35:51Z'
agent: claude-code
continues_from: 20260819T081336Z-ste100-controlled-language-pool-adr.md
---

## Current State Summary

Second half of a long session. Tree is clean and `main` is synced at `dc54c1f14`; every item below is committed and pushed.

Landed since the predecessor handoff, oldest first: `39591c7ec` (git-maintenance race in test fixtures), `eb12443d8` (session insights), `eda37ac98` (PyPI re-publish tolerance), `e5fab896a` (Python 3.13.15 via uv), `46af70856` (CI patch-locked to 3.13.15), `dc54c1f14` (the interpreter-pin validate arm).

Python moved 3.13.14 to 3.13.15, uv-managed, with uv itself refreshed 0.11.32 to 0.12.5 because uv's managed builds topped out at 3.13.14 and only system installs had 3.13.15. The project pin and all four CI sites now declare 3.13.15, and the new validate arm holds those five declarations in agreement — wired into `gz check` as step 14 of 56, with a registered negative control.

Three GHIs filed: #833 (index.lock residual), #834 (re-pushing a tag overwrites its release body), #835 (~46 validator steps run serially). #834 blocks #832 and is cross-linked there.

## Important Context

Four things a resuming agent will otherwise get wrong.

First, the CI failure diagnosis that arrived pre-written was wrong twice, and both corrections are recorded in commit bodies rather than only here. The maintenance-lock failure was a RACE, not a missing-file assumption — the error path is the copy SOURCE, and the same suite passed on the next commit. The release failure was NOT a version mismatch: the check printed matching versions and the failure was a 400 File already exists fourteen seconds later. The prescribed remedy for that one — bump to 0.34.5, tag, push — would have cut a spurious PyPI release of unreleased main.

Second, `gz check` is 136s and has NO dominant cost. Measured 2026-08-19 (dated record, re-derive): validators and friends ~55s, Test 45.3s, Behave 31.6s, mkdocs 4.0s. The test suite has been parallel since GHI #512 (see the runner in `src/gzkit/quality.py`, unittest-parallel). An agent reaching for "the tests are slow" is repeating the mistake this session made — the serial 111s figure is the ARB attestation path, which that same module keeps serial deliberately.

Third, adding a validate arm costs far more than three files, and every extra file is one of the repo's own controls refusing a weak gate. Validator-reachability holds a shrink-only list of ungated scopes, so a new scope MUST be wired into `gz check`. The qc-binding audit raises KeyError until the step is classified, then rejects it as green-by-emptiness unless an enforces-registered negative control exists. The exemption-controls audit demands an explicit exempts declaration after reading the gate end-to-end. Budget fourteen files, not three.

Fourth, handoff chaining: chain to the newest SUBSTANTIVE handoff, never to the newest file. The newest file at a session boundary is always a mechanical session-exit bookmark, which carries no Settled Rulings, and chaining there silently drops the whole accumulated ruling chain. The tool suggests the newest file, so following its suggestion is the trap.

## Decisions Made

- [operator-ruled] Patch-lock CI to 3.13.15 alongside the project pin (verbatim: "yes, patch-lock CI to 3.13.15"), after the agent flagged the minor-level CI pins as a separate call.
- [operator-ruled] Add a validate arm for the five version declarations (verbatim: "add a validate arm for the five version declarations").
- [operator-ruled] File a GHI for the index.lock residual and fix the release-workflow failure (verbatim: "file a GHI for the index.lock residual and fix this").
- [operator-ruled] Target the validator bucket rather than a test runner for the performance GHI, after the agent's parallel-test-runner premise was refuted by measurement.
- [operator-ruled] Move gzkit to Python 3.13.15 via uv (verbatim: "ensure gzkit is using this: Python 3.13.15" / "via uv").
- [agent-chose] Removed the CAUSE of the copytree race rather than ignoring the lock file. Rejected alternatives, each for its own reason: ignoring the lock by name treats the instance; ignoring the git directory entirely breaks tests that need it; a wildcard lock-ignore would drop the uv lockfile, which the patch-release command reads.
- [agent-chose] Fixed only the loud half of the release failure and filed the silent half as #834, because the remedy changes release semantics and deserves its own decision.
- [agent-chose] Routed the validate arm as a direct fix on measured precedent — every new validate arm in this repo landed as a fix commit — rather than as OBPI ceremony, which the routing table's CLI-surface criterion would have required.
- [agent-chose] Verified 3.13.15 exists in the actions/python-versions manifest by parsing it, after a WebFetch summary of the same file wrongly reported it absent from truncated content.

## Immediate Next Steps

1. Decide whether ASD-STE100 Issue 9 gets onboarded into project references. Still NOT onboarded and still carried from the predecessor handoff. The research-sources mechanism stores full extracted text, which for this document would redistribute copyrighted material; three options are recorded in that handoff and in the pool ADR.
2. GHI #835 is the live performance thread and its first deliverable is NOT a speedup. Establish for each `gz check` step whether it writes anything or depends on another step's side effect. A concurrent runner over steps with an undeclared dependency is a flaky gate, which is worse than a slow one.
3. GHI #834 blocks GHI #832. Do not run the 22-tag repair until the release-body rewrite is closed, or it will overwrite 22 curated release bodies with generated stubs — including the ones GHI #830 [settled] just curated.
4. GHI #833 (index.lock residual) is scoped and ready. Note the constraint recorded in it: a wildcard lock-ignore is the obvious fix and is wrong, because it drops the uv lockfile.
5. Adopt the two measured habits from this session: do not run `gz check` manually when about to push (the pre-push hook runs it, and this session paid ~9 minutes for that duplication), and batch pushes rather than pushing per commit.

## Pending Work / Open Loops

- GHI #835 open — ~46 serial validator steps, the largest bucket of the 136s gate.
- GHI #834 open — release-body rewrite on tag re-push. Blocks #832.
- GHI #833 open — the other transient git locks still race copytree; only the maintenance lock is quiesced.
- GHI #832 open — 22 repairable grandfathered release tags, blocked by #834.
- GHI #813 open — handoff authorship has no Layer-2 event. Untouched across both halves of this session.
- ASD-STE100 not onboarded into any references surface; the design-references bibliography pool ADR remains unpromoted.
- The interpreter version is declared in five places. The new validate arm now holds them in agreement, so this is closed as a drift risk, but a version bump must still move all five together.
- A workflow testing a MATRIX of interpreters would trip the new arm. None exists; an exemption mechanism was deliberately not built ahead of the case.
...
git status -sb
uv run python -V
uv run gz validate --python-version-pins
uv run gz check
for n in 813 832 833 834 835; do gh issue view $n --json number,state,stateReason; done
```

Expect HEAD at dc54c1f14, main synced with no tracked changes, Python 3.13.15, both gz invocations exit 0, and issues 813, 832, 833, 834, 835 all OPEN at authoring time. Re-resolve the issue states rather than trusting them, per the Claim Verification Gate.

To reproduce the gate breakdown rather than trusting the numbers in Important Context:

```bash
/usr/bin/time -p uv run gz check
/usr/bin/time -p uv run behave
```

## Evidence / Artifacts

- `src/gzkit/governance/trust_audits/python_version_pins.py` — the new audit; pure decision function plus a filesystem adapter, scanning the workflows directory rather than a hardcoded list
- `tests/governance/test_python_version_pins.py` — 12 tests covering agreement, the floor-is-not-a-pin distinction, missing authority, and end-to-end detection
- `src/gzkit/governance/trust_audits/_qc_negative_controls.py` — the planted-drift fixture the qc-binding audit demanded before it would accept the arm as non-theatre
- `src/gzkit/governance/trust_audits/_qc_claim_exemptions.py` — the no-exemption declaration with the end-to-end read recorded beside it
- `data/check_scope_membership.json` — in-check membership now 50, registry 91
- `tests/commands/common.py` — fixture git repos quiesced so background maintenance cannot race the per-test copy
- `tests/commands/test_common_fixtures.py` — the quiescence contract, RED-proven by reverting the helper
- `.github/workflows/release.yml` — skip-existing on the PyPI publish
