...
adr_id: null
branch: main
timestamp: '2026-08-20T09:50:09Z'
agent: claude-code
session_id: e685a9d4-dfd0-4dc9-ad4d-b53ef8035169
continues_from: .gzkit/handoffs/20260820T092034Z-pii-history-sweep-and-foundation-kind-seal.md
---

## Current State Summary

The PII history sweep is COMPLETE in this repository, and the foundation-kind canon repair that opened the session is landed. Verified at authoring time across every remote ref: `origin/main`, `origin/gh-pages` and all 71 tags return ZERO hits for both the operator's real name and personal address; commit metadata carries none; the working tree is clean. Author identities across all history are now only `g0`, `dependabot[bot]` and `Test User`.

This handoff folds in the dated correction its predecessor carried as an appendix, so a resuming agent reads one coherent document rather than a body plus a contradicting note.

Three movements landed. ONE — AGENTS.md taught `foundation` as a live authoring kind five weeks after ADR-0.34.0 sealed it; repaired through the corpus, never a hand-edit, per REQ-6 of the Completed OBPI-0.34.0-03. TWO — three defects fixed: `gz content commit` reported success without naming the playback writer, the adopter carve-out keeping `foundation` OPEN downstream had no mechanical witness, and two skill-audit tests pinned an exact warning census against calendar-dependent fixture data. THREE — three git-filter-repo passes swept the operator's name and personal address from all 3094 commits and 71 tags, force-pushed and verified.

TWO PII RESIDUALS REMAIN and neither is closable by an agent: GitHub retains unreachable pre-sweep objects until its own garbage collection, and five immutable `refs/pull/*` refs still resolve to commits carrying the address.

CRITICAL for any resuming agent: EVERY pre-sweep commit SHA is DANGLING. Confirmed by `git cat-file -e` for c2e79d5b1, 7c5f3d9a8 and 92bb60788 — all three fail. Every SHA cited in the ledger, ADR audit proofs, GHI comments and all prior handoffs is unresolvable.

## Important Context

Seven things a resuming agent will otherwise re-derive.

FIRST — the content pipeline is corpus -> compose -> commit -> playback, and the stages have DIFFERENT writers. `gz content commit` writes only the rendition; `gz agent sync control-surfaces` is the sole writer of AGENTS.md and its mirrors. Stopping after commit leaves `gz validate --invariant-coherence` red with the pending diff as its message. `agents.local.md` is FROZEN into the rendition, so editing it and running sync alone plays back the stale block — verified, not assumed. The commit verb now names the playback step in its own success output.

SECOND — the foundation-kind closure is PROJECT-LOCAL. ADR-0.34.0 Positive #8 keeps `gz init` scaffolding adopters OPEN. `src/gzkit/templates/agents.md` must never carry the closure; there is now a covering test asserting that in both directions. `gz validate --distribution` does NOT protect this — it enforces byte-parity between the two template copies, which an edit to BOTH satisfies.

THIRD — why the sweep needed three filter-repo passes, because the next person will assume one. Pass 1 (`--mailmap` plus `--replace-text`) collapsed nine author identities to `g0` and swept blobs. Pass 2 (a blob callback) was needed for a `.pptx` holding the name in `docProps/core.xml`, COMPRESSED inside a ZIP where text replacement cannot reach. Pass 3 (path purge) was needed for a 165-byte PowerPoint OWNER LOCK FILE storing the local username after a `\x0b` length prefix, which `--replace-text` skipped. `~$*` is now in `.gitignore` to close that class.

FOURTH — the sharpest lesson of the session, and it is recursive. The operator-PII rule seeded 93 occurrences of the value it prohibited, because its own worked example read 'use the operator's name only (e.g. <the name>)' while an invariant-tier corpus entry reserved those same attestor fields for `g0`. Then the handoff written to DOCUMENT the sweep quoted the personal address verbatim, putting it back on `main` one commit after 3094 were rewritten to remove it. A document about a forbidden value must never restate it. The failure mode survives active hunting.

FIFTH — NO GATE CATCHES A PLAIN ADDRESS. `gitleaks` passed on the re-leaking commit because it matches secret patterns and an email is not one. What caught it was running the pickaxe over every remote ref AFTER pushing rather than trusting the sweep had finished. If PII discipline needs a mechanical arm, this is the gap; none exists today.

SIXTH — hand-editing a handoff has a gotcha worth knowing. `gz validate --documents` PASSES a hand-edited handoff; the `gz check` 'Handoff documents' scope does not. `validate_handoff_document` fail-closes on scaffold-default tokens, and it rejects the literal word for a substitution marker even when used descriptively in prose. There is no amend verb, so the append-a-dated-correction pattern is the route — and it must clear that scope, not merely `--documents`.

SEVENTH — a measured finding that narrows GHI #834. A bare `git push --force refs/tags/*` moved all 71 tag refs with ZERO change to any release body; all 71 were byte-identical against a snapshot taken beforehand. So #834's overwrite belongs to the release-CEREMONY re-push path, not to moving a tag ref, which may unblock GHI #832 without waiting on #834. Re-measure before acting.

## Decisions Made

- [operator-ruled] Verify the sealed-kind contradiction rather than work any of the resumed handoff's five advised steps (four-option picker); all five were recorded set-aside.
- [operator-ruled] Fix it in the SOURCE, not the rendered surface (verbatim: 'AGENTS.md is wrong, but it is rendered, so fix in source').
- [operator-ruled] Fix the two gaps that had been written into a handoff instead of repaired (verbatim: 'fix:' followed by both). Writing a known gap into a handoff is the deferral pattern, not a fix.
- [operator-ruled] Stop using the operator's real name (verbatim: "we don't use my name for this"; 'we have a PII rule').
- [operator-ruled] Sweep the name and address from ALL history including sealed artifacts and the ledger (verbatim: 'yes, sweep it'), chosen over commit-metadata-only, main-only, and not rewriting. The cost — every pre-sweep SHA across the governance corpus going dangling — was stated in the question and accepted.
- [operator-ruled] Sweep LICENSE too, chosen over preserving the real legal name in the copyright notice. LICENSE now reads 'Copyright (c) 2026 g0'.
- [agent-chose] Captured the foundation closure at tier `invariant`, so it survives every compression setpoint and cannot be trimmed out by a future diet pass.
- [agent-chose] Snapshotted all 71 release bodies BEFORE touching tags as insurance against GHI #834. The insurance proved unnecessary and became the measurement that narrows the issue.
- [agent-chose] Backed up all refs to a bundle and verified locally to zero BEFORE any force-push. That bundle lived in the session scratchpad and is now GONE.
- [agent-chose] Did NOT re-stamp `gz-content-remember`'s `last_reviewed`; stamping a date to clear a warning would assert a review nobody performed (skill-surface-sync rule 6).
- [agent-chose] Repaired the re-leaking handoff by amend and force-push rather than a follow-up commit, so the offending commit is unreachable rather than merely superseded.
- [agent-chose] Appended a dated correction to the predecessor rather than editing its stale bullets, then folded that correction into this document's body.

## Immediate Next Steps

1. Ask GitHub Support to garbage-collect unreachable objects on tvproductions/gzkit. Force-pushing de-references the pre-sweep commits but does not delete them; anyone holding a pre-sweep SHA can still fetch one until GitHub collects them. This is the larger of the two remaining PII residuals.
2. Five `refs/pull/*` refs are IMMUTABLE and still resolve to commits carrying the personal address. Neither force-push nor GC clears them; only deleting those pull requests would.
3. Decide what to do about dangling SHA citations across the ledger, ADR audit proofs, GHI comments and prior handoffs. Rewriting the ledger to match is FORBIDDEN, so this needs an operator ruling, not an agent fix.
4. `gz-content-remember` (last_reviewed 2026-06-05) becomes a BLOCKING skill-audit error on 2026-09-03. It needs a real review, then a `skill-version` and `last_reviewed` bump in the same edit — never the date alone.
5. GHI #835 remains the live performance thread and its first deliverable is NOT a speedup: establish which `gz check` steps write, or depend on another step's side effect, before any concurrent runner. A flaky gate is worse than a slow one.
6. GHI #832 may now be unblocked by the #834 narrowing in § Important Context SEVENTH — but re-measure the ceremony path before running the 22-tag repair.
7. GHI #813 has now gone untouched across FIVE handoffs. It is the oldest open thread here.

## Pending Work / Open Loops

- GitHub unreachable-object GC not requested; pre-sweep objects remain fetchable by SHA.
- Five immutable `refs/pull/*` refs still carry the personal address.
- Every pre-sweep SHA cited anywhere in the governance corpus is dangling; no reconciliation attempted.
- No mechanical arm exists for PII discipline; `gitleaks` does not match a plain address.
- GHI #835 open — validator steps run serially, the largest bucket of the gate.
- GHI #834 open — narrowed this session to the release-ceremony path, not tag-ref moves.
...
- AGENTS.md is 38480 B against the codex 32768 B cap and the `operator-doctrine-verbatim-canon` section straddles it, so its tail is undelivered under codex. Budget work parked by standing ruling.
- ADR-pool.documentation-type-templates remains unpromoted and is named in no ADR checklist.
- The design-references bibliography pool ADR and the controlled-language pool ADR remain unpromoted.
...
git log --all -S '<operator personal address>' --oneline   # substitute; never commit the literal
uv run gz check
uv run gz plan create sunset-fidelity-probe --kind foundation --semver 0.0.75 --dry-run
```

Expect: the author list to contain ONLY `g0`, `dependabot[bot]` and `Test User`; the pickaxe to return nothing; `gz check` to exit 0; and the plan-create probe to REFUSE with ADR-0.34.0 guardrail prose — that refusal is the pass condition, being the ADR's own Fidelity Assertion with expected exit 1.

At authoring time: HEAD d2663b5a, 3100 commits, 71 tags, seven insights stamped 2026-08-20, and both `origin/main` and `origin/gh-pages` verified at zero hits for name and address. Re-resolve GHI states with `gh issue view` rather than trusting the numbers above; 813, 832, 834 and 835 were OPEN.

NOTE the backup is GONE: the pre-sweep all-refs bundle lived in the session scratchpad, which does not survive the session. There is no rollback for the history rewrite.

## Evidence / Artifacts

- `.gzkit/corpus/AGENTS.md.jsonl` — the foundation-closure entry, tier invariant, classification Mechanical, witness g0
- `AGENTS.md` § Kinds — the played-back result: the table row, the verbatim invariant paragraph, and the three rewritten enforcement bullets
- `.gzkit/agents.local.md` — the operator-PII rule, reworded to name the substitute (`g0`) instead of exhibiting a specimen of the forbidden class; its example was the seed for 93 occurrences across 27 files
- `docs/governance/behavior-rules.md` — the same correction in the expansion doc
- `src/gzkit/commands/content/commit.py` — the success path now names the playback writer
- `docs/user/manpages/content.md` — the same gap closed in the doc surface
- `tests/test_templates.py` — the adopter-open witness, asserting in both directions
- `tests/commands/test_skills.py` — skill-audit assertions decoupled from calendar drift
- `.gitignore` — the `~$*` rule closing the Office owner-lock-file class
- `docs/design/adr/pre-release/ADR-0.34.0-foundation-sunset/obpis/OBPI-0.34.0-03-terminal-partition-gate-and-doctrine-retirement.md` — line 87 the declared deferral no movement ever claimed, line 105 REQ-6 forbidding the hand-edit
- `.gzkit/insights/agent-insights.jsonl` — seven insights stamped 2026-08-20: the deferred capture, the handoff-scope reading, the skill review deadline, the PII example seed, the #834 narrowing, the history-rewrite residuals, and the handoff that re-leaked the swept address
