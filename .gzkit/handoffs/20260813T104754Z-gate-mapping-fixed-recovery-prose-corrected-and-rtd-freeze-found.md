...
adr_id: null
branch: main
timestamp: '2026-08-13T10:47:54Z'
agent: claude-code
session_id: 7d9ee56e-75a5-41f7-92db-bb85a5eb5dc5
continues_from:
- .gzkit/handoffs/20260812T104530Z-three-rulings-cleared-draft-holds-and-two-blockers-filed.md
- .gzkit/handoffs/20260813T011243Z-session-exit-bookmark.md
- .gzkit/handoffs/20260813T011255Z-session-exit-bookmark.md
- .gzkit/handoffs/20260813T011450Z-session-exit-bookmark.md
- .gzkit/handoffs/20260813T011509Z-session-exit-bookmark.md
---

## Current State Summary

Resumed the 2026-08-12 anchor, verified its five advised steps against live state, and worked the one the operator ruled. The session then became defect repair in two further surfaces the operator raised directly. Landed at 3a6a0771e across five commits; origin/main is 0/0 and the tree is clean.

Claim verification before presenting found the anchor's advised step 5 materially weakened: it justified re-triage with "the queue moved this session", and the open queue measured as the exact eight the anchor itself enumerated, unchanged. The operator ruled step 2 only (defect repair, no feature build) with steps 1, 3, 4 and 5 recorded set-aside via gz handoff decide.

GHI #798 is FIXED and CLOSED at 5494b4822. EnforcementClaimRecord now carries gate_targets, stamped at decoration time by reading the entrypoint's own import bytecode. Measured on the live population: 60 of the 71 disclosed undeclared claims resolve to a named gate, against 0 of 70 and 7 of 71 for the two heuristics that had previously failed. The exemption drain therefore goes from 71 blind delegation-chain walks to 9 one-line reads, and the shrink-only ratchet finally has something to shrink against.

Two further defects were raised by the operator mid-session and repaired. The verifier-pipe-gate's recovery prose named the highest-friction correct route in its NEXT STEP line while the one-call pipefail escape sat in a subordinate clause; the agent took the expensive route eleven times before the operator flagged it (ca39ecb6b). The Read the Docs check, carried unworked across three sessions, was finally answered: both documentation front doors have been serving 2026-05-05 content for roughly 100 days (4efe7fce9, 3a6a0771e, GHI #801).

Final uv run gz check exits 0 across 55 steps with QC binding, Exemption controls and Enforcement floor all green.

## Important Context

THE GHI #798 MAPPING WAS RECOVERABLE FOR A REASON THE ISSUE ITSELF DID NOT SEE. Its body says "Do not attempt a third heuristic; the mapping is not recoverable from what the registry stores." That sentence is precisely true and is also why the fix works. Attempt 1 scanned source_file, a string ABOUT the shim, and matched 0 of 70. Attempt 2 correlated claim ids against module stems, a convention ABOUT the gate, and matched 7 of 71. Both asked the registry what it had STORED. The registry does not only store strings, it HOLDS the live callable, and that callable's bytecode contains the actual from-gzkit-import statement it executes. Reading the delegation is not inferring it. The issue's own Scope hint had pointed here the whole time, asking for "a field on EnforcementClaimRecord populated at decoration time" -- populated, not declared.

WHY exempts STAYS DECLARED WHILE gate_targets IS STAMPED. These are different kinds of fact and the repo already draws the line. Whether a gate HAS an exemption surface is an authoring judgment no runtime can decide, which is why EXEMPTS_NONE exists as a real value rather than an absence. The delegated gate is a runtime fact. That is the same producer-versus-judgment split .claude/rules/task-discovery.md draws between its three producer-stamped channels and advisory @advances. Do not "finish the job" by making exempts derived; that would be the laundering the exemption-controls audit exists to refuse.

THE 11 CLAIMS THAT DO NOT RESOLVE ARE HONEST SILENCE, NOT A GAP TO CLOSE. Two are co-located entrypoints where the _ep_ function lives inside the gate's own module, so source_fn already names the gate. Nine are subprocess-backed shims whose gate is a command such as uv run ruff check, with no gzkit callable to name; their one-line body shows the command immediately. An empty gate_targets means read source_fn. Do not add a fourth derivation pass to chase them.

THE VERIFIER-PIPE GATE WAS NEVER THE PROBLEM, ITS ORDERING WAS. Both permitted routes preserve a verifier's exit status. NEXT STEP is the line an agent acts on, and it named the two-call file capture while framing set -o pipefail as a carve-out for an unusual need. Wanting a tail of the output is the normal case. The refusal predicate was not touched and must not be: a probe against a deliberately failing suite confirmed the promoted escape still surfaces REAL EXIT 1, which is the property the gate exists for. The function's own docstring had already claimed the fix it did not implement, promising to hand back "the corrected command shape" while emitting a generic stand-in instead.

THE RTD CHECK SAT UNWORKED THREE TIMES BEHIND A FALSE PREMISE. Successive handoffs recorded that app.readthedocs.org returns 403 unauthenticated, and each successor inherited that as settled fact rather than re-probing it. Measured 2026-08-13: the projects, versions and builds endpoints all return 200 with no token; only the write side needs credentials. The generalizable lesson is that a handoff's BLOCKING claims are the ones most worth re-measuring, because they are exactly the claims that stop anyone from ever testing them again. The working curl commands are now inlined in both .readthedocs.yaml and GHI #801 so a fourth carry cannot happen.

THE RTD FREEZE IS A TWO-SYSTEM CONTRADICTION NEITHER HALF CAN SEE. The pre_build hook cancels every non-tag build, which is correct for the release-only contract. But latest is a branch AND is default_version, so the hook guarantees the surface every visitor lands on can never rebuild. RTD serves default_version at both front doors, and gzkit.org is an RTD custom domain rather than a separate site. The file-level contract and the admin-level contract are each internally coherent and jointly wrong, and nothing in either can observe the other. The stable alias is unaffected because it is version-type tag, which is why the README badge target is the one current surface and why an earlier operator ruling to repoint that badge has been quietly load-bearing.

THE SESSION COMMIT SPLIT IS A STANDING FENCE, NOT A PREFERENCE. gz git-sync fails closed when staged files mix source scope with other artifacts, per the settled "Refuse the bundle (fence at sync)" ruling. The exit bookmarks were therefore committed separately from the source fix rather than bundled with it.

## Decisions Made

- [operator-ruled] Work advised step 2 only and set the other four aside (verbatim selection: "Step 2 only — defect repair, no feature build"), chosen from a four-option picker whose alternatives were the campaign spine (step 1), spine-plus-defect (steps 1+2), and hold-without-working. Booked via gz handoff decide with steps 1, 3, 4 and 5 recorded set-aside.
- [operator-ruled] Commit the four session-exit bookmarks as-is with the session's work (verbatim selection: "Commit them as-is with the session's work"), chosen over folding them into an authored successor, deleting them, and leaving them staged. They carry no authored content, and leaving them untracked orphans a Layer-2 handoff_path in a fresh clone.
- [operator-ruled] Fix the verifier-pipe-gate recovery prose by handing back the caller's own command corrected (verbatim selection: "Hand back the corrected command"), chosen from a four-option picker. The alternatives declined were a prose-only reorder, relaxing the gate to permit filter pipes, and recording the ergonomics as accepted.
- [operator-ruled] Do the RTD fixes (verbatim: "do the RTD fixes"). This reversed the set-aside that advised step 3 had carried, and it was the third session in which that step had been presented.
- [operator-ruled] File a GHI to track the RTD dashboard half rather than leaving it to an immediate manual fix (verbatim selection: "File a GHI to track it"), chosen over no-GHI and over re-measuring immediately on the assumption it had already been flipped.
- [operator-ruled] The verifier-pipe gate friction was worth raising at all (verbatim: "tired of this bullshit:" followed by the block message quoted in full). Recorded as an improvement insight under scope guardrail-recovery-prose per Behavior Rule 11 before the corrected work completed.
- [agent-chose] Derive gate_targets from the entrypoint's import bytecode rather than adding a declared parameter to @enforces. A declared field would have cost the same 71 manual gate readings the operator had already ruled to avoid, merely relocating them from the drain to the authoring pass. Measured 60 of 71 before committing to the approach.
- [agent-chose] Report the derivation as reading the delegation rather than as a third heuristic, and record that distinction in the code, the commit and the GHI closure. The issue explicitly warns off further attempts, so the reason this one is admissible had to be written down or the next reader would reasonably revert it.
- [agent-chose] Leave the 9 subprocess-backed claims unresolved rather than adding a fourth derivation pass to extract their command strings. An empty gate_targets pointing the reader at source_fn is honest and their bodies are one line.
- [agent-chose] Argue against relaxing the verifier-pipe gate predicate when the operator raised the friction, and prove the point by probing a deliberately failing suite through the promoted escape before recommending it. Sycophantic agreement with the relax option would have traded the gate's only property for ergonomics.
- [agent-chose] Split the source fix and the exit bookmarks into two commits rather than one, because the settled "Refuse the bundle (fence at sync)" ruling fails gz git-sync closed on staged files carrying mixed scope.
- [agent-chose] Re-probe the RTD 403 claim rather than inherit it from the handoff. It was false, and it is why the check had been carried unworked three times.
- [agent-chose] Withhold the runtime label on GHI #801. The predicate does not fire: no src/gzkit path, no gz CLI symptom, no fix commit landing under src. Applying it would have injected a false qualifier into gz patch release, which selects on runtime intersected with a src diff.
- [agent-chose] Leave GHI #801 open with a blocker comment rather than routing it to a destination, after verifying in-session that no agent-side route exists: no RTD credential in the environment, the automation-rules endpoint 404s unauthenticated, and RTD's config-file v2 schema has no default_version key.
- [agent-chose] Cross-reference GHI #801 from .readthedocs.yaml in a follow-up commit, so a reader of the runbook finds the tracker rather than only the measurement.
...
5. CONSIDER WHETHER GHI #766 SHOULD BE PULLED. Four mechanical floor bookmarks were committed this session, every section boilerplate and Pending Work reading "Unknown to the writer". They are the live instance of the issue and the second session in a row to produce them.

## Pending Work / Open Loops

GHI #801 IS OPEN BY CREDENTIAL BOUNDARY, NOT BY OMISSION, and carries a blocker comment naming the next concrete operator action plus the closing evidence commands. Verified in-session that no agent-side route exists: no RTD credential in the environment or user config, the automation-rules endpoint returns 404 unauthenticated where the read endpoints return 200, and Read the Docs' config-file v2 schema carries no default_version key. The repo-side half is landed and needs nothing further.

THE 71 DISCLOSED EXEMPTION CLAIMS REMAIN DISCLOSED. GHI #798 [settled] was the mapping gap, not the drain; the drain was routed to WAIT behind it by operator ruling on 2026-08-12 and is now unblocked rather than done. The shrink-only ratchet keeps the list from growing but nothing shrinks it automatically.
...
GHI #766 GAINED FOUR MORE INSTANCES THIS SESSION. The floor bookmarks committed at 419f3ab5c are exactly the constants-only documents the issue describes.

AGENTS.MD IS STILL OVER THE CODEX DELIVERY CAP at 33153 bytes against 32768, with the must-survive architectural-boundaries section straddling the boundary. Tracked on GHI #533; every gz check reports it as an advisory. Note the standing operator ruling that all instructions-file budget work is parked until the product stabilizes, so this is disclosed rather than actionable.

THE VERIFIER-PIPE GATE'S SIBLING SURFACES WERE NOT AUDITED. This session fixed one guardrail whose recovery prose named a higher-friction route than necessary. Whether other fail-closed surfaces carry the same ordering defect was not measured, and .claude/rules/guardrail-feedback-prose.md is explicit that the global three-part shape is advisory with no mechanical witness planned. Any promotion would need observed drift instances rather than a backlog sweep.
...
gh issue view 801 --json number,state                      # OPEN, blocker comment recorded
jq '.accepted_claims | length' data/exemption_control_grandfather.json   # still 71, unblocked but undrained. CORRECTED 2026-08-14 by operator ruling: the form written here was `grep -c undeclared_accepted`, which returns 1 — that string is a measurement key whose VALUE is 71, not a per-entry field, so the check never tested the claim it asserted.
curl -s -o /dev/null -w "%{redirect_url}" https://gzkit.readthedocs.io/   # currently /en/latest/; expect /en/stable/ once GHI #801 is closed
curl -s -o /dev/null -w "%{http_code}" https://gzkit.org/config/gz-cookiecutter-python-stack/   # currently 404; expect 200 once GHI #801 is closed

Note on the first two unittest lines: when tailing their output, prepend "set -o pipefail;" rather than capturing to a file. The verifier-pipe gate permits both, and as of ca39ecb6b its refusal prose hands back the corrected command directly.

## Evidence / Artifacts

Source changed this session:

- `src/gzkit/enforcement.py` — `gate_targets` field and `_derive_gate_targets`
- `src/gzkit/governance/trust_audits/exemption_controls.py` — `_registry_gate_hints` and the gate-naming disclosure finding
- `src/gzkit/verifier_pipe_gate.py` — recovery prose leads with the one-call escape, handed back corrected
- `tests/governance/test_enforces_registry.py` — derivation and live-population regression witnesses
- `tests/governance/test_exemption_controls.py` — gate-hint resolution and finding enrichment
- `tests/hooks/test_verifier_pipe_gate.py` — recovery-prose ordering and paste-readiness
- `.readthedocs.yaml` — corrected admin runbook, measured state, GHI #801 cross-reference
- `.gzkit/insights/agent-insights.jsonl` — improvement insight under scope guardrail-recovery-prose

Surfaces read but not changed:

- `src/gzkit/governance/trust_audits/_qc_nc_entrypoints.py` — the delegation shims whose bytecode the derivation reads
- `src/gzkit/governance/trust_audits/qc_binding.py` — claim registration path
- `data/exemption_control_grandfather.json` — the 71-entry disclosed list
- `.claude/rules/guardrail-feedback-prose.md` — the bar the recovery prose is measured against
- `.claude/rules/task-discovery.md` — the producer-versus-judgment split the derivation follows
- `docs/governance/build-to-1.0-campaign-2026-07-18.md` — Magna Carta sequencing

Commits, oldest first: 5494b4822, 419f3ab5c, ca39ecb6b, 4efe7fce9, 3a6a0771e.
