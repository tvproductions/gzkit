...
4. ADR-0.35.0 remains campaign TOPMOST with closeout BLOCKED; run `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing` for its landed count rather than trusting a figure carried in prose. OBPI-0.35.0-08 reads in_progress solely from a one-way pipeline_launched latch; the lock was released 29 days ago. #930 carries the finding with this session's mechanical evidence. Only the operator can move it.
...
CARRIED, NOT MINE TO MOVE. ADR-0.35.0 with closeout BLOCKED, and OBPI-0.35.0-08 stuck on a one-way latch. Read its landed count with `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing`. GHI #930 owns it and now carries the mechanical evidence: pipeline_launched is set at ledger.py:1154 and cleared by nothing, the lock was released 2026-08-23T13:42:46, and TWO OBPIs are in that state (the other is OBPI-0.47.0-02-owasp-chore-runner).
