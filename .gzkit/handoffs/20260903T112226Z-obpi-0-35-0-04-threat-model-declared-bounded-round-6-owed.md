...
- THE PIPELINE SKILL NEEDS THE THREAT-MODEL FIX, and it is recorded only as an insight, not as a rule edit. A trust-chain OBPI must declare its threat model BEFORE the first Step 4b round, and the Step 4b prompt must state the boundary and forbid out-of-scope findings. Also unrecorded as a rule: the convergence rule and the same-root escalation rule. All three are operator-ruled and live only in `.gzkit/insights/agent-insights.jsonl` — they will not bind a future session until they reach a rule surface.
