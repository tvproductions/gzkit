...
adr_id: null
branch: main
timestamp: '2026-08-20T09:20:34Z'
agent: claude-code
session_id: e685a9d4-dfd0-4dc9-ad4d-b53ef8035169
continues_from: .gzkit/handoffs/20260820T004345Z-agents-md-foundation-kind-sealed-in-source.md
---

## Current State Summary

Session ran three movements: a handoff review, a canon repair, and an operator-authorized PII history sweep that rewrote every commit in the repository.

MOVEMENT 1 — the resumed handoff (20260819T105428Z) was stale by six commits; its advised step 3 was already discharged. The operator set all five advised steps aside and ruled a different item.

MOVEMENT 2 — AGENTS.md taught `foundation` as a live authoring kind five weeks after ADR-0.34.0 sealed it. Repaired through the corpus (never a hand-edit, per REQ-6 of the Completed OBPI-0.34.0-03). Two further defects were then fixed on operator instruction: `gz content commit` reported success without naming the playback writer, and the adopter carve-out keeping `foundation` OPEN for downstream projects had no mechanical witness. A third, pre-existing and date-triggered, surfaced when the pre-push gate went red: two skill-audit tests pinned an exact warning census against calendar-dependent fixture data.

MOVEMENT 3 — the operator's real name and PERSONAL EMAIL were found across history: 270 commits carried the operator's personal address in author/committer metadata, which is the fail-closed clause of the operator-PII rule. Three git-filter-repo passes swept all 3094 commits and 71 tags. main and tags are force-pushed and verified clean. THE SWEEP IS NOT FINISHED — see Immediate Next Steps.

CRITICAL for any resuming agent: EVERY pre-sweep commit SHA is now DANGLING. Confirmed by `git cat-file -e` at authoring time for c2e79d5b1, 7c5f3d9a8 and 92bb60788 — all three fail. Every SHA cited in the ledger, ADR audit proofs, GHI comments, and all prior handoffs is unresolvable.

## Important Context

Five things a resuming agent will otherwise re-derive.

FIRST — the content pipeline is corpus -> compose -> commit -> playback, and the stages have DIFFERENT writers. `gz content commit` writes only the rendition; `gz agent sync control-surfaces` is the sole writer of AGENTS.md and its mirrors. This was invisible until this session and is now stated in the command's own success output and in the content manpage. `agents.local.md` is FROZEN into the rendition, so editing it and running sync alone plays back the stale block — verified, not assumed.

SECOND — the foundation-kind closure is PROJECT-LOCAL. ADR-0.34.0 Positive #8 keeps `gz init` scaffolding adopters OPEN. `src/gzkit/templates/agents.md` must never carry the closure, and there is now a covering test asserting that in both directions. `gz validate --distribution` does NOT protect this: it enforces byte-parity between the two template copies, which an edit to BOTH satisfies.

THIRD — why the PII sweep needed three passes, because the next person will assume one. Pass 1 (`--mailmap` + `--replace-text`) collapsed nine author identities to `g0` and swept blobs. Pass 2 (a blob callback) was needed for a `.pptx` holding the name in `docProps/core.xml`, COMPRESSED inside a ZIP where text replacement cannot reach. Pass 3 (path purge) was needed for `~$complete-series-slides.pptx`, a 165-byte PowerPoint OWNER LOCK FILE storing the local username after a `\x0b` length prefix, which `--replace-text` skipped. `~$*` is now in `.gitignore`.

FOURTH — a measured finding that narrows GHI #834. A bare `git push --force refs/tags/*` moved all 71 tag refs with ZERO change to any release body: all 71 were byte-identical afterward, compared against a snapshot taken beforehand. So #834's overwrite belongs to the release-CEREMONY re-push path, not to moving a tag ref. This may unblock GHI #832 without waiting on #834 — but re-measure before acting on it.

FIFTH — dated measurement, not a standing fact: AGENTS.md is 38480 B against the codex 32768 B delivery cap. The overflow is pre-existing; the must-survive section `operator-doctrine-verbatim-canon` straddles the cap, so under a codex delivery the tail of the operator's own verbatim canon is undelivered. Budget work remains parked by standing ruling.

## Decisions Made

- [operator-ruled] Verify the sealed-kind contradiction rather than work any of the five advised steps (four-option picker).
- [operator-ruled] Fix it in the SOURCE, not the rendered surface (verbatim 2026-08-20: 'AGENTS.md is wrong, but it is rendered, so fix in source').
...
- [operator-ruled] Sweep the name and email from ALL history including sealed artifacts and the ledger (verbatim: 'yes, sweep it'), chosen from a four-option picker over commit-metadata-only, main-only, and not rewriting. The cost of dangling SHAs across the governance corpus was stated in the question and accepted.
- [operator-ruled] Sweep LICENSE too, chosen over preserving the real name in the copyright notice. LICENSE now reads 'Copyright (c) 2026 g0'.
...
- [agent-chose] Rewrote three adjacent Mechanical-enforcement bullets in AGENTS.md under coupled-surface coherence; they would otherwise contradict the new invariant paragraph. Prose the operator did not explicitly authorize.
- [agent-chose] Did NOT re-stamp `gz-content-remember`'s `last_reviewed`. Stamping the date to clear a warning would assert a review nobody performed (skill-surface-sync rule 6).
- [agent-chose] Left records intact in the forward-looking content pass — then the operator's full-sweep ruling superseded that and filter-repo swept them too.
...
1. THE SWEEP IS INCOMPLETE. `gh-pages` — the deployed docs site, one squashed commit — still carries the operator's name in its built HTML on the remote. It is swept LOCALLY; the force-push was refused by the harness permission classifier, so the operator must run it: `git push --force origin gh-pages:gh-pages`. Local is clean, remote is not.
2. Ask GitHub Support to garbage-collect unreachable objects on tvproductions/gzkit. Force-pushing de-references the old commits but does not delete them; anyone who knows a pre-sweep SHA can still fetch one until GitHub collects them.
3. Five `refs/pull/*` refs are IMMUTABLE and still resolve to commits carrying the personal email. Neither force-push nor GC removes those; closing that requires deleting the pull requests.
4. Decide what to do about dangling SHA citations across the ledger, ADR audit proofs, GHI comments and prior handoffs. Rewriting the ledger to match is FORBIDDEN, so this needs an operator ruling rather than an agent fix.
5. `gz-content-remember` (last_reviewed 2026-06-05) becomes a BLOCKING skill-audit error on 2026-09-03. It needs a real review, then a `skill-version` and `last_reviewed` bump in the same edit.
6. GHI #835 remains the live performance thread and its first deliverable is NOT a speedup — establish which `gz check` steps write or depend on another step's side effect first.
7. GHI #813 has now gone untouched across FIVE handoffs.

## Pending Work / Open Loops

- `gh-pages` force-push outstanding — the only unswept remote surface.
- GitHub unreachable-object GC not requested; pre-sweep objects remain fetchable by SHA.
- Five immutable `refs/pull/*` refs still carry the personal email.
...
- GHI #832 open — 22 repairable grandfathered release tags; may be unblocked by the #834 narrowing.
...
- AGENTS.md is over the codex delivery cap and the operator-verbatim-canon section straddles it; budget work parked by standing ruling.
- ADR-pool.documentation-type-templates remains unpromoted.
- The design-references bibliography pool ADR and the controlled-language pool ADR remain unpromoted.

## Verification Checklist

Run these before trusting anything above. Note the exit-code integrity rule: a piped verifier reports the filter's status, so use `set -o pipefail` — a PreToolUse hook enforces it.
...
Expect: the pickaxe search to return NOTHING; the author list to contain only `g0`, `dependabot[bot]` and `Test User`; `gz check` to exit 0; and the plan-create probe to REFUSE with ADR-0.34.0 guardrail prose — that refusal is the pass condition, being the ADR's own Fidelity Assertion with expected exit 1.

To confirm the sweep gap is still open, compare `git rev-parse gh-pages` against `git rev-parse origin/gh-pages`; they diverged at authoring time and converge once step 1 is run.

At authoring time 3094 commits and 71 tags survived the rewrite, the full suite was 8523 tests OK, and a pre-sweep all-refs backup bundle plus the pre-sweep HEAD were written to the session scratchpad. That backup is EPHEMERAL and will not survive the session.

## Evidence / Artifacts

- `.gzkit/corpus/AGENTS.md.jsonl` — the foundation-closure entry, tier invariant, classification Mechanical, witness g0
- `AGENTS.md` — the played-back result; the table row, the verbatim invariant paragraph, and the three rewritten enforcement bullets under § Kinds
- `.gzkit/agents.local.md` — the PII rule, reworded to name the substitute (`g0`) instead of exhibiting a specimen of the forbidden class; this example was the seed for 93 occurrences across 27 files
