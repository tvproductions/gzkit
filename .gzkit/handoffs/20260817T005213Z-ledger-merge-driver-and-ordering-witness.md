...
adr_id: null
branch: main
timestamp: '2026-08-17T00:52:13Z'
agent: claude-code
session_id: 22b53689-8a07-493d-9a9a-072ae67ce924
continues_from: .gzkit/handoffs/20260816T213401Z-prose-as-state-discharged-and-measured.md
---

## Current State Summary

Worked two GHIs end-to-end from a single incident and closed both. The session
opened as a routine `/git-sync`, which hit a rebase conflict in
`.gzkit/ledger.jsonl` — 44 upstream appends against 1 local append, disjoint.
Resolving it by hand surfaced two distinct defects, filed as GHI #811 (no merge
driver, so every concurrent sync forces a hand-edit) and GHI #812 (the ledger's
timestamp ordering was a de-facto invariant with no witness).

Both are now CLOSED/COMPLETED. GHI #812 landed first as commit `50ca56fb` —
`validate_ledger` gained the cross-row phase it never had. GHI #811 landed as
`e7cc132a`, adding `gz ledger merge-driver` plus `.gitattributes` wiring and
self-healing registration. Branch `main` is synced (ahead=0 behind=0, clean);
full unit tier is 8436 tests green; no active OBPI locks.

Sequencing was load-bearing: #812 is the witness that makes #811 safe, because
git's built-in `merge=union` appends without sorting and would have written a
descending pair silently.

## Important Context

The two issues are coupled in the fix direction, not merely in origin. The
obvious one-line fix for #811 (`merge=union` in `.gitattributes`) is WRONG:
union concatenates each side's unique lines without ordering them, and in the
incident that surfaced this, the local append (ts 01:14:20) preceded all 44
upstream appends (01:47:37 through 21:23:55). Union would have produced a
descending pair that, before #812, every gate called clean.

Two measurements shaped the work and should not be re-derived:

- `id` uniqueness does NOT hold on the ledger — 2577 of 15037 ids repeat,
  because `id` names the ARTIFACT, not the event (`AGENTS.md` appears 64
  times). GHI #812's body proposed it as an obvious companion check; enforcing
  it would fail every real ledger. The cross-row phase deliberately enforces
  ordering only.
- Ordering must compare parsed INSTANTS, not strings. `10:00:00+01:00` sorts
  lexically after `09:30:00+00:00` while preceding it in real time, so a string
  comparison fails a correct ledger. `test_ordering_compares_instants_not_strings`
  pins this.

Registration is the non-obvious constraint on #811: git reads a merge driver
COMMAND from local config only, and config cannot be committed. So
`.gitattributes` can ship an attribute with no driver behind it. `gz git-sync
--apply` seeds it idempotently before rebasing (the ceremony that trips the
conflict is the one place guaranteed to run in every clone that can hit it,
including clones predating the fix, which `gz init` can never reach), and
`gz init` covers new clones.

`gz ledger merge-driver` exits 1 rather than guessing whenever the sides are
not plain appends. Refusing never destroys evidence; a wrong auto-merge does.

## Decisions Made

- [operator-ruled] Work both GHIs to completion (verbatim: "yes, do it, work
  the ghi magic"), after the two blocker rulings were left open.
- [agent-chose] GHI #812 fails CLOSED rather than warning for one release.
  Rationale: consistency with every other `--ledger` error, and a warning
  nobody reads is a witness in name only. Rejected the warn-then-fail
  alternative because the most likely source of an adopter inversion is the
  hand-merge #811 describes, so a grace period tolerates damage this repo's own
  missing driver caused. Flagged to the operator as an agent pick, not a ruling.
- [agent-chose] GHI #811 resolved as a real `gz` verb plus self-healing
  registration, rather than a documented setup step. Rationale: the defect is
  that AGENTS.md § Never #2 forbids hand-editing the ledger AND no `gz` command
  could do it instead; a loose script would have fixed the conflict without
  discharging that contradiction. Rejected `merge=union` outright (unsafe) and
  a docs-only setup step (relies on humans reading docs).
- [agent-chose] The cross-row phase enforces ts ordering ONLY, not id
  uniqueness, decided by measuring the live ledger rather than by assumption.
- [agent-chose] Scoped the `.gitattributes` rule to the whole class
  (`agent-insights.jsonl` and `corpus/*.jsonl` alongside the ledger) rather
  than the one file that conflicted.
- [agent-chose] Recorded the `.claude/rules/cli.md` conflict via
  `gz insights remember` rather than filing a third GHI, since it needs an
  operator ruling rather than repair. Surfaced rather than resolved.

## Immediate Next Steps

1. Rule on the `.claude/rules/cli.md` conflict recorded in
   `.gzkit/insights/agent-insights.jsonl` (type `defect`, scope
   `.claude/rules/cli.md`). That rule states a new CLI subcommand is Heavy lane
   and "runs `gz obpi pipeline`, not a freeform direct fix", requiring an ADR or
   brief. AGENTS.md § Operator Doctrine states GHI-tracked defect repair routes
   to direct fix "regardless of the OBPI ceremony required when ANY hold
   criteria". Canon pre-adjudicates and is the higher surface, so GHI #811 [settled]
   landed direct — but `.claude/rules/cli.md` still routes every new subcommand
   to ceremony, and the next agent hits the same wall. Decide whether that rule
   gains a GHI-repair carve-out, or whether ceremony overrides canon for CLI
   surface.
2. Decide whether the adopter-facing arm of GHI #812 [settled] needs a release note. The
   ordering check ships in the wheel and runs in every adopter's `gz check`; an
   adopter carrying a pre-existing inversion goes red on upgrade. gzkit's own
   ledger measures clean, so nothing surfaces locally.
3. Resume the Build-to-1.0 campaign at its topmost unchecked item (Movement B,
   calibrate before widening). This session was defect repair surfaced in
   flight, not campaign work, so the campaign position is unchanged.
4. Consider whether `gz init --update` should re-seed the merge driver on
   existing adopter clones. Currently only full `gz init` and `gz git-sync
   --apply` register it; the `--update` refresh path does not.

## Pending Work / Open Loops

- The `.claude/rules/cli.md` versus AGENTS.md § Operator Doctrine conflict is
  recorded but unruled. Tracked in `.gzkit/insights/agent-insights.jsonl`, not
  as a GHI, because it needs an operator ruling rather than repair.
- Adopter blast radius for the GHI #812 [settled] ordering check is accepted but
  unannounced. No release note has been authored; `RELEASE_NOTES.md` is
  authored at release time by the `gz-patch-release` ceremony, never by hand.
- `gz init --update` does not seed the merge driver (only full `gz init` and
  `gz git-sync --apply` do). Whether that matters depends on how adopters
  refresh; not investigated.
- Campaign Movement B remains at its pre-session position. Nothing in this
  session advanced or blocked it.
- Three pre-existing suite blockers print during runs and are unrelated to this
  work (declared Sunset prerequisites ADR-0.0.37 / 0.0.54 / 0.0.64 / 0.0.65 /
  0.0.72 with no foundation package on disk). They do not fail the suite.
...
```bash
# Both GHIs closed, with the commits that closed them
gh issue view 811 --json number,state,stateReason
gh issue view 812 --json number,state,stateReason
git log --all --grep="GHI #811" --format="%H %s"
git log --all --grep="GHI #812" --format="%H %s"

# Tree and branch state
uv run gz git-sync            # expect ahead=0 behind=0 diverged=False dirty=False
uv run gz obpi lock list      # expect: No active locks.

# The ordering witness actually bites (the point of GHI #812)
uv run gz validate --ledger

# The merge driver is registered in this clone and idempotent
git config --get merge.gzkit-jsonl.driver
uv run gz git-sync --apply    # expect NO re-registration warning on a second run

# Full gates
uv run gz cli audit                        # expect 135/135 fully covered
uv run gz validate --documents --surfaces --distribution --cli-alignment --skill-alignment --ledger --line-endings
uv run -m unittest discover -s tests -t .  # expect 8436 tests OK
```

Do not cite a piped verifier run as evidence: the `verifier-pipe-gate` hook
refuses it, and the shell would report the filter's exit rather than the
verifier's. Capture to a file and read the real exit, or cite the ARB receipt's
`exit_status`.
...
- `50ca56fb` — fix(validate): reject a ledger whose timestamps run backwards (GHI #812)
- `e7cc132a` — fix(ledger): merge concurrent append-only JSONL instead of hand-editing it (GHI #811)

Source authored or changed:

- `src/gzkit/ledger_merge.py` (new) — the three-way append-only merge
- `src/gzkit/commands/ledger.py` (new) — git-facing adapter and driver registration
- `src/gzkit/validate_pkg/ledger_check.py` — cross-row ordering phase; `parse_ledger_ts` made public so driver and validator provably share parsing
- `src/gzkit/commands/sync.py` — `_register_jsonl_merge_driver` seeded before rebase
- `src/gzkit/commands/init_cmd.py` — seeds the driver for new clones
- `src/gzkit/cli/parser_maintenance.py` — `gz ledger merge-driver` registration
- `src/gzkit/cli/parser_handler_manifest.py` — lazy handler entry

Tests:

- `tests/test_ledger_merge.py` (new) — 7 tests over the merge contract
- `tests/test_ledger_merge_driver.py` (new) — adapter and registration idempotence
- `tests/test_validate.py` — 4 new ordering tests in `TestValidateLedger`

Surfaces:

- `.gitattributes` — `merge=gzkit-jsonl` for the three append-only JSONL paths
- `docs/user/manpages/ledger-merge-driver.md` (new)
- `docs/user/manpages/index.md`, `docs/user/runbook.md`, `docs/governance/governance_runbook.md`
- `config/doc-coverage.json` — declares the new command
- `.gzkit/skills/git-sync/SKILL.md` — skill-version 1.3.0, wields the new verb
- `.gzkit/insights/agent-insights.jsonl` — records the unruled conflict in `.claude/rules/cli.md`

Receipt:

- `artifacts/receipts/arb-step-unittest-b9fd1945f3aa4452bcb63532a628e5fa.json` (exit_status 0, 8426 tests at emission)
