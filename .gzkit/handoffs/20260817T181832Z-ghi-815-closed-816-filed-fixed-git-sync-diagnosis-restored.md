...
adr_id: null
branch: main
timestamp: '2026-08-17T18:18:32Z'
agent: claude-code
session_id: 1fc95331-a12d-4cbb-a6b6-f4fe450bf27e
continues_from: .gzkit/handoffs/20260817T172834Z-ghi-815-reprioritised-closed-superseded-witness-prose-repaired.md
---

## Current State Summary

Supersedes `20260817T172834Z-ghi-815-reprioritised-closed-superseded-witness-prose-repaired.md`, which was authored before GHI #816 existed and therefore understated the session. Nothing in it is wrong; this document adds the arc it could not carry.

Resumed `20260817T130147Z`, booked `proceed` with advised step 2 only (steps 1, 3, 4, 5 `--set-aside`). Worked step 2, then three operator rulings extended the session well past the handoff scope.

Four commits, all synced, `origin/main` `0 0`, tree clean:

- `b20b210e4` — GHI #815 re-prioritisation recorded into `ADR-pool.render-order-truncation-survival` on a live re-measurement
- `01daaf8ae` — the two consumers that cited the now-closed #815 as their live tracking record, repaired
- `3791b1e7d` — the predecessor handoff
- `94997208f` — GHI #816 filed and direct-fixed: `gz git-sync` no longer discards a refusing hook's diagnosis

Two GHIs reached terminal state: **#815 closed `superseded`** into the pool ADR; **#816 filed and closed `fixed`** in the same session under the standing direct-repair grant.

No ADR or OBPI advanced. The campaign spine has now been set aside for two consecutive sessions.

## Important Context

**The GHI #816 defect was one underscore, twice.** `sync.py` bound `git_cmd`'s stdout to `_out_push` and `_out_commit` — Python's convention for *deliberately unused* — and appended only stderr to `blockers`. Hook frameworks write their check report to stdout; git writes a one-liner to stderr. So a pre-push refusal surfaced as thirteen lines naming no gate, while the same operation as a bare `git push` returned ~150 lines naming the failing check, its file, its line, and its recovery step. Nothing was broken: a name announced an intention and no reader questioned it. The lesson generalises — an underscore prefix is a claim about intent that no checker verifies.

**Nothing global could have caught #816, and that is deliberate.** `.gzkit/rules/guardrail-feedback-prose.md` § Enforcement posture rejects a global prose-grader by name as *"grading by shape"*, and enforces the three-part bar **per-surface, in each surface's own covering test**. `gz git-sync` simply never had that assertion. When adding a fail-closed surface or a wrapper around one, the covering test is where the bar lives — do not go looking for a scope flag that enforces it.

**Closing a GHI is a code change when a validator cites it.** `surface_delivery_witness.py` told every `gz check` run *"the live record is GHI #815"*. The instant #815 closed, a fail-closed surface pointed agents at a terminal artifact. The repair was made at the class: the prose now asserts that NO issue is live (*"if no OPEN GHI covers this exceedance, file one — resolve before citing, never assume"*). Substituting a fresh issue number would have rebuilt the trap for the next close.

**Two consumers of one closed issue took OPPOSITE correct treatments.** Runtime prose must be rewritten; a dated record must be appended to. `data/instructions_files_budget.json` states its own convention — *each dated entry states the values true on its date; the LAST entry is what is enforced today* — and already leaves four stale `ADR-0.0.37` mentions untouched on that principle. Editing its 2026-08-17 entry to correct the `#815` pointer would have falsified the record the convention depends on.

**A number transcribed from a markdown doc was stale by 2096 B, and re-deriving it strengthened the case.** Must-survive was carried as `21582 B` from GHI #580's July ratification; live measurement gave `23678 B`. The load-bearing fact — that exactly ONE 595 B section is the entire breach — only appears when per-section offsets are computed against the live surface, never from a cumulative total.

**The pre-push gate caught a defect the agent authored, in the class fixed one commit earlier.** The first draft of the predecessor handoff transcribed live ADR counts; `gz validate --transcribed-adr-counts` refused the push. `c7aa2888d` — the commit immediately before this session opened — had fixed that exact class. The validator flags only the newest handoff and named one line; a second authored instance in another section was the same defect and was removed too. Logged as a `defect` insight under scope `handoff-authoring`.

**The reorder fix is heavier than an earlier turn presented it.** `gz content compose` takes an agent-supplied candidate and exposes NO render-order control; `Pillar.order` carries document order for round-trip fidelity, and `gz validate --invariant-coherence` byte-compares playback against committed `AGENTS.md` in the default `gz check` scope. The fix is a hand-permutation of Layer-1 canon, possible only because the recompose ceremony commits surface and rendition together. Do not re-offer it as a flag.

**The survival declaration declares exactly one surface.** `data/agents_md_survival_declaration.json` covers `AGENTS.md` only, and `test_absent_declaration_is_not_a_finding` confirms an undeclared surface is silently unmeasured BY DESIGN. Recorded as the one empty right cell in #815's close-comment cause-to-witness table rather than papered over.

## Decisions Made

- [operator-ruled] Explain the `gz git-sync` diagnostic defect rather than let the closing note stand unexamined (verbatim: "explain: One thing I did not do, and it's a judgment worth your eye: gz git-sync dropping the blocker's cause is a real defect..."). The explanation was grounded by reading `sync.py` and `git_cmd`'s return contract rather than reasoning from the symptom, which is what surfaced the second blind call site.
- [operator-ruled] File the defect as a GHI and repair it directly (verbatim: "yes, give directly via ghi"), read against operator canon that a GHI is itself the work order and the receipt and needs no further ceremony. Produced GHI #816, commit `94997208f`, closed `fixed` the same session.
- [operator-ruled] Refresh the handoff so the record carries the #816 arc (verbatim: "yes, refresh"), authored as a successor rather than an edit because a committed handoff is a sealed record.
- [agent-chose] Fixed BOTH hook-bearing call sites rather than only the push that was observed failing. `git commit` (pre-commit) was equally blind, and a push-only fix would have been the instance-not-class failure DO IT RIGHT #1 names.
- [agent-chose] Wrote three negative controls into the #816 test set — stderr survives, stderr-only unchanged, both-streams-empty falls back — which passed at RED and so are demonstrably not vacuous. They would catch a fix that swapped or replaced the streams rather than combining them.
- [agent-chose] Left the three non-hook-bearing `git_cmd` sites (`fetch`, `pull`, `add -A`) discarding stdout, and said so in the close comment rather than folding a second finding in silently. `git pull` is the one with residual value: merge-conflict detail goes to stdout.
- [agent-chose] Removed BOTH transcribed-ADR-count instances from the predecessor handoff when the gate flagged only one, and left the carried Settled Rulings' figures untouched — they are sealed records of their own sessions, and the validator's own recovery prose forbids rewriting a historical count to match today.
- [agent-chose] Authored this successor rather than amending the predecessor, so the chain stays append-only and `continues_from` remains a true lineage.

## Immediate Next Steps

1. **Resume the campaign spine.** `ADR-0.35.0-canon-entry-corpus-landing` is the reachable topmost item and has been set aside two consecutive sessions. Read its lifecycle and OBPI figures from `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing`; this line deliberately transcribes no count. Its § Decision 3 is the destination for BOTH the budget-stay exit condition and GHI #533's registry-projection migration, so two parked items move when it lands.
2. **Rule on whether the not-in-force gap needs its own tracker.** The Codex cap breach is live with NO open GHI: `AGENTS.md` renders 1586 B past the cap and `architectural-boundaries` — six binding "Do not" statements — is undelivered. Filing a successor was offered at the #815 [settled] close and declined. The witness still fires on every `gz check`, so this is a question about tracker visibility, not detection.
3. **Consider declaring `CLAUDE.md` in the survival declaration.** `data/agents_md_survival_declaration.json` declares only `AGENTS.md`, so any other control surface crossing a vendor cap is silently unmeasured by design. Not live — `CLAUDE.md` is 2554 B against 32768 B — but it is the one empty cell in #815 [settled]'s cause-to-witness table.
4. **Consider surfacing `git pull` conflict detail.** The #816 [settled] fix covered both hook-bearing sites; `git pull` at `src/gzkit/commands/sync.py` still discards stdout, where a merge conflict writes its file list. No hook fires there so no recovery prose is lost, which is why it was scoped out rather than folded in.
...
- **The Codex vendor-cap breach is LIVE and has no open tracker.** `AGENTS.md` renders 34354 B against the 32768 B cap; the must-survive section `architectural-boundaries` begins at byte 33759, wholly past the cap rather than straddling it. By the witness's own doctrine a declared-unrecoverable section that is not delivered is not in force. What still watches it: `surface_delivery_witness` fires on every `gz check`, and `ADR-pool.render-order-truncation-survival` carries the measurement.
- **`git pull` still discards stdout** at `src/gzkit/commands/sync.py`, so merge-conflict file lists do not reach the blocker. Deliberately out of GHI #816 [settled]'s scope — no hook fires there, so the guardrail-feedback-prose invariant is not violated — and untracked by any GHI.
- **GHI #533 remains open, ranked second, and its body's blocker is stale.** It names `ADR-0.0.37` as still mid-implementation; that ADR has reached a terminal `Validated` lifecycle and the dependency relocated to `ADR-0.35.0` § Decision 3, still Pending with briefs in draft. Read both from `uv run gz adr status <ADR-ID>`. Corrected in a routing comment; the issue body was deliberately not rewritten.
...
- **`OBPI-0.33.0-06` attestation question stays deliberately undecided**: it reached `attested_completed` citing a test that does not exist, and whether that is a defective attestation or an honest one with a false supporting sentence has no ruled scope.
...
Expected, each observed at authoring rather than assumed: bare `validate` reports 13 scopes passed; `--instructions-files-budget` exits 0 while printing two `[advisory] WARNING` lines about the Codex cap (exit 0 WITH warnings is the ruled posture — a non-zero exit there is a regression of the budget stay); `--transcribed-adr-counts` exits 0; the three test modules run 31 tests and exit 0; `gz adr status` reports its own lifecycle and OBPI count, which this checklist deliberately does not transcribe; the branch count is `0 0`.

Two witnesses worth reading rather than merely running. First, the delivery advisory text must contain "If no OPEN GHI" and must NOT claim any issue is the live record — that is the GHI #816-adjacent repair from `01daaf8ae`. Second, `tests/commands/test_sync_hook_diagnosis.py` carries three negative controls that passed at RED; if a future change makes all six pass trivially, the controls have gone vacuous and the suite no longer proves the fix.
...
- `src/gzkit/commands/sync.py` — `_hook_failure_detail` added; both hook-bearing call sites (`git commit`, `git push`) now keep the refusing hook's stdout
- `tests/commands/test_sync_hook_diagnosis.py` — 6 tests, three of them negative controls that passed at RED
- `docs/design/adr/pool/ADR-pool.render-order-truncation-survival.md` — § Intent carries two dated measurement records with authorities cited instead of a bare "do not re-derive" number; § Decision records the fired trigger and the promotion fence
...
- `.gzkit/handoffs/20260817T172834Z-ghi-815-reprioritised-closed-superseded-witness-prose-repaired.md` — the predecessor this refreshes
- `.gzkit/insights/agent-insights.jsonl` — one `defect` insight under scope `handoff-authoring`
- `tests/governance/test_surface_delivery_witness.py` — 15 tests, the detection-arm witnesses cited in #815's close comment
