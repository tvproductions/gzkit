...
adr_id: null
branch: main
timestamp: '2026-08-09T17:42:58Z'
agent: g0
session_id: 970da8fc-e56e-4287-b30f-45b5974353d7
continues_from: .gzkit/handoffs/20260809T170440Z-sloc-cleared-gate-wired-critic-staged.md
---

## Current State Summary

Successor handoff authored at operator request to re-prioritize around the four GHIs this session filed, after the operator's assessment: "this is whack-a-mole, one step forward, four steps back." Predecessor `20260809T170440Z-sloc-cleared-gate-wired-critic-staged.md` carries the full work record; this document supersedes its Immediate Next Steps ordering only.

THE WHACK-A-MOLE READING WAS TESTED AGAINST THE DATA AND DOES NOT HOLD IN AGGREGATE, THOUGH IT HOLDS FOR TODAY. Measured 2026-08-09 via `gh issue list`:

  open right now      10
  last 30 days        110 opened, 145 closed   net -35
  last 60 days        186 opened, 214 closed   net -28

The backlog is shrinking substantially. Today alone was 1 closed and 4 filed, which is where the feeling comes from, but NONE of the four is a regression introduced by this session. #783, #784 and #786 are pre-existing defects that were invisible until a gate or a probe exposed them; #785 is the class behind the defect that WAS closed (#782). The count rose because detection improved, not because breakage did.

WHERE THE INSTINCT IS CORRECT, AND IT MATTERS FOR SEQUENCING: #785 is the one item that genuinely will breed more filings. Its remedy makes dark gates visible, so its first honest run surfaces findings nobody has seen. That is exactly why it leads the queue — it is the only one of the four that converts an unbounded stream of surprise defects into a finite, counted, shrink-only list. Every other item is instance work.

WORK LANDED THIS SESSION (seven commits, all pushed, `origin/main` 0/0, tree clean): all four module-SLOC breaches cleared by reduction with the grandfather file untouched (`c3d9a99a0`); the shrink-only ratchet given an automatic caller at `gz check` step 4 of 53, with QC-registry enrolment and a negative control (`59931cb07`); GHI #782 closed `fixed` on a fourth option none of its three arms listed (`eba0b0828`), then the same shape hunted across all 39 chores and the AST detector widened before the last two greps were retired (`96f5c0d77`); the campaign amended twice to pull the cross-family critic ahead of `ADR-0.35.0` and then stage its delivery on a `PERFORATED-BUT-NARROWABLE` adversary re-run (`99d98db45`, `e4c8eff44`).

## Important Context

WHY #785 LEADS THE QUEUE, STATED SO THE NEXT SESSION DOES NOT RE-ARGUE IT. Three of the four new GHIs are instance work: a fixed count of files to prune (#783), one frontmatter line to rule (#784), one design premise to re-scope (#786). #785 is the only one whose subject is the DISCOVERY MECHANISM itself. Every reachability mechanism in the repo polices its own membership — the QC registry fail-closes on an unclassified step already in `_build_check_steps()`, the enforcement floor fail-closes on an enrolled claim with no negative control, the default-tier fence fail-closes on a default-tier scope outside the gate. Each is sound; none can ask "what exists that is in none of us?", and that is the population both of this week's observed instances came from. Until something asks that question, uncalled gates keep arriving one accident at a time, which is precisely the churn the operator named.

EVERY REAL FINDING THIS SESSION CAME FROM A GATE REFUSING THE AGENT, NOT FROM ITS OWN READING. The `verifier-pipe-gate` refused a piped `unittest` twice, before a false green could be read back. The QC registry refused to build for an unclassified `gz check` step and named the remedy in its own exception. `gz validate --transcribed-adr-counts` caught a transcribed OBPI count typed into the campaign plan. The chore's own criterion 5 caught a regression a targeted test run had passed. The agent's confident claims were the unreliable input; the mechanical witnesses were the correction. This is the anti-vibing thesis working, and it is also why the count of open findings rises when the machinery improves.
...
- [operator-ruled] Author a successor handoff prioritizing the newly filed GHIs (verbatim: "write handoff prioritizing these new GHIs - this is whack-a-mole, one step forward, four steps back."). The ordering rationale is recorded in Immediate Next Steps; the churn assessment was tested against measured issue data rather than accepted or dismissed.
- [agent-chose] Led the queue with GHI #785 rather than with the campaign's topmost item. The operator's stated pain is the pattern, and #785 is the only one of the four that addresses the pattern rather than an instance. ALTERNATIVE ORDERING, if the campaign's sequencing authority is read strictly: #786 first, because it gates the critic promotion which the campaign now names topmost. Both orderings are defensible; the operator can flip them by ruling.
- [agent-chose] Reported the measured backlog trend (net -35 over 30 days, 10 open) rather than agreeing with the whack-a-mole framing. Agreeing would have been sycophantic and would have mis-sequenced the queue toward instance work; dismissing it would have missed that the instinct correctly predicts #785's behaviour.
- [operator-ruled] Work all four live advised steps of the resumed anchor (verbatim: "All four steps — 2, 3, 4, 5 in order"), selected from a four-option picker. Advised step 1 was recorded set-aside as already discharged.
- [operator-ruled] GHI #782 takes a fourth option the issue body never listed — delete criterion 6 as redundant with criterion 3 (verbatim selection: "Delete criterion 6 — redundant with criterion 3").
- [operator-ruled] Route the pre-existing `--sensitivity` red to a GHI rather than fixing the brief in-session (verbatim selection: "File a GHI alongside step 5").
- [operator-ruled] Amend the campaign and pull the cross-family critic ahead of `ADR-0.35.0` (verbatim selection: "Amend the campaign — pull the critic now").
...
- [operator-ruled] Stage the critic's delivery while keeping the pull-ahead (verbatim selection: "Amend to staged delivery, keep the pull-ahead"), on the adversary's `PERFORATED-BUT-NARROWABLE` verdict.
...
- [agent-chose] Kept the six presence-greps in `agents-md-architectural-boundaries`. They are the SOLE witness for the six Architectural Boundaries sentences, which are not corpus-owned, and a presence assertion cannot mistake narration for an instance.

## Immediate Next Steps

Ordered by leverage against the pattern, not by issue number. The operator's stated concern is churn — "this is whack-a-mole, one step forward, four steps back" — so the class item leads and the instance items follow.

1. GHI #785 — THE ONLY ITEM THAT ENDS THE PATTERN. Nothing in the repo asks which gates have no automatic caller, so uncalled gates are discovered one accident at a time. Measured on `main`: 88 registry scopes, 13 default (all reachable via the bare invocation), 75 explicit, 41 explicit UNREACHED, plus 2 of 3 chore gate scripts with no caller. Build the INVENTORY AND DISCLOSURE first, not enrollment: make "this gate has no automatic caller" a counted, visible fact with an accepted-list that can only shrink, on the `data/module_size_grandfather.json` pattern. EXPECT ITS FIRST HONEST RUN TO SURFACE MORE FINDINGS — that is the point, not a setback. It converts an unbounded stream of surprise defects into a finite list that can only get shorter. Do NOT read 41 as a remedy size: several scopes are deliberately explicit because they are expensive or single-artifact scoped, and which of them deserve callers is a separate per-scope ruling that must not block making the count visible.

2. GHI #786 — CAMPAIGN-CRITICAL; it gates the topmost item. The critic ADR's R4 rules the shipped Anthropic plugin already supplies the adversary transport. It does not: `adversarial-review` carries branch diffs, not decisions, and the `codex:codex-rescue` forwarder returns nothing on failure. Until this is scoped, the campaign's topmost item is building on a false premise. The remedy is a gzkit-owned surface that carries a decision to the adversary and a verdict back — which is an argument FOR the skill R2 ruled, and it weakens the adversary's own strongest no-build argument.

3. GHI #783 — bounded and the class fix is known. 71 `runtime_state` proof files across 29 slugs ship in the wheel. Deletion alone fixes the instance; the class fix is a converging sync that prunes `runtime_state` from the package side, or a distribution check that asserts absence rather than exempting presence. Without one, the next `gz chores run` writes a `CHORE-LOG.md` and the count climbs again.

4. GHI #784 — one frontmatter line, but the authoring call is not this session's to make. `OBPI-0.35.0-02-content-withdraw-verb.md` omits `sensitivity` while its Allowed Paths intersect `ledger_integrity`. Either declare `sensitivity: security` or narrow the paths; `src/gzkit/cli/**` is a broad glob and may be the real overlap cause. The brief belongs to `ADR-0.35.0`, so the decision arguably belongs with that ADR's own work.

5. ONLY THEN the critic promotion authoring. `uv run gz adr promote` is fail-closed today because the pool ADR has no `## Target Scope` and no `## Proposed OBPI Decomposition` table. Decompose against the THREE DOORS (operator, agent, gate), not the hook — the skill is the unit and the `PreToolUse` adapter is one OBPI that must land DARK. Carry the staged shape the campaign box requires. Target `--semver 0.36.0 --kind feature --lane heavy`; `0.36.0` is free. Do NOT hand-wire a hook; that option was explicitly declined.

## Pending Work / Open Loops

- THE OPEN-GHI COUNT IS THE WRONG INSTRUMENT FOR THE QUESTION IT KEEPS GETTING ASKED. It cannot distinguish "we broke four things" from "we found four things that were already broken", and this session was entirely the second. Nothing in the repo tracks defect RATE or time-to-detection, so the only visible signal is a count that rises whenever detection improves — which punishes exactly the work that reduces risk. UNFILED; needs an operator ruling on whether a discovery-versus-regression distinction is worth instrumenting, or whether the aggregate trend (net -35 over 30 days, 10 open) is sufficient.

- THE RATCHET DOES NOT TIGHTEN ON SHRINK, AND THIS SESSION CREATED THE SLACK. `parser_maintenance.py` fell from 1667 to roughly 1388 SLOC while its grandfather entry still records 1582, so it may regrow about 194 SLOC without tripping. The gate's `_doc` says the LIST shrinks, not that recorded values tighten, and `0a2672562` set the precedent of not touching the file on shrink. UNFILED; needs a ruling on whether a shrink should re-record the lower ceiling.
...
- GHI #785 SUBSUMES AN OLDER UNRULED LOOP carried across several handoffs: "54 of 89 registered validator scopes bind no scorecard row. Needs an operator ruling on whether the inverse direction gets an owner." Same inverse-direction question on the scorecard surface rather than the gate surface. Work them together.

- `ADR-0.35.0-canon-entry-corpus-landing` IS NOW SECOND IN THE QUEUE, not first. It is `Draft` and unstarted, so the pull-ahead exchanged which feature is in flight rather than running two; the one-feature-at-a-time ruling is NOT relaxed.
...
Reproduce the backlog trend that this handoff's priority ordering rests on:

```
gh issue list --state open --limit 200 --json number --jq 'length'
gh issue list --state all --search 'created:>=2026-07-10' --limit 300 --json number --jq 'length'
gh issue list --state closed --search 'closed:>=2026-07-10' --limit 300 --json number --jq 'length'
```

Expect roughly 10 open, 110 opened and 145 closed over the trailing 30 days. If the open count has climbed materially, the ordering below should be re-argued rather than inherited.
...
GHI state, which the priority ordering assumes:
...
Reproduce the GHI #785 measurement (88 scopes, 13 default, 75 explicit, 41 explicit unreached) by counting `VALIDATOR_REGISTRY` entries by tier and matching them against the `gz validate --<flag>` string literals in `src/gzkit/quality.py`. The coupling is a string literal, not a function call, so a name-match against the registry finds nothing.

Confirm the module-size gate is genuinely wired rather than merely present, and that its negative control has teeth:
...
Predecessor handoff superseded by this one for ordering only (its work record stands):

- `.gzkit/handoffs/20260809T170440Z-sloc-cleared-gate-wired-critic-staged.md`
