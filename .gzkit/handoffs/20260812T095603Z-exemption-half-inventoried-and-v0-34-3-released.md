...
adr_id: null
branch: main
timestamp: '2026-08-12T09:56:03Z'
agent: claude-code
session_id: 10d8768c-a612-4445-a08e-a7416f4273e2
continues_from: .gzkit/handoffs/20260812T073455Z-three-defects-fixed-selection-drift-banner-and-false-facade.md
---

## Current State Summary

Released v0.34.3 (12 user-visible GHIs) and then closed four GHIs of one family, all landed and synced at 1e0459325; origin/main...main is 0 0, tree clean. The session opened as a patch release and became a hunt: reading the commit log against the release enumeration by hand found two discovery gaps that had silently dropped three GHIs from the release. Repairing those surfaced the same shape twice more, and the operator ruled it a class worth its own instrument. Landed: cf15c9102 (multi-GHI subject regex), 92b879128 (the v0.34.3 release), 5e164e76d (unclassified_reference bucket, GHI #794), 53258c56b (resume-gate booking coupling, GHI #795), 980dd2ad3 (verifier-pipe escape must be used not named, GHI #796), bfa247daa and d2f74b3d5 (the exempts declaration and its inventory scope, GHI #797), 1e0459325 (Read the Docs badge). Every gz check in the session exited 0; the final one runs 55 steps with Exemption controls green at 46/55.

## Important Context

THE FAMILY IS SHARPER THAN 'A MECHANISM ASSERTS A COUPLING IT DOES NOT ENFORCE'. That framing describes half the advisory scorecard and cannot be built on. The checkable claim is GATE BUGS LIVE IN THE EXEMPTION, NOT IN THE RULE, AND THE ENFORCEMENT FLOOR TESTS ONLY THE RULE. Measured at the cutover: 28 source files carry an exemption surface, 55 negative controls were registered, 0 exercised an exemption. GHI #795 and #796 are the proof — handoff-resume-unauthorized-write, handoff-resume-unauthorized-bash and verifier-exit-status-masked were registered, enrolled and PASSING on every gz check for the entire life of both holes, because they assert the rule and never touch the exemption. THE OBVIOUS FIX FOR #795 WAS ACTIVELY HARMFUL AND THE FENCE SAYS SO. Comparing handoff_path inside _lifts_the_gate is per-handoff arming reached from the other direction: the instant any new handoff becomes newest_handoff mid-session (a completion record per GHI #619, an exit bookmark, a checkpoint) an already-granted clearance stops matching and the gate re-arms against a session the operator cleared minutes earlier (GHI #755). The coupling therefore lands at BOOKING time, and LiftTimeStaysSessionScopedTests is the fence that keeps a future reader from adding the equality check. Both function docstrings cross-reference the reason. 71 OF 76 ENFORCEMENT CLAIMS REMAIN UNDECLARED. That is a disclosed backlog, not coverage. The ratchet in data/waiver_ratchet_registry.json is what makes it shrink; the accepted-list can only decrease. THE EXEMPTION-CONTROLS GATE FLAGGED ITSELF during its own test run — registering its negative control created a claim that had declared nothing. Its exemption surface IS the accepted_claims list, so by its own rule it owes a control; promoting it needs _QC_NEGATIVE_CONTROL_TABLE to carry an exempts field, which is a separate change. A PER-CLAIM HINT WAS BUILT AND DROPPED ON MEASUREMENT: it scanned each claim's EnforcementClaimRecord.source_file for an exemption-surface naming convention and matched 0 of 70, because source_file points at the negative-control shim in _qc_nc_entrypoints.py rather than at the gate the shim delegates to. THE SHRINK-ONLY MODULE RATCHET FORCED A BETTER SHAPE: the new handler took validate_cmd.py from 1309 to 1336 SLOC, so both inventory handlers moved to validate_inventory_scopes.py and the module finished at 1293, sixteen BELOW the prior ceiling. READ THE DOCS WAS NEVER BROKEN. The shields badge with no version queries the default version latest, which .readthedocs.yaml cancels on every build with exit 183 by design, so it read 'no finished builds' permanently. gzkit.readthedocs.io/en/stable/ serves the docs and img.shields.io/readthedocs/gzkit/stable reports passing.

## Decisions Made

- [operator-ruled] Run the patch release and nothing else (verbatim: 'Patch release only'), with all five advised steps of the resumed handoff recorded set-aside.
- [operator-ruled] Approve the v0.34.3 release notes (verbatim: 'Approved'), which started the Iron Law run through Steps 4a-4e without pauses.
- [operator-ruled] Fix the multi-GHI subject regex immediately and file a GHI for the chore-closure question rather than fixing both (verbatim: 'Fix regex now, GHI the chore rule').
- [operator-ruled] Resolve GHI #794 by adding the unclassified_reference disclosure bucket rather than widening the closure types (verbatim: 'rule 794 - add the unclassified_reference bucket').
- [operator-ruled] Close the resume-gate handoff_path coupling gap, lifting the set-aside that item carried from the 'Patch release only' ruling (verbatim: 'first, fix this:').
- [operator-ruled] Fix the verifier-pipe gate's escape predicate (verbatim: 'fix this:').
- [operator-ruled] Route the exemption-untested class as disclose-now-enroll-as-it-drains, the gz validate --gate-callers shape, rather than fixing only the two proven gates or writing rule prose.
- [operator-ruled] Repoint the Read the Docs badge (verbatim: 'fix the badge').
- [agent-chose] Enforce the #795 coupling at BOOKING time rather than lift time, because lift-time path equality reintroduces the GHI #619 and #755 mid-session re-arm regressions. Neither arm of the handoff's bounded question as posed; both halves discharged.
- [agent-chose] Make the exempts declaration a claim id rather than a description, so the audit checks a reference instead of grading prose.
- [agent-chose] Give exempts three states rather than two, because 'this gate has no exemption' and 'nobody has looked' are different facts and collapsing them is what made the exemption half invisible.
- [agent-chose] Drop the per-claim hint in the disclosed list after measuring it at 0 of 70, and record in the file's _doc why it is uniform rather than shipping decoration.
- [agent-chose] Extract both inventory handlers to validate_inventory_scopes.py rather than raising the shrink-only module-size entry, following the c3d9a99a0 precedent from this same release.
- [agent-chose] Emit fresh ARB receipts before every GHI close after finding that gz check exit 0 is not a receipt and the newest on disk were three days stale.
- [agent-chose] Reconcile #789 into the v0.34.3 notes by hand and disclose it in Gate Evidence rather than silently omitting it or re-typing its chore commit.

## Immediate Next Steps

1. RULE THE ADR-0.35.0 LIFECYCLE STEP. Carried unworked from five predecessors and set aside a seventh time. Run uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing for the live lifecycle and landed count; it reports Lifecycle Pending while the ADR frontmatter says status: Draft. These are different axes, not drift - the Lifecycle column is Layer-3 derived from Layer-2 events, frontmatter is Layer-1 authorship - so this is a fresh authoring decision about whether Draft is right for a feature about to receive OBPI work. It gates step 2.
2. PULL OBPI-0.35.0-01-corpus-tombstone-schema-and-fold. It heads the declared 01 then 02 then 03 chain and carries the fold algebra the ADR pinned in its Decision section as its one irreversible commitment. Read that algebra from the ADR rather than re-deriving it: single reverse pass, never a fixpoint iteration, and unset tombstone fields MUST be omitted from serialization or corpus_fingerprint re-fingerprints the whole corpus on the landing commit. ADR-0.35.0 is the in-flight feature by the 2026-08-11 amendment.
3. CHECK WHETHER NEW RTD TAGS AUTO-BUILD. gzkit.readthedocs.io/en/stable/ serving proves at least one tag build succeeded, never that the v0.34.3 one did. The three one-time admin toggles .readthedocs.yaml's own header calls for at lines 11-17, notably the tag Automation Rule, cannot be confirmed from the repo and app.readthedocs.org returns 403 unauthenticated. Two-minute check in the RTD dashboard under Automation Rules.
4. DRAIN THE EXEMPTION DISCLOSURE LIST, OR RULE THAT IT WAITS. 71 of 76 claims are undeclared. Each needs one reading of the gate its entrypoint delegates to, then exempts='none' or a control claim id. The cheapest first cut is the claims whose gate carries an obvious allowlist - the resume gate's _PERMITTED_BASH family is already done, so start from the audits with _WAIVERS or _GRANDFATHER constants.
5. RULE WHETHER THE ADVISORY SCORECARD SHOULD COVER ADR-DECLARED DOCTRINE. Recorded on GHI #792 [settled] and still not acted on; set aside three times now. docs/governance/advisory-rules-audit.md scopes to .gzkit/rules/** clauses, so ADR-0.0.33 anti-pattern 3 sat outside the instrument that tracks enforcement for the project's entire life. Bounded either way: widen the scope, or state in the audit that ADR anti-patterns are deliberately out of scope and name what does track them.

## Pending Work / Open Loops

AGENTS.md IS 385 B OVER THE CODEX DELIVERY CAP and the must-survive section 'architectural-boundaries' straddles the boundary at bytes 32558-33153. Bytes past the cap are not delivered under codex at all, so a declared-unrecoverable section is silently absent rather than merely late. Tracked on GHI #533; every gz check reports it as an advisory. Verified byte-identical to v0.34.3, so this session did not cause it. Recovery is uv run gz chores show instructions-files-diet, or the GHI #580 [settled] reorder half.
THE EXEMPTION-CONTROLS GATE OWES ITS OWN CONTROL. Its exemption surface is the accepted_claims list; promoting the covering unit tests to a registered exemption control needs _QC_NEGATIVE_CONTROL_TABLE to carry an exempts field. Disclosed with that reason in data/exemption_control_grandfather.json rather than silently exempted.
THE INVENTORY DOES NOT VERIFY THAT A NAMED CONTROL REACHES ITS GATE'S EXEMPTION. It verifies the control is registered; the floor verifies the control catches. Whether a fixture exercises the right surface is the judgment @enforces already leaves to the author, and grading it would be the prose-grader this design refuses.
LIFT TIME REMAINS UNCOUPLED IN THE RESUME GATE, on purpose. If a decision event reaches the ledger by any route other than gz handoff decide, nothing downstream compares its handoff_path. That is the accepted cost of not re-arming cleared sessions.
LEGACY AUTHORIZATION EVENTS ARE NOT RETROACTIVELY VALIDATED. Every handoff_resume_authorized and handoff_resume_decided already on the ledger was booked before the #795 [settled] check existed. They still lift by design - a gate that stopped reading them would retroactively un-authorize the committed ledger - and none has been re-derived against the handoff it names.
GHI #789 [settled]'s CHORE-CLOSURE CASE IS DISCLOSED, NOT FIXED. A GHI whose only closure commit carries a non-code Conventional-Commits type now surfaces as unclassified_reference rather than vanishing, but the operator still adjudicates it per release at SKILL.md Step 1c.

## Verification Checklist

uv run gz check                                   # exit 0, 55 steps, Exemption controls green at 46/55
uv run gz validate --exemption-controls           # exit 0, 76 claims / 5 declared / 71 disclosed
uv run gz validate --gate-callers                 # exit 0, 44 gates / 4 called / 40 accepted
uv run gz cli audit                               # 134/134 commands fully covered
uv run python .gzkit/chores/module-sloc-cap-radon/check_module_size.py   # PASS, validate_cmd.py 1293 <= 1309
git rev-list --left-right --count origin/main...main                     # 0 0
gh issue view 797 --json number,state             # CLOSED (as are 794, 795, 796)
uv run gz patch release --dry-run                 # v0.34.3 is the latest tag

## Evidence / Artifacts

`src/gzkit/governance/trust_audits/exemption_controls.py`
`src/gzkit/commands/validate_inventory_scopes.py`
`data/exemption_control_grandfather.json`
`tests/governance/test_exemption_controls.py`
...
`docs/releases/PATCH-v0.34.3.md`
`RELEASE_NOTES.md`
`CHANGELOG.md`
