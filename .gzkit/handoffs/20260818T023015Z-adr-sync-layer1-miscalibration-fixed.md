...
branch: main
timestamp: '2026-08-18T02:30:15Z'
agent: g0
obpi_id: OBPI-0.35.0-09-codex-playback-wiring
session_id: 8bd5d70c-7290-43cd-b65c-89d08fed5df5
continues_from: .gzkit/handoffs/20260818T014902Z-obpi09-halted-adversary-refuted.md
---

## Current State Summary

Session ran `/gz-adr-sync` (full three-layer sync), then corrected a defect the operator caught in that skill, then unblocked and completed a push. All work is landed and pushed; `main` is level with `origin/main` and the tree is clean.

Layer 1 (evidence): 4,307 @covers annotations, REQ-anchored. Real coverage 1753/2706 REQs (64.8%).
Layer 2 (ledger): booked ADR-pool.skill-efficacy-measurement, which had landed on disk and in git 2026-08-17 but was never recorded as an adr_created event.
Layer 3 (registration): adr-status.md regenerated; count holds at 87 (pool ADRs are ledger-tracked but deliberately absent from that Layer-3 view, whose declared source is the foundation and pre-release ADR trees).

Three commits pushed: 350ddc804, ec211baa6, c62903655.

## Important Context

Three doctrine points this session established or confirmed, each grounded in a measured instance:

1. COVERAGE BINDS AT REQ GRANULARITY, NOT ADR. Per ADR-0.0.59 a BEHAVIOR REQ's only proof channel is a @covers test. The gz-adr-sync full-sync Layer 1 grepped for the ADR-prefixed @covers form — a surface that barely exists — and returned 4 hits, which reads as "no orphan claims". That is a FALSE GREEN. `gz covers` is the governed verb and reports at ADR, OBPI, and REQ granularity repo-wide.

2. A STRUCTURED OBPI BRIEF'S ALLOWLIST BINDS IN FRONTMATTER, NOT PROSE. The reconcile_brief function reads the parsed allowlist from YAML frontmatter — see `src/gzkit/governance/brief_reconcile.py` line 219. The "Allowed Paths" markdown section is a stale restatement: on OBPI-0.35.0-09 it carried 6 entries against frontmatter's 30-plus. This is the hazard `.claude/rules/governance-core.md` names — a value in a Markdown doc is illustrative, never authoritative.

3. THE RECONCILER DERIVES A BRIEF'S WORKING SET FROM THE COVERAGE GRAPH, NOT THE DIFF. The module `tests/governance/test_rendition_floor_coherence.py` carries 4 REQ-0.35.0-09 @covers tags and exercises the corpus store, which bound `src/gzkit/content/corpus_store.py` into OBPI-09's working set. A brief can be accurate the day it is written and drift the moment a test binds a new module to its REQs.

UNIFYING PATTERN, worth carrying forward: three separate false-signal mechanisms fired in one session — the ADR-prefixed grep, a piped verifier (gz validate into tail, caught by the verifier-pipe-gate hook), and a repair verb writing to a non-binding surface. Only the middle one had a mechanical witness. The repo defends hard against PIPED false greens and not at all against MIS-AIMED ones.

## Decisions Made

1. ROUTED THE SKILL FIX AS A DIRECT FIX, not OBPI ceremony. Routing facts computed per AGENTS.md Defect-fix routing: 422 fix commits in 60 days (threshold 3 or more), one-line diffs, single named surfaces, defect surfaced in flight. Precedent shape copied from GHI #806 — literally the same defect class (a skill prescribing the wrong command for agents to copy), fixed with a patch skill-version bump plus refreshed last_reviewed.

2. FIXED THE CLASS, NOT THE INSTANCE. Swept all SKILL.md surfaces with rg --hidden (the first sweep missed dot-directories, since rg skips them by default — itself an instance of the session's pattern). Two instances found and fixed: gz-adr-sync (7.1.0 to 7.1.1) and gz-adr-audit (6.13.1 to 6.13.2).

3. LEFT gz-adr-map UNTOUCHED — it is NOT a defect. It already calls `gz covers` as the canonical traceability walker, labels its rg an explicitly complementary raw survey, and states the doctrine inline: "Decorators are REQ-level, not ADR-level." The correct calibration was already written down in a sibling skill; gz-adr-sync and gz-adr-audit had drifted from a standard the repo already held.

4. OPERATOR ATTESTED THE OBPI-09 ALLOWLIST AMENDMENT AS g0 (verbatim selection: "Attest as g0, then push"). Attestation was requested rather than fabricated, because gz obpi brief-drift --apply requires an attestor and human attestation is sacrosanct.

5. MOVED THE ATTESTED AMENDMENT TO THE BINDING SURFACE. The brief-drift apply path wrote it into the markdown prose section, where the reconciler never reads it; the drift was IDENTICAL after apply. The verb's misplaced write was reverted and the path added to the frontmatter allowlist instead. Same semantic change the operator attested, correct surface.

6. DID NOT FIX the brief-drift apply path ITSELF, to keep the push-unblock surgical. Tracked below as an open loop awaiting an operator GHI ruling.

## Immediate Next Steps

1. OPERATOR RULING NEEDED on two unfiled GHI candidates (see Pending Work). The agent proposed filing via the ghi-author skill and the operator has not yet ruled. Do not file without that ruling; never call gh issue create directly (AGENTS.md Behavior Rules, Always #13 [settled]).

2. If resuming OBPI-0.35.0-09: read the Step 4b adversary verdict at receipt arb-step-codexadversary-fc821cac161042538c772cb58d0433a6 BEFORE trusting any coverage number for it. That adversary REFUTED the claim that the eight covering tests prove their REQs. This carries forward unresolved from the prior handoff.

3. If resuming OBPI-0.35.0-09: obtain an operator ruling on the brief contradiction between REQ-0.35.0-09-02 (exactly one destination) and REQ-0.35.0-09-06 (both the claude and codex destination files). REQ-06's covering test cannot be written correctly until this is resolved. Also carried forward unresolved.

4. Confirm with the operator who is driving OBPI-0.35.0-09 before touching it. A prior session was halted so the operator could run the skill themselves.

## Pending Work / Open Loops

GHI CANDIDATE 1 (high confidence, fully diagnosed, unfiled): the gz obpi brief-drift apply path writes amendments to a surface its own checker never reads. Its _compute_amendments helper — `src/gzkit/commands/brief_reconcile.py` line 36 — emits markdown bullets into the "Allowed Paths" prose section, but reconcile_brief reads the frontmatter allowlist for a structured brief, at `src/gzkit/governance/brief_reconcile.py` line 219. Consequence: the verb reports drift its own repair path can never clear, on repeat runs, forever. Reproduced live this session — applied, re-reconciled, byte-identical DRIFT. This is a governance verb that cannot discharge its own finding, and is coupled-surface incoherence per the AGENTS.md DO IT RIGHT rule 1a.

GHI CANDIDATE 2 (lower confidence, NOT investigated, unfiled): duplicate OBPI indices across ADR-0.35.0, ADR-0.36.0, and ADR-0.37.0 — for example OBPI-0.35.0-01-arb-ruff alongside OBPI-0.35.0-01-corpus-tombstone-schema-and-fold, and OBPI-0.37.0-01-charter alongside OBPI-0.37.0-01-parent-invariant-threading. Names like arb-ruff, ruff-format, ty-check, and forbid-pytest look like a chore namespace colliding with brief numbering rather than authored briefs. Observed in gz state JSON output; the agent stopped rather than widen scope. Needs investigation before filing.

OBPI-0.35.0-09 REMAINS INCOMPLETE. Not completed, not attested. Coverage 10 of 11 REQs (90.9%). Its Step 4b adversary verdict is REFUTED and the REQ-02 versus REQ-06 brief contradiction is unresolved. The allowlist now reconciles clean, which was the only thing this session changed about it.

PRIOR-HANDOFF STEP CARRIED FORWARD UNRESOLVED: read GHI #824 before any further TASK minting — the _find_brief_path helper can write TASK declarations into a plan file instead of the brief.

ADVISORY, NOT GATING: gz drift reports 715 unlinked specs (REQs with no test) repo-wide. Whole OBPI families under ADR-0.35.0, ADR-0.36.0, and ADR-0.37.0 sit at 0% coverage — verified NOT a governance breach, because none of those OBPIs are completed or attested. Zero coverage on unstarted briefs is expected.

NOT BOOKED: no gz handoff decide ruling was recorded against the prior handoff 20260818T014902Z-obpi09-halted-adversary-refuted. The operator ruled only on the allowlist attestation and on syncing; they did not rule on that handoff's advised steps as a set, so no proceed was fabricated. The resume gate was retired 2026-08-15 and gates nothing.

## Verification Checklist

Commands run and their OBSERVED results this session:

- gz register-adrs --dry-run: would append 1 adr_created; applied: "Registered ADR: ADR-pool.skill-efficacy-measurement", "Regenerated adr-status.md (87 ADRs)".
- gz validate --adr-status-fresh: "Validated: adr_status_fresh", exit 0.
- gz covers: "Summary: 1753/2706 REQs covered (64.8%)", exit 0. THIS IS THE CORRECTED LAYER 1 RESULT.
- gz validate --surfaces --cli-alignment: "All validations passed (2 scopes)", exit 0.
- gz obpi brief-drift OBPI-0.35.0-09-codex-playback-wiring: BEFORE, DRIFT with allowlist=1, exit 3. AFTER the frontmatter amendment, "clean" with allowlist=0, exit 0.
- unittest on tests.governance.test_brief_reconcile: OK (55 tests), exit 0. Previously FAILED on test_clean_repo_tree_has_no_errors.
- Pre-push hook ran the FULL suite: 8,461 tests, passed, before allowing the push.
- gz git-sync --apply: "Git sync completed", exit 0. Branch main is level with origin/main, tree clean.

MIRROR PARITY: all five copies of each fixed skill are MD5-identical — the .gzkit canonical, the .claude, .agents, and .github mirrors, and the wheel surface under src/gzkit/skills.

METHOD NOTE FOR THE NEXT SESSION: every verifier above was run with pipefail when piped. The verifier-pipe-gate hook blocks piping gz validate precisely because the shell reports the filter's exit, not the verifier's. Do not cite a piped run as attestation evidence.

## Evidence / Artifacts

COMMITS PUSHED (all authored g0, all hook-verified, none with a verify bypass):
...
FILES CHANGED:
- `.gzkit/skills/gz-adr-sync/SKILL.md` plus its four mirrors — Layer 1 now runs gz covers; version 7.1.0 to 7.1.1
- `.gzkit/skills/gz-adr-audit/SKILL.md` plus its four mirrors — coverage-discovery row now runs gz covers; version 6.13.1 to 6.13.2
- `docs/design/adr/pre-release/ADR-0.35.0-canon-entry-corpus-landing/obpis/OBPI-0.35.0-09-codex-playback-wiring.md` — frontmatter allowlist gained src/gzkit/content/corpus_store.py
- `docs/governance/GovZero/adr-status.md` — regenerated
- `.gzkit/ledger.jsonl` — one adr_created plus two agent_sync_completed events
- `.gzkit/insights/agent-insights.jsonl` — course-correction record

INSIGHT RECORDED (Behavior Rule 11): gz insights remember, type improvement, scoped to the gz-adr-sync SKILL.md surface, timestamp 2026-08-18T02:04:13Z.

KEY SOURCE ANCHORS for the two defects:
- `src/gzkit/governance/brief_reconcile.py` line 219 — structured briefs read the frontmatter allowlist
- `src/gzkit/commands/brief_reconcile.py` line 36 — the _compute_amendments helper emits prose bullets; this is the defect
- `src/gzkit/traceability.py` — the canonical find_covers_in_source scanner masks non-docstring string literals so test fixtures cannot register as phantom coverage anchors (GHI #390 Case A). The raw grep counted exactly those phantoms: 3 of its 4 hits were fixtures in `tests/commands/test_runtime.py`.
