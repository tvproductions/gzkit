...
adr_id: null
branch: main
timestamp: '2026-08-20T00:43:45Z'
agent: claude-code
session_id: e685a9d4-dfd0-4dc9-ad4d-b53ef8035169
continues_from: .gzkit/handoffs/20260819T105428Z-session-closeout-references-onboarded.md
---

## Current State Summary

Session opened on a review of the resumed handoff and closed on a canon repair. The resumed anchor (20260819T105428Z-session-closeout-references-onboarded.md, pinned at 92bb60788) was STALE by six commits: an intervening session closed GHI #833, recorded two insights, and pooled ADR-pool.documentation-type-templates, then exited leaving only mechanical CHECKPOINT bookmarks. Of its five advised steps, step 3 was already discharged; the operator set all five aside and ruled a different item instead.

The work done was the AGENTS.md foundation-kind contradiction. AGENTS.md taught `foundation` as a live authoring choice while ADR-0.34.0 (Foundation Sunset) sealed it on 2026-07-12 and both `gz plan create --kind foundation` and `gz adr promote --kind foundation` refuse it at the command layer. Repaired through the corpus, played back to AGENTS.md, all gates green. Nothing is committed at authoring time; the tree carries the corpus entry, the rendition, AGENTS.md, two insights, and two staged session-exit bookmarks.

## Important Context

Four things a resuming agent will otherwise re-derive.

First, the corpus route is MANDATED, not preferred. REQ-6 of OBPI-0.34.0-03 (status: Completed, attested) reads 'NEVER: Hand-edit AGENTS.md / CLAUDE.md to update the Kinds table - route through the corpus (gz content remember)'. Hand-editing AGENTS.md produces identical bytes and still violates a live requirement of a closed OBPI.

Second, the content pipeline is corpus -> compose -> commit -> playback, and the stages have DIFFERENT writers. `gz content commit` writes only the rendition under .gzkit/renditions/; it does NOT write AGENTS.md. `gz agent sync control-surfaces` is the playback writer. Stopping after commit leaves `gz validate --invariant-coherence` red at exit 3 with the diff as its message - that is the gate working, not a defect.

Third, the closure is PROJECT-LOCAL and must stay that way. ADR-0.34.0 Positive #8 keeps `gz init` scaffolding adopters OPEN, because early adopter projects are exactly when identity-shaping foundations make sense. The wheel template at src/gzkit/templates/agents.md is deliberately UNTOUCHED and byte-identical to its .gzkit counterpart (gz validate --distribution enforces that). Do not propagate the closure into the wheel template. data/vendor-manifest.json confirms AgentContract routes to 'root' only; codex appears there as a delivery CAP (32768 B), never as a route, so the one-rendered-contract canon holds and the on-disk codex rendition is a leftover of the drifted per-consumer shape, not a live route.

Fourth, a dated measurement, not a standing fact: AGENTS.md went 37329 B to 38416 B against the codex 32768 B cap. The overflow is PRE-EXISTING (already 4561 B over before this session); this session added 1087 B, making it 5648 B over. Advisory, never blocking, and covered by the standing 'park instructions-file budget work' ruling - but the surface-delivery witness now reports must-survive section operator-doctrine-verbatim-canon straddling the cap at 26285-35481, so under a codex delivery the tail of the operator's own verbatim canon is undelivered. Re-measure before acting.

## Decisions Made

- [operator-ruled] Verify the sealed-kind contradiction rather than work any of the five advised steps (selected from a four-option picker; GHI #835 side-effect map, GHI #813 provenance gap, and committing the staged bookmarks were the unselected options).
- [operator-ruled] Fix it in the SOURCE, not the rendered surface (verbatim, 2026-08-20: 'AGENTS.md is wrong, but it is rendered, so fix in source'). This arrived mid-turn and pre-empted any hand-edit of AGENTS.md.
- [operator-ruled] Booked proceed on the resumed handoff with all five advised steps recorded set-aside (gz handoff decide, session e685a9d4).
- [agent-chose] Captured the closure at tier invariant rather than compressible, so it survives every compression setpoint and cannot be trimmed out by a future instructions-files-diet pass. Classified Mechanical because it has real witnesses (two CLI refusals plus the --taxonomy closed-kind and terminal-partition assertions).
- [agent-chose] Left the wheel template untouched. Rejected alternative: editing it to say 'closed', which would have crippled adopters and contradicted ADR-0.34.0 Positive #8.
- [agent-chose] Rewrote the three adjacent Mechanical-enforcement bullets (plan create / adr promote / --taxonomy) in the same pass, under DO IT RIGHT 1a coupled-surface coherence, because leaving them would have contradicted the new invariant paragraph two lines below. This is prose the operator did not explicitly authorize and is the item most worth review.
- [agent-chose] Fenced ADR-0.0.18 inside the invariant text as superseded-in-part and frozen-historic, because AGENTS.md pointed at it as live 'when to choose which kind' guidance and that ADR already carries its own supersession marker.

## Immediate Next Steps

1. Review AGENTS.md lines 204-214, specifically the three rewritten Mechanical-enforcement bullets. They are the only prose in this change the operator did not explicitly authorize; the invariant paragraph at line 207 is corpus-sourced and attested, the bullets are not.
2. Commit and sync. At authoring time nothing is committed: the corpus, the rendition and its provenance sidecar, AGENTS.md, the insights file and the ledger are modified, and two session-exit bookmarks are staged.
3. GHI #835 is the live performance thread and its first deliverable is NOT a speedup. Establish for each gz check step whether it writes anything or depends on another step's side effect. A concurrent runner over an undeclared dependency is a flaky gate, which is worse than a slow one. Carried unworked from the predecessor.
4. GHI #834 blocks GHI #832. Do not run the 22-tag repair until the release-body rewrite is closed, or it will overwrite 22 curated release bodies with generated stubs. Carried unworked from the predecessor.
5. GHI #813 has now been untouched across FOUR handoffs and it reproduced live this session: six commits landed with no Layer-2 authorship event, and the only record was a mechanical bookmark whose own text says 'Unknown to the writer'.
6. The delivery-cap overflow is worse by 1087 B and the operator's verbatim-canon section now straddles the codex cap. Budget work is parked by standing ruling, so unparking needs an operator ruling rather than an agent decision.

## Pending Work / Open Loops

- GHI #835 open - validator steps run serially, the largest bucket of the 136s gate.
- GHI #834 open - re-pushing a tag overwrites its curated release body. Blocks #832.
- GHI #832 open - 22 repairable grandfathered release tags, blocked by #834.
- GHI #813 open - handoff authorship has no Layer-2 event; untouched across four handoffs and observed reproducing this session.
- AGENTS.md is 5648 B over the codex delivery cap; must-survive section operator-doctrine-verbatim-canon straddles it. Advisory only; instructions-file budget work is parked by standing operator ruling.
- ADR-pool.documentation-type-templates landed 2026-08-19 (commit 7c5f3d9a8) and is named in no prior handoff. Unpromoted.
- The design-references bibliography pool ADR remains unpromoted; the ASD-STE100 cite-only entry does not substitute for it.
- The controlled-language pool ADR is registered and parked post-1.0; promotion needs its own ruling.
- The codex and claude rendition artifacts under .gzkit/renditions/AGENTS.md/ are leftovers of the retired per-consumer shape. AgentContract routes to root only; nothing reads them and no gate binds them. Cleanup unrouted.
...
git log --oneline -5
git status -sb
uv run gz validate --taxonomy --invariant-coherence --rendition-freshness --rendition-floor-coherence
uv run gz plan create sunset-fidelity-probe --kind foundation --semver 0.0.75 --dry-run
uv run gz check
```

Expect the four validator scopes to pass at exit 0, and the plan-create probe to REFUSE with ADR-0.34.0 guardrail prose - that refusal is the pass condition, being ADR-0.34.0's own Fidelity Assertion with expected exit 1. Re-resolve GHI states rather than trusting them with `gh issue view` for 813, 832, 833, 834 and 835. At authoring time 813, 832, 834 and 835 were OPEN and 833 was CLOSED by c2e79d5b1. Note the exit-code integrity rule: a piped gz validate reports the filter's status, not the validator's - use `set -o pipefail`, which a PreToolUse hook enforces.

## Evidence / Artifacts

- `.gzkit/corpus/AGENTS.md.jsonl` - the source of truth; entry corpus-gate-covenant-2026-08-20T00:31:11.884248+00:00, tier invariant, classification Mechanical, witness g0, origin naming the OBPI-0.34.0-03 deferral and the operator ruling
- `.gzkit/renditions/AGENTS.md/root.corpus.json` - provenance sidecar, corpus_fingerprint=3e359d7502b0, entries=66, attestor g0
- `AGENTS.md` - the played-back result at lines 204-214; line 204 the table row, line 207 the verbatim invariant paragraph, lines 211-214 the rewritten enforcement bullets
- `docs/design/adr/pre-release/ADR-0.34.0-foundation-sunset/obpis/OBPI-0.34.0-03-terminal-partition-gate-and-doctrine-retirement.md` - line 87 is the declared deferral no movement ever claimed, line 105 is REQ-6 forbidding the hand-edit, line 422 records the sweep as deferred to the corpus route
- `docs/design/adr/pre-release/ADR-0.34.0-foundation-sunset/ADR-0.34.0-foundation-sunset.md` - Negative #1 names the AGENTS.md/CLAUDE.md Kinds table as a coupled surface that must move in the same movement
- `data/vendor-manifest.json` - AgentContract routes to root only; codex is a delivery cap, not a route
- `.gzkit/insights/agent-insights.jsonl` - the deferred-capture defect (governance.obpi-deferred-capture) recorded this session, and the predecessor's adr.kind-selection improvement that surfaced the contradiction
- `.gzkit/ledger.jsonl` - corpus_entry_appended, rendition_committed, and the handoff-decide events for session e685a9d4
