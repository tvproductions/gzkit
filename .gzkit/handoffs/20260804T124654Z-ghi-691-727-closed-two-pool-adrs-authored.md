...
adr_id: null
branch: main
timestamp: '2026-08-04T12:46:54Z'
agent: claude-code
session_id: 350c15d7-dec7-4994-bdf5-8d2c310374f3
continues_from: .gzkit/handoffs/20260804T085420Z-ghi-753-closed-tasks-channel-schema-enforced.md
---

## Current State Summary

Closed two GHIs against registered destinations and refreshed the triage ranking. Three commits on main, all synced (origin/main 0 0 at handoff time, tree clean). No parent ADR: the session was GHI routing, not ADR implementation.

GHI #691 (rules carry no aging mechanism while skills do) closed superseded against a new pool ADR carrying the operator ruling booked 2026-07-27. GHI #727 (mechanism objectives and scope unrecorded) closed superseded against a second new pool ADR that RECORDS its unruled fork rather than resolving it.

Open queue went 12 -> 10. Triage re-ranked all 10 remaining. Final quality state: uv run gz check exit 0, all checks passed on the landed tree.

## Important Context

The resume gate fired at session start and refused every mutating call until the ruling was booked. Reads stayed permitted, so the whole Phase-1 evidence pass ran before authorization. That ordering is the gate working as designed, not an obstacle.

**691 needed only a home.** Its direction was settled by operator ruling on 2026-07-27 (90 days, honest per-rule backfill, advisory-then-fail-closed) and marked not-to-be-re-adjudicated. What kept it open was that RuleFrontmatter is frozen with extra=forbid, so last_reviewed cannot land incrementally: the first rule to carry it makes the other 24 fail to parse. All 25 change in one landing with the schema, which is why it is not defect repair and the GHI-direct-fix override does not reach it.

**A naive backfill would launder the staleness it exists to expose.** Commit b89754166 (diet pass) touched 9 canonical rules in one mechanical pass that only deleted prose into a doc. Nobody re-read a line against code. Those 9 include pythonic, cli, and adr-audit, three of the four rules #691 names as worst-drifted. git log -1 stamps them fresh as of 2026-08-02 and buys 90 days of silence. The promotion must define and mechanize substantive. The diet pass postdates the GHI by two weeks, so its body could not have known.

**ADR-0.0.52 three-tier framing is false twice over, not once.** ADR-pool.skill-version-review-coupling had already caught that the rule-level coupling is doctrine-only. The deeper defect: the field named as the rule-level witness does not exist on rules at all. It governs skills, and skill-surface-sync rule #6 ends with the words skills only. Both ADRs inherited the mislabel; both now carry dated correction blockquotes.

**727 is different in the way that mattered: its fork was never ruled.** Documentary vs mechanical changes what gets built, so no agent should pick it. Pool status is why closing was still legitimate: a pool ADR is the design-conversation home, and the fork reaches the promotion ceremony with its evidence attached instead of decaying in an issue thread.

**The #727 residual is latent, not live.** audit_code_contract_mismatches returns 0 findings on the current tree because there are zero dataclasses under src/gzkit. It is a loaded gun with no round chambered: it fires the moment anyone adds a legitimately-waived dataclass, at which point the two mechanisms disagree by construction.

**One instance found while re-deriving, recorded nowhere before.** The sole _DATACLASS_WAIVERS entry is inert. CheckResult is a BaseModel now (obpi_precomplete.py:43), so the waiver exempts nothing, and the stale-waiver check at models.py:81 misses it because its predicate is whether the class still exists, not whether it still needs the exemption. The waiver objective was never recorded as a condition, so the staleness check tests the wrong predicate.

**Two triage freshness findings the structural-only rank schema cannot carry.** The top-ranked item names ADR-0.0.37 at 5/10 OBPIs as its blocker, but 0.0.37 went terminal 2026-07-18 with its registry-spine OBPIs permanently withdrawn as obsoleted. The path it waits on was never built and was re-homed to ADR-0.35.0, the campaign next pull. The rule surface already repointed (agents-md-map-doctrine 0.3.0); the issue did not. Separately, the second-ranked item declares itself deferred pending GHI #519, which is now settled, so its gate lifted and nothing said so.
...
- [operator-ruled] run triagr and create new handoff, git sync that (verbatim, operator spelling preserved) -- the triage re-run, this document, and the sync that follows.
- [agent-chose] Authored a NEW pool ADR for #691 rather than folding into ADR-pool.skill-version-review-coupling. That ADR draws the boundary itself in its Notes: it enforces the coupling at edit time and names calendar staleness adjacent scope, not a blocker. Folding would have overridden its own stated boundary.
- [agent-chose] Corrected the ADR-0.0.52 mislabel at both sources in a second commit rather than only recording it in the new ADR. Additive dated blockquotes; originals preserved rather than rewritten, following the GHI #615 close-comment precedent that a silently tidied record is worse than a flagged one.
- [agent-chose] Recorded the #727 fork rather than resolving it, and closed on the pool ADR anyway. Pool status is the design-conversation home; a promise to author later would have been the dead-letter the ghi-close doctrine forbids.
- [agent-chose] Did NOT consolidate the two Pydantic validators. They differ in intent as well as engine: one is conditional on the instruction document, one unconditional. Picking one unilaterally is the silently-picking-one-interpretation failure AGENTS.md Behavior Rules Always #9 names.
- [agent-chose] Did NOT fix the GHI #607 arming residual under the #727 close. Fixing it closes two named instances and leaves the class open, which is the instance-not-class failure DO IT RIGHT #1 names.
- [agent-chose] Ranked the AGENTS.md budget item top of triage because its recorded dependency is stale, not because it is the largest. Its blocker chain waits on an ADR that went terminal with the relevant OBPIs withdrawn.

## Immediate Next Steps

1. **Return to the campaign.** Movement A item 2 is ADR-0.35.0-canon-entry-corpus-landing (Pending, 0/10 OBPIs). The Magna Carta governs sequencing; the queue advises. Seven consecutive sessions have worked the queue instead.
2. **Or rule the GHI #727 fork** so ADR-pool.mechanism-objective-and-scope-record can promote: is the per-mechanism objective and scope obligation documentary or mechanical? The lodestar half is rulable independently of the fork and is the smaller of the two.
3. **Or work the refreshed triage queue.** The ranking at .gzkit/cache/triage/rank.json is current as of this session and covers all 10 open. The top item needs its stale ADR-0.0.37 dependency re-derived against ADR-0.35.0 before it is pulled.
4. **Or rule on the Settled Rulings threshold.** The block is past 65 entries and grows every session. The skill contract names routine growth as the signal that rulings belong in a durable ruling store, campaign Movement D box 3.

## Pending Work / Open Loops

- **ADR-pool.rule-surface-aging-clock** -- Pool, awaiting promotion. Carries the booked 2026-07-27 ruling plus one design question the ruling could not have answered: substantive needs a definition that excludes bulk mechanical passes, or the honest backfill launders the staleness it exists to expose.
- **ADR-pool.mechanism-objective-and-scope-record** -- Pool, awaiting the operator ruling on documentary vs mechanical before promotion. Carries three candidate halves (lodestar tech-choices, scorecard per-mechanism columns, and the capture channel from the 2026-07-28 comment).
- **The GHI #607 residual is live but latent** -- arming is still the bare substring Pydantic anywhere in the instruction body, and the regex validator still has no waiver affordance. Zero dataclasses under src/gzkit is the only thing keeping it quiet. Deliberately not fixed; it is the new pool ADR worked example.
- **The inert _DATACLASS_WAIVERS entry** -- exempts nothing and is invisible to the stale-waiver check. Recorded in the pool ADR as its third worked instance rather than deleted, because deleting it destroys the evidence the ADR is built on.
- **Two pool ADRs now carry dated correction blockquotes** -- ADR-pool.artifact-staleness-propagation and ADR-pool.skill-version-review-coupling. Both are marked re-verify before promotion, and both edit skill-surface-sync rule #6 from opposite directions, so whichever promotes second must reconcile against the first.
- **Settled Rulings block is past 65 entries** -- carried forward every session and now the bulk of the handoff. Signal, not defect; see next-step 4.
...
git log --oneline -3                                  # 80f1c123a, f1bf9fdcd, 1fd64ce38
gh issue view 691                                     # CLOSED, cites 1fd64ce38 and f1bf9fdcd
gh issue view 727                                     # CLOSED, cites 80f1c123a
uv run gz adr report                                  # both new pool ADRs present in the Pool table

To confirm the destinations are registered rather than trusting the close comments:

grep -c rule-surface-aging-clock docs/design/adr/pool/ADR-pool.rule-surface-aging-clock.md
grep -c "Correction (2026-08-04, GHI #691)" docs/design/adr/pool/ADR-pool.artifact-staleness-propagation.md

To confirm the #727 residual is latent rather than fixed:

uv run python -c "from pathlib import Path; from gzkit.instruction_audit import audit_code_contract_mismatches; print(len(audit_code_contract_mismatches(Path(chr(46)))))"   # 0

## Evidence / Artifacts

Commits: 1fd64ce38 authored ADR-pool.rule-surface-aging-clock; f1bf9fdcd corrected the inherited three-tier mislabel in two pool ADRs; 80f1c123a authored ADR-pool.mechanism-objective-and-scope-record.
...
Triage ranking refreshed: `.gzkit/cache/triage/rank.json` (10 ranked of 10 open, fix precedent 60d = 338)

Surfaces read for the re-derivation, unchanged: `src/gzkit/rules/__init__.py`, `src/gzkit/skills_audit.py`, `src/gzkit/instruction_audit.py`, `src/gzkit/governance/trust_audits/models.py`, `src/gzkit/commands/obpi_precomplete.py`, `.gzkit/rules/skill-surface-sync.md`, `docs/design/lodestar/architectural-identity.md`

No ARB receipts emitted this session: every commit was doc-only, and gz check exit 0 is the cited evidence.
