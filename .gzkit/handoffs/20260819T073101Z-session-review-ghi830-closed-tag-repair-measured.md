...
adr_id: null
branch: main
timestamp: '2026-08-19T07:31:01Z'
agent: g0
session_id: 970067d7-eb8c-4e25-b39d-6dc2bee76fbd
continues_from: .gzkit/handoffs/20260819T003142Z-v0344-release-and-release-tag-audit-repair.md
---

## Current State Summary

Session reviewed the resumed handoff `20260819T003142Z-v0344-release-and-release-tag-audit-repair.md`, booked a `proceed` ruling, and worked its advised steps 1 through 3 to completion. That handoff is now fully discharged and this one supersedes it.

Branch main is level with its remote (`git rev-list --left-right --count` returns 0 0), tree clean at fcc2b7632, no active OBPI locks. `gz validate --version-release`, `--waiver-ratchet`, and `--changelog` all exit 0. Full canonical suite green: 8,498 tests, receipt arb-step-unittest-c085247a523a4c99ae6eb8bc5b86dfaa, exit_status=0.

Three bodies of work landed. **(1) GHI #830 closed** — the release audit now sweeps in both directions, and the three releases documented nowhere in the repo are backfilled. **(2) An orphaned pool ADR recovered** — ADR-pool.controlled-language-for-control-surfaces, 276 lines, authored at a prior session's exit beat and left untracked, committed as 0fc82fe5f. **(3) GHI #832 filed** — a measured 22-of-34 repair path through the reachability grandfather set, deliberately not taken.

No ADR or OBPI was touched. `gz obpi status` and `gz state` are not relevant to verifying this handoff; every claim resolves against GitHub, git refs, or a `gz validate` scope.

## Important Context

**The 2026-04-19 filter-repo rewrite remains the load-bearing fact for anything touching tags**, and this session sharpened what is known about it. The rewrite removed operator-PII email and re-SHA'd history; tags survived the force-push, their commits did not. But orphaned is not the same as unrepairable, and that distinction had never been measured.

Measured 2026-08-19: for 22 of the 33 seeded grandfather entries there is a commit on origin/main whose tree is byte-identical to the orphaned tag's. The repair is therefore mechanical — retag at the identical-tree commit, exactly what GHI #828 did for v0.7.0. The 11 that cannot be repaired are the contiguous block v0.25.0 through v0.25.11: those are the commits filter-repo actually rewrote, so no surviving tree matches. Everything older passed through untouched.

Worked instance, showing the match is exact rather than approximate: v0.18.1 tree 54fb6b2f348b, target c1392fcde tree 54fb6b2f348b, identical timestamp 2026-03-21 19:33:04 -0500, identical subject. Same commit either side of the rewrite; only the author email differs.

**The grandfather set is an intersection, not a list.** This is the non-obvious dependency that bit mid-session. `data/release_tag_reachability_grandfather.json` holds releases that are BOTH documented in `RELEASE_NOTES.md` AND rewrite-orphaned. Its shrink-only ratchet implicitly assumed the first roster was frozen. Backfilling three previously-undocumented releases moved the intersection and turned the tree red on three tags whose condition had not changed at all — they were always orphaned, merely invisible while the roster omitted them. Any future change to the notes roster will move this set again.

**`CHANGELOG.md` does not cover the pre-0.33 era and must not be backfilled into.** It was created at f64857e1d on 2026-07-13 under GHI #685 and carries 9 versions, oldest v0.33.1. Its own header declares entries to be the derived projection of GHIs closed since the previous tag, which does not exist for releases from 2026-03 and 2026-04. GHI #830's issue body lists a CHANGELOG column as absent for all three releases, which overstates what was owed; the close comment records the correction.

**GHI #205 scope-gated network, it did not forbid it.** Its closing text reads: "Consideration: this check requires gh auth and network. Scope-gate it: skip under `--offline` or `GZKIT_NO_NETWORK=1`; otherwise it becomes brittle in CI or sandboxes." The source comment in `src/gzkit/governance/trust_audits/release.py` had paraphrased this as "GHI #205 scoped network out of this audit", which is a stronger claim than #205 makes, and the prior handoff inherited the stronger reading. The local-only posture is correct but stands on COST — 71 gh round trips at pre-commit — not on a settled prohibition. The comment now says so.

**The ceremony's tag/notes ordering is what makes the new predicate exact.** `RELEASE_NOTES.md` is written in the release commit; the tag is created afterwards by `gh release create`. So unlike the in-flight manifest arm — which exists precisely to cover a bump that PRECEDES its tag — there is no window in which a tag legitimately precedes its notes entry. That asymmetry is why a local proxy is exact here rather than approximate, and it is the reason to be suspicious of any future proposal to add a grace period to this arm.

## Decisions Made

- [operator-ruled] Proceed on the resumed handoff's advised steps 1 through 3 (verbatim: "Proceed on steps 1-3"), booked via `gz handoff decide --decision proceed`. Options to hold for the campaign's topmost item, to take step 1 alone, or to pause after review were declined.
- [operator-ruled] Commit the orphaned pool ADR as-is (verbatim: "Commit as-is"), over reviewing it first or leaving it untracked.
- [operator-ruled] Close GHI #830 with the strict local proxy predicate (verbatim selection: "Strict local proxy") — every tag matching strict vMAJOR.MINOR.PATCH owes a RELEASE_NOTES entry. A local proxy plus a disclosed exemption file was declined as speculative machinery with an empty exemption set; a scope-gated network arm was declined on pre-commit cost.
- [operator-ruled] Take the minimum tag repair (verbatim selection: "Minimum: unblock the three") — retag only v0.18.1 and v0.24.1, grandfather v0.25.1, and file the remaining 22 rather than sweeping them. The full 24-tag sweep and the no-retagging-at-all option were both declined.
- [agent-chose] Measured the "would fire on any legitimately unreleased tag" objection instead of reasoning about it. 71 tags, 68 documented, exactly 3 undocumented, zero false positives. The objection was about future tags, not present ones, and the ceremony's ordering shows it cannot arise.
- [agent-chose] Ran the backfill BEFORE the witness, per the resumed handoff's ordering constraint. Adding the witness first fails the tree closed on its own population.
- [agent-chose] Backfilled RELEASE_NOTES only and left CHANGELOG untouched, because the changelog postdates all three releases by months and its stated derivation does not exist for that era.
- [agent-chose] Transcribed v0.25.1 mechanically from its GitHub release body via a shell pipeline rather than retyping it, so the word "verbatim" in its provenance block is literally true.
- [agent-chose] Disclosed the two reconstructed entries AS reconstructions, with a Provenance block naming the commit range, the file and insertion counts, and the fact that no contemporaneous record exists. Presenting archaeology as authored notes would be the narrative-recall failure the anti-vibing doctrine names.
- [agent-chose] Matched tags strictly rather than reusing the glob, so a future v1.0.0-rc1 cannot become a permanent defect. Covered by its own test.
- [agent-chose] Did not skip the current version in the inverse sweep, unlike the forward sweep which skips it because its caller already judged it. Nothing else checks the current version's notes entry, and the ceremony guarantees no transient failure.
- [agent-chose] Repaired two of the three newly-visible tags rather than exempting all three, because the repair was available and is strictly more correct than a waiver.
- [agent-chose] Amended the grandfather doc-string to state the ONE admissible growth shape — a pre-cutover release entering the documented roster for the first time, with the repair attempted and unavailable — rather than silently bumping the count. A shrink-only surface that grows without disclosing why is how a ratchet decays.
- [agent-chose] Corrected the release.py characterisation of GHI #205 in the same commit as the coupled work, per the coupled-surface coherence duty.
- [agent-chose] Filed the 22-release repair path as GHI #832 with the operator's ruling recorded as its blocker, rather than widening the session's scope mid-fix or leaving the measurement in session memory only.

## Immediate Next Steps

1. Nothing is owed from the superseded handoff. Its four advised steps are discharged: the GHI #830 [settled] predicate was ruled and built, the backfill landed before the witness, and the v0.7.0 consistency question was answered with measurement and routed to GHI #832. Do not re-derive any of them.
2. Rule on GHI #832 when it is next in scope — whether to take the 22-tag repair sweep, and in one pass or batched. No code decision is pending: the target commit for each entry is mechanically derivable by matching the orphaned tag's tree against commits reachable from refs/remotes/origin/main. The action is outward-facing (each retag moves what a published GitHub release points at), so it needs explicit operator authorization, not a routing decision.
3. Consider whether the campaign's topmost item should now take the session. This session's work came from a handoff, and per operator canon "handoffs and triage advise, the campaign governs." Build-to-1.0 Movement B item 0 (calibrate the airlock before widening, carried by ADR-0.37.0-airlock-calibration-and-compulsion) was raised at review time and the operator chose the handoff lane instead. That choice covered this session, not the next one.
4. If any future change adds or removes a version heading in RELEASE_NOTES, re-check the reachability grandfather set in the same change. The set is the intersection of the notes roster and the orphaned-tag set, so moving the roster moves it — this session's red tree was exactly that, and nothing warns about it in advance.

## Pending Work / Open Loops

- **GHI #832 is OPEN by design, with its blocker recorded in the body.** 22 of the 34 grandfather entries are repairable by the identical-tree retag method; the 11 that are not are the contiguous v0.25.0-v0.25.11 block. Deferred by the operator's "Minimum: unblock the three" ruling, not by oversight. Verified OPEN at handoff time.
- **The grandfather list now stands at 34 with a growth amendment disclosed in its own doc-string.** It remains shrink-only. The one admissible growth shape is written into the file; a release tagged after the 2026-04-19 cutover is still never admissible. The waiver ratchet registry baseline moved 33 to 34 to match.
- **v0.25.1 is genuinely unrepairable and will stay grandfathered** until or unless the whole v0.25.x block is addressed some other way. It sits inside the rewritten block; no surviving commit carries its tree.
- **Two GitHub releases now point at different commits than they did.** v0.18.1 moved to c1392fcde and v0.24.1 to 6b86bdf07, both force-pushed. Trees are identical so the released content is unchanged, but anyone holding the old SHAs will find them unreachable from origin/main.
- **The recovered pool ADR is committed but unpromoted**, which is its correct resting state. ADR-pool.controlled-language-for-control-surfaces proposes a computable directive-versus-narrative criterion for the control-surface diet, with ASD-STE100 as prior art and a dated 2026-08-18 measurement as its warrant. Its measuring script was run from a scratchpad and is not committed; the ADR says so and instructs a reader to re-derive.
- **RELEASE_NOTES is not machine-checkable for structure or tone** by design (`.gzkit/rules/changelog-release-notes.md`: no mechanical release-notes validator exists). The three backfilled entries were not validated by any gate beyond the new tag-presence witness. A human read of their provenance blocks is the only check they will get.

## Verification Checklist

```bash
# The inverse sweep is non-vacuous, not merely clean. The count is the half that
# matters: a sweep exempting every tag would satisfy an emptiness check forever.
uv run python -c "
from pathlib import Path
from gzkit.governance.trust_audits.release import _release_tag_versions, _undocumented_tag_errors
print('judged', len(_release_tag_versions(Path('.'))))
print('errors', len(_undocumented_tag_errors(Path('.'))))
"                                                        # expect 71 and 0

# Both sweep directions are empty
uv run python -c "
import re, subprocess
from pathlib import Path
notes = Path('RELEASE_NOTES.md').read_text(encoding='utf-8')
documented = set(re.findall(r'^## v(\d+\.\d+\.\d+)', notes, re.MULTILINE))
tags = {t.strip()[1:] for t in subprocess.run(['git','tag','--list','v*'],capture_output=True,text=True).stdout.splitlines() if t.strip()}
print('tagged but undocumented:', sorted(tags - documented))
print('documented but untagged:', sorted(documented - tags))
"                                                        # expect [] and []

# Gate scopes touched this session
uv run gz validate --version-release                     # expect exit 0
uv run gz validate --waiver-ratchet                      # expect exit 0
uv run gz validate --changelog                           # expect exit 0

# The two repaired tags agree with the remote
git ls-remote origin refs/tags/v0.18.1                   # expect c1392fcde
git ls-remote origin refs/tags/v0.24.1                   # expect 6b86bdf07
git fetch --tags --prune origin                          # expect NO rejected-tag line

# Grandfather and ratchet agree at 34
uv run python -c "
import json
print(len(json.load(open('data/release_tag_reachability_grandfather.json'))['accepted_unreachable_versions']))
print([s['baseline_count'] for s in json.load(open('data/waiver_ratchet_registry.json'))['surfaces'] if 'release_tag' in s.get('data_file','')])
"                                                        # expect 34 and [34]

# GHI state
gh issue view 830 --json state --jq .state               # expect CLOSED
gh issue view 832 --json state --jq .state               # expect OPEN

# Full gates
uv run gz arb step --name unittest -- uv run -m unittest -q   # expect exit_status=0
```

Claim-verification note for the resuming agent: no OBPI or ADR was touched this session and no lock was held at authoring time — read `uv run gz obpi lock list` live rather than trusting that. Every claim above resolves against GitHub, git refs, or a `gz validate` scope.

## Evidence / Artifacts

Commits, both pushed to the remote main branch:

- 0fc82fe5f — docs(adr): pool the controlled-language-for-control-surfaces ADR
- fcc2b7632 — fix(release-audit): sweep the inverse direction, so a published release cannot go undocumented (GHI #830)

Tags force-pushed to the remote:

- v0.18.1 — repointed from 21020794b to c1392fcde (identical tree 54fb6b2f348b)
- v0.24.1 — repointed from b13cad910 to 6b86bdf07 (identical tree 683984340baa)

Files:

- `RELEASE_NOTES.md` — three backfilled entries at their chronological positions, each carrying a Provenance block
- `src/gzkit/governance/trust_audits/release.py` — the two new helpers, the corrected GHI #205 comment, and the entrypoint docstring's new arm
- `tests/governance/test_promoted_advisory_audits.py` — VersionReleaseAuditUndocumentedTagSweep, 5 tests including a negative control and a non-vacuity witness
- `data/release_tag_reachability_grandfather.json` — 34 entries, doc-string amended with the admissible growth shape, measurement block carrying both the amendment and the 22-of-33 repair-headroom record
- `data/waiver_ratchet_registry.json` — baseline_count 33 to 34
- `docs/design/adr/pool/ADR-pool.controlled-language-for-control-surfaces.md` — the recovered pool ADR
- `CHANGELOG.md` — deliberately unmodified; the reason is recorded in the GHI #830 close comment
- `.gzkit/handoffs/20260819T003142Z-v0344-release-and-release-tag-audit-repair.md` — the superseded predecessor
- `.gzkit/handoffs/20260819T011037Z-session-exit-bookmark.md` — the prior session's mechanical floor bookmark, committed this session

ARB receipts, canonical invocations, all exit_status=0:

- arb-ruff-382e42da1106452094a8128533859e61
- arb-step-typecheck-201902098d194d97ad8791ef989899ac
- arb-step-unittest-c085247a523a4c99ae6eb8bc5b86dfaa (8,498 tests)

GitHub:

- GHI #830 CLOSED, citing fcc2b7632, with the CHANGELOG-scope correction recorded
- GHI #832 OPEN, carrying the 22-of-34 measurement and the operator ruling as its blocker
