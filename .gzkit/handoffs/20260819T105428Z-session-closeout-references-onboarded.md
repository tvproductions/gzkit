...
adr_id: null
branch: main
timestamp: '2026-08-19T10:54:28Z'
agent: claude-code
continues_from: 20260819T103551Z-python-3-13-15-ci-pins-and-validator-arm.md
---

## Current State Summary

Session close-out. Tree is clean and `main` is synced at `92bb60788`; nothing is uncommitted and nothing is owed.

Landed since the predecessor handoff: `ebaa9f254` (ASD-STE100 onboarded as a cite-only reference) and `92bb60788` (that discharge appended to the predecessor as a dated correction). Two insights recorded this turn — a measurement defect and a gate-invocation improvement — and committed alongside this handoff.

The ASD-STE100 references question, carried open across three handoffs, is CLOSED. It is onboarded as a cite-only entry: publisher, issue, date, lineage and URL, with no extracted text, because gzkit ships publicly and extraction would republish a document whose default deny covers exactly that.

Five GHIs remain open and none is blocked on this session: #813, #832, #833, #834, #835. #834 blocks #832 and is cross-linked there.

## Important Context

Three things carried forward that a resuming agent will otherwise re-derive.

First, `gz check` is 136s with NO dominant cost, and "the tests are slow" is the wrong answer. Measured 2026-08-19 (dated record, re-derive before acting): validators and friends about 55s, Test 45.3s, Behave 31.6s, mkdocs 4.0s. The suite has been parallel since GHI #512. The 111s figure that circulates is the serial ARB attestation path, which the runner module keeps serial deliberately. This session asserted 82-percent-tests from that wrong invocation and nearly had a duplicate GHI filed on the strength of it; GHI #835 targets the validator bucket instead.

Second, two gate habits were adopted after measurement and should persist. Do NOT run `gz check` manually when about to push — the pre-push hook is the only push-stage hook and runs the identical gate, so a manual run followed by a push pays 136s twice. Batch pushes rather than pushing per commit; commits are nearly free.

Third, a cite-only reference entry is a citation home, NOT a bibliography surface. The design-references bibliography pool ADR is still unpromoted and nothing this session did pre-empts its design. Do not read "references onboarded" as "the bibliography question is settled" — they are different questions and only the first is answered.

## Decisions Made

- [operator-ruled] Onboard ASD-STE100 as a cite-only reference (verbatim: "onboard ASD-STE100 as a cite-only reference") — option 1 of three carried across two prior handoffs.
- [operator-ruled] Append the references discharge to the predecessor handoff rather than leaving it stale (verbatim: "append the correction to the handoff").
- [operator-ruled] Target the validator bucket rather than a parallel test runner for the performance GHI, after the agent's premise was refuted by measurement.
- [agent-chose] Kept the cite-only entry in the existing research-sources directory using that directory's header convention, rather than creating a new references surface. Rejected alternative: standing up the bibliography surface ad hoc, which would pre-empt the unpromoted pool ADR that owns that design.
- [agent-chose] Appended the handoff correction as a dated section rather than editing the stale Pending bullet, borrowing the reading convention the instructions-files budget file already establishes. Editing in place would have falsified what was genuinely pending at authoring time.
- [agent-chose] Placed that correction above the Settled Rulings block rather than at end-of-file, because the rulings run 395 lines and would have buried it.
- [agent-chose] Re-validated the hand-edited handoff against the production corpus gate rather than assuming the edit was safe, since there is no amend verb and the authoring path is fail-closed.

## Immediate Next Steps

1. GHI #835 is the live performance thread and its first deliverable is NOT a speedup. Establish for each `gz check` step whether it writes anything or depends on another step's side effect. A concurrent runner over an undeclared dependency is a flaky gate, which is worse than a slow one.
2. GHI #834 blocks GHI #832. Do not run the 22-tag repair until the release-body rewrite is closed, or it will overwrite 22 curated release bodies with generated stubs.
3. GHI #833 is scoped and ready — the transient git locks other than the maintenance lock still race the fixture copy. Note the recorded constraint: the obvious wildcard lock-ignore is wrong because it drops the uv lockfile.
4. GHI #813 has been untouched across three handoffs. It is the handoff-authorship provenance gap and is the oldest open thread here.
5. If promoting the controlled-language pool ADR is ever considered, it needs its own operator ruling. It is parked post-1.0 behind the in-flight feature ADR and nothing this session changed that.
...
- GHI #833 open — transient git locks other than the maintenance lock still race copytree.
- GHI #832 open — 22 repairable grandfathered release tags, blocked by #834.
- GHI #813 open — handoff authorship has no Layer-2 event; untouched across three handoffs.
- The design-references bibliography pool ADR remains unpromoted. The cite-only entry does not substitute for it.
- The controlled-language pool ADR is registered and parked post-1.0; promotion needs its own ruling.
- A workflow testing a MATRIX of interpreters would trip the new interpreter-pin arm. None exists; an exemption mechanism was deliberately not built ahead of the case.
...
Expect HEAD at or after 92bb60788, main synced with no tracked changes, both gz invocations exit 0, and issues 813, 832, 833, 834 and 835 all OPEN at authoring time. Re-resolve the issue states rather than trusting them, per the Claim Verification Gate.
...
- `docs/governance/research_sources/asd-ste100-issue-9.md` — the cite-only entry; header convention preserved, body deliberately absent, licence position and the not-encumbered half both recorded
- `docs/design/adr/pool/ADR-pool.controlled-language-for-control-surfaces.md` — sibling table now names the entry that exists; Notes records the ruling and fences the bibliography ADR as un-pre-empted
- `.gzkit/handoffs/20260819T103551Z-python-3-13-15-ci-pins-and-validator-arm.md` — predecessor, now carrying an appended dated correction; its seven authored sections are byte-unchanged
- `.gzkit/insights/agent-insights.jsonl` — six insights recorded this session across both halves
- `src/gzkit/quality.py` — the runner module naming both the parallel gate path and the deliberately serial attestation path
- `.pre-commit-config.yaml` — confirms the pre-push gate is the only push-stage hook
