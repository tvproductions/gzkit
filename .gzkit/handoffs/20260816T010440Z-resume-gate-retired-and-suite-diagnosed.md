...
adr_id: null
branch: main
timestamp: '2026-08-16T01:04:40Z'
agent: claude-code
continues_from: .gzkit/handoffs/20260815T092541Z-d1-withdrawn-on-measurement-six-briefs-authored.md
---

## Current State Summary

Six commits landed on main (`556ae768` -> `e51a3a09`); tree clean, level with
`origin/main`, `uv run gz check` exit 0, 8380 tests OK with **zero skips**.

The session opened as a routine `/git-sync`, hit a rebase conflict on the
append-only ledger, and became two connected pieces of work: the handoff resume
gate was retired ENTIRELY on operator ruling, and the test suite was measured,
diagnosed, and the stalled tautological-decommission chore restarted with a
first wave plus a REQ re-kind pilot.

No OBPI lock is held and no pipeline is in flight.

## Important Context

FIRST, the resume gate is GONE, not narrowed. `bc9b72f6` (2026-08-14) had
already dropped its Bash arm; `c772497c` drops the `Write|Edit|NotebookEdit`
arm, deregisters the hook, and DELETES `.claude/hooks/handoff-resume-gate.py`
together with its generator template `src/gzkit/hooks/scripts/handoff.py`. Both
negative controls are retired, because a control asserting enforcement that no
longer happens reports green while blind. Gate 5, the pre-commit hook and the
pre-push `gz check` are untouched. `gz handoff decide` still books the
operator's verbatim ruling to Layer 2 — it records provenance and now gates
nothing. The arm's entire measured lifetime was 9 lifts to 1 block across the
single day refusal-recording existed, against 13 admission-breadth corrections
in 29 days.

SECOND, and this reframes the whole test-suite question: test count is a
MECHANICAL FUNCTION of governance surface area, not of product capability. 3044
REQs are declared; 1651 carry a `@covers` test at 2.6 tests each, and 3044 x 2.6
is approximately the 8380-test suite. Every BEHAVIOR REQ owes a covering test
and per GHI #537 that cannot be waived on any lane, so authoring governance
mints tests — and gzkit's product IS governance, so the growth is
self-referential and accelerating: 2316 tests on 2026-05-15, 3839 on 07-15, 4847
on 08-15 by one consistent git-grep metric, a 26% rise in the final month.

THIRD, the shrink half of that system was built and abandoned. The
`decommission-tautological-tests` chore processed five files on 2026-05-27 and
stopped for eleven weeks. It stalled INVISIBLY because all three of its
acceptance criteria PASS while 287 ops stand outstanding: they measure the
ratchet holding, never the debt shrinking.

FOURTH, duration is a separate and much smaller problem than volume. 76% of
tests run under 10ms, and 57% of the 343s clock sits in 15 modules doing real
filesystem scaffolding — `tests/commands/test_init.py` runs a real `gz init` 47
times at about 0.9s each, and one single test costs 15.31s.

FIFTH, there is NO mutation testing in the repository. Nothing measures whether
any of the 8380 tests would catch a bug, so the tautological audit's static AST
heuristic is the only instrument and it cannot answer that question. Adding one
is a STDLIB-FIRST named departure requiring an ADR.

## Decisions Made

- [operator-ruled] Retire the handoff resume gate's last arm (verbatim
  2026-08-15: "the handoff should be an advisor, not a gate-keeping nanny"), and
  amend `REQ-0.37.0-05-02` plus campaign line 340 in the SAME commit rather than
  silently contradicting them. Both said the arm retires WITH the unbuilt
  session-entry door and that a retirement preceding the door is "a regression,
  not an increment"; `ADR-0.37.0` is Pending at 0/6 with no implementation, so
  that premise was unmet and unscheduled.
- [operator-ruled] No skipping tests, whatsoever (verbatim: "no skipping tests,
  whatsoever"). Drove removal of every skip mechanism under `tests/`: a win32
  `skipIf`, 10 dead red-phase `skipUnless` guards, 43 artifact-existence
  `skipTest` guards, 2 TDD-red import guards, 2 registry-content guards, and 2
  genuine platform guards restructured to assert BOTH branches instead of
  skipping either.
- [operator-ruled] Union-merge the conflicted append-only ledger — chosen over
  `git rebase --skip`, which would have destroyed 4 local-only session-exit
  bookmarks, and over `--abort`.
- [operator-ruled] Restart the stalled `decommission-tautological-tests` chore,
  chosen over attacking the duration tail or adding mutation testing first.
- [operator-ruled] Authorize re-kinding prose-shaped REQs from BEHAVIOR to
  SUPPORT inside Completed/attested briefs, unblocking 146 REQs and 172
  deletable tests across 29 OBPIs.
- [agent-chose] Fix the duplicate hook group at the CLASS level: `_merge_hook_phase`
  now decides ownership per HOOK rather than by matcher identity, so a renamed
  matcher's orphan dies. Rejected simply deleting the stale `Write|Edit` group,
  which would have left the same bug armed for the next rename.
- [agent-chose] Re-key 3 entries in `data/tautological_test_baseline.json`
  (`is_file`/`exists` -> `read_text`) with the COUNT UNCHANGED at 290. The
  baseline keys on the first filesystem call in a function, so removing an
  existence check re-keys the entry onto the next call — a genuine shrink that
  identity-matching reads as new. The registered mechanism is a shrink-ratchet on
  the count.
- [agent-chose] Keep `HandoffResumeBlockedEvent` readable while deleting its
  emitter, because one such record sits on the append-only ledger. Removed its
  `trust_audits.events` waiver (keyed on emitters) but KEPT its
  `ontology.corpus` disposition (keyed on the live model union). Conflating those
  two registries broke 17 tests before the distinction was read correctly.

## Immediate Next Steps

1. Rule on how many of the remaining 28 OBPIs to re-kind per wave. Each is
   mechanically similar but surfaces its own latent incoherences, as the
   ADR-0.0.54 pilot did — so this is 28 small audits, not a batch operation. Do
   NOT sweep them unattended.
2. Rule on the `decommission-tautological-tests` acceptance criteria, which PASS
   while 287 ops stand. They measure the ratchet holding rather than the debt
   shrinking, which is exactly why the chore stalled unnoticed for eleven weeks.
   Changing them is a Heavy-lane chore-definition change and wants a GHI filed
   through `/ghi-author`.
3. Decide whether mutation testing is adopted. There is none today; it is a
   STDLIB-FIRST named departure requiring an ADR, and mutation runs are far
   slower than the current 343s, so it wants scoping to a subset such as
   `src/gzkit/governance`.
4. Decide whether to attack the duration tail. 197s of 343s sits in 15 modules;
   sharing scaffolded trees across read-only tests in `test_init` and
   `test_sync_cmds`, plus fixing the one 15.31s test, would cut roughly 40% of
   runtime with zero tests removed and zero governance risk.
5. Reconcile the 714 unlinked-spec advisory against the 146 re-kind candidates
   before acting on either. 8380 tests coexisting with 714 uncovered REQs means
   test volume is not tracking spec coverage, and the two numbers should be read
   together rather than worked separately.

## Pending Work / Open Loops

The tautological chore is RESTARTED but far from finished: 287 ops -> 280 across
93 files, with 240 of the 287 carrying disposition `convert`. Converting a
doc-grep needs either a REQ re-kind or a new validator scope, so the remaining
work is gated on operator rulings rather than on effort. The re-kind pilot
covered ONE OBPI of 29.

GHI #805 is CLOSED, resolved by deletion — the template it described no longer
exists. The class-level check it suggested (assert an emitted docstring's tool
list against the `settings.json` matcher) was deliberately NOT built, because its
only subject was the hook that was deleted; the general shape survives and is
worth building the next time a generated hook grows a self-describing docstring.

Two defects are recorded as insights only and are NOT tracked in GitHub: the
chore's defective acceptance criteria, and the fact that every partially-tagged
brief in the corpus is one edit away from surfacing latent Allowed-Paths
incoherence (the ADR-0.0.54 brief never listed `tests/**` while demonstrably
producing a test file).

The 866 undeclared output/render assertions reported by `gz test-shape` are
untouched and advisory. No mutation testing exists. The behave tier's log noise
and the per-clone `g0` authorship guard carry forward unworked from predecessor
handoffs.

## Verification Checklist

Run `git log --oneline -6` (expect `e51a3a09`, `e3e0d6a4`, `c5b3b2aa`,
`6c63b1e2`, `c3b4ee2c`, `c772497c`); `git status --short --branch` (expect clean,
level with `origin/main`); `git log -1 --format=%an` (expect `g0`).

Run `uv run gz check` (expect exit 0; the 714-finding spec-test-drift advisory is
pre-existing and does not affect the exit code). Run `uv run -m unittest -q`
(expect 8380 tests OK and ZERO skips — a nonzero skip count is a regression of
this session's work).

Confirm the gate is really gone rather than trusting this document: searching
`.claude/settings.json` for `handoff-resume-gate` must return nothing, and
`.claude/hooks/handoff-resume-gate.py` must not exist. Confirm no skip mechanism
returned: searching `tests/` for `skipTest(`, `skipIf(` or `skipUnless(` should
match only a docstring mention in `tests/commands/test_upgrade.py`.

Re-measure the chore instead of trusting the numbers above: `uv run gz test-shape`
for the live op count, and `uv run gz chores advise decommission-tautological-tests`
— noting that it reports PASS regardless, which IS the defect named in Immediate
Next Steps item 2.

Confirm the hook duplication is gone by reading the `PreToolUse` matchers in
`.claude/settings.json`: expect three groups, not four.

## Evidence / Artifacts

Session commits: `c772497c`, `6c63b1e2`, `c5b3b2aa`, `e3e0d6a4`, `e51a3a09`.

Resume-gate retirement: `src/gzkit/handoff_resume_gate.py`,
`src/gzkit/hooks/claude.py`, `src/gzkit/enforcement.py`, `src/gzkit/events.py`,
`src/gzkit/ledger_events.py`, `src/gzkit/ontology/corpus.py`,
`src/gzkit/governance/trust_audits/events.py`,
`.gzkit/skills/gz-session-handoff/SKILL.md`,
`docs/user/manpages/handoff-decide.md`, `docs/user/runbook.md`,
`docs/governance/governance_runbook.md`.

Amended surfaces: `docs/governance/build-to-1.0-campaign-2026-07-18.md`.

Hook-orphan fix and skip sweep: `tests/test_hooks.py`,
`tests/commands/test_upgrade.py`, `tests/governance/test_handoff_archive.py`,
`tests/governance/test_qc_binding_scope.py`,
`tests/governance/test_skill_self_close_drift.py`,
`tests/governance/test_attestation_receipt_validator.py`.

Chore wave 1 and the re-kind pilot:
`tests/governance/test_agents_md_map_doctrine_application.py`,
`data/tautological_test_baseline.json`,
`.gzkit/chores/decommission-tautological-tests/proofs/CHORE-LOG.md`.

Insights recorded this session: `.gzkit/insights/agent-insights.jsonl`.
Predecessor handoff:
`.gzkit/handoffs/20260815T092541Z-d1-withdrawn-on-measurement-six-briefs-authored.md`.
