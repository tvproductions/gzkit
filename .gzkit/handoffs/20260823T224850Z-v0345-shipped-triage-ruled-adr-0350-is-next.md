...
**`ADR-0.35.0-canon-entry-corpus-landing` is the next work, and the campaign says so in its own arithmetic.** Movement B is TOPMOST *and gated*; Movement A is HELD as a Movement while its ADR-0.35.0 box holds the in-flight feature position. ADR order is absolute (operator verbatim, campaign line 49: *"i will NOT go out of adr order, whatsoever."*). Live counts: ADR-0.35.0 1/10 (one `attested_completed`, one `in_progress`, eight `pending`), ADR-0.36.0 0/9, ADR-0.37.0 0/6. Read them from `uv run gz adr status <ADR-ID>`, never from a transcribed figure.
