...
adr_id: null
branch: main
timestamp: '2026-08-17T11:05:11Z'
agent: claude-code
continues_from:
- .gzkit/handoffs/20260817T070534Z-session-exit-bookmark.md
- .gzkit/handoffs/20260817T005213Z-ledger-merge-driver-and-ordering-witness.md
---

## Current State Summary

An architectural and roadmap review of gzkit against the August-2026 agentic-development stack was run, retained, dispositioned through a one-question-at-a-time operator interview, and the campaign was amended with the results. Five commits, `d82206df0` to `ba60027fa`, tree clean and `0 0` against origin at handoff time.

Landed: the review plus the operator's prompt retained under `docs/evals/`; campaign § Amendments gains a dated 2026-08-17 entry with seven subsections A through G; §5's 1.0 definition re-cut against a declared target and gaining a new five-layer gate; Movement D re-scoped to absorb orientation and the `finding` entity; **Movement E created** and given a binding internal sequence; § Rulings Register gains 15 rows covering 12 numbered rulings.

Gate evidence observed, not asserted: `uv run gz validate` 13 scopes passed; `uv run -m unittest discover -s tests -t .` ran 8436 tests, exit 0; pre-push `gz check` passed on every sync. The `--cli-alignment` gate **rejected the first push** because the review wrote two unregistered verbs as if they existed; that rejection was correct and the references were rephrased.

No ADR or OBPI was advanced. No box was checked. This session was campaign-level governance authorship only.

## Important Context

**The review carried errors, and the corrections are the load-bearing part of this session.** Eight of its characterizations were wrong, every one because the campaign's transcription of an ADR was read instead of the ADR body. Those transcriptions are lossy in one consistent direction: they preserve status, counts, and citations and drop the reasoning. Operator correction, verbatim: *"i fear that you riff on things without rereading docs."* The corrections are recorded in the review's own § Corrections and in the campaign amendment's § Corrections so a later reader does not inherit them.

**Two review recommendations were overturned by ruling and must not be revived.** The airlock is NOT deferred post-1.0, and `ADR-0.36.0` OBPI-09 is NOT dropped. `ADR-0.33.0` § Persona says the airlock *"earns trust by biting, never by ceremony"* and its Positive #1 is *"Prosthetic memory"*, so it is memory AND gate by design. `ADR-0.36.0`'s first sentence is *"Install an always-on 2nd opinion"*, and OBPI-09 is the always-on arm; its metaphor is Crew Resource Management, not adversarial review.

**The 20-of-23 empty seam-maps are a disclosed residual, not an undetected defect** — operator-attested 2026-07-10 as `ADR-0.33.0` § Calibration frontier, with Stage 1 held diagnostic-only so a mis-calibrated gate cannot 2am-wall a real pipeline.

**`session_exit_bookmark_skipped` is the mechanism working.** `session_exit.py` lines 62 to 64: `skipped=True` means nothing needed booking. 36 of 44 carry `exit_reason: clear` because a covering handoff already exists. Any future citation of N-skips-and-0-writes as a defect must say which of the two states it means.

**ADR order is absolute and gates Movement E.** `ADR-0.38.0` is not authorable until `ADR-0.35.0`, `ADR-0.36.0`, and `ADR-0.37.0` land. Movement B stays TOPMOST, Movement A stays HELD, `ADR-0.35.0` is the in-flight feature and `Draft` HOLDS through implementation (ruled 2026-08-12, closed, do not re-raise).

**The five layer-parity findings survive the corrections unchanged** because they rest on measured absence via grep, ledger, and registry rather than on ADR prose: harness portability, foreign skills, MCP and tools, agent-and-session identity, eval.

## Decisions Made

- [operator-ruled] 1.0 is **both** stations of §1 (verbatim: *"I want it to be a complete and contemporary toolkit for agentic work, but I am willing to let scope change on some things. public product and personal toolkit."*). Scope negotiable per item; the standard is not.
- [operator-ruled] The five contemporary-stack layers are built **properly**, not thinly (verbatim: *"All five, properly built"*), selected against a stated cost of quarters of work and a stated risk of 1.0 receding.
- [operator-ruled] **Nothing comes out of 1.0** to fund that (verbatim: *"Nothing — move the date instead"*). Every §5 gate retained; target moves to roughly 2027-04 by declaration, never by slippage.
- [operator-ruled] Execution layer is **hybrid** (verbatim: *"Hybrid — runner for integrity gates only"*). Adapter by default; gzkit invokes the cross-vendor reviewer itself because a self-summoned adversary is not an adversary. Full orchestration declined.
- [operator-ruled] The external forcing function waits for `ADR-0.38.0` in **strict ADR order** (verbatim: *"Wait for ADR-0.38.0 in strict order"*). Accepted cost: it certifies rather than informs.
- [operator-ruled] Step 4b is **mandatory and unconditional** before Gate 5 on OBPI completion (verbatim: *"yes, absolutely yes"*).
- [operator-ruled] Step 4b does **not** extend to GHI direct-fix or MX work (verbatim: *"not presently. obpis are feature implementation - new intended behavior. ghi direct fix is repair and correction... akin to an 'airworthiness directive'... So, the 4b gate is QC for new feature work."*). This was already canon at `ADR-0.36.0` § Intent and was re-asked in error.
- [operator-ruled] Movement D **absorbs the orientation problem** (verbatim: *"Movement D should absorb the orientation problem. a handoff is there to orient a new session from the prior session - it is an advisory shift change orientation"*).
- [operator-ruled] Simultaneity substitute is **a plus b plus c** (verbatim: *"it really is a + b + c. yes to checklist, BUT, handoffs are advisory/informative to operator. also, a git-sync is always a permitted first operation after a handoff is created."*). Position Relief Checklist becomes a per-project schema-validated artifact.
- [operator-ruled] `prosthetic` and `synthetic` memory are one concept with two names (verbatim: *"probably the same"*).
- [operator-ruled] The airlock is **not a verification gate** — awareness and synthetic memory, orientation, contamination watch, disturbance monitoring; it and the handoff cooperate. Three of the four purposes appear nowhere in `ADR-0.33.0`.
- [operator-ruled] The campaign-narrative guard **was never written**; added to Movement D scope (verbatim: *"it was never written - add the guard to Movement D scope"*).
- [operator-ruled] The three coupled campaign body edits are **ratified** (verbatim: *"ratify the body edits and git-sync"*).
- [operator-ruled] The flight-test gate gets an owning §6 checkbox (verbatim: *"add the flight-test checkbox to §6"*).
- [operator-ruled] Movement E's internal sequence is **binding: vocabulary before sortie** (verbatim: *"sequence the vocabulary box before the sortie box"*).
- [agent-chose] **Movement E** as the home for the flight-test checkbox, rather than Movement C or Housekeeping. Grounded in §1 line 79, amended 2026-07-18: *"One external forcing function is a Movement, not a nicety."* §6 had never created one. Disclosed as an agent judgment in amendment § F; move it if the placement is wrong and the boxes travel intact.
- [agent-chose] Rephrased the two unregistered verb references rather than injecting `gz-validate-skip` markers, because all six sit mid-paragraph where an HTML comment splits the paragraph. This diverges from the campaign's marker convention at lines 279 and 1386, where the markers sit cleanly before checkbox bullets.
- [agent-chose] Appended amendment subsections E, F, and G rather than folding later rulings into the already-ratified and already-pushed A through D, so ratified text is never rewritten.

## Immediate Next Steps

1. **Resume the campaign at its topmost unchecked item.** Movement B is TOPMOST and Movement A is HELD, but ADR order is absolute: `ADR-0.35.0-canon-entry-corpus-landing` is the next work at 0/10. Verify with `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing` rather than trusting this line.
2. **Do not re-raise the `ADR-0.35.0` lifecycle question.** Ruled 2026-08-12: `Draft` HOLDS through implementation. It was carried unworked across seven handoffs before being written down.
3. **Capture the two owed doctrine items** surfaced this session and recorded as CARRIED with capture owed in § Rulings Register: the airlock's four stated purposes are absent from `ADR-0.33.0` (orientation, contamination watch, awareness score zero occurrences), and `AGENTS.md` § Operator Doctrine needs a cooperation clause so a reader meets the airlock-handoff join and not only the three-system fence. Route through `gz content remember`, never a hand edit.
4. **Settle `prosthetic` versus `synthetic` memory as one term.** PRD § Ubiquitous Language is the home. Measurable cost today: `rg synthetic` misses the airlock and `rg prosthetic` misses the handoff system.
5. **Narrow the README's agent-runner claim** to match ruling B.5. It currently reads *"governance meta-harness and agent runner"* and no code runs a harness; under gzkit's own enforcement-claim rule that is a facade at the doc layer.

## Pending Work / Open Loops

- **The `OBPI-0.33.0-06` attestation question is recorded and deliberately undecided.** That OBPI reached `attested_completed` with a resolution citing `test_campaign_has_no_stale_unchecked_narrative`, a test that does not exist. Whether that is a defective attestation or an honest attestation with a false supporting sentence has no ruled scope. `ADR-0.33.0` stays `Validated`. Amendment § E states the non-decision explicitly.
- **The false Layer-2 record stands unretracted.** `AGENTS.md` § Never #2 [settled] forbids editing the ledger, and an append-only Layer 2 correctable in place would not be evidence. The correction lives in the campaign amendment and in `.gzkit/insights/agent-insights.jsonl`.
- **Radar registry widening is ruled CARRIED but unbuilt.** The competitor radar tracks 15 entries, all authoring-side, and images none of the five layers where gzkit measured absent. Last scan `2026-05-07` against a monthly cadence; all six of its suggested MOVEs remain `pending-operator-session`, including MOVE-004 on typed review receipts, which is the same review-parity gap still open.
- **PRD rewrite is ruled CARRIED but unstarted.** `PRD-GZKIT-1.0.0` is dated 2026-01-22, status `Draft`, and its Non-Goals forbid multi-agent orchestration while `ADR-0.18.0` ships it in `pipeline_dispatch.py`.
- **Movement E item 2 blocks item 3 and neither is started.** `CANONICAL_STEP_COMMANDS` is `uv`, `ty`, `unittest`, `mkdocs`, so no non-Python project can produce a canonical receipt.
- **Carried unresolved from prior sessions, not touched here:** re-baseline the W1 target and measure the third cost axis for SUPPORT re-kinds. Both gate the same unauthorized re-kind sweep.
...
uv run -m unittest discover -s tests -t . -q
uv run -m unittest tests.governance.test_active_campaign_registry -q
uv run -m unittest tests.scripts.test_session_orientation -q
git rev-list --left-right --count origin/main...HEAD
```

Expected at handoff time, each observed rather than assumed: bare `validate` reports 13 scopes passed; the two-scope run passes; the unit suite ran 8436 tests at exit 0; the campaign registry suite is 7 of 7; the orientation suite is 68 of 68; the branch count is `0 0`.

Campaign shape checks, to confirm the amendment parses as intended:

```bash
grep -nE '^\*\*Movement [A-E]' docs/governance/build-to-1.0-campaign-2026-08-16.md
grep -cE '^\s*- \[[ xX]\]' docs/governance/build-to-1.0-campaign-2026-08-16.md
uv run python scripts/session_orientation.py
```

Movements A through E must be present; the box count is 25 with 7 done; orientation must report `7/25 checklist items done`. Every §9 register row must read CARRIED or WITHDRAWN, with none undispositioned.

## Evidence / Artifacts

- `docs/evals/gzkit-campaign-architecture-review-2026-08-17.md` — the retained review: measured baseline, per-exemplar parity across ten layers, ownership boundary, revised milestones, § Corrections, and a 16-row § Operator Decisions ledger
- `docs/evals/gzkit_campaign_architecture_eval_prompt.md` — the operator's review prompt, now under version control
- `docs/governance/build-to-1.0-campaign-2026-08-16.md` — amended: § Amendments 2026-08-17 subsections A through G, §5 re-cut plus a new five-layer gate, Movement D re-scoped, Movement E created and sequenced, § Rulings Register plus 15 rows
- `.gzkit/insights/agent-insights.jsonl` — three records appended: an `improvement` on reading Layer 3 as Layer 1, a `defect` on the fabricated guard claim, and a `defect-resolution` on its routing

Commit range `d82206df0` to `ba60027fa`, five commits, all through `gz git-sync --apply` with the pre-push `gz check` gate passing.
