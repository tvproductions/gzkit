...
adr_id: null
branch: main
timestamp: '2026-08-21T11:59:54Z'
agent: claude-code
session_id: 3c344686-e8d2-468f-8a14-febfe5604541
continues_from: .gzkit/handoffs/20260821T092739Z-obpi09-attested-unpushed-mx-open.md
---

## Current State Summary

Post-mortem of the 2026-08-21 OBPI-0.35.0-09 session, followed by repair of three findings it surfaced. Verified at authoring: branch `main` at `a7c4465a`, `git rev-list --left-right --count origin/main...HEAD` returns `0 0` (pushed, in sync).

The prior session's blocked push was cleared first. Its final two commits (`711fe562`, `4a919af1`) changed the pipeline-gate's behaviour in one file each, touched zero test files, and left `test_allows_when_richer_per_obpi_marker_matches` asserting the retired allow. The pre-push gate caught it: 1 failure in 8542. Repaired at the surface rather than deleted (`e1a3d6b2`), then pushed.

The post-mortem read the Layer-2 ledger, both prior session transcripts, and the GHI record. Its central measurement: across the three sessions that implemented OBPI-0.35.0-09, the tool census was `Bash=348, Write=0, Edit=0, Task=0`. Six governance hooks bind the `Write|Edit|NotebookEdit` matcher and key on `tool_input.file_path`, a field a Bash payload does not carry, so all six executed zero times while roughly 350 lines of production code were authored at `current_stage: verify`. The ledger is silent for 3h38m across that window because `ledger-writer.py` is a PostToolUse `Edit|Write` hook too.

Four findings were filed. GHI #844, #845, #846 were fixed, verified, closed, and pushed. GHI #847 was filed as #844's child and remains open. Three closed GHIs carry disclosed residuals in their close comments rather than clean claims.

MX hangar is OPEN and was not closed this session. `.gzkit/mx.json` records `session_id d3c16ae0-719a-4276-9fca-9c9739ee3be2`, `opened_at 2026-08-21T08:56:53Z`, `attestor g0`, `inspection_scope ADR-0.35.0`. `uv run gz obpi lock list` shows `mx-session ACTIVE agent=claude-code-d3c16ae0 elapsed=181m ttl=1440m`.

## Important Context

**MX narrows what every green in this session means.** `_run_scope_checks` at `src/gzkit/commands/validate_cmd.py:917` runs each scope then discards its errors unless `disposition.grounds()` holds. With the hangar open, ordinary `gz validate` scopes resolve `advisory` and their findings do not fail the build. Test, lint, typecheck, behave, and docs tiers DO bind regardless — the pre-push gate blocked a real test failure earlier in this same session. So every `gz check` exit 0 recorded here is strong on those five tiers and advisory on validate scopes. The three new validators were proven instead by their unit tests and by calling their registry runners directly, both of which bypass the MX layer.

**Gate 5 is NOT demoted.** Measured against `gzkit.mx.invariants.GATE5_INVARIANTS` with the hangar open, all five members (`gate5-attestation`, `grader-gaming`, `ledger`, `operator-pii`, `secrets`) resolve `checkpoint=aog-mx-hangar, grounds=True`. `.gzkit/rules/mx-mode.md` is accurate. An earlier claim in this session that Gate 5 measured `grounds=False` was wrong and is corrected on GHI #843.

**A separate observation, not a defect claim:** an unregistered scope name resolves to `advisory` rather than raising. That fallthrough is what let a mistyped identifier read as a finding.

**The agent can open and close the hangar.** `gz mx exit` is in no deny list; `.claude/settings.json` denies six patterns, all `git *--no-verify*`. The CLI requires `--attestor ATTESTOR`, documented as "Operator identity who signs airworthiness (required; never an agent)" — the constraint is on the VALUE, which must be `g0`, not on who runs the command. AGENTS.md operator canon settles it: no transport mechanism may be cited as a reason an agent cannot record human attestation. Prior claims in this session that `gz mx exit` was harness-denied were inherited from the resumed handoff and never re-derived; they were false.

**`gz mx exit` is a hard gate.** It re-runs every guard at full strength against the enter-time inspection scope, green-or-grounded, no `--force`, and is the only path that clears the marker. Closing it therefore verifies this session's work under non-advisory conditions.

**Coupled-surface cost is real here.** Adding one `gz validate` scope obligated six further surfaces before the tree went green: QC step classification, an exemption declaration, check-scope membership, the explicit-tier set, a per-flag manpage section, and a negative control. Each surfaced one at a time, each costing a full `gz check` cycle of roughly four minutes over an 8582-test suite.

## Decisions Made

- [operator-ruled] File all four post-mortem findings as GHIs before building anything (verbatim selection: "File all four as GHIs now"). Nothing was to be built without an explicit further ruling.
- [operator-ruled] Fix all three filed GHIs, direct-fix path (verbatim: "do it, fix it, all we do now in gzkit is fix things"). Per AGENTS.md operator canon a GHI is the work order and the receipt, so no ADR or OBPI ceremony was opened.
- [operator-ruled] The agent opens and closes the MX hangar, not the operator (verbatim: "the mx system isn't going to work if you expect me to open and close it"). Standing from this session forward.
- [agent-chose] Filed three GHIs rather than four. Step-0 prior-art found GHI #770 already named the absorb-pool-ADR class verbatim and fixed that instance on 2026-08-08; re-filing would have been the #459/#460 duplicate regression.
- [agent-chose] Moved the Stage-2 write fence to a pre-commit locus reading `git diff --cached` rather than widening the PreToolUse hook. A tool-keyed gate is bypassed by choosing a different tool; a filesystem-outcome gate is not.
- [agent-chose] Split the fence into TWO rules over one stage vocabulary. Authoring `src/**` at `sync` is refused; committing `src/**` at `sync` is permitted, because that stage is where the pipeline runs `gz git-sync --apply`. A single shared stage set would be wrong at one call site, and refusing the sync commit would have made the gate un-compliable.
- [agent-chose] Made declared single-driver PASS `gz obpi precomplete` while silent single-driver fails. A gate with no compliant path for a session that genuinely cannot dispatch gets worked around rather than obeyed.
- [agent-chose] Scoped the lifecycle-pointer validator to the CLAIM, never the citation. Citing a Superseded ADR stays legal so history references remain writable; only asserting something is pending from a terminal artifact is refused.
- [agent-chose] Posted corrections as GHI comments rather than editing issue bodies, so the errors stay in the audit trail instead of being erased.

## Immediate Next Steps

1. **Close the MX hangar.** Run `/gz-mx` and `uv run gz mx exit --attestor g0`. This is a hard gate that re-runs every guard at full strength, so it doubles as non-advisory verification of this session's three fixes. If a guard is red, exit refuses and names it. Per the operator ruling above, the agent runs this; the operator's word authorizes it.
2. **Decide whether to run the offered verification pass** over the close comments of GHI #844 [settled], #845 [settled], and #846 [settled]. Three verification errors were made this session, so the load-bearing claims in those permanent records deserve re-derivation. The offer was made and not yet ruled on.
3. **Rule on GHI #847** (OPEN) — four sibling hooks still key on `file_path` and remain bypassed by Bash writes. `ledger-writer` is the consequential one and is why the ledger went silent for 3h38m; its fix needs an operator decision on ledger volume before it is buildable.
4. **Rule on the GHI #842 and #843 direction.** They are two halves of one closed loop: the runtime emits a ledger that fails the ts-order gate, and the maintenance mode built to repair governance cannot commit the repair.
5. **Resume `ADR-0.35.0-canon-entry-corpus-landing`**, which is the work by ascending-semver ADR order. Run `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing` for the live landed count rather than trusting any figure written here. Per operator canon, run every OBPI through `gz-obpi-pipeline` end to end including the Stage-2 dispatch — which is now recordable via `uv run gz obpi dispatch` and fail-closed at precomplete.

## Pending Work / Open Loops

- **MX hangar open.** Marker `.gzkit/mx.json` present; `mx-session` lock ACTIVE at 181m elapsed against a 1440m TTL. Until it closes, every `gz validate` scope is advisory.
- **GHI #847 (OPEN)** — `session-staleness-check`, `obpi-completion-validator`, `instruction-router`, and `ledger-writer` still key on `tool_input.file_path`. `post-edit-ruff` is covered only incidentally, by the unrelated pre-commit `ruff-check` hook.
- **GHI #843 (OPEN)** — MX demotes `gz validate` guards but the hangar never reaches the pre-commit hooks. Carries this session's corrected measurement.
...
- **GHI #839 (OPEN)** — `gz arb red` uses HEAD as base, so already-landed work always witnesses `failure_class=none`. Leaves the RED witness degraded.
- **Stage-4 narrator dispatch has no channel.** The new dispatch roster covers Stage 2 only; `gz-obpi-pipeline` mandates four personas and three are now attestable.
- **`gz-adr-audit` and `gz-adr-closeout-ceremony` have no dispatch channel at all.** Named as GHI #770 [settled]'s residual and still open ground; their skill bodies now say so plainly rather than promising a promotion that cannot occur.
- **The new lifecycle-pointer validator scans two surfaces only** (`.gzkit/skills/*/SKILL.md`, `.gzkit/rules/*.md`). The identical defect in an OBPI brief or ADR body would pass.
- **`gz obpi precomplete` behaviour changed.** It now exits non-zero for an OBPI with an active pipeline marker whose Stage-2 dispatch is neither recorded nor declared. Every in-flight OBPI is affected; the escape is one command.
- **AGENTS.md delivery-cap overrun remains untracked.** `gz check` advisory reports AGENTS.md at 42235 B against the codex cap of 32768 B, 9467 B over, with two must-survive sections straddling or past the cap. Remedy is the `instructions-files-diet` chore, not a session edit. No GHI filed; surfaced to the operator twice without a ruling.

## Verification Checklist

- `git rev-list --left-right --count origin/main...HEAD` returns `0 0` and `git rev-parse --short HEAD` returns `a7c4465a`.
- `ls .gzkit/mx.json` confirms whether the hangar is still open; `uv run gz obpi lock list` shows the `mx-session` lock state.
- `uv run gz check` passes all 57 steps. Read it knowing the MX caveat in Important Context: the validate scopes are advisory while the hangar is open.
- `uv run gz validate --lifecycle-pointers` exits 0 on a clean tree. To confirm it bites, run `audit_lifecycle_pointers` directly or run its unit tests, since the CLI path is MX-demoted.
- `uv run -m unittest tests.test_pipeline_stage_fence tests.test_obpi_dispatch_channel tests.governance.test_lifecycle_pointers` returns OK for 38 tests across the three fixes.
- `uv run gz cli audit` reports 136 of 136 commands fully covered.
- `grep -rn "awaiting promotion" .gzkit/skills/ src/gzkit/skills/ .claude/skills/` returns nothing outside the unrelated `gz-design` usage.
- `uv run gz obpi dispatch --help` resolves, confirming the new verb is registered.
- `gh issue view 844 --json state` and the same for 845 and 846 each return CLOSED; 839, 842, 843, and 847 return OPEN.

## Evidence / Artifacts

- `src/gzkit/pipeline_stage_fence.py` — single authority for the post-Stage-2 rule, imported by both the generated hook and the pre-commit guard
- `tests/test_pipeline_stage_fence.py` — 19 tests including the write-versus-commit divergence at `sync`
...
- `tests/test_obpi_dispatch_channel.py` — 12 tests
- `src/gzkit/governance/trust_audits/lifecycle_pointers.py` — the stale-pointer class fence
- `tests/governance/test_lifecycle_pointers.py` — 7 tests, both polarities
- `docs/user/manpages/obpi-dispatch.md` — manpage for the new verb
- `src/gzkit/hooks/guards.py` — `forbid_post_authoring_src_commits`, the arm no tool choice evades
- `src/gzkit/commands/obpi_precomplete.py` — the `stage2_dispatch` Stage-5 check
- `.gzkit/insights/agent-insights.jsonl` — the improvement record for this session's three verification errors
- `.gzkit/mx.json` — the open hangar marker
- `.claude/settings.json` — the six-entry deny list showing no `gz mx` rule
