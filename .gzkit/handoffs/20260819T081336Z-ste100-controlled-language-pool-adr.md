...
adr_id: null
branch: main
timestamp: '2026-08-19T08:13:36Z'
agent: claude-code
continues_from: 20260819T073101Z-session-review-ghi830-closed-tag-repair-measured.md
---

## Current State Summary

Session opened on an operator question — had ASD-STE100 (asd-ste100.org) ever been considered for a pool ADR — and closed with the pool ADR authored, registered, and two GHIs closed. All work is committed and pushed to `main` at `050780628`; the tree carries no tracked changes.

Landed this session, oldest first: `0fc82fe5f` (pool ADR authored), `f5ff29e20` (dfch/AGPL/MCP survey folded in), `f255c5fea` (skill-doc fix closing GHI #831), `0c6f61614` (GHI #579 absorbed as arm 4, pool ADR registered), `050780628` (ASD licence claims corrected against the read source).

`ADR-pool.controlled-language-for-control-surfaces` now carries five arms in deliberate order: RFC 2119 modal discipline, a terminology registry modelled on `ledger-vocabulary-inertness`, a decrease-only prose-metric ratchet modelled on ADR-0.35.0 Decision 3, imperative-density measurement (absorbing GHI #579), and a licence-gated approved-word dictionary ranked last because it is the only arm needing anyone's permission. The ADR is registered in the ledger and appears as Pending/HEAVY in the Pool table of `uv run gz adr report`.

GHI #831 (skill doc claimed `gz plan create` books `adr_created` without the pool carve-out) is CLOSED fixed. GHI #579 (imperative-density as budget unit) is CLOSED superseded against the pool ADR.

## Important Context

Three things a resuming agent will otherwise re-derive the hard way.

First, the warrant for this ADR is a measurement, not a preference. Measured 2026-08-18 across `AGENTS.md`, `CLAUDE.md` and 26 `.claude/rules/*.md`: 1,466 sentences, 3,861 distinct word forms, 1,788 used exactly once. Purely directive files already pass a 20-word bar without anyone aiming at it (`changelog-release-notes.md` 6 percent over 20 words); narrative-carrying files do not (`tool-skill-runbook-alignment.md` 60 percent, `guardrail-feedback-prose.md` mean 28.3). The real line is binding-directive versus rationale-narrative, which gzkit already ruled on. An earlier agent framing drew it at doctrine-versus-procedure and the measurement refuted that; do not redraw it there.

Second, the licence position is now READ from the source, not recalled. Issue 9 front matter: section Copyright notices sets a default deny absent written authority of an ASD officer; a separate section Special usage rights grants irrevocable free permission to eight enumerated categories, of which 8 is universities and research institutes for educational purposes. That paragraph was WRONG when first committed (fetch had failed; written from a search summary plus recall) and carried a fabricated issue day. The provenance note inside the ADR records this deliberately.

Third, two measurement traps bit this session and both are recorded in-issue. A raw `grep` over `.gzkit/ledger.jsonl` is NOT a measurement of ledger state — the log has rename semantics and only the graph projection answers whether an artifact is registered. And `git diff --stat` settles "did I clobber prior work" questions that commit-message archaeology does not.

## Decisions Made

- [operator-ruled] Author a pool ADR for controlled language rather than filing ASD-STE100 as a bibliography entry (verbatim: "yes, draft it"). The agent had recommended the bibliography route; the operator overrode it on the ground that a named chore lacking a criterion is a destination, not a citation.
- [operator-ruled] Narrow GHI #831 to the documentation line and commit the ADR (verbatim: "narrow #831 to the doc line and commit the ADR").
- [operator-ruled] Fix GHI #831 and push (verbatim: "fix #831 and push").
- [operator-ruled] Register the pool ADR and absorb GHI #579 into it rather than closing it as a bare sibling (verbatim: "register the pool ADR, include 579 as a part of it").
- [agent-chose] Ranked the five arms so the licence-gated dictionary is last and arms 1 through 4 are explicitly not blocked by it. Rejected alternative: leaving the dictionary as arm 4 and treating the licence question as gating the whole ADR, which is the mistake the agent made earlier in the session.
- [agent-chose] Absorbed GHI #579 as a full arm rather than a cross-reference, on the argument that the length ratchet and the count ratchet are individually gameable and jointly sound. Recorded inside the arm so reopening it means defeating an argument.
- [agent-chose] Wrote the correction of the fabricated ASD source claims into the ADR as a visible provenance note rather than silently patching it. Rejected alternative: quiet amendment, which leaves a reader unable to tell a read claim from a recalled one.
- [agent-chose] Cited `src/gzkit/commands/plan.py` by symbol and path rather than by line number in both skill fixes, so the references do not rot on the next edit.
- [agent-chose] Kept the ledger write for registration out of the skill-doc fix commit, so a documentation fix did not carry a governance state change.

## Immediate Next Steps

1. Rule on whether ASD-STE100 Issue 9 gets onboarded into project references, and by which mechanism. It is NOT onboarded today. The usual mechanism is unavailable: `docs/governance/research_sources/` holds full extracted text of its 32 sources, and doing that for this document would redistribute copyrighted material the project has no established right to redistribute. Three options: cite-only entry naming publisher, issue, date and URL with no text; promote `ADR-pool.design-references-bibliography` so a real `docs/REFERENCES.md` surface exists; or leave the pool ADR itself as the reference record, which is the status quo.
2. Decide the operator's own standing under Special usage rights category 8. This is the single fact that unblocks or permanently gates arm 5 of the pool ADR, and only the operator can answer it. Arms 1 through 4 do not wait on it.
3. GHI #813 (handoff authorship has no Layer-2 event; 387 documents, zero provenance) is the nearest live thread and was deliberately untouched this session. Its design note describes a documents-on-disk versus authorship-events-in-ledger reconciler; if that is built, scope it over (document glob, authorship event type) rather than hardcoding handoffs.
4. Do NOT promote `ADR-pool.controlled-language-for-control-surfaces` without a separate ruling. It is parked post-1.0 because `ADR-0.35.0-canon-entry-corpus-landing` is the in-flight feature and one-feature-at-a-time holds.

## Pending Work / Open Loops

- ASD-STE100 is not onboarded into any references surface. `docs/REFERENCES.md` and `docs/references/` do not exist; `ADR-pool.design-references-bibliography` (pool, Lite, dated 2026-04-13) is unpromoted, so there is no live bibliography surface to onboard into.
- GHI #813 open and untouched. Related by family to the pool-ADR registration work but NOT a sibling cut of GHI #831 [settled] as narrowed; the overstated parallel was retracted in a follow-up comment on #813 itself.
- GHI #533 open. Sibling of the absorbed GHI #579 [settled] on the byte axis rather than the language axis; unaffected by the absorption.
- Arm 5 of the pool ADR is gated, not scheduled. Blocked on the category-8 question in next step 2.
- Arm 4's metric choice (binding-bullet count, imperatives per 1k chars, or classification-weighted count) is deliberately left open to promotion rather than pre-empted in the pool ADR.
- The measuring script behind the section Intent numbers was run from a scratchpad and is NOT committed. The method is stated in the ADR so the figures can be re-derived; they are marked as a dated record, not authoritative values.
...
```bash
git log --oneline -5                      # expect 050780628 at HEAD
git status -sb                            # expect main synced, no tracked changes
uv run gz adr report                      # expect the controlled-language pool ADR Pending HEAVY in the Pool table
uv run gz register-adrs --pool-only --dry-run   # expect no unregistered pool ADRs
gh issue view 831 --json number,state,stateReason
gh issue view 579 --json number,state,stateReason
gh issue view 813 --json number,state,stateReason
uv run gz validate --taxonomy --documents --adr-status-fresh
uv run mkdocs build --strict
```

Expected issue states at authoring time: 831 CLOSED/COMPLETED, 579 CLOSED/COMPLETED, 813 OPEN. Re-resolve rather than trusting these, per the Claim Verification Gate.

To confirm the skill-doc fix propagated to every surface:

```bash
grep -rl 'books nothing' .gzkit/skills .claude/skills .agents/skills .github/skills src/gzkit/skills
```

Expect ten files: `gz-adr-create` and `gz-plan` across five surfaces.

## Evidence / Artifacts

- `docs/design/adr/pool/ADR-pool.controlled-language-for-control-surfaces.md` — the ADR authored and amended this session; five arms, five constraints, seven alternatives, plus the source-provenance note
- `.gzkit/skills/gz-adr-create/SKILL.md` — Trust Model now states the non-pool carve-out
- `.gzkit/skills/gz-plan/SKILL.md` — step 8 now scopes automatic registration to non-pool kinds and names `gz register-adrs --pool-only --dry-run` as the pool verification
- `docs/governance/GovZero/adr-status.md` — regenerated by registration, 87 ADRs
- `.gzkit/ledger.jsonl` — carries the `adr_created` event for the pool ADR at 2026-08-19T07:54:55Z plus the `agent_sync_completed` row from the mirror sync
- `data/instructions_files_budget.json` — the surface whose diet chore this ADR aims to give a criterion; read for the 2026-08-17 cadence rulings that constrain arms 1 through 4 to chore-time rather than per-turn
- `docs/design/adr/pool/ADR-pool.design-references-bibliography.md` — the unpromoted destination named in pending work
