...
adr_id: null
branch: main
timestamp: '2026-08-17T21:10:11Z'
agent: claude-code
session_id: 2ad6f031-07a4-4a62-b3ad-1abf2ae38ae7
continues_from: .gzkit/handoffs/20260817T190544Z-session-close-815-open-816-817-fixed-narrative-insight.md
---

## Current State Summary

Resumed the predecessor, booked proceed on three of its five advised steps, then six operator rulings carried the session well past that scope into a doctrine alignment.

Five commits, all on main:

- `56123ad9c` - the delivery witness stopped transcribing GHI state into its recovery prose
- `02205b10b` - memory-hygiene migrations routed to sources rather than renders
- `6918949e4` - `agents-md-architectural-boundaries` chore retired
- `8eb58648a` - `instructions-files-diet` v3.0.0: render/check/recommend/consult
- `cdcb92d95` - the 1.0 fail-closed exit condition retired across three surfaces; witness quieted

GHI #818 filed (governance: Architectural Boundaries rest on a memo, not an ADR) and left OPEN with a blocker comment. Evidence recorded on #815; cross-link posted on #799.

Issue state at authoring: #815 OPEN, #818 OPEN, #799 OPEN, #816 and #817 closed.

No ADR or OBPI advanced. The campaign spine has been set aside for a third consecutive session, this time by operator ruling rather than drift.

## Important Context

**The per-turn surface is a versioned build, and that is the fact the whole session turned on.** `sync_agents_md` plays back a Gate-5-attested rendition verbatim; `.gzkit/renditions/AGENTS.md/<consumer>.corpus.json` is a release manifest carrying `corpus_fingerprint`, `corpus_entry_count`, `committed_ts`, `attestor`. `gz content remember` appends to the corpus and moves NOTHING. Only `gz content commit` changes the rendered build. So the rendered surface cannot drift on its own, which is why a fail-closed budget gate had nothing to bite on and why the per-turn alarm was structurally misplaced - it alarmed about the release rather than about unreleased accumulation.

**The corpus is only part of AGENTS.md.** 59 entries, 16321 B of text against a 34354 B rendition. 55 of 59 are `tier: invariant` (14427 B) and verbatim-preserved by the composer; only 4 are `compressible` (1894 B), and three of those four are `prime-directive-ownership` and `operator-doctrine-verbatim-canon`. Any claim that the diet chore can close a gap by compression must be measured against 1894 B, not against the surface size.

**A compliant codex rendition exists and is delivered to nothing.** `.gzkit/renditions/AGENTS.md/codex.md` is 15764 B with `architectural-boundaries` at byte 5605; the claude rendition is 34354 B with it at 33759. `sync_surfaces.py:374` and `compose.py:29` both hardcode the string "claude", while `data/vendor-manifest.json` routes AgentContract to both consumers. GHI #815's breach is therefore a playback-routing defect, not a size defect. Recorded on the issue; UNRULED.

**The Coverage Ledger is a real forcing function.** Bumping a rule version refuses `gz validate --advisory-scorecard` until the scorecard rows are re-read. That refusal is what found row 58 stale in two opposite directions - calling the shape check forthcoming when it ships, and the budget arm enforced when it is disarmed.

**A chore's registry class sets its blast radius.** No `projectLocal: true` means canonical class: shipped in the wheel, scaffolded into every adopting project by `gz init`. Check the class before scoping a chore defect.

## Decisions Made

- [operator-ruled] Fix the stale witness prose, sweep the sibling chores, and run the repaired diet chore (verbatim multi-select: "Fix the stale witness prose first, Step 1 - sweep sibling chores, Step 3 - run the repaired diet chore"). Booked via `gz handoff decide`; advised steps 2, 4 and 5 recorded set-aside. Step 3 was NOT executed - the § 2 check it now prescribes showed compression to be the wrong instrument.
- [operator-ruled] Retire `agents-md-architectural-boundaries` and file the missing ADR gap (verbatim: "retire it and file the missing ADR gap"), after asking what purpose the chore had served and being shown it had never performed its stated work.
- [operator-ruled] The chore must render, check, trim, compress, and ask questions when needed (verbatim: "the chore should render, check, trim, compress. and ask questions if needed.").
- [operator-ruled] The chore recommends and consults before acting, not after (verbatim: "it should recommend compression and trimming and consult the operator before acting."). This moved the consult gate upstream of the first edit.
- [operator-ruled] Exceedance of accumulated render sources is PERMITTED; the chore handles overages on a cadence (verbatim: "in fact, permit exceedances of accumulated render sources, then, let the chore handle overages. It keeps the management of control surface sizes trimmed and managed at a more realistic/manageable cadence.").
- [operator-ruled] Division of labour: ordinary sessions add to sources and never resize a render surface; the chore is where surfaces are brought back into trim; operator-on-demand is a first-class trigger (verbatim: "let the chore manage the limits. let normal discovery and operations add to or suggest modifications to sources. then, the chore is what gets render surfaces back into shape. otherwise, we churn. If the operator wants immediate effect, then the operator can run the chore.").
- [operator-ruled] Reminders, never show stoppers (verbatim: "I accept that having limits on these control surfaces is a part of best practice for agent guidance, but I can't be stopping to trim them at every turn.").
- [operator-ruled] Quiet the delivery witness to one line pointing at the chore, selected over silencing it entirely and over leaving it as-is.
- [operator-ruled] Ratify Option B-explicit and align all three surfaces now, retiring the 1.0 fail-closed exit condition. Selected over keeping a softened 1.0 backstop and over discussing further.
- [operator-ruled] The pending-accumulation signal rides `OBPI-0.35.0-06-validate-rendition-lineage` in the campaign rather than a direct fix, selected over landing a minimal fingerprint comparison now.
- [agent-chose] Fixed the witness prose as a class - removed every issue number rather than correcting closed to open - and added `DeliveryRemediationOutputContract` so a transcribed record cannot return. The string had decayed twice.
- [agent-chose] Filed GHI #818 OPEN with a blocker comment rather than closing it superseded, because authoring a Negative Architecture ADR is a design conversation and canon forbids spinning one up merely to discharge a GHI.
- [agent-chose] Made the witness's quiet mechanical via `_MAX_ADVISORY_CHARS = 400` rather than trusting the shorter wording to stay short.
- [agent-chose] Did NOT run the diet chore despite it being authorized, because the measurement its own new § 2 prescribes showed the gap is a routing defect and the compressible budget is 1894 B of operator canon.

## Immediate Next Steps

1. **Rule on the delivery-routing question recorded on GHI #815.** A compliant 15764 B codex rendition exists and `sync_agents_md` hardcodes "claude", so nothing plays it back. Both consumers currently read root `AGENTS.md` because `CLAUDE.md` is a 2554 B `@AGENTS.md` redirect stub, so only one rendition can occupy that path. Options recorded on the issue: two targets with CLAUDE.md carrying its own rendition (contradicts the GHI #525 [settled] redirect doctrine and needs an amendment), swap the playback literal (costs Claude 18.6 KB), or route it to design. This is the item that would actually close #815.
2. **Rule on GHI #818 question 1 - shape.** New `foundation` ADR package, or an amendment surface on ADR-0.0.9 / ADR-0.0.10. The blocker comment carries all three questions and pre-answers the Boundary 2 objection. Nothing proceeds without this ruling.
3. **Resume the campaign spine.** `ADR-0.35.0-canon-entry-corpus-landing` is `Pending` at 0 of 10 OBPIs, set aside three consecutive sessions. Its item #6 [settled] `OBPI-0.35.0-06-validate-rendition-lineage` now also carries the pending-accumulation signal by operator ruling - read its lifecycle from `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing`.
4. **Consider declaring `CLAUDE.md` in the survival declaration.** `data/agents_md_survival_declaration.json` declares only `AGENTS.md`, so any other control surface crossing a vendor cap is unmeasured by design. Carried from the predecessor, still not live.
5. **Consider whether the diet chore needs a declared cadence.** `.gzkit/chores/registry.json` entries carry `slug`, `title`, `version`, `path`, `lane`, `timeoutSeconds` - there is no cadence field, so "on a cadence" currently has no mechanical home and depends on the operator remembering to run it.

## Pending Work / Open Loops

- **GHI #815 is OPEN and its breach is live but re-diagnosed.** `AGENTS.md` renders 34354 B against the 32768 B codex cap with `architectural-boundaries` at byte 33759. The new evidence says this is a playback-routing defect: a compliant codex rendition exists, is Gate-5 attested, and is delivered to nothing. Both prior remedies - reorder via `ADR-pool.render-order-truncation-survival` and shrink via the diet chore - answer a size problem the measurement does not support.
- **GHI #818 is OPEN and blocked on an operator design ruling.** The Negative Architecture ADR was never authored, so six boundaries cited repo-wide as binding rest on a 2026-03-29 planning memo. The campaign quotes Boundary 2 to refuse a pool ADR outright; the metacognition note defers the graph engine on Boundary 3.
- **GHI #799 remains OPEN and is the inverse cut of #818.** ADR-declared doctrine with no surface mirror versus a surface mirror with no ADR. Cross-linked at authoring; neither subsumes the other, and a remedy making the ADR-to-surface link explicit would serve both.
- **The diet chore has no declared cadence.** The registry schema has no cadence field, so the operator ruling that the chore manages limits on a cadence currently rests on memory.
- **`git pull` still discards stdout** at `src/gzkit/commands/sync.py`, so merge-conflict file lists do not reach the blocker. Deliberately out of GHI #816 [settled]'s scope and untracked by any GHI. Carried from the predecessor.
- **GHI #533 remains open and its body's blocker is stale.** It names `ADR-0.0.37` as still mid-implementation; that ADR is terminal and the dependency relocated to `ADR-0.35.0` § Decision 3.
...
- **Carried untouched from prior sessions:** the PRD rewrite (`PRD-GZKIT-1.0.0`, `Draft`, Non-Goals forbidding multi-agent orchestration that `ADR-0.18.0` ships); radar registry widening; the W1 re-baseline and third cost axis for SUPPORT re-kinds.

## Verification Checklist

```bash
uv run gz validate
uv run gz validate --advisory-scorecard --instructions-files-budget --invariant-coherence --documents --surfaces
...
Expected, each observed at authoring rather than assumed: bare `validate` reports 13 scopes passed; the five-scope run exits 0 while printing exactly two `[advisory] WARNING` lines about the codex cap (exit 0 WITH warnings is the ruled posture); `gz chores list` no longer lists `agents-md-architectural-boundaries`; the two witness modules run 27 tests and exit 0.

Three witnesses worth reading rather than only running. Each `[surface-delivery-witness]` advisory line must stay at or under `_MAX_ADVISORY_CHARS` = 400 and must contain no `GHI #<n>` - both are asserted by `DeliveryRemediationOutputContract`, and the length arm was observed biting at 406 during the change. The advisory-scorecard Coverage Ledger row for `agents-md-map-doctrine.md` must read `0.5.0`; if a future rule edit bumps it again, the scorecard refuses until the rows are re-read, which is the forcing function that found row 58 stale. And `data/instructions_files_budget.json` must still carry its original `files` and `globs` values - only `_doc` grew, verified by diffing every other key against HEAD.
...
- `.gzkit/chores/instructions-files-diet/CHORE.md` - v3.0.0; new §§ 1 Render, 2 Check, 3 Recommend, 4 Consult, 5c Re-render, § When to stop and ask, § Posture; criteria 12-16
- `src/gzkit/chores/instructions-files-diet/CHORE.md` - wheel copy, byte-identical after sync
- `.gzkit/chores/memory-hygiene/CHORE.md` - v1.1.0; § Migration targets routes each correction kind to its actual source
- `.gzkit/chores/memory-hygiene/acceptance.json` - `--invariant-coherence` added and leading; two criteria to three
- `.gzkit/chores/registry.json` - `agents-md-architectural-boundaries` entry removed
- `src/gzkit/chores/registry.json` - same removal on the wheel side
- `docs/design/ARCHITECTURE-PLANNING-MEMO.md` - § Retirement note dated 2026-08-17 beside the two unmodified stale claims
- `src/gzkit/governance/trust_audits/surface_delivery_witness.py` - `_DELIVERY_REMEDIATION` carries no issue number and no remedy catalogue; `_MAX_ADVISORY_CHARS` added
- `tests/governance/test_surface_delivery_witness.py` - `DeliveryRemediationOutputContract`, two assertions, 27 tests in the module set
- `.gzkit/rules/agents-md-map-doctrine.md` - v0.5.0; exit condition retired, division of labour stated
- `docs/governance/advisory-rules-audit.md` - row 17b retirement, row 58 re-scored, Coverage Ledger at 0.5.0
- `data/instructions_files_budget.json` - fifth dated entry appended; budget values unchanged
- `.gzkit/renditions/AGENTS.md/codex.md` - the 15764 B compliant rendition that nothing plays back
- `.gzkit/renditions/AGENTS.md/claude.corpus.json` - the release manifest the versioning argument rests on
- `.gzkit/handoffs/20260817T190544Z-session-close-815-open-816-817-fixed-narrative-insight.md` - the predecessor this continues
