...
branch: main
timestamp: '2026-08-18T08:45:42Z'
agent: g0
obpi_id: OBPI-0.35.0-09-codex-playback-wiring
session_id: 8bd5d70c-7290-43cd-b65c-89d08fed5df5
continues_from: .gzkit/handoffs/20260818T023015Z-adr-sync-layer1-miscalibration-fixed.md
---

## Current State Summary

All work requested this session is landed and pushed. `main` is level with `origin/main`, tree clean.

Started as a `/gz-adr-sync` full run. The operator caught that the skill's Layer 1 grepped for ADR-anchored @covers when evidence is REQ-anchored — a false green. Fixing that as a class surfaced three further defects, all filed and all closed with fixes.

SUPERSEDES the prior handoff 20260818T023015Z-adr-sync-layer1-miscalibration-fixed, whose Pending Work listed the three GHI candidates as unfiled. They are now #825, #826, #827 — all CLOSED with fix commits.

Seven commits: 350ddc804, ec211baa6, c62903655, 3588c8232, 1d6555e8e, 5e5cb655d, d82817f65, 1257a8ea5.

## Important Context

THE UNIFYING FINDING, and the reason one sync run surfaced four defects: every one was AN INSTRUMENT AIMED AT THE WRONG TARGET, and not one could fail loudly. Each returned a confident, well-formed, wrong answer.

1. gz-adr-sync Layer 1 grepped ADR-anchored @covers where evidence is REQ-anchored (ADR-0.0.59). Returned 4 hits, 3 of them test fixtures the canonical scanner deliberately masks. Read as "no orphans"; truth was 1753/2706 REQs covered.
2. GHI #825 — `gz obpi brief-drift --apply` wrote amendments to the markdown prose section while the engine reads frontmatter for a structured brief. Reported drift its own repair path could never clear, forever.
3. GHI #826 — three lookups derived a bare OBPI prefix from the caller's id and searched THAT first, so a fully-qualified id resolved to a prefix sibling under a different parent ADR.
4. GHI #827 — the handoff settled-citation annotator matched any bare `#N`, claiming this repo's own rule numbering.

DOCTRINE ESTABLISHED OR CONFIRMED:

- Coverage binds at REQ granularity, never ADR. `gz covers` is the governed verb; a raw rg is a mis-aimed probe.
- A structured OBPI brief's allowlist binds in FRONTMATTER. The `## Allowed Paths` prose section is a stale restatement — 6 entries against frontmatter's 30-plus on OBPI-0.35.0-09. See src/gzkit/governance/brief_reconcile.py line 219.
- An `OBPI-<semver>-<index>` prefix is NOT an identity. Demoting a feature ADR to pool releases its semver for reuse while parked OBPI ids keep it, so one prefix can name two OBPIs under two parent ADRs.
- The reconciler derives a brief's working set from the COVERAGE GRAPH, not the diff. A brief can be accurate the day it is written and drift the moment a test binds a new module to its REQs.
- Check for prior art before adding a helper. Two of this session's own commits needed correcting for this: a duplicate frontmatter appender, and a first class-sweep that missed dot-directories because rg skips them by default.

METHOD NOTE: rg output was observed MANGLING file content in this environment (the literal string "parked" rendering as "ln", "brief-drift" as "ln"). File paths were unaffected. Every code read this session was done with sed or cat for that reason. Do not trust rg content lines here; trust sed.

## Decisions Made

1. ROUTED EVERY DEFECT AS A DIRECT FIX, never an OBPI. Per AGENTS.md Defect-fix routing and the operator's own canon that a GHI is the work order and the receipt. 422 fix commits in 60 days is the precedent. No OBPI was authored for any of the four.

2. LEFT gz-adr-map UNTOUCHED. It is not a defect — it already calls `gz covers` as the canonical walker and states the doctrine inline: "Decorators are REQ-level, not ADR-level." The correct calibration was already written down in a sibling skill.

3. OPERATOR ATTESTED THE OBPI-09 ALLOWLIST AMENDMENT AS g0, verbatim selection "Attest as g0, then push". Requested rather than fabricated.

4. MOVED THAT AMENDMENT TO THE BINDING SURFACE by hand after `--apply` proved inert, then fixed the verb under #825 so the next one does not need a human workaround.

5. FOR #826, CHOSE REMOVAL OVER A NEW BRANCH. The bare-form derivation was pure liability: a bare id still resolves because it globs as its own prefix. Deleted the derivation, the pattern constant, and the `re` import.

6. FOR #827, CHOSE A MEASURED DENY-LIST over three alternatives, and recorded why each was rejected: resolving against GitHub cannot discriminate (issue 13 genuinely exists and is genuinely closed, which is WHY it fired); capitalization cannot ("Fixes #696" is capitalized and real); requiring the GHI prefix would delete the bare form and reintroduce the GHI #696 decay the feature exists to catch.

7. DID NOT RE-KEY THE PARKED OBPI IDS. They are append-only ledger history and rewriting them violates AGENTS.md Never #2. The fix makes the prefix non-load-bearing instead of making the ids unique.

8. IMPROVED THE SHARED HELPER RATHER THAN JUST LIFTING IT. Consolidating onto `_append_frontmatter_list_value` exposed a fixed two-space emit that reformatted whole blocks; threading the measured indent fixed that for every caller, including the `tasks:` stamping in src/gzkit/commands/task.py.

9. LEFT THE STALE `[settled]` MARKER IN THE PRIOR HANDOFF. Hand-editing a gate-written artifact to tidy its own output is the worse trade; this handoff supersedes it instead.

## Immediate Next Steps

1. Nothing is blocked and nothing is owed. Every item the operator asked for this session is landed, pushed, and closed.

2. GHI #824 is the nearest live thread — `_find_brief_path` at src/gzkit/commands/task.py line 524 matches by substring over rglob and can pick up a plan file, so TASK stamps land in the wrong artifact. VERIFIED THIS SESSION as a sibling cut of #826 [settled], NOT the same code: #826 [settled] fixed prefix derivation in src/gzkit/commands/obpi_precomplete.py; this is a different function with a different and looser mechanism. Same family, its own scope. Route it as a direct fix via the ghi-close skill.

3. If resuming OBPI-0.35.0-09: read the Step 4b adversary verdict at receipt arb-step-codexadversary-fc821cac161042538c772cb58d0433a6 BEFORE trusting any coverage number. It REFUTED the claim that the eight covering tests prove their REQs. Unresolved since two sessions ago.

4. If resuming OBPI-0.35.0-09: obtain an operator ruling on the brief contradiction between REQ-0.35.0-09-02 (exactly one destination) and REQ-0.35.0-09-06 (both the claude and codex destination files). REQ-06's covering test cannot be written correctly until this resolves. Also unresolved.

5. Confirm with the operator who is driving OBPI-0.35.0-09 before touching it. An earlier session was halted so the operator could run the skill themselves.

## Pending Work / Open Loops

OBPI-0.35.0-09 REMAINS INCOMPLETE — not completed, not attested, coverage 10 of 11 REQs. Its Step 4b adversary verdict is REFUTED and the REQ-02 versus REQ-06 contradiction is unresolved. This session changed exactly one thing about it: the allowlist now reconciles clean.

GHI #824 OPEN — the last live defect thread from this session's investigations. Diagnosis is complete in its body; it needs a fix, not more analysis.

ONE UNCOVERED CAUSE, DISCLOSED AT CLOSE: #826 [settled] added `sorted()` to make brief-path resolution deterministic, because Path.glob is filesystem-ordered and the unsorted pick made the answer vary by machine. That change carries NO TEST — asserting a specific filesystem ordering is not something I could drive deterministically. Recorded as an empty row in the #826 [settled] close table rather than hidden.

ONE RESIDUAL BY DESIGN: #827 [settled]'s fix is a lead-in deny-list derived from a census of 168 occurrences across the handoff corpus. It closes every form that census found and will misfire on a section-numbering word this repo has not used yet. The census command is in the commit body so the list can be regenerated.

715 UNLINKED SPECS repo-wide (REQs with no test), advisory not gating. Whole OBPI families under ADR-0.35.0, ADR-0.36.0, ADR-0.37.0 sit at 0% — VERIFIED NOT a governance breach, because none of those OBPIs are completed or attested. Zero coverage on unstarted briefs is expected.

NO `gz handoff decide` RULING was booked against either prior handoff. The operator ruled on the attestation, on syncing, on filing, and on each close — never on a handoff's advised steps as a set, so no proceed was fabricated. The resume gate was retired 2026-08-15 and gates nothing.

## Verification Checklist

OBSERVED results, all run with pipefail when piped:

- gz covers: "Summary: 1753/2706 REQs covered (64.8%)", exit 0. The corrected Layer 1 number.
- gz validate --adr-status-fresh: "Validated: adr_status_fresh", exit 0.
- gz validate --surfaces --cli-alignment: "All validations passed (2 scopes)", exit 0.
- gz obpi brief-drift OBPI-0.35.0-09-codex-playback-wiring: "clean", allowlist=0, exit 0. Was DRIFT allowlist=1 exit 3.
- #826 fix, observed: OBPI-0.35.0-01-arb-ruff now resolves to None; before it resolved to OBPI-0.35.0-01-corpus-tombstone-schema-and-fold.md, a different OBPI under a different parent ADR. Bare OBPI-0.35.0-01 still resolves.
- #827 fix, observed: "(AGENTS.md Behavior Rules, Always #13)" extracts no references and takes no marker; "Per AGENTS.md Always #13, resolve GHI #824 and see #696" still extracts both 824 and 696.
- Consolidation, observed on the live OBPI-09 brief: appending one allowlist path changes ONE line and is idempotent. Before the indent fix it would have reformatted all 31.
- Pre-push hook ran the full suite on every push: 8461-plus tests, passed.
- gz git-sync --apply: "Git sync completed", exit 0, four times.

RECEIPTS, each confirmed to resolve on disk before being cited, and regenerated whenever ruff-format touched the tree after they were cut:
- 825: arb-ruff-ab19a9c9560f442a9ab0b492be2231fc, arb-step-typecheck-81490bf2b6ec4a69a1adfd13e974c9a5, arb-step-unittest-f9139abf828e4ad69ab8b1f9cf10950c
- 826: arb-ruff-7f50bba737324f51aa4ffed1ce5a1945, arb-step-typecheck-f78022cbc6074e6d98c83d6ad1b9aece, arb-step-unittest-a21685876d0a4696a82a3614b9799893
- 827: arb-ruff-d2534bf8997549ab8c28f829de7fa3fd, arb-step-typecheck-8ecede4416a7436fa6b8b11bb149a091, arb-step-unittest-69711e8935494f0eaf01f2edbffee056
- consolidation: arb-ruff-06371e6eb4ed443aab04cdc1d6c6a4ca, arb-step-typecheck-dadc707a18e5420d8cfb1901a40b6eba, arb-step-unittest-7b160589b92045859831c9683d62b2f6

METHOD NOTES FOR THE NEXT SESSION:
- The verifier-pipe-gate hook fired twice on compound commands ending in a pipe. It is right every time: a piped verifier reports the filter's exit, not the verifier's. Use pipefail, or split the command.
- Generate ARB receipts AFTER confirming ruff-format is a no-op. A receipt cut before the formatter touches the tree describes a tree that never landed; that happened once and the receipts were regenerated.

## Evidence / Artifacts

COMMITS, all authored g0, all hook-verified, none with a verify bypass:
- 350ddc804 chore(adr-sync): regenerate ADR status index after pool registration
- ec211baa6 fix(skills): route ADR-sync/audit Layer 1 through gz covers, not a raw ADR grep
- c62903655 fix(obpi-brief): add corpus_store.py to OBPI-0.35.0-09 allowlist frontmatter
- 3588c8232 docs(handoff): checkpoint the gz-adr-sync Layer-1 miscalibration traversal
- 1d6555e8e fix(brief-drift): amend the frontmatter allowlist for structured briefs (GHI #825)
- 5e5cb655d fix(obpi-precomplete): match the supplied OBPI id, never a derived prefix (GHI #826)
- d82817f65 fix(handoff): stop reading rule numbers as issue citations (GHI #827)
- 1257a8ea5 refactor(brief-drift): reuse the frontmatter list appender instead of a duplicate

GHIs FILED AND CLOSED THIS SESSION, each with a cause-to-test table:
- GHI #825 brief-drift: --apply writes where the reconciler never reads — CLOSED fixed
- GHI #826 gz state: parked OBPI ids keep a semver a later ADR reuses — CLOSED fixed
- GHI #827 handoff: settled-citation annotator matches any #N — CLOSED fixed
Cross-link posted on GHI #776 naming #826 as the OBPI-id arm of its root cause, at authoring time.

TWO OF MY OWN CLAIMS WERE DISPROVED BY RE-DERIVATION, and the corrections are recorded in the #826 close comment: gz state does carry parked/parked_to/parent and never rendered the sets identically; and the handoff reference checker resolves only GHI kinds, so OBPI references were never ambiguous. The real mechanism was prefix derivation in obpi_precomplete, which is worse than what was filed.

KEY SOURCE ANCHORS:
- src/gzkit/governance/brief_reconcile.py — line 219 reads the frontmatter allowlist for a structured brief
- src/gzkit/commands/obpi_precomplete.py — the three lookups #826 fixed
- src/gzkit/handoff_api.py — the reference patterns #827 fixed
- src/gzkit/commands/closeout_form.py — the shared frontmatter list appender, now indentation-preserving
- src/gzkit/commands/task.py — line 524 is GHI #824's resolver, a different function
- src/gzkit/traceability.py — the canonical find_covers_in_source scanner, which masks non-docstring string literals so fixtures cannot register as phantom anchors (GHI #390)

INSIGHT RECORDED: gz insights remember, type improvement, scoped to the gz-adr-sync SKILL.md surface, timestamp 2026-08-18T02:04:13Z.
