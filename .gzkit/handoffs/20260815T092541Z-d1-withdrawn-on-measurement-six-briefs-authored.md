...
adr_id: ADR-0.37.0
branch: main
timestamp: '2026-08-15T09:25:41Z'
agent: claude-code
session_id: ca40a011-5ec7-4735-8aaa-bfe890b714e2
continues_from: .gzkit/handoffs/20260814T123053Z-airlock-rehoming-ratified-resume-gate-descoped.md
---

## Current State Summary

Session opened on "review handoff" and ended having re-decided the load-bearing decision of ADR-0.37.0 on measurement, authored its six OBPI briefs, and filed-fixed-closed GHI #806. Two commits landed and pushed; tree clean and in sync with origin/main.

The chain: reviewed the resumed anchor -> found four of its five advised steps already discharged by an intervening session that left no successor -> operator ruled the ADR's declared SINGLE-DRIVER review gap closed first -> both mandated personas returned FAIL -> the decisive finding was MEASURED, not argued -> operator re-opened D1 -> design dialogue produced three rulings -> ADR amended in place -> operator corrected the ceremony order (evaluate runs AFTER brief authoring) -> six briefs authored -> a perfect structural-completeness score.

Commits: d9b697ede (fix(skills): airlineops invocations in Gate-5 template assets, GHI #806) and 86f71e7d4 (gz git-sync sweep carrying the ADR amendment and the six briefs).

ADR-0.37.0 is Draft/Pending with all six briefs authored and passing the authored gate; read its landed count from `uv run gz adr status ADR-0.37.0-airlock-calibration-and-compulsion`. No implementation code was written this session.

## Important Context

THE ANCHOR WAS STALE AND THE REVIEW THAT MISSED IT WAS MINE. Session f73586eb booked its own proceed ruling on the same anchor at 00:01:35Z, worked advised steps 1-4, authored ADR-0.37.0 with six OBPIs, and exited leaving only a mechanical CHECKPOINT bookmark. I verified the GHI tracker, the campaign marker and the ledger counts but never the ADR tree, and presented four discharged steps as live. The operator ruled on that false premise and had to re-rule. The evidence was in the one commit since the anchor -- 61c9bbc59's ledger-event trailer records adr_created ADR-0.37.0 plus six obpi_created events. Recorded as an improvement insight under scope handoff-resume-presentation.

D1'S ORIGINAL DECISION WAS WITHDRAWN ON MEASUREMENT, NOT ARGUMENT. Re-measure rather than transcribe: reachable_from(<OBPI>) returns [] for every OBPI tested (the confirmed cause of the 20-of-23 empty seam-maps), and the proposed inverse returns exactly {<parent ADR>, PRD-GZKIT-1.0.0} -- IDENTICAL for OBPI-01, -02 and -05, because ancestors of an OBPI is its lineage chain, a function of the artifact tree alone. The accounting predicate then auto-accounts the first: the parent ADR id occurs 11 times in a scaffold brief by construction. Inverting reach converts an EMPTY seam-map into a CONSTANT one.

THE OLD FIDELITY ASSERTION WOULD HAVE CERTIFIED THE REJECTED ALTERNATIVE. Row 2 asserted d['push'] or d['pull'] -- a disjunction, under a Decision whose own text said "Both arms are required", while Alternative 1 ("wire pull only") was listed REJECTED. Non-emptiness cannot distinguish a gate that bites from a constant that does not, which is why the replacement is a DIFFERENTIAL pair.

THE DESIGN WEIGHT MOVED TO THE ACCOUNTING PREDICATE, AND IT INVERTS RATHER THAN TIGHTENS. The defect is DIRECTION, not scope: under any substring rule the brief accounts for a seam by NAMING it, and the brief is a file the entering agent controls. Both new arms bind through surfaces the entering work does not own -- the parent's Boundary Invariants via _fence_obpi_anchored, or a STRUCTURAL-FENCE REQ that gz validate --req-kind-discipline already checks.

TWO PRIMITIVES WERE REUSED RATHER THAN INVENTED. req_kind_fence.py:43 already carries GHI #538's lesson in its own docstring -- "Heading presence alone is NOT proof -- an invariant list naming no OBPI cannot say which invariant proves which fence" -- which is exactly the discipline the airlock's predicate lacked. The STRUCTURAL-FENCE channel supplies the brief-side arm.

THE CORPUS IS NOT READY AND THE ADR SAYS SO OUT LOUD. Measured 2026-08-15: only 19 of 166 ADRs declare a Boundary Invariants section at all, and across those 19, 89 of 93 modelled entries (95%) would HOLD because invariants are rarely bound to an OBPI. ADR-0.37.0 itself bound 1 of 6 before this session; all 9 are now bound. That 95% is why the gate ships WARN and why the flip criteria exist.

BOTH REVIEWS RETURNED FAIL ON DIFFERENT GROUNDS AND BOTH WERE RIGHT. Three claims the artifact made about its own contents were false: "the flip criterion is written and owned" (no criterion existed), "all four call sites" (there are five; commands/airlock.py:95, the door the ADR's own fidelity commands measure through, was omitted), and "Inherited unchanged from ADR-0.33.0" on Boundary Invariants 2 and 3. ADR-0.33.0's Boundary Invariants 1 and 3 had also been DROPPED and are restored as 7 and 8.

SEAM-SOURCE ALTERNATIVES WERE PRICED AGAINST REAL DATA. File-coupling (.gzkit/ontology/source_anchors.json, coupling_edges, 3947 edges already built) genuinely varies with the work -- 68, 98, 101, 248, 258 and 270 across real briefs -- but is unbounded, so it is deferred WITH AN OWNER. Brief-territory overlap is useless today: median 86 co-claimants, max 173 of 174, because allowlists are src/gzkit/** coarse.

THE SEAM SOURCE IS PLUGGABLE BY DESIGN. _default_reach is injected as reach_fn for hexagonal rule 6, so changing what a seam IS swaps an adapter rather than forking the primitive -- no tension with ADR-0.33.0's one-primitive invariant.

SKILL GATES CAUGHT TWO REAL AUTHORING DEFECTS. tests/airlock/** does not exist (the convention is flat, tests/test_airlock_enter.py) -- a fabricated path I introduced earlier in the session, caught by gz-obpi-specify's Pre-Save Ground-Truth Check. And the Objective section's scaffold comment contains "one-sentence", which _is_placeholder matches, so the section is unpassable until the comment is deleted regardless of what is written beneath it.

THE REFUSE-THE-BUNDLE FENCE FIRED AND WAS RIGHT. gz git-sync blocked the first sweep because it would have staged src/gzkit/skills/*/SKILL.md under chore: with Task: TASK-gz-git-sync. Its own diagnostic named the cost: gz validate --commit-trailers would still exit 0, so the mis-attribution is invisible to enforcement, and git log --grep='^fix(' -- the precedent query AGENTS.md prescribes for its own routing decisions -- could never see the fix.

## Decisions Made

- [operator-ruled] Work advised steps 1 and 3 of the resumed anchor, the campaign spine (verbatim: "Steps 1 + 3 — the campaign spine"). Booked via gz handoff decide; step 2 recorded set-aside as DISCHARGED (GHI #804 filed 2026-08-15T00:03:36Z by the intervening session), steps 4 and 5 recorded set-aside. The ruling was given on an agent presentation that was wrong: steps 1 and 3 were already complete.
- [operator-ruled] Close the ADR's declared review gap BEFORE authoring the six briefs (selected "Close the review gap first"), on the ground that ADR-0.37.0 was authored and self-scored by one session that then vanished without a handoff. Both mandated Step-2 personas were dispatched and both returned FAIL.
- [operator-ruled] Re-open the D1 design question rather than repair prose or file the findings (selected "Re-open the D1 design question"). This was the correct call: the measurement showed D1's seam source was wrong, not merely under-specified.
- [operator-ruled] A seam is DECLARED LAW the work may breach (selected "Declared law you might violate"). The PULL arm is load-bearing; the PUSH arm is not its equal.
- [operator-ruled] Severity is GRADUATED (selected "Graduated: HOLD on unaddressed, WARN on absent"): unaccounted law holds, a parent declaring no law proceeds with a counted warning.
- [operator-ruled] The brief earns accounting through a STRUCTURAL-FENCE REQ as a second arm, and severity ships staged WARN-then-flip against a written criterion (selected "Brief-side STRUCTURAL-FENCE arm + staged severity"). Ruled AFTER the agent surfaced that the first framing produced a 95% day-one hold rate.
- [operator-ruled] Book the amendment and REPURPOSE OBPI-01 rather than withdraw it (selected "Book it, but repurpose OBPI-01"), keeping six OBPIs and the checklist 1:1 with the briefs on disk, accepting the slug/content mismatch as tracked rename debt.
- [operator-ruled] gz adr evaluate runs AFTER OBPI authoring, not before (verbatim: "no, we run evaluate after obpi authoring"). Borne out mechanically: dimension 4 scored 3 against scaffold briefs and 4 against authored ones, so the pre-authoring run had measured the template. Recorded as an improvement insight under scope gz-design-ceremony-order.
- [operator-ruled] Sync the session's work and file the GHI (verbatim: "sync and file ghi").
- [agent-chose] Execute gz-design directly rather than spawn its self-escalation subagent: the session is already opus-tier, and a design dialogue's next step depends on operator answers a subagent has no channel to receive.
- [agent-chose] Keep SeamKind.PUSH in the model rather than delete it. ADR-0.33.0's attested checklist commits to it; what ADR-0.33.0 DEFERRED is which source yields a meaningful map, so changing the source is inside the deferral and deleting the edge kind would not be.
- [agent-chose] Amend the authored sections and add an explicit Amendment section rather than rewrite the Q&A Transcript, so the 2026-08-14 authoring interview stays an honest historical record and a banner states which text governs.
- [agent-chose] Add Boundary Invariant 9 (no OBPI may write the ADR that grants its accounting) and bind all nine invariants to OBPIs, so the ADR practises the discipline it introduces rather than declaring it.
- [agent-chose] Scope OBPI-02 to the accounting predicate in _reconcile and OBPI-03 to the decision ladder in _decide, resolving the collision both reviewers flagged by reading the source rather than by sequencing prose.
- [agent-chose] File GHI #806 and then fix and close it in the same session, per the ghi-author constraint that a GHI must never substitute for a fix that can land now.
- [agent-chose] Scope GHI #806 by prescriptive-vs-descriptive rather than by the airlineops string, so airlineops-parity-scan and the provenance notes in gz-arb / gz-adr-evaluate / gz-obpi-pipeline stay untouched and correct.

## Immediate Next Steps

1. RULE ON WHETHER ADR-0.37.0 IMPLEMENTATION STARTS, AND FROM WHICH OBPI. The ADR is Draft/Pending with six authored briefs and no implementation code -- read the landed count from Layer-2 via `uv run gz adr status ADR-0.37.0-airlock-calibration-and-compulsion` rather than any figure transcribed here. The ADR's own sequencing says OBPI-01 + OBPI-02 alone deliver the value (calibrate before compel), and OBPI-04, OBPI-05 and OBPI-06 have no code dependency on OBPI-01, OBPI-02 or OBPI-03 -- nothing mechanically stops the compel half landing first, which is the "widening an uncalibrated gate" failure the ADR exists to prevent. If implementation starts, start at OBPI-01. Do not re-measure the design; DO re-measure the baselines before writing tests against them.

2. FIX THE airlock_exit COUNTERPART DEFECT. src/gzkit/airlock/exit.py:238 carries the identical `parent_invariants: tuple[str, ...] = ()` default and no caller passes it, and its _default_reach uses the same descendants-based compute_reach. ADR-0.37.0 explicitly scopes it OUT and OBPI-01's Denied Paths says so, while the ADR's own Negative Consequence 6 calls exit "the load-bearing half" if work discovers its blast radius mid-flight. Route it: a GHI direct fix, or an amendment scoping it into OBPI-01. It is currently owned by nothing.

3. RENAME OBPI-0.37.0-01. Its slug is `ontology-inverse-reach` and its content is now parent-invariant threading; the operator accepted the mismatch as tracked debt when ruling repurpose-not-withdraw. It must go through `uv run gz migrate-semver` because obpi_created for the current id is already in Layer 2 -- do not hand-edit the id.

4. RULE ON THE PAIRED-EVENT FAMILY, WHICH IS NOW FOUR INSTANCES. The resume gate (160 lifts / 0 blocks, fixed), session_exit (37+ skips / 0 writes, GHI #766 open), the airlock (23 in / 5 out, re-measured unchanged this session), and the seven session-exit bookmarks this session alone produced. ADR-0.37.0 deliberately does not absorb it. No owner asks "does this event's decision have a counterpart for its other branch?"

5. DECIDE WHETHER THE SUBSTANCE CHANNEL EVER GETS AN EMITTER. Every ADR scorecard reads `Substance: UNGRADED` and `DISPATCH MODE: SINGLE-DRIVER` because nothing emits a gzkit.adr_eval_dispatch receipt -- stated in gz-adr-evaluate/SKILL.md:81, pending ADR-pool.obpi-pipeline-dispatch-attestation Target Scopes 5 and 6. Two genuine independent reviews ran this session and the label could not follow them. This is a live member of the doctrine-declared-without-mechanism family the campaign's Movement C box tracks.

## Pending Work / Open Loops

- ADR-0.37.0 IS AUTHORED BUT UNIMPLEMENTED. Draft/Pending, six briefs passing `gz obpi validate --authored`, zero lines of implementation; read the landed count from `uv run gz adr status ADR-0.37.0-airlock-calibration-and-compulsion`, never from a figure copied into prose. Movement B remains blocked on it landing, and Movement B is TOPMOST by ratified amendment.
- THE FLIP-CRITERIA GATE-2 THRESHOLD IS AGENT-PROPOSED, NOT RATIFIED. Gate 1 (25% modelled hold rate, against the measured 95% baseline) was ratified in the design dialogue. Gate 2's "90% of src/** and tests/** commits in a trailing 30-day window" was never put to the operator and is marked as a draft inside the ADR. It needs a ruling before OBPI-06 can flip anything.
- airlock_exit CARRIES THE SAME DEFECT AND HAS NO OWNER (see Immediate Next Steps 2).
- OBPI-0.37.0-01's SLUG UNDERSTATES ITS CONTENT (see Immediate Next Steps 3).
- THE PAIRED-EVENT FAMILY IS STILL UNDISPOSITIONED, now at four instances (see Immediate Next Steps 4).
- GHI #804 (attested deferred frontier needs no owner), #805 (generated resume-gate still documents a removed Bash arm), #803 (docs-build mkdocs downgrades), #802 (gzkit.org stale VPS, operator-only), #799 (advisory scorecard), #767 (handoff carries no transcript reference), #766 (session-exit bookmark emits constants) all remain OPEN and unworked this session.
- SEVEN session-exit-bookmark FILES were produced by this single session and swept into the sync. GHI #766 is the open home for that behaviour; the volume is new evidence for it.
- THE ASSETS-NOT-IN-WHEEL QUESTION IS RECORDED BUT UNVERIFIED. No skill ships an assets/ directory (`find src/gzkit/skills -type d -name assets` returns nothing) while gz-obpi-specify/SKILL.md -- which does ship -- calls reading assets/HEAVY_LANE_PLAN_TEMPLATE.md "mandatory". Whether that is a declared exclusion or a gap was deliberately not asserted in GHI #806 [settled] and remains open.
- THE EXEMPTION INVENTORY still owes differential controls; unchanged this session.
- THE MAINTENANCE RATIO REMAINS UNRULED, carried across five handoffs now.
- THE INSTRUCTIONS-FILES BUDGET ADVISORIES stay green-but-warning on every gz check; parked by standing ruling.

## Verification Checklist

uv run gz check
uv run gz obpi validate --adr ADR-0.37.0 --authored
uv run gz adr status ADR-0.37.0-airlock-calibration-and-compulsion
uv run gz adr fidelity ADR-0.37.0-airlock-calibration-and-compulsion --check
uv run gz validate --req-kind-discipline
git rev-list --left-right --count origin/main...HEAD
gh issue view 806 --json number,state
grep -c "event.:.airlock_in" .gzkit/ledger.jsonl
grep -c "event.:.airlock_out" .gzkit/ledger.jsonl
...
- `docs/design/adr/pre-release/ADR-0.37.0-airlock-calibration-and-compulsion/EVALUATION_SCORECARD.md`
...
- `src/gzkit/req_kind_fence.py`
- `src/gzkit/commands/airlock.py`
- `.gzkit/ontology/source_anchors.json`
- `.gzkit/skills/gz-obpi-specify/assets/HEAVY_LANE_PLAN_TEMPLATE.md`
- `.gzkit/skills/gz-obpi-specify/assets/OBPI_BRIEF-template.md`
