...
adr_id: ADR-0.33.0
branch: main
timestamp: '2026-08-14T12:30:53Z'
agent: claude-code
session_id: e7f2646c-1751-41da-9950-c4f0c8648489
continues_from: .gzkit/handoffs/20260814T014658Z-tier-b-cleared-803-filed-and-766-ruled-parked.md
---

## Current State Summary

Session opened on "review handoff" and ended by re-sequencing Magna Carta. Five commits landed and pushed.

The chain: a false-refusal complaint against the handoff resume gate -> refusal telemetry -> removal of the gate's whole Bash arm -> root-cause (a category error, not a tuning problem) -> reading ADR-0.33.0 properly -> operator ratifying Movement B ahead of Movement A.

Commits: 96584b6ec (reserved words read as syntax; git fetch admitted), 2a326f042 (handoff_resume_blocked telemetry), bc9b72f67 (Bash arm removed; -2036 lines), 714244e04 (Magna Carta amendment: Movement B TOPMOST, Movement A HELD, calibration item 0 added), d5eb544d8 (restored the topmost marker the amendment broke, plus a fence).

The campaign spine was NOT worked. ADR-0.35.0 remains 0/10 and is now HELD by ratified amendment rather than set aside by default.

## Important Context

THE RESUME GATE WAS A CATEGORY ERROR, NOT A MIS-TUNED ALLOWLIST. A handoff is a synthetic memory refresh (AGENTS.md three-systems canon, invariant tier, 2026-08-06); entry/exit authorization is TRANSIT's subject (the airlock, ADR-0.33.0). A memory artifact has no natural blast radius, so "what should reading a reminder prevent?" had no principled answer -- the invented answer was maximal (every mutating tool call) and 44 read-exceptions were carved back one complaint at a time. All 13 admission-breadth corrections in 29 days were Bash refusals; the Write/Edit arm needed none because it has no allowlist to be incomplete.

THE GATE WAS ATTACHED WHERE THE FAILURE WAS OBSERVED, NOT WHERE THE CONCERN LIVES. GHI #574 quotes the remedy it was built to apply -- "place the human at a mechanical gate, not at every keystroke" -- and shipped as a PreToolUse hook on every Bash call. It also scoped itself "an OBPI, not a fix()" with a named destination and a blocker, then landed as a direct fix under none of them.

ADR-0.33.0 IS VALIDATED AND ATTESTED ITS OWN INCOMPLETENESS. Four acceptance criteria (04-01, 04-03, 05-01, 05-03) name a "deferred calibration frontier" in the attested REQ text, and the ADR body carries "Calibration frontier (FC-2 tracer finding, operator-attested 2026-07-10)". A disclosed deferral inside an attested artifact is invisible to every mechanical check, because the artifact is telling the truth -- gz adr status reports Validated 6/6 and that is accurate. Searched the issue tracker open and closed: ZERO GHIs track it. Specified precisely; scheduled not at all.

THE AIRLOCK PREDICTED BOTH OF ITS OWN FAILURE MODES. Negative #1 is the theater pre-mortem ("seam-maps rubber-stamped, GO always reached"); WWHTBT-(a) ranks the seam-map calibration as the load-bearing condition; WWHTBT-(c) names the route-around risk. Both materialised. The forecasting instrument worked and the scheduling instrument does not exist.

THE DOORS DO FIRE -- NOTHING COMPELS THEM. REQ-0.33.0-04-01 (attested) has gz mx enter calling airlock-IN for any --reason; REQ-0.33.0-05-01 (attested) has permitted-entry ALWAYS firing the gate with no bypass path. The gap is that 524 fix commits in 90 days never invoke a door at all. "Wire the door" is done; "entry triggers the door" was never in ADR-0.33.0's checklist.

FOUNDATION IS SEALED, SO ANY NEW ADR IS A FEATURE ADR. ADR-0.34.0: "no new foundation ADR may be authored", enforced at both adr_created ingresses and by gz validate --taxonomy in gz check; its own fidelity assertion pins gz plan create --kind foundation to exit 1. Next free feature slot is 0.37.0.

RE-HOMING, NOT REOPENING, IS THIS REPO'S PRECEDENT. ADR-0.35.0 exists because ADR-0.0.37's composition engine was withdrawn and re-homed as a feature. Appending OBPIs to a Validated ADR would drag it back to Pending and retroactively falsify an operator attestation that was honest when given. This corrects an agent proposal made earlier in the session (corrective work under ADR-0.33.0).

A PAIRED-EVENT FAMILY SHOWED UP THREE TIMES IN ONE DAY: the resume gate recorded 160 lifts and 0 blocks (fixed), session_exit records 37 skips and 0 writes (GHI #766, open), and the airlock pairs 23 entries to 5 exits. Worth dispositioning once rather than three times.

MEASURED LIVE THIS SESSION (re-measure, do not transcribe): 23 airlock_in vs 5 airlock_out; 20 of 23 transits computed an EMPTY seam-map and auto-proceeded while 3 bit correctly (3, 4, 7 unaccounted -> hold); 524 fix commits in 90 days.

## Decisions Made

- [operator-ruled] Fix the resume gate's false refusals (verbatim: "this is ridiculous, and keeps triggering almost each run, FIX IT"). Booked via gz handoff decide with all five advised steps of the resumed handoff recorded --set-aside; that ruling authorised the repair only.
- [operator-ruled] Prioritise the recurrence fix (verbatim: "I am this close to removing that hook if it is so poorly designed that we can't control this - PRIORITIZE THIS FIX"). Produced the handoff_resume_blocked telemetry.
- [operator-ruled] gz git-sync is unconditionally permitted (verbatim: "I EXPLICITLY want this"). Corrected an agent framing that had called the git-sync carve-out evidence of abandoned strictness; it is deliberate canon, and an agent-sketched refuse-set would have re-blocked it.
- [operator-ruled] Remove the Bash arm (verbatim: "why is the handoff gate so picky? it is a reminder of what we were doing and what to do next, not a nanny", then "i say the word").
- [operator-ruled] The airlock's incompleteness takes priority (verbatim: "the airlock's incompletemess is a problem that needs priority address.", ratified "yes, ratify"). Movement B is TOPMOST; Movement A is HELD.
- [agent-chose] Derive reserved-word handling from bash's CLOSED compgen -k set rather than adding allowlist rows, so membership can be complete rather than exemplary; case and [[ excluded on purpose with a test saying so.
- [agent-chose] Record refusal SHAPE, never command text, and make record_refusal fail-open by contract -- an unwritable ledger must never convert a refusal into a hook crash.
- [agent-chose] Retire the handoff-resume-unauthorized-bash negative control with its arm rather than leave a control asserting enforcement the gate no longer performs.
- [agent-chose] Re-purpose GitSyncIsNeverGatedOnAHandoffTests as an explicit regression pin once its assertions became trivially true, paired with test_bash_is_not_in_the_gated_set on the set itself.
- [agent-chose] Fence the campaign's topmost marker with a live-plan test after breaking it, and prove the fence RED by re-breaking it -- every prior test in that module ran against fixtures and passed while the real plan had no extractable marker.

## Immediate Next Steps

1. CORRECT THE MOVEMENT B ITEM 0 DESTINATION LINE in docs/governance/build-to-1.0-campaign-2026-07-18.md. Item 0 (calibrate the seam-map) currently reads as work inside Movement B without naming where it lands. Per the re-homing precedent it belongs to the new feature ADR, not to corrective work under the Validated ADR-0.33.0. Small prose edit; no code. Do not restate the measured figures -- re-measure them.

2. FILE THE ATTESTED-DEFERRAL-OWNER GHI via /ghi-author. The rule: an acceptance criterion declaring a deferred frontier must name an owner (GHI or successor OBPI) before Gate 5 accepts it. This is NOT a new ADR -- gz validate --req-kind-discipline already parses acceptance criteria, so the check tightens an existing scope rather than adding CLI surface, which keeps it in direct-fix territory. Evidence for the body: ADR-0.33.0's four deferral REQs, the operator-attested 2026-07-10 frontier note, and the zero-GHI search result.

3. AUTHOR ADR-0.37.0 VIA /gz-design -- a FEATURE ADR (foundation is sealed) that RE-HOMES ADR-0.33.0's residual. Two gaps, sequenced calibrate-then-compel: (a) the attested calibration frontier -- reach() returns transitive dependents so a leaf OBPI has none, and parent_invariants is never passed, leaving both edge sets empty; (b) compulsion -- entry must TRIGGER a door, which was never in ADR-0.33.0's checklist. Widening without calibrating installs inert gates; calibrating without compelling leaves a sharp gate nobody walks through. This is a real design dialogue and needs the operator present.

4. RETIRE THE RESUME GATE'S SURVIVING Write/Edit/NotebookEdit ARM INTO ADR-0.37.0, as the precondition of the session-entry door rather than as a loose direct fix. By the operator's framing a roadsign carries no gate at all, and the arm is an improvisation of Movement B item 3 ("Session entry triggers the airlock"). Removing it before the governed door exists would open a gap in front of it; removing it as part of that door closes the improvisation and the hole in one move.

5. DO NOT WORK ADR-0.35.0 UNTIL MOVEMENT B RELEASES THE QUEUE. It is HELD by ratified amendment, not set aside by default, and one-feature-at-a-time is unchanged. This step is listed to record that the queue position is RULED, not to invite a re-decision.

## Pending Work / Open Loops

- THE CAMPAIGN'S MOVEMENT B CHECKLIST HAS NO ADR BEHIND IT. Movement B is described as "new feature ADR extending ADR-0.33.0; heavy" and none is authored. Until ADR-0.37.0 exists the movement has no governed path and its five checkboxes cannot be worked through the OBPI pipeline.
- THE PAIRED-EVENT FAMILY IS UNDISPOSITIONED. Three instances found 2026-08-14; one fixed, GHI #766 open for the second, the airlock's 23/5 recorded as a Movement B box. No single owner asks "does this event's decision have a counterpart for its other branch?"
- ATTESTED DEFERRALS HAVE NO OWNER MECHANISM. Step 2 above is the fix; until it lands, any acceptance criterion may declare a frontier and nothing schedules it. ADR-0.33.0 is the worked example, disclosed four times and tracked nowhere.
- GHI #766 (session-exit bookmark constants), #767 (transcript reference), #799 (advisory scorecard), #802 (gzkit.org stale VPS, operator-only), #803 (docs-build mkdocs downgrades) all remain OPEN and unworked this session. #802 and #803 are unchanged from the resumed handoff.
- THE EXEMPTION INVENTORY still owes 55 differential controls; unchanged, and enrollment needs its own ruling.
- THE MAINTENANCE RATIO REMAINS UNRULED, carried across four handoffs now.
...
uv run gz adr status ADR-0.33.0-airlock-membrane             # read lifecycle and landed count from Layer-2, never transcribed
uv run python scripts/session_orientation.py                 # first campaign line must read "Movement B is TOPMOST; Movement A is HELD"
uv run -m unittest tests.scripts.test_session_orientation.LiveCampaignCarriesItsSteeringMarkerTests   # the marker fence
uv run -m unittest tests.governance.test_handoff_resume_gate # 49 tests; Bash-not-gated contract plus the git-sync pin
git rev-list --left-right --count origin/main...HEAD         # expect 0 0
grep -c '"event":"airlock_in"' .gzkit/ledger.jsonl           # re-measure; was 23
grep -c '"event":"airlock_out"' .gzkit/ledger.jsonl          # re-measure; was 5
gh issue view 766 --json number,state                        # OPEN; the second paired-event instance
...
docs/design/adr/pre-release/ADR-0.33.0-airlock-membrane/obpis/OBPI-0.33.0-04-airlock-mx-door.md
docs/design/adr/pre-release/ADR-0.33.0-airlock-membrane/obpis/OBPI-0.33.0-05-airlock-permitted-entry-door.md
docs/design/adr/pre-release/ADR-0.34.0-foundation-sunset/ADR-0.34.0-foundation-sunset.md
src/gzkit/handoff_resume_gate.py
src/gzkit/shell_reading.py
