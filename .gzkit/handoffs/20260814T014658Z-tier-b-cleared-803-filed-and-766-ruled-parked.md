...
continues_from: .gzkit/handoffs/20260814T013249Z-tier-b-cleared-and-the-docs-build-gap-filed.md
---

## Current State Summary

Two operator rulings were worked this session, in sequence, both booked via `gz handoff decide`.

FIRST — advised step 3 of the anchor (verbatim: "3"): the exemption drain's Tier-B backlog is CLEARED. All 17 claims the first pass left unreached were read end-to-end; eight carry no admit path and are declared `exempts='none'`, nine carry one and moved to Tier A with the path named. Disclosed 63 -> 55, `baseline_count` decremented in the same commit, 76 inventoried / 21 declaring. The state change matters more than the number: NOTHING in this inventory is unread, so the remaining 55 are ENROLLMENT debt — one differential control each — never reading debt.

A finding surfaced during that pass was routed rather than fixed: GHI #803, the `docs-build` gate's mkdocs validation downgrades, filed via `/ghi-author` after a clean Step-0 prior-art lookup and left OPEN with a blocker comment.

SECOND — advised step 4 of the successor (verbatim: "4"): considered whether GHI #766 should be pulled. RECOMMENDATION IS DO NOT PULL, and the measurement is banked on the issue rather than left in session memory. Three gates hold and none of them is a severity gate, so the stronger evidence moves none of them. No repo change was made by this step.

Four commits landed and pushed: `723a50352` (the drain), `db4dc2121` (the #803 cross-reference), `f4a123931` (the predecessor handoff), and this handoff's own commit. The campaign spine was NOT worked. `ADR-0.35.0` remains 0/10, every brief pending and draft, set aside for a SEVENTH consecutive session by explicit ruling.

## Important Context

THE SEVERITY LINE IS THE DRAIN PASS'S REAL PRODUCT, now written into both `_qc_claim_exemptions.py`'s docstring and the triage. Eight of the seventeen gates turned on a case the original five-bullet bar does not settle: a gate that FINDS something and does not exit non-zero on it. The separator is WHO CONTROLS THE ADMISSION. A finding held non-blocking by a FIXED CODE PROPERTY (a separate finding type, a `required: False` in an in-code check table, a question declaring no validator) is outside the judged set — the gate never claimed to enforce it. A finding the gate WOULD fail on, admitted by a PROJECT-CONTROLLABLE INPUT (an off-by-default flag, a manifest entry, a config value, a data file, a marker, a ledger booking) is an exemption, however well justified. Without that line `readiness-audit` and `skill-audit` are indistinguishable — both split blocking from non-blocking — and would have been ruled the same way. They are not: one's split is a constant in a table, the other's sits behind an off-by-default `--strict`.

A TOKEN SCAN WAS STRUCTURALLY INCAPABLE OF FINDING WHAT MATTERED, the third heuristic to fail this exact question in this repo. All nine newly-located admit paths live in modules containing ZERO waiver/grandfather/excluded tokens; `grep` clears every one. The compressible-tier witness in `bullet_retention`, the `CaptainOverride` in the airlock's `_decide`, and the floor's `_GATE5_NAMED_NOT_ENFORCED` subtraction are each stated in prose in a docstring and implemented as ordinary control flow. Prior failures: a naming-convention scan matched 0 of 70; claim-id-to-module-stem correlation matched 7 of 71. Do not re-derive membership from a scan.

THE DECLARATION MAP IS NOT THE ONLY INLET. `QC_CLAIM_EXEMPTS` feeds `register_qc_negative_controls` and nothing else. Three of the eight declarations register from `gzkit.mx.invariants` and `gzkit.mx.proxy_reality`, and `gz validate --exemption-controls` failed closed on all three until they carried `exempts=` at their own `enforces(...)` call sites. Check where a claim actually registers before declaring it.

GHI #766's LEDGER IS INVERTED, and this is new evidence the issue did not previously carry. `ledger_events.py:1050` defines `session_exit_bookmark_skipped_event` with no written-side counterpart, so Layer-2 holds 36 records of bookmarks that were NOT written and ZERO records of the 21 that WERE. Any Layer-2 query asking "did this session leave a bookmark?" is structurally guaranteed to answer no, while the 21 files are visible only on the filesystem and in `git log`. `docs/governance/state-doctrine.md` makes Layer-2 the system of record; here the record is precisely inverted. This strengthens rather than redirects the issue — the ruled Option-B shape already requires both branches to book.

GHI #766's HARM IS STEADY, NOT DECAYING, which is why the park deserves a different kind of patience than a shrinking defect would. All 21 bookmark files postdate the `c94de7e4b` intentionality mitigation (2026-08-05): 21 constants in 9 days against 36 skips, with six landing on 2026-08-12 alone. The mitigation suppresses roughly three exits in five; the other two still emit a constant across exactly two byte-lengths (1753 and 1765).

TWO CONSECUTIVE ITEMS HAVE NOW ROUTED BACK TO THE CAMPAIGN SPINE, and that is the session's real signal. The exemption inventory ran out of cheap work — its reading is finished and everything left is enrollment needing a fresh ruling. GHI #766 is correctly parked and what unblocks it is `ADR-0.35.0` finishing, not more evidence. The cheap-remainder argument that won the last several rulings against the spine no longer has a subject.

THE MAINTENANCE RATIO REMAINS UNRULED, carried unchanged across three handoffs now. Measured 2026-08-14 over 30 days on main: 592 commits — 260 `fix`, 232 `chore`, 74 `docs`, 13 `feat`; 116 GHIs filed, 112 closed, every gate green throughout.

## Decisions Made

- [operator-ruled] Work advised step 3 only — continue the exemption drain from the triage (verbatim: "3"), chosen from a four-option picker whose alternatives were the campaign spine, ruling on GHI #766, and booking a hold. Booked via `gz handoff decide` with steps 1, 2, 4 and 5 recorded `--set-aside`.
...
- [operator-ruled] Author a successor handoff and sync it (verbatim: "author new handoff, git sync it"). Landed as `f4a123931`.
- [operator-ruled] Work advised step 4 of that successor — consider whether GHI #766 should be pulled (verbatim: "4"). Booked via `gz handoff decide` with steps 1, 2, 3 and 5 recorded `--set-aside`, reversing the step-4 set-aside carried from the prior booking.
- [operator-ruled] Author this fresh handoff (verbatim: "write fresh handoff").
- [agent-chose] Declare only the eight claims whose gates admit nothing, and move the other nine to Tier A rather than leaving any in a third "read but undecided" bucket. All 17 reached a decision; the asymmetry rule — a wrong declaration launders an admit path, an absent one costs a later reading — was honored by declaring conservatively, never by deferring.
...
- [agent-chose] Declare the three `mx`-registered claims at their own `enforces(...)` call sites rather than widening `QC_CLAIM_EXEMPTS` to reach foreign registration sites, which would have coupled two deliberately separate tables.
- [agent-chose] Record on GHI #803 the precision that `validation.nav.not_found` is unset and still catches the docstring's stated origin incident, rather than filing the stronger and simpler claim that the gate is wholly withdrawn. The simpler claim would have been false and would have been discovered false by whoever worked the issue.
- [agent-chose] Recommend AGAINST pulling GHI #766, on the ground that all three of its gates — blocked by #767, parked behind an unpromoted pool ADR, and one-feature-at-a-time — are structural rather than severity gates, so sharper severity evidence moves none of them.
- [agent-chose] Bank the #766 re-measurement as an issue comment rather than leave it in session memory or in a handoff alone, so the next consideration starts from numbers instead of a re-read.

## Immediate Next Steps

1. PULL OBPI-0.35.0-01-corpus-tombstone-schema-and-fold. The campaign spine, Magna Carta by operator canon, and the topmost Movement A item whose gate is met. Now set aside SEVEN sessions running, every time by explicit ruling. What changed across this session is that the competing work ran out: the exemption inventory has no reading debt left, and GHI #766's blocker resolves only by landing this feature. Two consecutive items routed back here. Verified live this session as unstarted — every brief pending and draft, closeout BLOCKED. Read the fold algebra from the ADR's Decision section rather than re-deriving it, and run `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing` for the landed count rather than trusting any figure transcribed here.
...
4. DO NOT RE-CONSIDER GHI #766 UNTIL ADR-0.35.0 LANDS. Considered and answered this session; the analysis is banked as an issue comment. Its three gates are structural, not severity: blocked by #767 (OPEN), parked behind `ADR-pool.primary-source-corroboration` (verified `status: Pool`, unpromoted), and behind one-feature-at-a-time. Re-considering it before the spine moves will reach the same answer at the same cost. This step is listed to record that it is CLOSED as a question, not to invite a repeat.
5. AFTER ADR-0.35.0 OBPI-05 AND OBPI-07 LAND, WORK GHI #799. Unchanged and still correctly gated; its gate is unmet while ADR-0.35.0 has unlanded OBPIs — confirm with `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing` rather than assuming either way. Do not attempt it earlier, and do not score clauses 1, 2 or 3 as Judgment to make them fit.
...
- THE EXEMPTION INVENTORY HAS NO READING DEBT LEFT. Tier B is empty; all 76 claims have been read. A future pass looking for cheap declarations here will find none and should not spend a session confirming that.
- GHI #803 IS OPEN with three dispositions and an unmeasured backlog. Its blocker comment names the measurement as step 1.
- THE SESSION-EXIT LEDGER INVERSION is recorded on GHI #766 but unfixed and correctly parked: Layer-2 holds 36 skip records and zero written records against 21 files on disk. It rides with #766 behind `ADR-pool.primary-source-corroboration`; do not fix it standalone, because the ruled Option-B shape may retire the document the event would describe.
- GHI #767 IS OPEN and is #766's blocker by operator ruling — the transcript channel lands first. It sits behind the same unpromoted pool ADR.
- THE MAINTENANCE RATIO IS UNRULED. 260 fix, 232 chore, 74 docs and 13 feat commits over 30 days with every gate green, carried across three handoffs now without a ruling being sought on whether it should change how sessions are spent.
- MODULE-SIZE HEADROOM at `_qc_negative_controls.py` is unchanged at roughly 17 SLOC against the block band. This session added nothing to that file — the eight declarations went into `_qc_claim_exemptions.py` (136 raw lines) exactly as advised. The next addition to the negative-controls table will still trip the pre-push gate.
- SIX CLAIMS INHERIT THEIR EXEMPTION FROM AN EXTERNAL TOOL — lint, format, typecheck, test, behave, and now `docs-build`. Worth deciding once as a family rather than six times; GHI #803's disposition 3 is the family-shaped answer.
...
uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing   # read the landed count from Layer-2; never transcribe it back into this file
git rev-list --left-right --count origin/main...HEAD         # expect 0 0
grep -c session_exit_bookmark_written .gzkit/ledger.jsonl    # 0 — the inversion recorded on GHI #766
grep -c session_exit_bookmark_skipped .gzkit/ledger.jsonl    # 36 against 21 files on disk
...
gh issue view 766 --json number,state            # OPEN, correctly parked; re-measurement banked as a comment
gh issue view 767 --json number,state            # OPEN, #766's blocker by operator ruling
