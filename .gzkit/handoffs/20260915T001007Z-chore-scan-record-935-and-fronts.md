...
adr_id: null
branch: main
timestamp: '2026-09-15T00:10:07Z'
agent: claude-code
session_id: 863d2b33
continues_from: 20260914T231625Z-chores-status-936-and-fronts.md
---

## Current State Summary

Session 863d2b33 resumed `20260914T231625Z-chores-status-936-and-fronts.md` (ruling booked: proceed, option (a); (b), (c), (d) set aside). It fixed the reopened GHI #935 interval-gate deadlock in `7e306e4cb`, synced it to origin/main, and closed #935 with evidence.
...
At authoring: HEAD `7e306e4cb`, level with origin/main, no OBPI locks.
...
- [operator-ruled] Resume ruling on `20260914T231625Z`: "a — fix the #935 deadlock". The 35-overdue board, R&D skill design, and the #1010/#1008 short fixes set aside.
- [operator-ruled] GHI #935 witness shape: "Declare it (Recommended)". Add `staleness.artifacts` to the chore declaration, read by both the gate and `gz chores status`. Undeclared elapsed-time chores keep the PASS-block reading in status and are refused by the gate. Inferring the record from `proofs/` and a gate-only fix were set aside.
- [agent-chose] The newest change among declared records dates the scan, rather than requiring every record to change. An identically regenerated file makes no commit, so requiring every record would re-create a deadlock.
...
- [agent-chose] Removed a registry-reading coherence test that the tautological-test audit refused. The gate's exit-1 refusal already makes an undeclared gated chore announce itself.
...
- `.gzkit/handoffs/20260914T231625Z-chores-status-936-and-fronts.md` (predecessor).
...
- Commit `7e306e4cb`; GHI #935 contract-amendment and close comments.

## Settled Rulings

865 rulings booked and carried forward. The corpus lives in `.gzkit/handoffs/rulings.jsonl` — read it with `gz handoff rulings`.
