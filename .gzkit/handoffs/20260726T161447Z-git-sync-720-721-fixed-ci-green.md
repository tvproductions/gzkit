...
adr_id: null
branch: main
timestamp: '2026-07-26T16:14:47Z'
agent: claude-code
session_id: 5eed8cd8-58a7-48c0-9d4b-0280c2ebecd4
continues_from: .gzkit/handoffs/20260726T113150Z-swarm-forge-pool-adrs-718-fixed-719-open.md
---

## Current State Summary

Ran the guarded git-sync ritual, then filed, fixed, and closed two defects that
the sync itself surfaced. HEAD at `2a0652b6`, tree clean and level with origin.
CI is green after five consecutive red runs (`5ef2a1d5`, `47c99613`, `e16831a4`,
`2e1793c8`, `02ed4e83` all failed; `d9224500` and `2a0652b6` pass).

Three commits landed: `02ed4e83` (the sync's own governance-file chore commit),
`d9224500` (GHI #721 brief-reconcile), `2a0652b6` (GHI #720 git-sync ordering).
GHI #720 and #721 are CLOSED with evidence comments citing their SHAs. No active
OBPI lock, no in-flight pipeline, 13 open GHIs.

The session's original ask was only "git-sync". Everything after it was defect
repair the sync exposed, plus one operator-authorized config change.

## Important Context

FIRST, the two defects are the same shape and it is worth naming: a predicate
evaluated against state that something else had already changed. In GHI #721 the
state was the filesystem and the mutator was whichever repositories happen to sit
beside gzkit; in GHI #720 the state was ahead/behind and the mutator was the
ceremony's own auto-commit. Both produced verdicts correct on the machine that
computed them and wrong everywhere else, and both hid because the environment
where they were false was never the environment where they were tested.

SECOND, GHI #721 was invisible locally BY CONSTRUCTION and will look
fixed-or-never-broken to anyone reproducing on this machine. `../airlineops` is
checked out beside gzkit here, so the out-of-tree existence check resolved and the
suite ran OK with 7482 tests while CI failed on the identical commit. Any agent
that runs the suite locally and sees green may wrongly conclude the CI log was
noise. The guard now skips the path BEFORE `.exists()` is consulted, so sibling
presence is structurally irrelevant rather than incidentally absent.

THIRD, and the most likely repeat offender: negative-path tests print fail-closed
prose into CI logs that reads exactly like a real failure. This session's CI
diagnosis initially targeted `Fidelity validation failed [surface-weight]: Surface
weight limit exceeded / File not written.` That string is intended stderr from the
PASSING `test_save_hook_blocks_write_on_violation`
(`tests/content/test_validation_hooks.py:401`), which patches `validate_save` with
`side_effect=FidelityHookError`; ADR-0.0.34's own AUDIT.md line 158 already
documents it as intended negative-path output. Both remedies suggested for it
would have destroyed the negative control -- there is no payload to trim because
it is a mock, and stopping the implementation from breaching the budget would make
the control unable to fail. This is the SECOND occurrence: the predecessor chain
records the same misread of `OBPI-unwaivered-pre-cutoff-01`. Recorded as a
discovery insight; still has no GHI.

FOURTH, the `g0` authorship guard is PER-CLONE and this is PII-adjacent. Global
git config still holds the operator's personal email; only a local override keeps
it out of commits. Local config previously pinned `user.email` but NOT
`user.name`, so the global personal name bled through on `02ed4e83` -- 19 of the
preceding 20 commits are `g0`, that one is the exception. Local now pins both to
`g0` and the GitHub noreply address. A fresh clone of any gzkit-governed repo
falls through to the personal email again until someone sets it. No leak occurred;
the email override held throughout.

FIFTH, a test-harness trap that cost a false GREEN during authoring and will cost
the next person one too. `_compute_git_sync_state` lives in `gzkit.git_sync` and
resolves `git_cmd` in THAT namespace. Patching only `gzkit.commands.sync.git_cmd`
leaves real git running against the fixture root, which reports behind=0 and makes
every pull assertion vacuously pass. Three tests passed for no reason before this
was caught. Both namespaces must be patched; the reason is documented at the patch
site in `tests/commands/test_sync_pull_ordering.py`.

SIXTH, the near-edge skill-staleness cohort that the predecessor chain was written
to carry is RESOLVED upstream and needs no action: `uv run gz skill audit` passes,
68 skills, Blocking 0. Commits `cc659dc2` (review-and-stamp the cohort) and
`c5c878aa` (warn band before the gate blocks) landed in the 24 commits pulled at
session start. Do not re-sweep it.

## Decisions Made

- [operator-ruled] A git-sync is ALWAYS authorized and never gated on a handoff
  (operator verbatim 2026-07-26: "yes, just do my git-sync - a git-sync will
  ALWAYS be authorized - think about it, if we need to sync with remote, your
  local handoff is almost always likely to have been superseded by something on
  remote. Challenging me on a handoff for a git-sync is silly."). Booked via
  `gz handoff authorize` against both the gating handoff and, after the pull
  replaced it, its successor. Borne out mechanically in-session: the 24 pulled
  commits had already landed all five advised steps of the handoff being gated on.
- [operator-ruled] Add the local `user.name = g0` guard (operator verbatim
  2026-07-26: "yes, do the user.name = g0 guard.").
- [operator-ruled] Fix GHI #720 with the recompute approach rather than reordering
  the ceremony to pull before committing (operator verbatim 2026-07-26: "fix #720
  with the recompute").
- [agent-chose] Fixed the CLASS in GHI #721, not the reported instance. Three
  existence-checking dimensions shared the predicate; `_compute_allowlist_delta`
  GATES via `gating_allowlist_absence` and was already reporting three
  `../airlineops` paths for OBPI-0.0.13-06, ungated only because that brief is
  unstarted. 92 briefs cite such paths, so repairing one row would have moved the
  next failure rather than removed it.
- [agent-chose] Rejected the initially-proposed CI diagnosis and its two remedies
  after tracing the actual failure, rather than applying a fix that would have
  turned CI green while leaving the real defect in place. Evidence: the named test
  passes, and its assertion is already the contract the remedy proposed adding.
- [agent-chose] Extended GHI #720's scope once, to `_plan_git_sync`. Fixing
  execution alone would have made the dry-run contradict the run it describes --
  plan promising `--ff-only` while apply rebased, and omitting the push
  `_push_if_ahead` then performs. The git-sync skill directs operators to preview
  first and flags "Dry-run run without follow-through apply", so a lying preview
  attacks that contract.
- [agent-chose] Did NOT change `assess_git_sync_readiness` in `git_sync.py`
  despite its parallel stale prediction. It advises the operator what to run by
  hand (where `--ff-only` on a behind tree is correct guidance) and never executes,
  so it cannot break a tree. Recorded on GHI #720 so nobody "fixes" a planner.
- [agent-chose] Resolved the mid-sync divergence by dropping the local chore commit
  and re-emitting its two events through `gz handoff authorize` and
  `gz insights remember`, rather than hand-resolving the append-only conflict in
  `.gzkit/ledger.jsonl`. Editing the ledger directly is a hard Never; the governed
  verbs own those writes.
- [agent-chose] Used `g0` as the git AUTHOR field rather than a trailing message
  line. Operator canon defines it as "git author name", all 19 preceding canon
  commits do this, and a literal footer would displace the `Task:` trailer from the
  final paragraph and break `gz validate --commit-trailers`.
- [agent-chose] Verified the test count with the canonical ARB step instead of
  asserting it from arithmetic. The drafted commit message said 7487; the observed
  figure is 7491.

## Immediate Next Steps

1. NOTHING IS AUTHORIZED. This session's work is closed and needs no follow-up;
   every step below requires an explicit operator ruling before execution.
2. Rule the negative-path log-legibility finding (Important Context THIRD): file a
   GHI, or accept it as known noise. It has now caused two wrong diagnoses in two
   sessions and is tracked only as an insight, which is not a tracker. The proposed
   remedy is a marker convention distinguishing exercised fail-closed prose from
   genuine failures in CI logs.
3. Rule the `g0` guard's durability (Important Context FOURTH): leave it per-clone,
   change the global config, or add a `gz init` step that writes the local guard.
   The PII rule treats a personal-email leak as needing a filter-repo rewrite to
   recover, so the per-clone gap is a standing risk on every fresh clone.
4. The predecessor handoff's advice is UNWORKED and still live: GHI #719 (pool
   ...
   errors); promotion of the two pool ADRs when wanted, with the ephemeral-worktree
   doctrine carve-out ratified FIRST for ADR-pool.worktree-parallel-agents.

## Pending Work / Open Loops

THE NEGATIVE-PATH LOG-LEGIBILITY FINDING is untracked by deliberate choice
pending an operator ruling. It is recorded in
`.gzkit/insights/agent-insights.jsonl` as a discovery insight with evidence and a
proposed next action, but an insight is not a tracker and this is its second
occurrence.

THE `g0` GUARD IS PER-CLONE. Fixed for this working copy only; global git config
still holds the operator's personal email.

CARRIED FROM THE PREDECESSOR, all unworked: GHI #719; campaign Movement A
(ADR-0.35.0 at 0/9, ADR-0.34.0 at 2/5); promotion of
ADR-pool.worktree-parallel-agents and ADR-pool.ledger-concurrency-substrate. The
predecessor's own first step reads "NOTHING IS AUTHORIZED" and that remains true.

13 GHIs open. The three-way file contention recorded earlier on
`src/gzkit/governance/brief_reconcile.py` is partly relieved: #615 cuts 2+3 and
#641 landed upstream before this session, and #581 remains OPEN against that file.
This session also modified it, so re-read before assuming the contention shape.

SPEC-TEST DRIFT ADVISORY stands at 2031 findings (2020 unlinked specs, 10 orphan
tests, 1 unjustified code change). Advisory only; does not affect exit code.

TWO OBSERVATIONS NOT ACTED ON. `gz check` does not emit a "Ran N tests" line, so a
test-count claim cannot be sourced from its log -- use the canonical ARB step. And
`gz git-sync --apply` refuses to auto-commit while `.git/COMMIT_EDITMSG` holds a
stranded conventional-commit subject; that guard fired correctly this session after
a dropped commit, and it names its own release path, but it means a failed sync can
require two manual interventions.

## Verification Checklist

`git log --oneline -3` (expect `2a0652b6`, `d9224500`, `02ed4e83`);
`git status --short --branch` (expect a clean tree level with origin/main);
`git log -1 --format=%an` (expect `g0`);
`git config --local user.name` (expect `g0` -- if empty, the guard is missing on
this clone);
`gh issue view 720 --json state` and `gh issue view 721 --json state` (expect
CLOSED);
`gh run list --workflow=CI --limit 2` (expect success on `2a0652b6` and
`d9224500`);
`uv run gz check` (expect exit 0, all 43 steps, with the pre-existing
spec-test-drift and complexity advisories);
`uv run gz skill audit` (expect exit 0, Blocking 0, 68 skills -- confirms the
staleness cohort is resolved upstream and needs no sweep);
`uv run gz obpi lock list` (expect no active locks);
`uv run gz validate --commit-trailers` (expect exit 0).

To confirm GHI #721 is genuinely fixed rather than masked by this machine's
layout, note that the guard skips an out-of-tree path BEFORE `.exists()` runs:
`_escapes_project_root` returns True for `../airlineops/.claude/hooks/` regardless
of whether the sibling repo is present. Reproducing the ORIGINAL failure requires
a clone without `../airlineops` beside it.

To confirm GHI #720's tests still discriminate, pin sync state to entry values
(ahead=0, behind=N, diverged=False) and re-run
`tests.commands.test_sync_pull_ordering`; the headline test must fail. If it passes
under pinned state, the ordering property is no longer being tested.

## Evidence / Artifacts

Session commits: `02ed4e83` (git-sync governance-file sweep), `d9224500`
(GHI #721 brief-reconcile out-of-tree guard), `2a0652b6` (GHI #720 git-sync pull
ordering).
...
`tests/commands/test_sync_pull_ordering.py`,
`tests/commands/test_sync_sweep_guard.py`, `docs/user/manpages/git-sync.md`.

The planner deliberately left unchanged: `src/gzkit/git_sync.py`.
