...
adr_id: null
branch: main
timestamp: '2026-08-11T01:33:59Z'
agent: claude-code
session_id: f1827b6c-5fe7-43d4-8147-422fac7c4389
continues_from: .gzkit/handoffs/20260811T010851Z-structural-repairs-and-solution-approach.md
---

## Current State Summary

Collapsed the forked handoff chain — advised step 4 of the resumed anchor, the only step the operator authorized this session (verbatim: "do step 4"). The chain carried TWO live heads whose `Settled Rulings` corpora were DISJOINT: `20260809T210209Z-critic-promoted-briefs-deferred.md` held 145 rulings, `20260811T010851Z-structural-repairs-and-solution-approach.md` held 72, and their measured intersection was ZERO. This document is a single successor newer than both, carrying the union, so neither prior head can be selected as an anchor again. The session opened as a `/git-sync` which landed at `8a9fde21a` (ledger-only, two events: the `/clear` exit bookmark and the `handoff_resume_decided` ruling); tree clean, `main` level with `origin/main`. Advised steps 1, 2, 3 and 5 were recorded set-aside and remain unworked.

## Important Context

THE FORK DROPPED THE ENTIRE 2026-08-06..2026-08-09 RULING ACCUMULATION, and that is the damage — not the anchor ambiguity the predecessor named. `resume_handoff` selects element zero of a newest-first list, ordered by frontmatter timestamp GLOBALLY, never by chain traversal. So on any clone holding both, the newer head already won and the ambiguity was inert. What was NOT inert is `_carried_settled`, which composes a successor's rulings from ONE predecessor: the Windows session ran on a stale clone, wrote a successor rooted in a 2026-07-26 ancestor, and every later head inherited that stale corpus. 145 booked operator rulings stopped propagating — including `we will NOT alter the OBPI process, at all`, `we DO NOT go out of sequence (0.44.0)`, `only one feature at a time`, and the instructions-file-budget park. A future session resuming the pre-collapse head would not have seen them.

THE COLLAPSE NEEDED ONLY RECENCY, BUT CORRECTNESS NEEDED THE UNION. Because selection is by timestamp, any newer document retires both heads mechanically. Picking one branch would have retired the other's anchor while silently discarding 72 or 145 rulings, so the union is the only lossless collapse. `--settled` is documented as unioning with carried entries and both composition paths share `_dedup_rulings`, so seating the orphan's corpus against the carried one is the governed path rather than a hand-merge.

`continues_from` POINTS AT THE RULED ANCHOR, NOT THE ORPHAN. Both are genuine ancestors and the field holds one, so it names the document this session actually resumed and the operator actually ruled on. The orphan's content is preserved by value in `Settled Rulings` rather than by reference in lineage; the lineage pointer is therefore honest about provenance and lossless about content, which is the best a single-valued field can do. A multi-parent `continues_from` is the structural gap that made this repair manual, and it is unfiled.

THE SETTLED-RULINGS CHANNEL CARRIES UNHEALED HISTORICAL DAMAGE. Measured on the pre-collapse head: 3 entries clipped mid-sentence and 4 near-duplicate groups. `_section_items` in `src/gzkit/handoff_api.py` was already fixed to join wrapped continuations, and its own docstring predicts this — a clipped ruling `no longer dedups against its untruncated twin, so both propagate down the chain`. The fix is forward-only; nothing heals clips already in the chain, and `20260727T234748Z-handoff-review-shattered-rulings-reseated.md` carries the clipped and full forms of the same ruling as two bullets in one file. The 4 duplicate groups are a DIFFERENT failure: semantic abridgements, longer-first in every pair, which escape `_ruling_key` because it folds only quoting, whitespace and case. That narrowness is deliberate and defended in its docstring — collapsing two distinct rulings drops a booked operator ruling silently, so duplication is the chosen safe direction. This union inherits both conditions and does not repair either.

## Decisions Made

- [operator-ruled] Book the resumed anchor `proceed` with all five advised steps set aside, sync only (verbatim: "proceed with the git sync, set aside the rest").
- [operator-ruled] Work advised step 4, the fork collapse, after reviewing the handoff (verbatim: "review handoff", then "do step 4"). Steps 1, 2, 3 and 5 were not authorized and were not worked.
- [agent-chose] Collapsed by UNION into a new head rather than by selecting one branch. Selection is by timestamp, so either branch would have unforked the anchor; only the union preserves all 217 rulings. Picking a branch would have silently discarded 72 or 145 booked operator rulings.
- [agent-chose] Pointed `continues_from` at the ruled anchor rather than the orphan, and carried the orphan by value through repeated `--settled`. Both are true ancestors; the field holds one, so it names the document the operator ruled on.
- [agent-chose] Used `gz handoff create --settled` rather than editing a handoff by hand, so the union runs through `_dedup_rulings` and the fail-closed validation gate instead of a hand-merge.
- [agent-chose] Did NOT repair the 3 clipped and 4 duplicated rulings while unforking. The clips need upstream recovery and the duplicates need a dedup-policy change on a shared composition path; both are separate defects surfaced with routing facts, not fork damage.

## Immediate Next Steps

1. THE FOUR SET-ASIDE STEPS REMAIN UNWORKED and carry forward verbatim from the predecessor: rule on the AGENTS.md truncation (385 B past the codex delivery cap, `architectural-boundaries` straddling at byte 32558 — the GHI #580 [settled] reorder is a Layer-1 canon change requiring Gate-5, parked post-1.0, and must NOT be taken unilaterally); the growth brake; the witness-density check. Confirm with `uv run gz validate --instructions-files-budget`.

2. THE CAMPAIGN HAS STILL NOT MOVED. `ADR-0.36.0-convergence-moment-cross-family-critic` measured 0/9 this session with every brief `draft (scaffold)`; it is the topmost Magna Carta item. `ADR-0.35.0` measured 0/10 — TEN, not nine. Read both with `uv run gz adr status <ADR-ID>` rather than trusting this prose.

3. ROUTE THE SETTLED-RULINGS CLIP REPAIR. 3 entries are clipped mid-sentence and the full text is still recoverable upstream — `20260727T234748Z-handoff-review-shattered-rulings-reseated.md` holds the complete form of the clipped triage-list ruling. A one-time backward heal erases damage that otherwise propagates forever. Routing facts measured: single module, 374 `fix(` commits in 60 days, defect surfaced in flight — direct fix on the thresholds, but `_dedup_rulings` is shared by both composition paths deliberately, so land a covering test first.

4. CONSIDER FILING THE MULTI-PARENT GAP. `continues_from` is single-valued, so a genuine merge cannot be expressed and this collapse had to carry one parent by value. Unfiled.

## Pending Work / Open Loops

THE SETTLED-RULINGS CHANNEL IS UNREPAIRED — 3 clipped entries, 4 abridged-twin duplicate groups, both inherited into this union unchanged. Neither is fork damage; both predate it.

THE OPEN-COUNT INSTRUMENT IS STILL THE WRONG ONE and remains unbuilt, carried unfiled across several predecessors. Nothing tracks defect rate, found-versus-introduced, or time-to-detection, so the only signal reaching the operator is the raw count of problems, which a maturing system drives UP.

THREE DEFECTS TRACKED AND NOT FIXED, all via `gz insights remember`: no `*.jsonl merge=union` strategy in `.gitattributes`, so concurrent syncs reliably conflict on the ledger; the unresolved `Path.cwd()` fallback in the control-surface-sync hook; and `gz typecheck` corrupting diagnostic coordinates by expanding emoji shortcodes.
...
The collapse is verified by the corpus being a superset of both parents, not by the file existing:

```
uv run gz handoff list
uv run gz validate --lock-exchange-coupling
uv run gz check
```

Expect this document to be the newest entry, so resume selects it over both prior heads. The union assertion is mechanical — parse `Settled Rulings` here and confirm every normalized ruling key from BOTH `20260809T210209Z-critic-promoted-briefs-deferred.md` and `20260811T010851Z-structural-repairs-and-solution-approach.md` is present. A future collapse should re-run exactly that check rather than trusting this paragraph.

Session state confirmed before authoring: CI green on BOTH legs at `ff8ad605d` (`check (ubuntu-latest)` and `check (windows-latest)`, run 31448046820), discharging the predecessor's only unverified claim; `git rev-list --left-right --count origin/main...main` returned `0 0`; `--advisory-scorecard`, `--transcribed-adr-counts` and `--waiver-ratchet` exit 0; `--instructions-files-budget` exits 0 with the two by-design delivery warnings.

## Evidence / Artifacts

The two forked heads, both retired by this document:

- `.gzkit/handoffs/20260809T210209Z-critic-promoted-briefs-deferred.md` — 145 rulings, seated here by value
- `.gzkit/handoffs/20260811T010851Z-structural-repairs-and-solution-approach.md` — 72 rulings, carried here by lineage

Selection and composition surfaces read to ground the approach, all in `src/gzkit/handoff_api.py`: `resume_handoff` (newest-by-timestamp selection), `_carried_settled` (single-predecessor composition), `_dedup_rulings`, `_ruling_key` (deliberately narrow normalization), and `_section_items` (forward-only continuation join).

Unhealed clip with its recoverable original — both forms present as separate bullets in `.gzkit/handoffs/20260727T234748Z-handoff-review-shattered-rulings-reseated.md`.

Ledger events this session: `handoff_resume_decided` (proceed, five steps set aside), committed in `8a9fde21a`.
