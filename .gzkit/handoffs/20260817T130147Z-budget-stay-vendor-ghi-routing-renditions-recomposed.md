...
adr_id: null
branch: main
timestamp: '2026-08-17T13:01:47Z'
agent: claude-code
session_id: 1503933b-a8ba-44f5-82a6-53df9af8e676
continues_from: .gzkit/handoffs/20260817T110511Z-architecture-review-dispositioned-movement-e-created.md
---

## Current State Summary

Resumed handoff 20260817T110511Z, worked its advised steps 3/4/5 under operator ruling, then four further operator rulings extended the session well past the handoff scope. Five commits `3b394c8b5..f93897f80`, tree clean, `0 0` against origin at authoring time.

Landed: the two owed doctrine captures (airlock four purposes, airlock/handoff cooperation clause) via `gz content remember`; the agent-runner facade corrected in both places it appeared; a PRD `synthetic-memory` glossary entry; two operator-canon typos corrected through corpus retire+remember; the control-surface budget stay (two fail-closed arms flipped to warn); vendor-limit exceedances routed to GHI tracking; and both AGENTS.md renditions recomposed under Gate 5 and pushed.

Gate evidence observed, not asserted: `uv run -m unittest discover -s tests -t .` 8436 tests exit 0; `gz validate` 13 scopes exit 0; `gz validate --advisory-scorecard` exit 0; receipts `arb-ruff-a6fad85f40b24556b6c00729dc44d9e2`, `arb-step-typecheck-e9c35801b832460eaff9809648634b6b`, `arb-step-mkdocs-1b74708897ab422284ee39c3a944a93b` all exit 0; pre-push `gz check` passed.

No ADR or OBPI advanced. Advised steps 1 and 2 of the predecessor stay set aside.

## Important Context

**The predecessor handoff's step 4 rested on a false premise, and the operator caught it.** It advised settling `prosthetic` versus `synthetic` memory as ONE term. Measurement said otherwise on the framing, not the concept: an agent has used BOTH terms for the airlock (`ADR-0.33.0:19` prosthetic 2026-07-08; campaign row 406 synthetic) and both make sense — `prosthetic` names whose deficit the memory answers, `synthetic` names how it was made. The agent built a "they are not synonyms" case out of MECHANISM differences (blast radius, subject, source) and presented it as CONCEPT difference; that was wrong and was withdrawn. The real defect was three spellings with no definition anywhere, closed by a glossary entry that keeps both terms current. Do not reopen this as a naming contest.

**`prosthetic` is agent-coined and was never ratified.** It appears in ZERO operator-verbatim quotes repo-wide; in `ADR-0.33.0-interview.json` it occurs only in `intent` and `positive_consequences`, both agent-synthesis fields. First written 2026-07-08. The operator's own `sythetic` first appears 2026-08-06. Git sees files, not conversations, so an earlier spoken coinage cannot be ruled out.

**The verbatim-preservation rule was misapplied to typos, and that had a measurable cost.** Operator canon carried `sythetic` and `ecosysten`, stamped "(spelling preserved)". The rule exists so an agent cannot launder the operator's words or MEANING; a keystroke slip is neither. Cost: `rg synthetic` and `rg prosthetic` BOTH missed the bullet that defines the concept, because it was spelled a third way.

**`gz content compose --candidate` reads from STDIN when omitted.** Two runs silently validated empty input and reported an invariant-floor violation naming the TTY/PTY entry — the first entry checked, not the actual cause. That sent the agent hunting a duplicate-pair theory that was entirely wrong. Always pass `--candidate <path>` explicitly.

**GHI #814 should not have been filed.** `ghi-author` Step 0 searches GHIs and worked (it found #635 and produced the sibling analysis), but it does not search in-flight ADRs — and `ADR-0.35.0` § Forces item 2 had already measured the same seven duplicate pairs in the same words. When a finding names its own likely home, read that home BEFORE filing. The lesson was applied on the next filing (#815) and prevented a second duplicate.

**`ValidationError` has no severity field.** Every `gz validate` diagnostic is fail-closed by construction. A new invariant can therefore only be adopted at the moment the repo already satisfies it — which is why this codebase reaches for baselines and decrease-only ratchets, and why a measurable-but-not-yet-remediable defect has nowhere to live except a GHI.

**The budget stay and the vendor-limit ruling are DISTINCT rules and must not be collapsed.** Budgets are gzkit's own thresholds, stayed to advisory until 1.0. A vendor delivery cap is physical truncation owned by the consumer: no gzkit ruling can stay it and raising a budget cannot relieve it. Collapsing them would let a future reader ship an AGENTS.md whose binding rules never reach the agent.

## Decisions Made

- [operator-ruled] Work advised steps 3, 4 and 5 of the resumed handoff (verbatim: "Steps 3, 4, 5 — the cheap doctrine items"), selected from a four-option picker. Booked via `gz handoff decide`; steps 1 and 2 recorded `--set-aside`, step 2 as a fence rather than work.
- [operator-ruled] Correct the `sythetic` misspelling in all three files INCLUDING Layer-1 canon (verbatim selection: "All three, canon included"), chosen over code-docstrings-only and over filing it.
- [operator-ruled] Both `synthetic` and `prosthetic` are apt for the airlock and both stay (verbatim: "no, an agent, in reference to the airlock system, has used both terms, and both make sense"). This REVERSED the agent's recommendation to retire `prosthetic`, and the agent's supporting analysis was withdrawn as a mechanism/concept conflation.
- [operator-ruled] File the corpus/rendition coherence finding as a GHI and do it properly (verbatim: "file it, do it right"), read against the standing ruling "do it right - fix defects when found".
- [operator-ruled] Temporary stay of all control-surface budget limits until 1.0 — warn, never block (verbatim: "temporary stay of all control surface budget limits until version 1.0. I want to be warned, and we may lift the limits as needed, but no blockers. that may be the current behavior."). The closing guess did not hold: it blocked, in TWO arms.
- [operator-ruled] Vendor-limit exceedances route to a GHI rather than a block (verbatim: "ok, when we exceed vendor limits, ghi it so it doesn't block"). The non-blocking half was already true since 2026-07-06; the ruling added the tracking obligation.
- [operator-ruled] Gate 5 attestation for both rendition candidates (verbatim: "yes, push the rendition candidates"), attestor `g0`, knowingly widening the Codex cap breach from 385 B to 1586 B with that disclosure carried in the attestation text.
- [operator-ruled] Sync the session including the two untracked `docs/evals/` hexagonal-review files (verbatim: "git sync with the two files").
- [agent-chose] Repointed the `instructions-files-budget` negative control at the scope's surviving fail-closed property rather than deleting it, and widened its entrypoint to run both arms summed as `validate_cmd` composes them. Calling only the budget arm would have left the control structurally unable to fail.
- [agent-chose] Treated `REQ-0.0.54-03-01d` as SUPERSEDED with `ADR-0.0.54` left sealed, rather than amending a Validated ADR. The covering test keeps its proof channel and now asserts BOTH halves — an audit that had merely stopped looking would satisfy a no-finding assertion alone.
- [agent-chose] Scoped the stay to budget limits only; `agents_md_map_conformance` criteria (a) paragraph shape, (b) prohibited titles and (c) link resolution stay fail-closed.
- [agent-chose] Authored the rendition candidates programmatically from corpus text rather than transcribing them, so invariant-tier strings cannot drift from their entries.
- [agent-chose] Filed GHI #815 recording the measured breach and proposing no fix, because both remedies already have registered homes.
...
5. **Restore the budget gates to fail-closed at 1.0.** The exit condition is recorded in three places that must move together: `data/instructions_files_budget.json` last entry, `.gzkit/rules/agents-md-map-doctrine.md` v0.4.0, and advisory-scorecard row 17b (re-score Judgment to Mechanical when the stay lifts).
...
- `.gzkit/corpus/AGENTS.md.jsonl` — three invariant captures and two retirements this session; 59 rows (56 entries + 3 retractions)
- `.gzkit/renditions/AGENTS.md/claude.md` — recomposed, 34354 B, corpus fingerprint `b388afd9f5e3`
- `.gzkit/renditions/AGENTS.md/codex.md` — recomposed, 15764 B, same fingerprint
- `AGENTS.md` — played back from the rendition; typos corrected, retired GHI #635 bullet removed, two airlock captures seated
- `docs/design/prd/PRD-GZKIT-1.0.0.md` — § 2.1 gains the `synthetic-memory` glossary entry
- `README.md` — agent-runner claim narrowed
- `docs/design/adr/pool/ADR-pool.workflow-specification.md` — same claim, second instance
- `src/gzkit/governance/trust_audits/instructions_files_budget.py` — budget arm flipped to advisory
- `src/gzkit/governance/trust_audits/agents_md_map_conformance.py` — criterion (d) flipped to advisory
- `src/gzkit/governance/trust_audits/surface_delivery_witness.py` — recovery prose names the GHI route
- `src/gzkit/governance/trust_audits/_qc_negative_controls.py` — NC repointed at the surviving fail-closed property
- `data/instructions_files_budget.json` — two dated ruling entries; budget values unchanged
- `docs/governance/advisory-rules-audit.md` — row 17b re-scored; Coverage Ledger entry added
- `.gzkit/rules/agents-md-map-doctrine.md` — rule version 0.4.0
- `.gzkit/insights/agent-insights.jsonl` — one `improvement` under scope `term-doctrine-analysis`
