...
adr_id: null
branch: main
timestamp: '2026-08-12T06:31:18Z'
agent: claude-code
session_id: c0e35610-b8bd-4cd8-85d7-c679777a85ae
continues_from: .gzkit/handoffs/20260811T125121Z-three-owed-rulings-discharged.md
---

## Current State Summary

The operator ruled advised steps 3 and 4 of the anchor ('Steps 3+4 — clear the defect queue first'); steps 1 and 2 were recorded set-aside via 'gz handoff decide'. Both ruled steps proved deeper than the anchor described, and the session closed three GHIs rather than two. Three commits landed and are synced: 3d8e9e99b (GHI #791, the recalibration producer plus the ruled band raise), 3c768bb73 (GHI #790, multi-parent lineage), 1d376c61f (GHI #792, the band witness). 'uv run gz check' exits 0; tree clean; 'origin/main...main' is 0 0 at 1d376c61f.

The session's shape was set by one discovery. Step 4 asked for a band raise; verifying the ceremony BEFORE performing it found that the witness the operator had chosen could not be produced by any registered verb, which turned one ruling into a filed-and-fixed GHI (#791), and closing that one exposed its sibling half (#792) which the operator then ruled into scope with 'ghi to fix?'.

## Important Context

THE SURFACE-WEIGHT BANDS ARE NOW WITNESSED IN BOTH DIRECTIONS, AND THIS CHANGES WHAT A RULE EDIT COSTS. Bands are green <= 3000 / yellow <= 3400, floor re-snapshotted to 2600. 'uv run gz validate --surface-weight --recalibrate --attestor "<name>" --reason "<evidence>"' is the producer; the same scope now ALSO fail-closes when the live '_GREEN_CEILING'/'_YELLOW_CEILING' constants disagree with the bands recorded on the newest 'surface_weight_recalibrated' event. Editing those constants without recalibrating is now a red gate, verified by live probe (set 3200, observed exit 3, reverted). The corpus has roughly 400 lines of green headroom, about 47 days at the measured 8.4 lines/day.

THE CEREMONY HAD NEVER BEEN PERFORMABLE, WHICH IS WHY NOBODY PERFORMED IT. 'OBPI-0.0.33-02' REQ 4 named 'gz adr emit-receipt' as the producer of 'surface_weight_recalibrated'. That verb's '--event' is a closed enum of completed/validated/closed. Four surfaces prescribed the impossible command and the ledger carried zero such events across the project's life. The 2026-06-30 band change (green 1800 to 2600) therefore landed as exactly the config tweak ADR-0.0.33 anti-pattern 3 forbids — not negligence. A self-concealing property made it durable: '_check_floor_drift' returns clean when zero recalibration events exist, so the gate was silent precisely while its producer was missing. A check that fails open until its own producer exists cannot report that producer's absence.

TWO SILENT DIVERGENCES CAME FROM ONE 2026-06-30 CHANGE. Besides the missing event, its source comment asserted 'ADR-0.0.33 Decision is amended to match' — an amendment never written, so the ADR read green <= 1800 while the code read 2600 for 42 days. Both are now reconciled in ADR-0.0.33 § Amendment (2026-08-11).

'continues_from' IS NOW 'str | list[str]' AND HAS EXACTLY ONE READER. 'continues_from_refs()' in 'handoff_validation.py' is the single fold; '_carried_settled', 'load_handoff_chain', '_chain_target_keys', '_has_pointer' and '_warn_on_silent_chain_root' all delegate. Never branch on the type at a call site. A single parent still WRITES as a scalar, so the 297 authored documents keep their shape and the list form appears only on a genuine merge. '--continues-from' is repeatable.

THE ARCHIVE GUARD WIDENING WAS NOT OPTIONAL, CONTRARY TO GHI #790's OWN SCOPE NOTE. That note said archival was unharmed, and it was correct WHILE the field was scalar. Widening the read side without widening '_chain_target_keys' would have created the hole rather than found it: a second ancestor invisible to the guard is archivable while live.

THE ADVISORY SCORECARD HAS NO ROW FOR ADR ANTI-PATTERNS, AND NEVER DID. 'docs/governance/advisory-rules-audit.md' is scoped to '.gzkit/rules/**' clauses. ADR-0.0.33 anti-pattern 3 lives in an ADR, so it was outside the instrument that tracks whether declared doctrine is enforced — which is part of why it went unwitnessed for the project's whole life. Widening that scope is a real question carrying its own Coverage Ledger ceremony; it was recorded on GHI #792 rather than done unilaterally.

'OBPI-0.0.33-02' REQ 4 STILL NAMES THE WRONG VERB, DELIBERATELY. The brief is 'Completed' and a terminal brief is a sealed historical record. The ADR amendment carries the correction and names the REQ's error explicitly. Do not 'fix' the brief.

## Decisions Made

- [operator-ruled] Work advised steps 3 and 4 and set steps 1 and 2 aside (verbatim: "Steps 3+4 — clear the defect queue first"), selected from a four-option picker whose alternatives were the campaign spine (steps 1+2), all four steps, and the two cheap judgments (steps 1+4). Booked via 'gz handoff decide'.
- [operator-ruled] GHI #790 takes 'str | list[str]' on the same key (verbatim selection: "str | list[str] on the same key"), selected from a four-option picker. The alternatives declined were a second key alongside the scalar (two fields expressing one relation) and a 'list[str]' migration across 297 authored handoffs (rewriting sealed history).
- [operator-ruled] Raise '_GREEN_CEILING' rather than re-floor, update the covering waiver, or adopt a standing convention (verbatim selection: "Raise _GREEN_CEILING"), selected from a four-option picker after the agent corrected two arms of the anchor's own option set: adding a waiver entry is mechanically refused by the shrink-only ratchet at baseline_count 6, and the largest live waiver covers 340 against a delta of 742.
- [operator-ruled] Override ADR-0.0.33's 6-month recalibration cadence, 42 days after the last change, rather than amending the cadence or falling back to a cadence-respecting arm (verbatim selection: "Override the cadence, raise now"). The agent surfaced the collision rather than folding it silently into a constant edit.
- [operator-ruled] Bands become green 3000 / yellow 3400 (verbatim selection: "3000 green / 3400 yellow"), preserving the 400-line yellow-band width of both prior generations; sized from measured growth of 8.4 lines/day.
- [operator-ruled] File the GHI, build the emitter, and land the band raise witnessed, rather than landing it unwitnessed and emitting later, holding the raise, or falling back to the convention arm (verbatim selection: "File the GHI, build the emitter, land witnessed").
- [operator-ruled] The emitter lands as 'gz validate --surface-weight --recalibrate' (verbatim selection: "gz validate --surface-weight --recalibrate"), so the surface that reads the bands is the surface that records their change. Widening 'gz adr emit-receipt's enum and a dedicated 'gz governance' subcommand were declined.
- [operator-ruled] File and fix the missing band witness (verbatim: "ghi to fix?"), which produced GHI #792. The agent had left it unbuilt as beyond the ruled scope and surfaced it for routing.
- [operator-ruled] Sync the session's work (verbatim: "git sync", given twice).
- [agent-chose] Did NOT author a test asserting the band constants equal 3000/3400. The existing suite derives its boundaries from the constants precisely so it survives recalibration; a value-pinning test is the tautological shape '.gzkit/rules/tests.md' § The discriminator forbids.
- [agent-chose] Wrote the floor snapshot BEFORE appending the event, and pinned the order with a test. The failure modes are asymmetric: a failed append leaves a green gate and a re-runnable command, while the reverse order strands a red gate no operator action short of hand-editing the ledger could clear.
- [agent-chose] Kept 'previous_floor_lines' on the event so the 741-line accretion survives the floor overwrite, which resolves the objection that re-flooring erases the Movement C growth signal.
- [agent-chose] Derived the '_qc_nc_composite.build_surface_weight' fixture's line count from '_YELLOW_CEILING' instead of repairing its 3001 literal. The literal was one past the OLD red threshold, so the recalibration silently demoted its planted violation to yellow and the negative control reported FACADE; deriving it fixes the class rather than the instance.
- [agent-chose] Treated a recalibration event carrying no band fields as SILENT about bands rather than as drift, after the first implementation broke a pre-existing floor-drift fixture. Reporting drift there would fail for a reason the finding does not name. Not a bypass: the ledger schema marks both ceilings required.
- [agent-chose] Split 'run_surface_weight_recalibrate' into 'commands/validate_surface_weight.py' because 'validate_cmd.py' carries a shrink-only grandfather ceiling at 1309 SLOC that the handler would have breached.
- [agent-chose] Left 'OBPI-0.0.33-02' REQ 4 as authored — the brief is terminal and sealed — and put the correction in the ADR amendment instead.
- [agent-chose] Ran a live negative control for GHI #792 (set '_GREEN_CEILING = 3200', observed exit 3, reverted) rather than trusting the unit fixtures, so the gate is proven wired into the real path.

## Immediate Next Steps

1. RULE THE ADR-0.35.0 LIFECYCLE STEP. Carried unworked from three predecessors and now set aside FIVE times, which is a stronger signal than anything in its content. 'uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing' reports lifecycle Pending (run it for the live OBPI count) while the ADR frontmatter says 'status: Draft'. Note the correction the last review established: these are different axes, not a drift — the Lifecycle column is Layer-3 derived from Layer-2 ledger events, while frontmatter is Layer-1 authorship. So this is not a reconciliation, it is a fresh authoring decision about whether 'Draft' is the right status for a feature about to receive OBPI work. Cheap, and it gates step 2.
2. PULL 'OBPI-0.35.0-01-corpus-tombstone-schema-and-fold'. It heads the declared 01 then 02 then 03 chain and carries the fold algebra the ADR pinned in its Decision section as its one irreversible commitment. Read that algebra from the ADR rather than re-deriving it — single reverse pass, never a fixpoint iteration, and unset tombstone fields MUST be omitted from serialization or 'corpus_fingerprint()' re-fingerprints the whole corpus on the landing commit.
3. RULE WHETHER THE ADVISORY SCORECARD SHOULD COVER ADR-DECLARED DOCTRINE. Recorded on GHI #792 [settled] and not acted on. 'docs/governance/advisory-rules-audit.md' scopes to '.gzkit/rules/**' clauses, so ADR-0.0.33 anti-pattern 3 sat outside the instrument that tracks enforcement for the project's entire life and no row ever flagged it. The question is bounded — widen the scope, or state in the audit that ADR anti-patterns are deliberately out of scope and name what does track them. Carries its own Coverage Ledger ceremony either way.
4. WATCH THE NEW HEADROOM RATHER THAN ASSUMING IT. The bands bought roughly 47 days at the measured rate, and the campaign's Movement C is Reduce the accretion, so growing into the headroom is the outcome the raise was NOT for. The measurement is one command: count lines across 'AGENTS.md', 'CLAUDE.md' and '.claude/rules/**/*.md' against '_GREEN_CEILING'.

## Pending Work / Open Loops

ADR-0.36.0 IS DEFERRED, NOT ABANDONED. Three briefs (01 critic-skill-contract, 02 cross-family-transport, 03 operator-door) are authored and passing at 1b808b18d; six remain draft scaffolds — 04 agent-door, 05 decision-envelope, 06 risk-tiering, 07 verdict-resolution-transition, 08 pilot-instrumentation, 09 asked-question-gate-dark. Its Fidelity Assertions table still carries the scaffold's example row, so '--fidelity-presence' passes while the ADR asserts nothing about its own thesis. Its Forcing Functions are unfilled.

THE CROSS-FAMILY CRITIC STILL DOES NOT RUN. '.claude/hooks/' ships no critic script and 'PreToolUse' has no AskUserQuestion matcher. Every structured choice this session presented — seven pickers across two turns, including a Layer-1 doctrine override of ADR-0.0.33's recalibration cadence — went unchallenged. That override is exactly the consequential-decision class the critic exists for.

THE OPEN-COUNT INSTRUMENT IS STILL THE WRONG ONE and remains unbuilt, carried unfiled across several predecessors. Nothing tracks defect rate, found-versus-introduced, or time-to-detection. This session is a live example of why that matters: it closed three GHIs and opened three, two of which it opened itself by looking.

THREE DEFECTS TRACKED AND NOT FIXED, all via 'gz insights remember': no '*.jsonl merge=union' strategy in '.gitattributes'; the unresolved 'Path.cwd()' fallback in the control-surface-sync hook; and 'gz typecheck' corrupting diagnostic coordinates by expanding emoji shortcodes. A fourth from the predecessor, the zero-headroom surface-weight condition under scope 'surface-weight-headroom', is now DISCHARGED by GHI #791 [settled]/#792 [settled].
...
THE INSTRUCTIONS-FILE BUDGET WORK STAYS PARKED by standing ruling until the product stabilizes. Note this is ADJACENT TO BUT DISTINCT FROM the surface-weight gate discharged this session: that budget check is advisory, surface-weight is fail-closed.

## Verification Checklist

Confirm the session's claims against Layer 2 rather than trusting this document:

uv run gz check
uv run gz validate --surface-weight
uv run -m unittest tests.governance.test_surface_weight_recalibrate tests.governance.test_handoff_multi_parent_lineage
uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing
git rev-list --left-right --count origin/main...main
gh issue view 791 --json state
gh issue view 792 --json state

The band witness is reproducible as a live negative control, and running it is the honest way to confirm the gate is wired into the real path rather than only into fixtures: set '_GREEN_CEILING' to any value other than 3000 in 'src/gzkit/governance/trust_audits/surface_weight.py', run 'uv run gz validate --surface-weight', observe exit 3 naming both the code value and the witnessed value, then revert. It should NOT be left edited.

Read the ADR's landed count off 'gz adr status' rather than from any prose copy here; this document deliberately carries none, because a transcribed figure has no reconciliation path.

Do NOT pipe any of these through 'tail' or 'grep' — the verifier-pipe-gate hook refuses it, and the shell would report the filter's exit rather than the verifier's.

## Evidence / Artifacts

Commits landed and synced this session:

- '3d8e9e99b' — GHI #791: the recalibration producer, plus the ruled band raise witnessed by the first 'surface_weight_recalibrated' event ever emitted
- '3c768bb73' — GHI #790: multi-parent lineage via 'continues_from_refs'
- '1d376c61f' — GHI #792: band-coherence fail-close

Authored or changed:

- 'src/gzkit/governance/trust_audits/surface_weight.py' — 'recalibrate_surface_weight', 'RecalibrationOutcome', '_newest_recalibration', '_check_band_coherence', bands 3000/3400
- 'src/gzkit/commands/validate_surface_weight.py' — new; the CLI handler, split for the module-size ratchet
- 'src/gzkit/handoff_validation.py' — 'continues_from_refs', the single fold; 'continues_from: str | list[str] | None'
- 'src/gzkit/ledger_events.py', 'src/gzkit/events.py', 'src/gzkit/schemas/ledger.json' — the event factory, typed model, and schema entry
- 'src/gzkit/governance/trust_audits/events.py' — the NO_GRAPH_IMPACT waiver
- 'src/gzkit/governance/trust_audits/_qc_nc_composite.py' — fixture derived from '_YELLOW_CEILING' instead of a 3001 literal
- 'docs/design/adr/foundation/ADR-0.0.33-agent-control-surface-fidelity/ADR-0.0.33-agent-control-surface-fidelity.md' — § Amendment (2026-08-11), Invariant 2 bands, anti-pattern 3 validator citation
- 'docs/user/manpages/validate.md', 'docs/user/manpages/handoff-create.md', 'docs/governance/GovZero/session-handoff-schema.md', 'docs/governance/GovZero/handoff-validation.md' — operator-facing corrections
- 'tests/governance/test_surface_weight_recalibrate.py', 'tests/governance/test_handoff_multi_parent_lineage.py' — new covering tests
- 'data/surface_weight_floor.json' — re-snapshotted 1859 to 2600

Read to ground the rulings:

- 'data/waiver_ratchet_registry.json' — 'shrink-ratchet', 'baseline_count: 6'; the entry that made the anchor's waiver arm unavailable
- 'src/gzkit/governance/trust_audits/_qc_negative_controls.py' — the FACADE discrimination that caught the band change degrading its own control
