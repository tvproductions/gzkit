...
1. RULE THE ADR-0.35.0 LIFECYCLE STEP. Carried unworked from the predecessor and set aside twice now, which is itself the signal. 'gz adr status ADR-0.35.0-canon-entry-corpus-landing' reports lifecycle Pending with 0/10 OBPIs while the ADR frontmatter says Draft and ADR-0.36.0 says Proposed. Confirm what the in-flight feature's status must be before OBPI work begins, and move it if so. Cheap, and it gates step 2.
