...
branch: main
timestamp: '2026-08-11T12:51:21Z'
agent: claude-code
continues_from: .gzkit/handoffs/20260811T113901Z-campaign-amended-035-in-flight-review-repaired.md
---

## Current State Summary

The three rulings the predecessor listed as owed are discharged. Advised steps 3 (abridged twins), 4 (repo-wide [kind] tag case) and 5 (multi-parent lineage) were worked; steps 1 (the ADR-0.35.0 lifecycle question) and 2 (pull OBPI-0.35.0-01) were recorded set-aside via 'gz handoff decide' and remain unworked. Three commits landed: 662496e04 (twin witness plus head heal), c8d874a6b (tag-case ruling), cd8f3b2d4 (the git-sync chore carrying ledger and insights). 'uv run gz check' exits 0; tree clean; 'origin/main...main' is 0 0 at cd8f3b2d4. The predecessor was healed IN PLACE this session rather than superseded on the twin question, so it is both this document's parent and one of its artifacts.

## Important Context

THE TWIN REPAIR NEEDED NO DEDUP-POLICY CHANGE, which reverses the predecessor's framing of advised step 3. It described the repair as 'a dedup-policy change on _dedup_rulings'. It is not: '_carried_settled' reads ONLY the immediate predecessor, so a bad entry rides forward one hop at a time and never re-enters from deep history. Healing the head therefore stops propagation permanently with no code change to the shared composition path. Root cause is a single upstream document — '.gzkit/handoffs/20260809T174258Z-ghi-priority-785-leads.md' hand-wrote a shortened copy of a ruling into its own Decisions Made (line 49) while the full form already sat in its Settled Rulings (line 301); the composer then unions carried with booked and '_ruling_key' correctly declines to fold them.

THE TWIN COUNT WAS 6, NOT 3, AND THAT MATTERED TO THE WITNESS DESIGN. The predecessor said three; the option preview said five; a strict-prefix probe finds three. The other three diverge on ONE punctuation mark ('discharged.' against 'discharged:', 'as written.' against 'as written,'), so a strict-prefix witness would have under-reported its own repair — the false-Mechanical shape this repo has named before. '_twin_probe' strips one trailing sentence terminator, and its docstring justifies the looser predicate by CONSEQUENCE DIRECTION rather than by taste: '_dedup_rulings' collapses, so a false match there destroys operator words, while the witness only reports to a human reading both forms.

THE TAG-CASE RULING FORBIDS THE MIGRATION IT MIGHT LOOK LIKE IT AUTHORIZES. Case is declared meaningless (both readers are 're.IGNORECASE'), UPPERCASE is named as the authored form, and the 370 lowercase tags across 55 briefs are declared CORRECT and explicitly out of scope for rewriting. A future session reading only the UPPERCASE half and starting a sweep would be violating the ruling, not executing it. Scorecard row 59a records the promotion path as FORECLOSED rather than deferred: the only check anyone could build would flag 370 tags the ruling declares compliant.

THE RULES SURFACE HAS ZERO HEADROOM AND THIS WILL BITE THE NEXT RULE EDIT. Measured 2026-08-11: 'AGENTS.md' plus 'CLAUDE.md' plus '.claude/rules/**/*.md' totals exactly 2600 lines against '_GREEN_CEILING = 2600' in 'src/gzkit/governance/trust_audits/surface_weight.py'. An 18-line clause added to a rule file tipped 'gz check' into the yellow band (2619, delta +760 from floor 1859). The resolution taken was structural rather than a waiver — binding text folded into an existing line, rationale routed to 'docs/governance/' which is outside the corpus. This is ADJACENT TO BUT DISTINCT FROM the parked instructions-file budget work: that check is advisory, this one is fail-closed.

GHI #790 IS OPEN-WITH-BLOCKER BY DESIGN, NOT NEGLECT. The operator ruled file-not-fix, declining the direct-fix arm, so there is no same-session destination. The blocker comment names the three widening shapes and states the decision is a shape ruling, not an investigation — the surface is already read and enumerated in the issue body.

## Decisions Made

- [operator-ruled] Work the three owed rulings, setting the two campaign-sequencing steps aside (verbatim: 'address these from the handoff: "Three rulings still owed — abridged twins, repo-wide tag case, and whether to file the multi-parent lineage gap"'). Booked via 'gz handoff decide'; advised steps 1 and 2 recorded set-aside.
- [operator-ruled] Heal the head and flip the test carve-out into a witness, rather than prefix-collapsing in '_dedup_rulings' (selected from a four-option picker with rendered previews). The alternatives declined were prefix-collapse keeping the longer text, heal-only with no witness, and leaving the duplicates standing. The collapse arm was declined on the ground that it would break '_ruling_key''s deliberate refusal to fold look-alikes.
- [operator-ruled] Tag case is binding-insensitive with UPPERCASE as the authored form, and existing lowercase tags are NOT to be rewritten (selected from a four-option picker). The alternatives declined were declaring lowercase canonical, rewriting all 370 lowercase tags, and declining to rule.
- [operator-ruled] File the multi-parent lineage gap as a GHI rather than direct-fixing it or dropping it (selected from a three-option picker). Produced GHI #790.
- [operator-ruled] Sync the session's work (verbatim: 'git sync').
- [agent-chose] Split the sync into two hand-authored commits before running 'gz git-sync --apply', because the 'Refuse the bundle (fence at sync)' ruling fails the sync closed on staged source scope and a bundled commit would have attributed the tests/ change to 'TASK-gz-git-sync'.
- [agent-chose] Amended 662496e04 before pushing: the first message cited the root-cause handoff as '20260809T174258Z' incorrectly transcribed, which would have sent the next reader to a file that does not exist.
- [agent-chose] Widened the twin probe to tolerate one trailing sentence terminator rather than matching '_ruling_key''s exact narrowness, on the stated ground that reporting and collapsing have opposite failure costs.
- [agent-chose] Corrected nine lowercase example lines across three documenting surfaces rather than only the two the ruling's preview named, because leaving four lowercase examples inside the document being declared UPPERCASE-authored would have re-created the self-contradiction the ruling exists to close.
- [agent-chose] Resolved the surface-weight breach by relocating rationale to 'docs/governance/' rather than adding a waiver entry, and recorded the zero-headroom condition as a 'discovery' insight rather than a GHI, because the operator's standing park covers budget work.

## Immediate Next Steps

1. RULE THE ADR-0.35.0 LIFECYCLE STEP. Carried unworked from the predecessor and set aside twice now, which is itself the signal. 'uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing' reports lifecycle Pending (run it for the live OBPI count) while the ADR frontmatter says Draft and ADR-0.36.0 says Proposed. Confirm what the in-flight feature's status must be before OBPI work begins, and move it if so. Cheap, and it gates step 2.
...
3. RULE GHI #790's WIDENING SHAPE. The issue is open-with-blocker and the blocker comment enumerates three arms: a 'str | list[str]' union on the same key, a second key alongside the scalar, or a 'list[str]' migration across the 297 authored handoffs. The surface is already read and enumerated in the issue body, so this is a shape ruling and not an investigation.
4. RULE THE SURFACE-WEIGHT HEADROOM, or decline to. The rules corpus sits at exactly 2600 of 2600 lines, so the next rule edit that adds a line fails 'gz check' closed. Options are a waiver entry in 'data/surface_weight_waivers.json', a recalibration of '_GREEN_CEILING', or a standing convention that rule edits land binding text inside existing lines. Declining is legitimate; leaving it unruled means the next rule author rediscovers it at gate time.
...
THE CROSS-FAMILY CRITIC STILL DOES NOT RUN. '.claude/hooks/' ships no critic script and 'PreToolUse' has no AskUserQuestion matcher, so every structured choice this session presented — four pickers — went unchallenged.
...
THE INSTRUCTIONS-FILE BUDGET WORK STAYS PARKED by standing ruling until the product stabilizes; the two 'surface-delivery-witness' warnings on 'gz check' are that parked condition, not new drift. AGENTS.md now renders 385 B OVER the codex delivery cap, with the must-survive 'architectural-boundaries' section straddling it.
...
gh issue view 790 --json state,title

The twin and clip arms are independent and mechanical — both must pass, and the twin arm is new this session. Read the ADR's landed count off 'gz adr status' rather than from any prose copy here; this document deliberately carries none, because a transcribed figure has no reconciliation path.
...
The surface-weight condition is reproducible: count lines across 'AGENTS.md', 'CLAUDE.md' and '.claude/rules/**/*.md' and compare against '_GREEN_CEILING' in 'src/gzkit/governance/trust_audits/surface_weight.py'. It should read exactly at the ceiling, not under it.

## Evidence / Artifacts

Landed this session:

- 'tests/governance/test_settled_ruling_integrity.py' — the abridged-twin witness ('TestNoAbridgedTwinRidesAlongsideItsFullerForm', '_twin_probe') and the clip arm's carve-out rerouted from silence to that class
- '.gzkit/handoffs/20260811T113901Z-campaign-amended-035-in-flight-review-repaired.md' — this document's parent, healed in place; six abridged twins removed, 229 rulings down to 223
- '.gzkit/rules/tests.md' — rule 0.16.0 to 0.17.0, the tag-case clause folded into an existing line
- 'docs/governance/req-scope-discipline.md' — the self-contradiction repaired, new Tag case section carrying the rationale and the measured split
- 'docs/governance/advisory-rules-audit.md' — scorecard row 59a, ledger row to 0.17.0, Summary roll-up recounted 41 to 42 Judgment
- '.gzkit/insights/agent-insights.jsonl' — the 'surface-weight-headroom' discovery record

Read to ground the rulings:

- 'src/gzkit/handoff_api.py' — '_ruling_key', '_dedup_rulings', '_carried_settled'; the immediate-predecessor read is what made the heal durable
- 'src/gzkit/handoff_validation.py' — 'HandoffFrontmatter.continues_from', the GHI #790 surface
- 'src/gzkit/governance/trust_audits/surface_weight.py' — the 2600 green ceiling
- '.gzkit/handoffs/20260809T174258Z-ghi-priority-785-leads.md' — the upstream document that authored the six twins
