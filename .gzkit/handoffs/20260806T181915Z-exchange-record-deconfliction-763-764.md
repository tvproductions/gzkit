...
adr_id: ADR-0.0.41
branch: main
timestamp: '2026-08-06T18:19:15Z'
agent: claude-code
session_id: 31d94676-2b78-48d2-8180-851ce84f62a4
continues_from: .gzkit/handoffs/20260806T122310Z-term-deconfliction-transit-exchange-handoff.md
---

## Current State Summary

GHI #763 and #764 were both worked and closed `fixed`, discharging the two sibling cuts the predecessor handoff filed. The OBPI token block's register entry is now an EXCHANGE RECORD in name, location, and content: `gzkit.exchange_records` owns it, `.gzkit/locks/exchange/` stores it, and it carries the observation report half of what operator canon says it is. Three commits sit on main, unpushed at authoring time: fe69a835c (#763), 4b35a4a44 (the stray exit bookmark), 286f255b6 (#764). `uv run gz check` exits 0; 8016 unit tests OK. The predecessor's advised step 1 (RED renditions) was already discharged before this session opened, by an attested recompose at 12:34Z on 2026-08-06 that no document recorded.

## Important Context

**The gitignore trap, and why it is a doctrine split rather than a config tweak.** The operator-approved location `.gzkit/locks/exchange/` sat inside `.gzkit/locks/`, which .gitignore line 57 excluded. Every exchange record would therefore have failed GHI #759's own index-durability arm (`_git_index_paths` in the coupling validator), so the relocation would have broken the validator it exists to serve on the first release after landing. The fix excludes the CONTENTS of the locks tree and re-includes the exchange subtree; that form is load-bearing because git does not descend into an excluded DIRECTORY, so a negation under a directory-form exclusion would have been silently inert. The split is the right doctrine independently: a lock is ephemeral runtime state, an exchange record is audit evidence.

**The class fix is the predicate inversion, not the rename.** `find_exchange_for_release` admitted by default and subtracted known-bad shapes one reactive commit at a time — abandoned, then CHECKPOINT under GHI #756 — so each exclusion could only be written AFTER the harm it prevents. `is_exchange_register_entry` is now default-DENY: it admits only mode CREATE and not-abandoned, the shape the writers emit, so RESUME and any mode invented later are refused without being enumerated. This is a behavior change beyond a rename and is why a hand-authored RESUME-mode document no longer pairs a release.

**Two tests had become tautologies and one encoded the defect.** After the directories split, `test_the_bookmark_cannot_surrender_a_token` would have passed on the directory mismatch alone — it would not have failed with the shape check deleted. `TestFullSlugReleasePairing` was worse: it wrote a SESSION handoff and asserted the TOKEN finder returned it, which is this GHI's conflation as a passing test. Both re-pointed, with `test_a_session_handoff_never_pairs_a_token_release` added as the fence.

**The observation report already existed; it had no channel.** #764 looked like an authoring gap and was a plumbing gap: the brief's Value Narrative and Tracked Defects sections are where the completing agent already writes what the traversal learned and what it leaves behind. Measured across all 533 briefs, 252 carry real Value Narrative content and 383 carry real Tracked Defects. That reframing is what let GHI #619's input-free floor survive — no new operator input is required on the normal path. 116 of the 368 briefs carrying a Value Narrative write it INSIDE the scaffold comment that the sanitizer strips by design, so the fallback is a measured case, not defensive padding.

**Registries are what made a 47-file rename safe.** The check-scope membership data file, the QC step classification map, and the advisory scorecard's version ledger each failed closed the moment a scope name changed. None was consulted deliberately; all three fired.

## Decisions Made

- [operator-ruled] Work GHI #763, then #764 (verbatim: "Work #763, then #764", selected via AskUserQuestion and booked through `gz handoff decide` — this is the ruling that lifted the resume gate and scoped the session). Advised steps 4 (re-run GHI triage) and 5 (return to ADR-0.35.0) were recorded set-aside in the same booking.
- [operator-ruled] Author a fresh handoff and then sync (verbatim: "git sync that work, do fresh handoff first") — this document and the `gz git-sync` that follows it, in that order.
- [agent-chose] Un-ignore the exchange subtree rather than relocating the store somewhere already tracked. The operator approved the path, and the alternative silently contradicts GHI #759's index-durability arm. Flagged to the operator as a change to what gets committed going forward.
- [agent-chose] Invert the finder's predicate to default-DENY rather than adding RESUME to the existing blocklist. Adding an entry would have repeated the pattern that produced the defect; only the inversion refuses the shape nobody has thought of yet.
- [agent-chose] Do NOT move the 90 historical exchange records out of the handoffs store. The ledger is append-only and the validator resolves each event's recorded path, so moving them would break 90 references to fix a naming concern. New records relocate; historical ones stay where their events cite them.
- [agent-chose] Fold the new default-deny scorecard row into existing row 73 rather than minting a new row for it. A new invariant-tier Mechanical bullet trips the surface-fidelity bullet-retention gate, which would have forced a corpus append plus an attested rendition recompose for what is an enforcement detail of a clause already scored.
- [agent-chose] Source #764's inlets from the brief rather than adding CLI flags. Requiring input would have re-created exactly the friction GHI #619 removed when it made surrender mechanical.
- [agent-chose] Leave the Immediate Next Steps and Evidence sections with inlets but no automatic brief source, and say so in the close comment. The former's boilerplate is genuinely derivable and correct rather than vacuous; inventing a source to fill a section already saying something true would be motion, not correction.
- [agent-chose] Commit the stray session-exit bookmark separately (4b35a4a44) rather than bundling it into the #763 fix, keeping both commits atomic.

## Immediate Next Steps

1. Run `uv run gz git-sync --apply` to push the three commits. This is the operator's standing ruling for this session and the only authorized remaining step.
2. Re-run GHI triage before pulling anything else. The triage rank cache was last written 2026-08-05T17:20 and its top-ranked entry #732 is CLOSED, so any pull order derived from it is untrustworthy. Set aside by the operator's ruling this session.
3. Return to the campaign's topmost item, ADR-0.35.0-canon-entry-corpus-landing, Pending at 0 of 10 OBPIs and untouched across seven handoffs. Set aside by the operator's ruling this session.
4. Consider whether the airlock's Build door is now cheaper to wire. #763 gave the token system its own module and store, so `gzkit.exchange_records` is a single seam an airlock-out could hang from — but this is an observation, not a plan, and no GHI owns it.

## Pending Work / Open Loops

- The three commits are LOCAL at authoring time; the revision count against origin/main is 3. The sync in step 1 clears this.
- The airlock's Build door remains unwired: zero airlock references in the completion, lock-command, and lock-manager modules; the ledger carries 23 airlock-in against 5 airlock-out, so 18 transits entered and were never accounted for. That asymmetry is unowned by any open GHI. Carried unruled from the predecessor.
- Three handoff-system event types (handoff_resume_authorized, handoff_resume_decided, session_exit_bookmark_skipped) cite handoff_path with no durability check. All are inside git's index today, so the exposure is theoretical. Recorded on #759's close comment, deliberately not filed. Carried unruled.
- ExitBookmarkResult.skipped still has no operator-visible surface. Carried unruled across three handoffs now.
...
- The 90 historical exchange records remain in the handoffs store, mixed with session handoffs. This is deliberate and correct — their ledger events cite those paths — but it means the corpus is only cleanly typed by location going FORWARD. No action is owed; recorded so a later reader does not mistake it for incomplete work.

## Verification Checklist

- `uv run gz check` exits 0 with an All-checks-passed line at line 115 of the captured log. Every exit code this session was read from an explicit echo after redirecting to a file, never from a pipe — the verifier-pipe-gate hook refused one such pipe mid-session and was correct to.
- `uv run -m unittest -q` reports Ran 8016 tests / OK (9 added this session).
- Receipts: arb-step-unittest-0f7781a264124a34a3f593fe74f36402, arb-ruff-789568b5dc7b49c4bfd3d62ac1c7e241, arb-step-typecheck-06b88df61bc04687bcb5e8b960fdb039.
- `gh issue view 763 --json state` and `gh issue view 764 --json state` both return CLOSED.
- git check-ignore exits 1 for a path under the exchange store (not ignored) while reporting the locks-contents rule for a lock file. Both arms of the gitignore split verified directly.
- `git rev-list --count origin/main..HEAD` returns 3 before the sync and must return 0 after.

## Evidence / Artifacts

- `src/gzkit/exchange_records.py` — the token-owned module; is_exchange_register_entry is the default-deny predicate.
- `src/gzkit/governance/trust_audits/lock_exchange_coupling.py` — the renamed ledger-replay validator.
- `src/gzkit/handoff_validation.py` — now the session system only; its docstring carries the scope boundary.
- `src/gzkit/commands/obpi_complete.py` — _read_observation and _read_open_loops are the #764 brief-sourcing readers.
- `src/gzkit/quality.py` — run_handoff_document_audit now scans both register-entry stores.
- `.gitignore` — the locks-contents exclusion plus the exchange-subtree re-inclusion.
- `.gzkit/locks/exchange/README.md` — the store's own explanation of why it is tracked while its siblings are not.
- `.gzkit/rules/token-block-discipline.md` — version 0.6.0; Sub-Invariant 7 is new.
- `docs/governance/token-block-doctrine.md` — line 189 no longer defines the correct term using the wrong one.
- `docs/governance/advisory-rules-audit.md` — rows 73 and 74; coverage ledger bumped.
- `tests/governance/test_obpi_complete_lock_release.py` — TestObservationReport and TestObservationSourcedFromBrief.
- `.gzkit/handoffs/20260806T122310Z-term-deconfliction-transit-exchange-handoff.md` — the predecessor this supersedes.
...
- Rule GHI #759 and stop there (verbatim: "Rule #759 only, then stop", selected via AskUserQuestion and booked through `gz handoff decide`). Advised steps 2, 4, and 5 were recorded `--set-aside` in the same booking; step 3 was already discharged by observation at session start.
- `obpi_lock_released` is not part of the handoff system (verbatim: "this has nothing to do with the handoff system"). This corrected an agent sweep that had grouped four event types on the shared `handoff_path` key; an `improvement` insight was recorded via `gz insights remember` before the corrected work proceeded, per Behavior Rule 11.
- The two systems collide on one English word with two meanings — the token is exchanged in the railway sense, while the session handoff is a shift-change briefing. Simple grepping caused the misdirection.
- Separate the terms: `handoff` is NOT critical to the token block system and `exchange` can be substituted; `handoff` IS critical to the session system. These are GHI-level repairs deconflicting terms, and `.gzkit/locks/exchange` is an acceptable new location.
- Retain BOTH the document and the event record — the case exists for both, and what exists is an incomplete implementation rather than a redundant one.
- Do not dismiss the unused sections; fix why they are overlooked. Important Context, Immediate Next Steps, and Evidence/Artifacts each hold real value, and an incomplete implementation is not evidence the sections are worthless.
- All three terms are correct and distinct (verbatim: "transit (how we enter and leave the designed ecosysten); exchange (noting block vacation and an observation report of what happened); handoff (sythetic memory refresh, from agent session to agent session, for context management). Three vital features, that, as it turns out, are vital for campaign success."). The OBPI token system IS how features are implemented and is the sole mechanism for it — the airlock's Build door.
- Capture the canon then file the GHIs (verbatim: "do all of the above").
- Author a successor handoff and sync it (verbatim: "create fresh handoff and git sync") — this document and the `gz git-sync` that follows.
- Work #763, then #764 — booked via gz handoff decide against the 20260806T122310Z handoff; this is the ruling that lifted the resume gate and scoped the session to those two issues in that order.
