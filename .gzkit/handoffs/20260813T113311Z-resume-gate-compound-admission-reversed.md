...
adr_id: null
branch: main
timestamp: '2026-08-13T11:33:11Z'
agent: claude-code
session_id: b060003b-a8c0-4247-b345-d7a7f06cd3fe
continues_from: .gzkit/handoffs/20260813T104754Z-gate-mapping-fixed-recovery-prose-corrected-and-rtd-freeze-found.md
---

## Current State Summary

Resumed the 2026-08-13 anchor, reviewed it, and booked a proceed ruling with all five advised steps set aside — the operator directed different work mid-review. That work was a single defect: the handoff-resume gate refused a compound Bash command on SHAPE before reading any verb, so a chain of three admitted reads was blocked. Landed at 927273927 across three commits; origin/main is 0/0 and the tree is clean. GHI #798 was NOT touched this session; the anchor's five advised steps remain unworked and are carried forward below unchanged. uv run gz check exits 0 across 55 steps.

## Important Context

THE GATE ALREADY HELD THE INFORMATION TO ADMIT AND DECLINED TO USE IT. The refusal prose listed the refused segments back to the caller as 'permitted RIGHT NOW', because _shape_refusal_note called _admissible_segments to build its error text. The decider ignored that same set. This is why the friction read as arbitrary rather than as a rule: the block message was, in effect, a correct admission decision printed as an apology. The fix wired the existing computation into the verdict rather than adding a new predicate.

THE SUPERSEDED RATIONALE WAS DISPROVED BY THE ISSUE IT CITED. The _is_compound docstring recorded the 2026-08-12 disposition-1 ruling and asserted that narrowing to admit-when-all-segments-admitted 'would reopen both holes' named by its two lexer facts. GHI #798-era reasoning aside, that claim is false on GHI #800's own body, which states in § Candidate dispositions item 2 that substitution tokens and redirections would still refuse. The mechanism: admission re-enters _segment_is_admitted, which calls _bash_is_read_only, which calls _is_compound — so _can_expand fires on a backtick or dollar-paren INSIDE each segment. A redirect refuses on a different fact entirely: its target is a segment matching no allowlisted head. Neither hole is reached by the operator token.

DISPOSITION 2 AS WRITTEN COULD NOT HAVE FIXED ITS OWN EXAMPLE. GHI #800's canonical shape is git status --short && echo dashes && uv run gz git-sync. The echo segment was on no allowlist, so the all-segments-admitted narrowing alone would still have refused the very command the issue was filed about. echo was admitted alongside it: it has no write form, and directing its output at a file takes a redirect whose target is then its own unadmitted segment. Any future narrowing must be tested against the issue's own example, not only against the abstract rule.

THE ANCHOR'S VERIFICATION CHECKLIST CARRIES A BROKEN LINE, FOUND DURING THE REVIEW. It prescribes grep -c undeclared_accepted data/exemption_control_grandfather.json with the expectation 'still 71'. The command returns 1: the string occurs once, as a measurement key whose value is 71. The claim is true and the check does not test it. The working form is grep -c for the quoted claim key, which returns 71. This is untracked and was surfaced to the operator rather than fixed, because the ruled scope was the gate.

REDIRECTION AND 2>&1 REMAIN REFUSED, DELIBERATELY. A redirect's target is a segment and matches no allowlisted head, which is also the mechanism by which 2>&1 stays refused: its trailing 1 is an unadmitted part. Admitting either would require modeling redirect targets, which is a different question from chaining and has not been ruled. Do not read the chaining ruling as covering it.

## Decisions Made

- [operator-ruled] Fix the compound-command refusal immediately, at highest priority (verbatim: 'I want this fixed now, highest priority, I can't tolerate this:' followed by the block message quoted in full, then 'do not let this problem persist'). This REVERSES the operator's own 2026-08-12 disposition-1 ruling on the same question.
- [operator-ruled] Book the resumed anchor as proceed with every advised step set aside, because the session's work was the gate fix rather than any advised step. Booked via gz handoff decide with all five recorded --set-aside.
- [operator-ruled] Route the fix to a successor handoff (verbatim: 'did you route this fix to a new handoff?'), which the agent had not done — the ruling was booked in the ledger and the commits landed, but no successor document existed, so the anchor still stood as newest with its five steps reading as live.
- [agent-chose] Name the conflict with the prior ruling in one sentence and proceed rather than blocking on it. Canon settles the precedence (MY WORD IS AUTHORITY IN ALL CASES); a newer operator directive supersedes an older operator ruling, and stopping to re-ask would have spent the operator's typing budget on a question canon already answers.
- [agent-chose] Single-source the segment split into _segments and _segment_is_admitted so the decider and the error prose cannot disagree about what a part is. A second copy of the membership test is precisely how the gate came to refuse what its own prose promised.
- [agent-chose] Admit echo rather than leave the operator's exact command refused by a second cause. Verified against the stated membership predicate rather than added for convenience: echo has no write form at all.
- [agent-chose] Rewrite the refusal prose to name the REFUSED part first. Since a wholly-admitted chain now runs, anything reaching the note is a genuine mix, and the offending part is the one fact the caller cannot derive — it can see its command but not the allowlist that judged it.
- [agent-chose] Correct the _is_compound docstring in place rather than delete the superseded ruling. A reversed ruling that vanishes is indistinguishable from one that was never made, and this question had already been re-litigated five times across three sessions for exactly that reason.
- [agent-chose] Update the gz-session-handoff SKILL.md Trust Model in the same push as the predicate change, as a coupled surface under DO IT RIGHT 1a. It documented compound commands as failing closed and named the old example.
- [agent-chose] Split the landing into three commits — source, skill surface, governance artifacts — under the settled 'Refuse the bundle (fence at sync)' ruling.
- [agent-chose] Reopen GHI #800 to record the reversal, then close it fixed, rather than closing it silently or filing a new issue. The reversal belongs on the issue whose disposition it overturns.

## Immediate Next Steps

1. FLIP THE RTD DEFAULT VERSION, THEN CLOSE GHI #801. Carried forward unchanged from the anchor and still the single highest-value operator-only action. Settings, Default version = stable, at the Read the Docs project admin page. Both documentation front doors have served 2026-05-05 content for roughly 100 days. Closing evidence needs no credentials: curl the site root and expect a redirect to /en/stable/ rather than /en/latest/, and curl gzkit.org/config/gz-cookiecutter-python-stack/ and expect 200 where it currently returns 404. NOT re-measured this session — the gate refused curl (its -o flag is a write form), which is correct and is not a defect.

2. PULL OBPI-0.35.0-01-corpus-tombstone-schema-and-fold. The campaign spine and the topmost unchecked Movement A item whose gate is met. Now set aside THREE sessions running, every time by explicit ruling rather than neglect. Verified live this session as unstarted, with every brief pending and draft and the ADR at Draft, which the 2026-08-12 lifecycle ruling settled as correct through implementation; run `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing` for the landed count rather than trusting a figure transcribed here. Read the fold algebra from the ADR's Decision section rather than re-deriving it: single reverse pass, never a fixpoint iteration, and unset tombstone fields MUST be omitted from serialization or corpus_fingerprint re-fingerprints the whole corpus on the landing commit.

3. RULE ON THE ANCHOR'S BROKEN VERIFICATION LINE. Its checklist prescribes grep -c undeclared_accepted against data/exemption_control_grandfather.json expecting 71; the command returns 1 because the string is a measurement key, not a per-entry field. One-line correction to a superseded handoff, or leave it. Surfaced during the review and untracked until ruled.

4. DRAIN THE EXEMPTION CLAIMS NOW THAT THE MAPPING EXISTS. The 71 disclosed undeclared claims in data/exemption_control_grandfather.json are unblocked but undrained; the count was re-measured live this session and is still 71. Each entry now resolves to a named gate through the gz validate --exemption-controls finding itself. Drain by declaring exempts on each claim, either the no-exemption token or the claim id of the control that exercises it, and decrement baseline_count in data/waiver_ratchet_registry.json as entries are surrendered. Do not add entries to silence anything.
...
6. CONSIDER WHETHER GHI #766 SHOULD BE PULLED. Carried forward from the anchor. No new floor bookmarks were produced this session, so the instance count did not grow.

## Pending Work / Open Loops

THE ANCHOR'S FIVE ADVISED STEPS WERE ALL SET ASIDE AND NONE WAS WORKED. They are carried forward above as steps 1, 2, 4, 5 and 6, renumbered only. The set-aside was an operator ruling, not neglect, and the ledger carries all five --set-aside records against this session.

GHI #801 IS OPEN BY CREDENTIAL BOUNDARY, NOT BY OMISSION, and carries a blocker comment naming the next concrete operator action plus the closing evidence commands. The repo-side half is landed and needs nothing further. Not re-probed this session.
...
ADR-0.35.0 IS UNSTARTED, with no OBPI landed, now set aside by explicit ruling in each of the last three sessions. Its status is Draft and that is RULED correct through implementation; the Lifecycle column reading Pending is Layer-3 derived from the absence of completion events and is not drift.
...
REDIRECTION AND 2>&1 REMAIN REFUSED BY THE RESUME GATE and are NOT covered by the chaining ruling. Admitting them would require modeling redirect targets. No GHI was filed: this is a declared boundary of the ruling, recorded in the `_compound_is_wholly_admitted` docstring and in the skill Trust Model, not a defect awaiting repair.

AGENTS.MD IS STILL OVER THE CODEX DELIVERY CAP at 33153 B against 32768 B, with the must-survive architectural-boundaries section straddling the boundary. Tracked on GHI #533; reported as an advisory by every gz check. The standing operator ruling parks all instructions-file budget work until the product stabilizes, so this is disclosed rather than actionable.

## Verification Checklist

uv run gz check                                              # exit 0, 55 steps; three pre-existing advisories only
uv run -m unittest tests.governance.test_handoff_resume_gate # 67 tests OK, including the four new admission witnesses
git rev-list --left-right --count origin/main...HEAD         # 0 0
git log --oneline -3                                         # 927273927, d6e3031c3, f32456fa1
gh issue view 800 --json number,state                        # CLOSED, reopened and re-closed with the reversal recorded
gh issue view 801 --json number,state                        # OPEN, operator-only RTD toggle
uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing   # read the landed count from Layer-2; all briefs pending and draft
grep -c "claim" data/exemption_control_grandfather.json       # 71 accepted_claims entries; NOTE the anchor's grep -c undeclared_accepted returns 1, not 71

## Evidence / Artifacts

Source changed this session:

- `src/gzkit/handoff_resume_gate.py` — `_segments`, `_segment_is_admitted`, `_compound_is_wholly_admitted`, `_refused_segments`, `echo` in the read allowlist, corrected `_is_compound` docstring, rewritten refusal prose
- `tests/governance/test_handoff_resume_gate.py` — admission witnesses plus the paired negative, and both prose tests rewritten to the new contract
- `.gzkit/skills/gz-session-handoff/SKILL.md` — Trust Model compound rule, skill-version 6.27.0
- `.gzkit/insights/agent-insights.jsonl` — improvement insight under scope handoff-resume-gate
- `.gzkit/ledger.jsonl` — the proceed ruling with five set-aside records

Surfaces read but not changed:

- `docs/governance/advisory-rules-audit.md` — row 66 describes the shared shell_reading lexer and is unaffected
- `data/exemption_control_grandfather.json` — the 71-entry disclosed list, re-measured
- `.gzkit/handoffs/20260813T104754Z-gate-mapping-fixed-recovery-prose-corrected-and-rtd-freeze-found.md` — the anchor

Commits, oldest first: f32456fa1, d6e3031c3, 927273927.
