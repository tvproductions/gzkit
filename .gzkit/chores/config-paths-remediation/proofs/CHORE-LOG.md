...
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run -m unittest -q` => rc=0 (102.05s) -- exit 0 == 0
  - [PASS] `uv run gz check-config-paths` => rc=0 (0.68s) -- exit 0 == 0
...
Traceback (most recent call last):
  ...
  File "C:\Users\Jeff\AppData\Roaming\uv\python\cpython-3.13-windows-x86_64-none\Lib\encodings\cp1252.py", line 23, in decode
    return codecs.charmap_decode(input,self.errors,decoding_table)[0]
           ~~~~~~~~~~~~~~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
UnicodeDecodeError: 'charmap' codec can't decode byte 0x9d in position 29: character maps to <undefined>
Skipping unparseable file: C:\Users\Jeff\AppData\Local\Temp\tmp1fprvedr\test_broken.py
Malformed REQ line (skipped): - [ ] REQ-X-Y-Z: Malformed (non-numeric).
Malformed REQ line (skipped): - [ ] REQ-: Empty body.
[1] metric=radon_cc value=12.0 band=block
  Archetype: long_parameter_list
...
[1] metric=radon_cc value=12.0 band=warn
  ...
  Recommended move: Extract Parameter Object
----------------------------------------------------------------------
Ran 4671 tests in 100.877s

OK (skipped=2)
[uv run gz check-config-paths] stdout:
Config-path audit passed.
```
## 2026-06-29T21:37:12-05:00
- Status: PASS
- Chore: config-paths-remediation
- Title: Config Paths Remediation
- Lane: lite
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run -m unittest -q` => rc=0 (82.39s) -- exit 0 == 0
...
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run -m unittest -q` => rc=0 (81.40s) -- exit 0 == 0
...
/Users/jeff/Documents/Code/gzkit/src/gzkit/pipeline_runtime.py:376: DeprecationWarning: Brief 'OBPI-0.0.37-07-test.md' lacks structured frontmatter fields (allowlist, reqs, verification); loading as LegacyBriefShape. Migrate to structured frontmatter per OBPI-0.0.37-04.
  parsed = parse_brief(brief_path)
----------------------------------------------------------------------
Ran 6660 tests in 80.880s

OK
[uv run gz check-config-paths] stdout:
Config-path audit passed.
```
## 2026-07-07T06:00:15-05:00
- Status: PASS
- Chore: config-paths-remediation
- Title: Config Paths Remediation
- Lane: lite
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run -m unittest -q` => rc=0 (85.31s) -- exit 0 == 0
  - [PASS] `uv run gz check-config-paths` => rc=0 (0.41s) -- exit 0 == 0
...
/Users/jeff/Documents/Code/gzkit/src/gzkit/pipeline_runtime.py:376: DeprecationWarning: Brief 'OBPI-0.0.37-07-test.md' lacks structured frontmatter fields (allowlist, reqs, verification); loading as LegacyBriefShape. Migrate to structured frontmatter per OBPI-0.0.37-04.
  parsed = parse_brief(brief_path)
----------------------------------------------------------------------
Ran 6808 tests in 84.659s

OK
[uv run gz check-config-paths] stdout:
Config-path audit passed.
```
## 2026-07-31T18:54:39-05:00
- Status: PASS
- Chore: config-paths-remediation
- Title: Config Paths Remediation
- Lane: lite
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run -m unittest -q` => rc=0 (78.88s) -- exit 0 == 0
  - [PASS] `uv run gz check-config-paths` => rc=0 (0.45s) -- exit 0 == 0
