...
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run gz cli audit` => rc=0 (3.34s) -- exit 0 == 0
  - [FAIL] `uv run -m unittest -q` => rc=1 (111.53s) -- exit 1 != 0
...
Skipping unparseable file: C:\Users\Jeff\AppData\Local\Temp\tmpk_esplua\test_broken.py
...
[1] metric=radon_cc value=12.0 band=warn
  ...
  Recommended move: Extract Parameter Object
======================================================================
FAIL: test_audit_completes_under_two_seconds_on_repo_root (tests.governance.test_audit_chores_layout.PerformanceBudgetTests.test_audit_completes_under_two_seconds_on_repo_root)
----------------------------------------------------------------------
Traceback (most recent call last):
  File "C:\Users\Jeff\source\repos\va\gzkit\tests\governance\test_audit_chores_layout.py", line 154, in test_audit_completes_under_two_seconds_on_repo_root
    self.assertLess(
    ~~~~~~~~~~~~~~~^
        elapsed,
        ^^^^^^^^
    ...
    ^
AssertionError: 2.1863245999993524 not less than 2.0 : audit took 2.186s; budget is <2s; errors=0

----------------------------------------------------------------------
Ran 4671 tests in 110.098s

FAILED (failures=1, skipped=2)
```
## 2026-05-10T13:02:57-05:00
- Status: PASS
- Chore: cli-contract-governance
- Title: CLI Contract Governance (Drift & Evolution)
- Lane: heavy
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run gz cli audit` => rc=0 (2.99s) -- exit 0 == 0
  - [PASS] `uv run -m unittest -q` => rc=0 (111.26s) -- exit 0 == 0
...
FAIL 1 covers-backfill finding(s):
  tests/x.py:1 REQ REQ-0.1.0-01-01 introduced @ aaaaaaa (0c / 0d before receipt
r1); see .claude/rules/tests.md § Invariant 6f for remediation
Bootstrap-mode: C:/Users/Jeff/AppData/Local/Temp/tmpjv2omcvp/.gzkit/rules/complexity-thresholds.md declares a Bootstrap absolutes carve-out section; portability checks against bootstrap rows are skipped per ADR-0.0.28 § Bootstrap absolutes (REQ-11). This is informational, not a policy breach — review tracked GHIs (#404 parser zeros, #405 polarity-aware model) for resolution.
Hook 'complexity-advisor-auto-chain' already present in .pre-commit-config.yaml
Installed hook 'complexity-advisor-auto-chain' in .pre-commit-config.yaml
  Replaced: xenon-complexity -> complexity-advisor-auto-chain
  Skip:     SKIP=complexity-advisor-auto-chain git commit
...
Traceback (most recent call last):
  ...
  File "C:\Users\Jeff\AppData\Roaming\uv\python\cpython-3.13-windows-x86_64-none\Lib\encodings\cp1252.py", line 23, in decode
    return codecs.charmap_decode(input,self.errors,decoding_table)[0]
           ~~~~~~~~~~~~~~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
UnicodeDecodeError: 'charmap' codec can't decode byte 0x9d in position 29: character maps to <undefined>
Skipping unparseable file: C:\Users\Jeff\AppData\Local\Temp\tmp05wkk8e2\test_broken.py
Malformed REQ line (skipped): - [ ] REQ-X-Y-Z: Malformed (non-numeric).
Malformed REQ line (skipped): - [ ] REQ-: Empty body.
[1] metric=radon_cc value=12.0 band=block
  Archetype: long_parameter_list
...
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run gz cli audit` => rc=0 (2.48s) -- exit 0 == 0
  - [PASS] `uv run -m unittest -q` => rc=0 (82.35s) -- exit 0 == 0

```text
[uv run gz cli audit] stdout:
CLI audit passed.
Cross-coverage: 114/114 commands fully covered.
[uv run -m unittest -q] stderr:
[1/1] Test
...
/Users/jeff/Documents/Code/gzkit/src/gzkit/pipeline_runtime.py:376: DeprecationWarning: Brief 'OBPI-0.0.37-07-test.md' lacks structured frontmatter fields (allowlist, reqs, verification); loading as LegacyBriefShape. Migrate to structured frontmatter per OBPI-0.0.37-04.
  parsed = parse_brief(brief_path)
----------------------------------------------------------------------
Ran 6660 tests in 81.827s

OK
```
## 2026-07-07T06:03:10-05:00
- Status: PASS
- Chore: cli-contract-governance
- Title: CLI Contract Governance (Drift & Evolution)
- Lane: heavy
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run gz cli audit` => rc=0 (2.78s) -- exit 0 == 0
  - [PASS] `uv run -m unittest -q` => rc=0 (84.89s) -- exit 0 == 0
...
- Version: 1.0.0
- Criteria Results:
  - [PASS] `uv run gz cli audit` => rc=0 (0.41s) -- exit 0 == 0
  - [PASS] `uv run -m unittest -q` => rc=0 (78.04s) -- exit 0 == 0
