...
- Lite lane requires Gates 1–2. Heavy lane requires all five and is for changes to a CLI, API, schema or runtime contract used by humans or external systems; documentation, process and template changes stay Lite.
- Gate 5 applies to every OBPI completion in every lane, kind and sensitivity: the operator attests, and the agent records their words with `--attestation-text`.
...
- Every BEHAVIOR REQ needs a passing `@covers` test before `gz obpi complete`; this cannot be waived. SUPPORT and STRUCTURAL-FENCE REQs use their declared proof channels.
