...
- Criteria Results:
  ...
  - [PASS] `uv run gz lint` => rc=0 (1.04s) -- exit 0 == 0
  - [PASS] `uv run gz check-config-paths` => rc=0 (0.67s) -- exit 0 == 0
  - [PASS] `uv run -m unittest -q` => rc=0 (104.31s) -- exit 0 == 0
...
Skipping unparseable file: C:\Users\Jeff\AppData\Local\Temp\tmp658zmkl8\test_broken.py
...
- Criteria Results:
  - [PASS] `grep -rn "Path(__file__).*parents" src/gzkit/eval/` => rc=1 (0.01s) -- output clean of 'parents'
  - [PASS] `grep -rn "Path(__file__).*parents" src/gzkit/hooks/` => rc=1 (0.01s) -- output clean of 'parents'
  - [PASS] `uv run gz lint` => rc=0 (0.96s) -- exit 0 == 0
  - [PASS] `uv run gz check-config-paths` => rc=0 (0.64s) -- exit 0 == 0
  - [PASS] `uv run -m unittest -q` => rc=0 (105.82s) -- exit 0 == 0
  - [PASS] `grep -rn "Path(__file__).*parents\[" src/gzkit/ --include=*.py --exclude=quality.py` => rc=2 (0.01s) -- output clean of 'parents['
...
FAIL 1 covers-backfill finding(s):
  tests/x.py:1 REQ REQ-0.1.0-01-01 introduced @ aaaaaaa (0c / 0d before receipt
r1); see .claude/rules/tests.md § Invariant 6f for remediation
Bootstrap-mode: C:/Users/Jeff/AppData/Local/Temp/tmpj2ubnslj/.gzkit/rules/complexity-thresholds.md declares a Bootstrap absolutes carve-out section; portability checks against bootstrap rows are skipped per ADR-0.0.28 § Bootstrap absolutes (REQ-11). This is informational, not a policy breach — review tracked GHIs (#404 parser zeros, #405 polarity-aware model) for resolution.
Hook 'complexity-advisor-auto-chain' already present in .pre-commit-config.yaml
Installed hook 'complexity-advisor-auto-chain' in .pre-commit-config.yaml
  Replaced: xenon-complexity -> complexity-advisor-auto-chain
  Skip:     SKIP=complexity-advisor-auto-chain git commit
...
Traceback (most recent call last):
  ...
  File "C:\Users\Jeff\AppData\Roaming\uv\python\cpython-3.13-windows-x86_64-none\Lib\encodings\cp1252.py", line 23, in decode
    return codecs.charmap_decode(input,self.errors,decoding_table)[0]
           ~~~~~~~~~~~~~~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
UnicodeDecodeError: 'charmap' codec can't decode byte 0x9d in position 29: character maps to <undefined>
Skipping unparseable file: C:\Users\Jeff\AppData\Local\Temp\tmpuiyk03gu\test_broken.py
Malformed REQ line (skipped): - [ ] REQ-X-Y-Z: Malformed (non-numeric).
Malformed REQ line (skipped): - [ ] REQ-: Empty body.
[1] metric=radon_cc value=12.0 band=block
  Archetype: long_parameter_list
...
[1] metric=radon_cc value=12.0 band=warn
  ...
  Recommended move: Extract Parameter Object
----------------------------------------------------------------------
Ran 4671 tests in 104.635s

OK (skipped=2)
[grep -rn "Path(__file__).*parents\[" src/gzkit/ --include=*.py --exclude=quality.py] stderr:
grep: Invalid regular expression
```
## 2026-06-29T21:57:55-05:00
- Status: PASS
- Chore: hardcoded-root-eradication
- Title: Config-First Enforcement (Anti-Vibe-Code)
- Lane: lite
...
- Criteria Results:
  ...
  - [PASS] `uv run gz check-config-paths` => rc=0 (0.40s) -- exit 0 == 0
  - [PASS] `uv run -m unittest -q` => rc=0 (82.32s) -- exit 0 == 0
  - [PASS] `grep -rn "Path(__file__).*parents\[" src/gzkit/ --include=*.py --exclude=quality.py` => rc=1 (0.03s) -- output clean of 'parents['
...
/Users/jeff/Documents/Code/gzkit/src/gzkit/pipeline_runtime.py:376: DeprecationWarning: Brief 'OBPI-0.0.37-07-test.md' lacks structured frontmatter fields (allowlist, reqs, verification); loading as LegacyBriefShape. Migrate to structured frontmatter per OBPI-0.0.37-04.
  parsed = parse_brief(brief_path)
----------------------------------------------------------------------
Ran 6660 tests in 81.808s

OK
```
## 2026-07-07T06:13:26-05:00
- Status: PASS
- Chore: hardcoded-root-eradication
- Title: Config-First Enforcement (Anti-Vibe-Code)
- Lane: lite
...
- Criteria Results:
  ...
  - [PASS] `uv run gz check-config-paths` => rc=0 (0.41s) -- exit 0 == 0
  - [PASS] `uv run -m unittest -q` => rc=0 (86.26s) -- exit 0 == 0
  - [PASS] `grep -rn "Path(__file__).*parents\[" src/gzkit/ --include=*.py --exclude=quality.py` => rc=1 (0.03s) -- output clean of 'parents['
...
/Users/jeff/Documents/Code/gzkit/src/gzkit/pipeline_runtime.py:376: DeprecationWarning: Brief 'OBPI-0.0.37-07-test.md' lacks structured frontmatter fields (allowlist, reqs, verification); loading as LegacyBriefShape. Migrate to structured frontmatter per OBPI-0.0.37-04.
  parsed = parse_brief(brief_path)
----------------------------------------------------------------------
Ran 6808 tests in 85.626s

OK
```
## 2026-07-31T19:06:36-05:00
- Status: PASS
- Chore: hardcoded-root-eradication
- Title: Config-First Enforcement (Anti-Vibe-Code)
- Lane: lite
...
- Criteria Results:
  ...
  - [PASS] `uv run gz check-config-paths` => rc=0 (0.44s) -- exit 0 == 0
  - [PASS] `uv run -m unittest -q` => rc=0 (78.27s) -- exit 0 == 0
  - [PASS] `grep -rn "Path(__file__).*parents\[" src/gzkit/ --include=*.py --exclude=quality.py` => rc=1 (0.03s) -- output clean of 'parents['
