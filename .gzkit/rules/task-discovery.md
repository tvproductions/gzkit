...
> **Rule version:** `0.5.2` — repoints the unruled witness question from GHI #731 (closed) to its successor GHI #752, which carries the declared residual: two of the four channels produce zero keys repo-wide, so Signature (c) compares 7 of 534 OBPIs. Prior `0.5.1` — commit-trailer channel is producer-stamped (`0.5.0`, GHI #731); prior version history lifted to [Rule Version History](../../docs/governance/rule-version-history.md#task-discoverymd). Binding rules unchanged.
...
> **Two of these four are unpopulated — GHI #752.** `@advances` and frontmatter
> `tasks:` produce **zero keys repo-wide**, so only `ledger` x `commit_trailer` can
> pair and Signature (c) compares 7 of 534 OBPIs. The emptiness is asserted, not
> assumed (`test_unpopulated_channels_are_named_not_assumed`). Read the declaration
> above as the design, not as the realized shape, until #752 settles which it is.
