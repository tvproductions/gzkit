...
Run the `pythonic-design-pattern-detection` chore to surface structural refactor candidates that mechanical metric chores (`pythonic-refactoring`, `complexity-reduction-xenon`, `module-sloc-cap-radon`) cannot catch. The chore's scanner walks `src/` AST-by-AST and flags class shapes whose Pythonic equivalent is cleaner — paired with the `refactoring.guru/design-patterns/python` catalogue as the absorption surface.
...
5. Open the candidates report. For each candidate:
   ...
   - Open the cited `refactoring.guru/design-patterns/<slug>/python/example` URL in a browser tab as the absorption reference (this is the absorption-relationship surface from AGENTS.md § Stdlib-First Doctrine — *learn* from the catalogue, do not *adopt* its hierarchies)
...
6. For reference-mode patterns (Bridge, Flyweight, Factory Method) — open the catalogue's Python example side-by-side with any module ranked B-or-worse by xenon. Add an inline `## Reference-mode candidates` section to the report.
...
- **Empty report (`NO_CANDIDATES_DETECTED`):** AST signals returned zero hits. Switch to reference-mode review against the catalogue; do not assume the codebase is Pythonic everywhere — Bridge, Flyweight, Factory Method are not detected mechanically.
