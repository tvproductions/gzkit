...
Operate the gz adr promote command surface to transition pool (backlog) ADRs into versioned, canonical ADR packages with preserved ledger lineage.
...
5.  **Verify the transition**:
    ...
    *   Confirm the ledger records an `artifact_renamed` event.
