...
def generate_manifest(
    ...
    manifest: dict[str, Any] = {
        ...
        # `has_vendor_declaration` gates every vendor-surface branch on its
...
def has_vendor_declaration(config: GzkitConfig) -> bool:
    """Whether the project opted into vendor-aware sync, read from canon.

    Distinguishes legacy projects (no vendor gating -- sync every surface) from
    projects that declared enablement per OBPI-0.16.0-03.

    Reads the parsed ``.gzkit.json`` config, NOT ``.gzkit/manifest.json``.
    Canon first (GHI #891, the same correction ``_shared_subtree_rules`` carries):
    the manifest is generated BY this module, so gating sync on it let a derived
    view decide what gets derived -- what ``AGENTS.md`` § Architectural
    Boundaries #6 forbids. It also could not work, because nothing emitted a
    ``vendors`` block: the answer was False on every project forever and each
    disabled vendor tree was written regardless of config. Reading canon also
    removes the one-sync lag, since the check no longer waits for a regeneration
    to observe what the operator already declared.

    ``model_fields_set`` rather than the field's value: ``VendorsConfig`` supplies
    a default for every vendor, so a parsed config cannot otherwise distinguish
    "declared" from "defaulted", and every legacy project would read as having
    opted in.

    Takes the config rather than a root because ``audit_skills`` gates mirror
    expectations on the same answer and is handed a config directly. The audit
    validates what this module writes, so a second predicate would let the two
    disagree about whether a vendor tree is expected -- demanding exactly the
    mirror sync declined to produce (GHI #921).
    """
    return "vendors" in config.model_fields_set
...
def sync_all(
    ...
    *,
    ...
    # Legacy projects declare no vendors and sync every surface.
