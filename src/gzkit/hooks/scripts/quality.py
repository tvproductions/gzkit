...
def _verifier_pipe_gate_script() -> str:
    """Return the PreToolUse verification exit-code integrity hook script (GHI #589).

    Refuses a Bash command that pipes a verifier into another process, because the
    shell then reports the last stage's exit status rather than the verifier's.
    Thin adapter: the DECISION is :func:`gzkit.verifier_pipe_gate.decide`, so the
    gate stays unit-testable and ``@enforces``-able (generated text is neither).
    ...
    hole it plugs. There is no evidence read here to fail closed on — the verdict
    is a pure function of the command string.
    """
    return dedent(
        """\
            #!/usr/bin/env python3
...
def _stop_turn_feedback_script() -> str:
    """Return the blocking stop-turn-feedback Stop hook script (ADR-0.0.70, GHI #620)."""
    return dedent(
        """\
            #!/usr/bin/env python3
