"""MX-mode hook script sources (ADR-0.0.74).

The generated-script home for the Maintenance Hangar (MX) vendor hook
adapters. Keeping these out of ``validation.py`` holds that module under the
600-line ceiling and keeps the MX surface a single discoverable concern.
"""
...
def _mx_awareness_script() -> str:
    """Return the UserPromptSubmit MX awareness hook script (OBPI-0.0.74-07).

    Injects the MX banner to stdout every agent turn while the marker is
    present; strict no-op when absent; fail-open (always exits 0). The marker
    read falls back to a stdlib-only path so the banner fires even when gz
    itself is the patient (the MX premise).
    """
    return dedent(
        """\
            #!/usr/bin/env python3
            ...
            def _banner_stdlib(cwd: str | None) -> str:
                ...
                _BANNER = (
                    "MX MODE ACTIVE — most guards advisory; "
