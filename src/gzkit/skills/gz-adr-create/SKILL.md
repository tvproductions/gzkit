...
- The ADR status index `docs/governance/GovZero/adr-status.md` — a Layer-3 derived view regenerated by `gz register-adrs`, never hand-edited (see `.claude/rules/governance-core.md` § ADR status index regeneration).
...
7. **Register the ADR in the ledger (Mandatory):** Run `uv run gz register-adrs ADR-X.Y.Z` immediately after the ADR file exists on disk. Emits the `adr_registered` ledger event so `gz adr report` and downstream gates recognize the ADR, AND regenerates the Layer-3 status index `docs/governance/GovZero/adr-status.md` from on-disk frontmatter. **Never hand-edit that index** — it is a derived view (`.claude/rules/governance-core.md` § ADR status index regeneration). Required for every kind — foundation, feature, and pool — regardless of whether OBPIs are co-created in the same session. Skipping this step is the canonical failure that produces "ADR exists on disk but is not registered in ledger" warnings.
