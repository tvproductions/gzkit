...
description: "ARCHIVED: Consolidated into gz-adr-audit. Use /gz-adr-audit for ADR evidence verification."
...
- `uv run gz adr audit-check <adr-id>` (evidence gate)
- `uv run gz adr status <adr-id> --json` (lifecycle summary)
- `rg -n '@covers("ADR-' tests` (coverage discovery)
