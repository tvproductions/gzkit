...
surface. The file accumulates two trust-bearing record kinds — defect
observations and post-correction lessons (Behavior Rule #11) — and was
hand-appended without enforcement until this lock landed; the existing
record drift (date-only timestamps, nested-object evidence, plural
`adr_ids` keys, prose `resolution` field) is preserved via line-keyed
waivers in `gzkit.governance.trust_audits._INSIGHTS_SHAPE_WAIVERS`
rather than rewriting closed-evidence history (trust-doctrine T2).
"""
...
class InsightRecord(BaseModel):
    ...
    type: InsightType = Field(..., description="Record kind enum")
