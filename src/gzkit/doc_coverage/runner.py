...
def _print_human_report(gap_report: DocCoverageGapReport) -> None:
    ...
    print("Documentation Coverage Gap Report")
    print("=" * 40)
    print()
    ...
    if gap_report.gaps:
        ...
        for gap in gap_report.gaps:
            ...
            print(f"    MISSING: {gap.surface} -- {gap.detail}")
        print()

    if gap_report.undeclared_commands:
        print("  Undeclared commands (not in manifest):")
    ...
    if gap_report.orphaned_docs:
        ...
        for orphan in gap_report.orphaned_docs:
            print(f"    - [{orphan.surface}] {orphan.reference}: {orphan.detail}")
        print()
    ...
    if gap_report.passed:
        ...
        print(f"FAILED: {total} issues found across {gap_report.commands_with_gaps} commands.")
