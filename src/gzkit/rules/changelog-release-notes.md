...
> **Rule version:** `1.2.1` — distinguish operator release approval from OBPI/ADR Gate 5 completion (GHI #1041), preserving release review and provenance. Prior history: [Rule Version History](../../docs/governance/rule-version-history.md#changelog-release-notesmd).
...
- **Release-notes:** conform to the template and require operator approval through `gz-patch-release`; structure and tone are judgment-class (no mechanical release-notes validator — the curated narrative is not machine-checkable). This release approval is not Gate 5; that name is reserved for completed OBPI/ADR work. Retain references to existing gate evidence where applicable.
