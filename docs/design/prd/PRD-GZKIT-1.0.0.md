...
- **term:** `synthetic-memory` · **scope:** cross-cutting · **provenance:** ADR-0.33.0-airlock-membrane, ADR-0.0.65-handoff-system-consolidation
  **definition:** The externalized project model an agent is handed rather than reconstructing each sortie; it answers the operator-named deficit, verbatim, of "the model's inability to retain a model of the project without having to reconstruct it on each sortie." Provided COOPERATIVELY by two mechanisms with different subjects — the airlock supplies the project's current shape on entry (bounded seam-set), the handoff carries the prior session's decisions and next steps forward — and neither alone gives an agent a resident model. **`prosthetic memory` names the same faculty and is equally current, not deprecated:** the two adjectives are different angles on one thing, `prosthetic` naming whose deficit it answers and `synthetic` naming how the memory was made. Search all three spellings — `synthetic`, `prosthetic`, and `sythetic` (the operator's original, preserved in canon before its 2026-08-17 correction) — when tracing this concept; no single one reaches every occurrence. Do not confuse with "synthetic fixture" under `tests/complexity/**`, an unrelated sense.
...
```
docs/design/adr/{foundation|pre-release|<major>.0}/ADR-X.Y.Z-{slug}/
...
- Create ADR folder structure: `docs/design/adr/{foundation|pre-release|<major>.0}/ADR-X.Y.Z-{slug}/`
...
- Place OBPIs in `docs/design/adr/{foundation|pre-release|<major>.0}/ADR-X.Y.Z-{slug}/obpis/`
