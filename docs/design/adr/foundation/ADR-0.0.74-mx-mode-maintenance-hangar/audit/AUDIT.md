...
| m2 | `src/gzkit/req_kind.py` is 765 lines (> 600-line limit, `.claude/rules/pythonic.md`). Pre-existing, exacerbated by OBPI-18 + the closeout-proof fix. | **GHI #652** (tech-debt). |
...
- [ ] **No unresolved MAJOR shortfalls** — ❌ open, now tracked: F1→#650, F2→#651, F3→#648, m2→#652
...
**Disposition (operator-chosen, path A — 2026-06-27):** GHIs filed — F1→#650, F2→#651, m2→#652; F3 rides #648 (new evidence comment added cross-linking #651). All drain as campaign Movement II patches (0.29.1+) per the operator's verbatim canon (a GHI is the direct-fix work order and the receipt). After the drain lands, **re-run `/gz-adr-audit 0.0.74`** — a green re-audit (no unresolved MAJOR shortfalls) is the gate to the `validated` receipt. Until then the ADR stays COMPLETED.
