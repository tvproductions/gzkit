...
- [ ] REQ-0.0.29-02-06: Given no archetype rule matches a metric crossing, when the engine runs, then `archetype=long_parameter_list` (default) is bound and `recommended_move` is populated from the distilled-characteristics document, never from a fabricated string. Amended 2026-09-26 by operator ruling on GHI #1103 (verbatim: "fix 1103 with an explicit unclassified archetype"; record reconciliation, verbatim: "Amend in place (Recommended)"): an unmatched crossing binds `archetype=unclassified`, not `long_parameter_list`. The attested default named, as a confident match, an archetype the function did not have (1178 of 2193 diagnoses over src/gzkit, one-parameter functions included). The `recommended_move` clause is unchanged. The attested text above is kept as the record of what was decided on 2026-05-06; this OBPI is Completed, so this is the attested-REQ transition `docs/governance/attested-req-subject-retirement.md` § Worked example 3 records, not pre-attestation repair.
