...
9. REQUIREMENT: Concurrent gather's cost is bounded by its inputs, not the corpus size. The expensive boundary is the `run_exec` subprocess spawn, and gather invokes it a fixed number of times — one ledger fetch + one `git log` + one `gh` per `related` anchor — regardless of how many ADRs/OBPIs exist on disk (no per-artifact subprocess fan-out). Measured via a test that compares the subprocess-call count across a small and a large representative fixture and asserts they are equal, and equal to the input-derived bound. (Originally worded as a `<3.0s` wall-clock budget; replaced 2026-06-02 — a wall-clock ceiling on ~1ms of in-process work measured scheduler jitter, not throughput, was widened 3.0s→6.0s under GHI #443, and still flaked under suite load. The deterministic call-count invariant catches the real regression — per-artifact fan-out — without timing dependence.)
...
- [ ] REQ-0.0.19-01-10: Given temp repos of small (~2 ADRs) and large (~80 ADRs, ~240 OBPIs) representative size, when `gather_evidence` runs against each, then its `run_exec` subprocess-call count is identical across both (cost bounded by inputs, not corpus) and equals the input-derived bound — one ledger fetch + one `git log` + one `gh` per related anchor — i.e. no per-artifact subprocess fan-out. (Superseded the original `<3.0s` wall-clock wording 2026-06-02; see REQUIREMENT #9 rationale.)
