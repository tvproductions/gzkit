---
id: OBPI-0.0.46-06-pool-docs-runbook
...
- **Checklist Item:** #6 - "OBPI-0.0.46-06: **pool-docs-runbook** — Add manpages, runbook entries, command docs, and the retire-or-absorb decision for the interim `pool-triage` chore."

**Status:** Draft

## Objective

Document the managed pool workflow and record whether the interim `pool-triage` chore is retired or absorbed into the canonical implementation.
...
1. REQUIREMENT: Documentation MUST describe triage, ranking, override, and show workflows using observed command output after implementation.
2. REQUIREMENT: The runbook MUST explain when to use computed rank versus operator override.
3. REQUIREMENT: The interim chore disposition MUST be explicit: retired with successor reference or absorbed as a wrapper.
...
- [ ] REQ-0.0.46-06-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.0.46-06-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.0.46-06-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
