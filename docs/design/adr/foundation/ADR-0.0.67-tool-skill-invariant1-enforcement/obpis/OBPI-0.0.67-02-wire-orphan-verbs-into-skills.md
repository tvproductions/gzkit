...
> Generated mirrors (`.claude/skills/**`, `.agents/skills/**`, `.github/skills/**`, `src/gzkit/skills/**`) are regenerated by `gz agent sync control-surfaces` — they are NOT allowed edit paths (see Denied Paths).
...
1. REQUIREMENT: `gz-obpi-reconcile` Phase 1 MUST invoke `gz obpi audit` (and `gz obpi audit --adr`) as the deterministic evidence step, replacing the ad-hoc agent Read/Grep/Bash audit (`SKILL.md:120-153`). The verb's ledger schema already matches the skill's documented Ledger Schema v1 (`SKILL.md:375-399` ↔ `obpi_audit_cmd._build_entry`).
...
- [ ] `obpi audit` → `src/gzkit/commands/obpi_audit_cmd.py:13` (single + `--adr`); reconcile `SKILL.md:120-153,375-399`
- [ ] `obpi withdraw` → `src/gzkit/commands/obpi_cmd.py:59`; ledger consumers `ledger.py:670`, `state.py:86`
...
`uv run gz validate --skill-alignment` green with zero of the 10 verbs in
`_NO_SKILL_VERBS`; `gz obpi audit --adr ADR-0.0.67` runs as the reconcile Phase-1
engine.

### Implementation Summary

- Files created/modified: 7 canonical skills + `cli.py` (waiver removal) + new tests + regenerated mirrors
- Tests added: `obpi_audit_cmd`, `obpi_withdraw_cmd`, wielding/no-waiver assertions
- Date completed:
- Attestation status:
