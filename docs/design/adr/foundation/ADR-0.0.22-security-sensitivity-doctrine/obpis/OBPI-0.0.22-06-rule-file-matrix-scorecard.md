...
Rule file + AGENTS.md matrix + advisory scorecard — Author `.gzkit/rules/security-sensitivity.md`; AGENTS.md three-axis matrix; advisory-rules-audit.md scorecard entry (Mechanical); `gz agent sync control-surfaces` propagates to mirrors; foundation-kind closeout walkthrough per ADR-0.0.18.
...
- `.gzkit/rules/security-sensitivity.md` — explicitly referenced by the checklist item
...
- [ ] REQ-0.0.22-06-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.0.22-06-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.0.22-06-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
