...
Without this ADR, security work has no third-axis attestation requirement — a lite-lane feature ADR touching authentication or cryptography can ship without security review, because the kind/lane attestation matrix has only two axes.

This ADR authors a port: the `sensitivity: security` axis contract that every brief/ADR validator and attestation surface must honor independently of lane and kind.
