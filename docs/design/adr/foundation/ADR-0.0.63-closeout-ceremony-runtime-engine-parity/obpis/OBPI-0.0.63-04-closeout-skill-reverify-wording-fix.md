...
- `.gzkit/skills/gz-adr-closeout-ceremony/SKILL.md` — "Does NOT re-verify" / spec-reviewer persona-dispatch contradiction (lines 60-65)
...
1. REQUIREMENT: This OBPI MUST deliver: **closeout-skill-reverify-wording-fix** — `.gzkit/skills/gz-adr-closeout-ceremony/SKILL.md:60-65`. Wording fix resolving the "Does NOT re-verify" vs spec-reviewer "Independent re-verification" contradiction (CLI pipeline does not re-execute; spec-reviewer persona-dispatch retains independent re-verification).
...
- [ ] Required path exists or is intentionally created in this OBPI: `.gzkit/skills/gz-adr-closeout-ceremony/SKILL.md:60-65`
...
test -f .gzkit/skills/gz-adr-closeout-ceremony/SKILL.md:60-65
```
...
- [ ] REQ-0.0.63-04-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.0.63-04-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.0.63-04-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
