...
# OBPI-0.0.63-07-verify-stage-command-shape-gate: Verify Stage Command Shape Gate
...
- `src/gzkit/commands/obpi_stages.py:140-147` — explicitly referenced by the checklist item
- `src/gzkit/quality.py:41-94` — explicitly referenced by the checklist item
...
1. REQUIREMENT: This OBPI MUST deliver: **verify-stage-command-shape-gate** — `src/gzkit/commands/obpi_stages.py:140-147` + `src/gzkit/quality.py:41-94`. The OBPI-pipeline verify stage and a fail-closed `gz validate` scope reject brief `## Verification` commands that are not single-program shell-less invocations (`&&`, `||`, `|`, `;`, `$(...)`, redirects), so authoring-vs-runtime mismatch (GHI #550) fails closed at authoring time rather than erroring confusingly at the verify gate. Reuses the shell-less command classifier built by OBPI-0.0.63-02.
...
- [ ] Required path exists or is intentionally created in this OBPI: `src/gzkit/commands/obpi_stages.py:140-147`
...
test -f src/gzkit/commands/obpi_stages.py:140-147
test -f src/gzkit/quality.py:41-94
```
...
- [ ] REQ-0.0.63-07-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.0.63-07-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.0.63-07-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
