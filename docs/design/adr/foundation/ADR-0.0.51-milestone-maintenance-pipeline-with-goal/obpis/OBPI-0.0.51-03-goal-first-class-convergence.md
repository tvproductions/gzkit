---
id: OBPI-0.0.51-03-goal-first-class-convergence
...
- **Checklist Item:** #3 - "OBPI-0.0.51-03: `/goal`-first-class convergence on Claude Code (goal-condition templating, evaluator-readable surfacing of findings); bounded-iteration fallback (max 8) for Codex/Copilot harnesses; harness detection utility shared with ADR-0.0.50"
...
`/goal`-first-class convergence on Claude Code (goal-condition templating, evaluator-readable surfacing of findings); bounded-iteration fallback (max 8) for Codex/Copilot harnesses; harness detection utility shared with ADR-0.0.50.
...
- [ ] REQ-0.0.51-03-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.0.51-03-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.0.51-03-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
