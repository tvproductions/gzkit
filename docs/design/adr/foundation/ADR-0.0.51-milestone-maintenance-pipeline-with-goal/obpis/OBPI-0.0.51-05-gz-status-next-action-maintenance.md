---
id: OBPI-0.0.51-05-gz-status-next-action-maintenance
...
- **Checklist Item:** #5 - "OBPI-0.0.51-05: `gz status --next-action` extension naming `gz-adr-milestone-maintenance` when an ADR is `Validated` without a fresher `milestone_maintenance_completed` event; integration with ADR-0.0.50's `--next-action` patch (no duplicate code paths)"
...
`gz status --next-action` extension naming `gz-adr-milestone-maintenance` when an ADR is `Validated` without a fresher `milestone_maintenance_completed` event; integration with ADR-0.0.50's `--next-action` patch (no duplicate code paths).
...
- [ ] REQ-0.0.51-05-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.0.51-05-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.0.51-05-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
