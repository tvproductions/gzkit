...
ceremony.
EXPANDED 2026-05-13 (14 → 15) to absorb T0 maintenance surfaces per
GHI #461 — companion CLI verbs for the T0 fail-closed mechanism shipped
under OBPI-06 (baseline freeze) and OBPI-07 (`gz validate --distribution`).
The validator detects two drift classes (`ON_DISK_NOT_BASELINE` for
Layer-3 baseline-manifest staleness; `ON_DISK_NOT_INCLUDED` for non-md
files under canonical surface trees with no class-classifier coverage)
that have no canonical-CLI recovery path today, forcing hand-edits that
violate AGENTS.md § Architectural Boundaries #6 ("do not let derived
views silently become source-of-truth"). OBPI-15 authors the baseline
regenerator surface symmetric to `gz register-adrs` and extends the
chores class-classifier doctrine to rules/skills/personas/templates.
Baseline Selected 10 → 11 absorbs the maintenance-surfaces narrative as
distinct from the enforcement-surface narrative (OBPI-07). No dimension
bump: Logic/Engine and Interface are already at the cap (2/2) — the
maintenance-surfaces work is breadth, not new architectural depth.
Sibling-cut precedent: GHI #455 (security-registry staleness from the
same canonical-surface-migration class) closed superseded against
`ADR-pool.agentic-security-review` 2026-05-12; OBPI-15 is the
distribution-axis counterpart, bundled into ADR-0.0.32 rather than
spawned as a fresh pool ADR because the T0 mechanism whose drift it
remediates was authored here.

Lifecycle note: this 14 → 15 expansion re-opens implementation from the
`Completed → Draft` transition. The first 14 OBPIs remain `attested_completed`
in the ledger; OBPI-15 alone runs through the pipeline. `gz adr status`
already reports `lifecycle_status: Pending` (Gates 2–5 pending) — the
frontmatter flip from `Completed` to `Draft` aligns Layer-1 authorship with
Layer-2 runtime state per `docs/governance/state-doctrine.md`.
...
- [ ] CLI surface: `gz init --update` manpage at `docs/user/manpages/init.md`; behave coverage in `features/init.feature`
