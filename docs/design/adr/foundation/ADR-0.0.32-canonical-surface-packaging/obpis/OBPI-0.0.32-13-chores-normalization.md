...
- `.claude/rules/skill-surface-sync.md` — add § "Chores class-classifier" section with the three-class table + binding rules
...
- `docs/governance/canonical-routing-doctrine.md` (new, optional) — short doc cross-linking the three-class table from `.claude/rules/skill-surface-sync.md` for the docs site
...
1. The three-class chores classifier MUST be authored in `.claude/rules/skill-surface-sync.md` as a § "Chores class-classifier" section. The classifier defines, for every file under `.gzkit/chores/<slug>/` or `src/gzkit/chores/<slug>/`, exactly one classification: **canonical**, **package_only**, or **runtime_state**.
...
6. `.claude/rules/skill-surface-sync.md` MUST cite `ADR-pool.canonical-vs-runtime-separation` as the long-term home for the runtime-state relocation question, so future readers know the classifier is a temporary accommodation.
...
- [ ] `.claude/rules/skill-surface-sync.md` — the rule this OBPI extends with the chores class-classifier section
...
- [ ] `.claude/rules/skill-surface-sync.md` § "Chores class-classifier" section landed; cites `ADR-pool.canonical-vs-runtime-separation`
...
# Rule re-affirmation
grep -A 3 "Chores class-classifier" .claude/rules/skill-surface-sync.md | head -10
```

## Acceptance Criteria

- [ ] REQ-0.0.32-13-01: `.claude/rules/skill-surface-sync.md` § "Chores class-classifier" section landed with the three-class table (canonical / package_only / runtime_state) and binding rules for each class
...
- [ ] REQ-0.0.32-13-06: `.claude/rules/skill-surface-sync.md` cites `ADR-pool.canonical-vs-runtime-separation` as the long-term home for the relocation question
...
```bash
diff -qr .gzkit/chores src/gzkit/chores | grep -v "Only in src/gzkit/chores: __" | grep -v "proofs/CHORE-LOG.md" | grep -v "proofs: .gitkeep"
# Expected: no diff (after normalization, only package_only + runtime_state classes remain divergent)
```
...
- Tests added:
- Classifier implementation shape (Python helper vs. JSON data file):
- Canonical-class normalizations landed:
- Date completed:
- Attestation status:
- Defects noted:
