...
15. REQUIREMENT: Attestor identity uses operator name only (e.g., `g0`); never the personal email per `.gzkit/rules/agent-contract.md` / AGENTS.md operator PII discipline. If a CLI requires email-shaped value, use the operator's GitHub noreply address.
...
uv run gz adr emit-receipt ADR-0.0.20 --event validated --attestor "g0"
```
