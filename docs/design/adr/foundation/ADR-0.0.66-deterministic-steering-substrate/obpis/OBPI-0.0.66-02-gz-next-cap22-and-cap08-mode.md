...
- [ ] REQ-0.0.66-02-05 [SUPPORT]: gz next exposes `--dry-run`, `--explain`, and `--json` and is covered in its manpage; `uv run gz cli audit` exits 0 with the verb covered across manpage, command doc, and index, and an `artifact_edited` event records the CLI addition.
