---
id: OBPI-0.0.44-02-predicate-parser
...
# OBPI-0.0.44-02-predicate-parser: **predicate-parser** — Add runtime-predicate body parser matching `src/gzkit/` path refs, `uv run gz <verb>` symptom lines, and `fix(...)` remedy shape.
...
- **Checklist Item:** #2 - "OBPI-0.0.44-02: **predicate-parser** — Add runtime-predicate body parser matching `src/gzkit/` path refs, `uv run gz <verb>` symptom lines, and `fix(...)` remedy shape."
...
- `src/gzkit/` — explicitly referenced by the checklist item
...
- [ ] Required path exists or is intentionally created in this OBPI: `src/gzkit/`
...
- [ ] REQ-0.0.44-02-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.0.44-02-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.0.44-02-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
