---
id: OBPI-0.0.44-01-local-repo-routing
...
- **Checklist Item:** #1 - "OBPI-0.0.44-01: **local-repo-routing** — Extend `gz issue file` to accept local-repo (`tvproductions/gzkit`) invocations alongside cross-repo filing."
...
- `src/gzkit/commands/issue_cmd.py` — `gz issue file` command surface; extend to accept local-repo invocations
- `src/gzkit/cli/parser_governance.py` — parser registration for the issue-file subcommand
- `tests/commands/test_issue_cmd.py` — unit tests for the local-repo routing path
...
- [ ] Required path exists or is intentionally created in this OBPI: `src/gzkit/commands/issue_cmd.py`
...
- [ ] REQ-0.0.44-01-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.0.44-01-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.0.44-01-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
