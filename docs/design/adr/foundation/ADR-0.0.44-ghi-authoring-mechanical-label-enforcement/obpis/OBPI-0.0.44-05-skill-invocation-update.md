---
id: OBPI-0.0.44-05-skill-invocation-update
...
- **Checklist Item:** #5 - "OBPI-0.0.44-05: **skill-invocation-update** — Update `ghi-author` skill Step 5 invocation from raw `gh issue create` to `gz issue file`; bump skill-version; sync mirrors via `gz agent sync control-surfaces`."
...
- `docs/design/adr/foundation/ADR-0.0.44-ghi-authoring-mechanical-label-enforcement/**` — parent ADR package scope
...
- [ ] Required path exists or is intentionally created in this OBPI: `docs/design/adr/foundation/ADR-0.0.44-ghi-authoring-mechanical-label-enforcement/**`
...
- [ ] REQ-0.0.44-05-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.0.44-05-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.0.44-05-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
