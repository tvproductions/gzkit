...
- [ ] REQ-0.0.59-03-03 [BEHAVIOR]: Given `ReqCoverageRecord` and `ReqCoverageSummary` Pydantic models, when constructed with valid inputs, then they satisfy `frozen=True`, `extra="forbid"`, and all required fields are present and correctly typed; invalid construction raises `ValidationError`. **SUPERSEDED (GHI #545, 2026-07-23):** the `ReqCoverageRecord`/`ReqCoverageSummary` models never reached the production enrichment path — `compute_three_channel_coverage` enriches `CoverageEntry` instead, and `CoverageEntry`/`CoverageReport` (`src/gzkit/traceability.py`) are a complete superset already wired into `gz covers --json`. The parallel models and their isolation tests were retired (hexagonal § Prefer subsumption to a parallel model). The surviving three-channel coverage record is `CoverageEntry`/`CoverageReport`, whose contract is proven by this OBPI's 03-01 and 03-05 acceptance criteria (the enrichment-output and rollup behaviors). This discharges the ADR-0.0.59 audit finding (`audit/AUDIT.md` #4: "populate them or remove from the model — lying schema is worse than missing schema"), which itself sanctioned "remove from the model" as one of two dispositions. Retirement authorized by the operator's design-dialogue ruling (2026-07-23): given CoverageEntry already fulfills the three-channel intent, retire the parallel duplicate.
