...
2. **Add `req_evidence` field to `obpi_brief_structure.json`.** Schema-level field with shape `{req_id: str, receipt_ids: list[str], file_lines: list[str]}`. Optional at authoring time, required by `gz validate --closeout-proof-binding` at closeout time (per closeout-runtime-parity pool ADR Decision item 5).
