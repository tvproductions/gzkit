---
id: ADR-pool.artifact-staleness-propagation
status: Pool
lane: heavy
parent:
---
...
Without this ADR, cross-artifact design-assumption coupling at the ADR/OBPI graph tier has no mechanical witness. Invariant 1a (coupled-surface coherence) holds at the file level; the `last_reviewed` ↔ skill-version coupling holds at the rule level; the artifact-graph tier — the upstream-canon → downstream-canon coherence boundary — is the missing third tier.

> **Correction (2026-08-04, GHI #691) — the middle tier does not hold, and did not when this was written.** The original text is preserved above; do not read it as current. The `last_reviewed` ↔ skill-version coupling is **false twice over** as a rule-level witness. (1) It is doctrine-only — no mechanical backstop — which `ADR-pool.skill-version-review-coupling` exists to fix. (2) More fundamentally, **`last_reviewed` does not exist on rules at all.** It governs *skills*: `RuleFrontmatter` (`src/gzkit/rules/__init__.py:501-513`) is exactly `id`, `paths`, `description` under `extra="forbid"`, and `skill-surface-sync.md` § #6 ends *"this clause applies to skills only."* "Rule level" here means "the level governed by rule #6", not the `.gzkit/rules/**` surface. The genuinely-missing rule-surface tier is homed at **`ADR-pool.rule-surface-aging-clock`**; the three-tier framing becomes true only when that ADR and `ADR-pool.skill-version-review-coupling` both promote. Re-verify this note before promoting 0.0.52. Without it, downstream artifacts carry silently-invalidated design assumptions through the lifecycle window with no surface to detect or attest. This is identity-shaping: gzkit's commitment to T1/T2/T3 trust invariants requires that every cross-artifact coupling surface have a mechanical witness, not just the file and rule tiers.
