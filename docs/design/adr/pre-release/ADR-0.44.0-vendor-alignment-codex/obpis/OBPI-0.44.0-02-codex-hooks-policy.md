...
# OBPI-0.44.0-02-codex-hooks-policy: **codex-hooks-policy** — Generate `.codex/hooks.json` only for gzkit behaviors whose semantics are safe under Codex hook execution, and document non-portable Claude hook behavior explicitly

## ADR Item

- **Source ADR:** `docs/design/adr/pre-release/ADR-0.44.0-vendor-alignment-codex/ADR-0.44.0-vendor-alignment-codex.md`
- **Checklist Item:** #2 - "OBPI-0.44.0-02: **codex-hooks-policy** — Generate `.codex/hooks.json` only for gzkit behaviors whose semantics are safe under Codex hook execution, and document non-portable Claude hook behavior explicitly"
...
**codex-hooks-policy** — Generate `.codex/hooks.json` only for gzkit behaviors whose semantics are safe under Codex hook execution, and document non-portable Claude hook behavior explicitly.
...
- `.codex/hooks.json` — explicitly referenced by the checklist item
...
- [ ] REQ-0.44.0-02-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.44.0-02-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.44.0-02-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
