...
3. The airlock (HATCH) gains a real seam-enumerator: re-sense can be certified against a graph, not asserted by judgment (wired through OBPI-03's `resense` diff surface, consumed by the HATCH re-sense gate).
4. Three fragmented graph representations (get_artifact_graph, triangle, dormant ontology) unify into one typed substrate; drift detection and artifact-graph become views over one replay path -- one queryable source of shape. (Honest accounting: the absorptions ship as compat views layered OVER the originals, not deletions, so this is net MORE code in the near term -- the win is a single typed replay path and one type system that resolves differing-semantics-under-a-shared-name, never fewer lines; deletion of the superseded surfaces is a deferred two-way-door move once the views are proven at parity.)
...
1. **Rebuild fidelity — the graph never lies about the shape.** The Tier-B projection
   ...
   `sense` self-reports its replay completeness and freshness. **Completeness is computed
   by diffing replayed event types against the live `TypedLedgerEvent` discriminator
   registry — never a hardcoded handled-type set** — so an event type added by a later ADR
   surfaces as `complete=False` (unaccounted) instead of being silently dropped while the
   report still reads `complete=True`. A graph that cannot confess an incomplete or stale
   replay is a defect. This is the load-bearing fence — the entire trust premise (the
   airlock certifying "every seam seen") rests here — so the deferred-breadth domains
...
<!-- KEEL/ADR-0.31.0 precedent: a heavy multi-OBPI ADR lands cleanly by shipping a
     tracer first and gating breadth behind a live landing falsifier — not by pulling
     all breadth in at once. This ADR adopts BOTH halves of that discipline. -->

**In this cut (the corpus-first MVP tracer — OBPI-01–04):** the typed object/link model
+ Harness-Purity fence (01), the networkx substrate + corpus projection + the
registry-coupled rebuild-fidelity fence (02), the `gz ontology` read-only sonar (03), and
the ownership/plane doctrine + Boundary-Invariants (04). This spine is a two-way door: rip
the graph out and L1/L2 are untouched.

**Deferred-breadth, gated behind the fidelity fence proven live (Boundary Invariant #1):**
the OKF Doc-absorption (05), the work-domain L2 edge schema + queue (06 — the one
irreversible surface, additionally guarded by its own fail-closed emission gate), and the
source-domain tree-sitter anchors (07). None of these begins until OBPI-02's fidelity
fence refuses a live registry-derived drift — imaging breadth onto a graph not yet proven
honest is the precise "wrong graph is more dangerous than none" failure this ADR names.

**Out of scope (named non-goals):** semantic-seam recall (RECALL / Phase-4, L3-advisory);
MCP-served ontology (`gz mcp serve`, a conscious later increment); hard-refusal on work
blocks (declared as OBPI-06's promotable torque-up milestone, not shipped here); any path
by which the graph gates a `gz validate` scope, gate, or closeout (Boundary Invariant #2).
