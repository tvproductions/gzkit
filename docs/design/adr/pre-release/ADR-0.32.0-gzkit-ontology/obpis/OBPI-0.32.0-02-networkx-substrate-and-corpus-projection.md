...
3. ALWAYS: The graph self-reports rebuild fidelity (replay completeness + freshness) and confesses an incomplete or stale replay — the runtime realization of parent ADR Boundary Invariant #1 (rebuild fidelity).
...
- [ ] REQ-0.32.0-02-05 [BEHAVIOR]: the projection emits a rebuild-fidelity self-report carrying replay completeness (every encountered ledger event type is accounted for; any unaccounted/dropped type is named) and freshness (latest ledger event timestamp vs projection build time), reporting `complete=False` / `fresh=False` so it confesses an incomplete or stale replay — the runtime realization of parent ADR Boundary Invariant #1; a `@covers(REQ-0.32.0-02-05)` test injects an unaccounted event type and a stale build and asserts the report confesses both, in `tests/test_ontology_corpus.py`.
