...
- `src/gzkit/ontology/model.py` — **CREATE**: `OntologyNode`/`OntologyEdge` frozen `extra="forbid"` Pydantic models, `LinkType`/`Ownership`/`Plane` closed `StrEnum`s, and `ontology_node_json_schema()` projector
...
- [ ] `.gzkit/governance/ontology.json` present — the object corpus the `plane` partition test (REQ-04) is validated against
...
- [ ] REQ-0.32.0-01-04 [BEHAVIOR]: the `plane` axis genuinely partitions the catalog — a `@covers(REQ-0.32.0-01-04)` test enumerates the object types this OBPI seats and asserts each maps to exactly one plane (`product` XOR `process`), with both planes non-empty, so the partition is proven by assignment and would fail if any type were assigned to neither plane or to both.
