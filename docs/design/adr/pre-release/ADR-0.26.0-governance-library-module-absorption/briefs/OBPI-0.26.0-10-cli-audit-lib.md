...
Evaluate `../airlineops/src/opsdev/lib/cli_audit.py` (238 lines) against
gzkit's partial CLI audit in `src/gzkit/cli.py` and determine: Absorb (opsdev
is better), Confirm (gzkit is sufficient), or Exclude (domain-specific). The
opsdev module provides dedicated CLI audit infrastructure for verifying command
contracts, checking help text compliance, validating exit codes, and auditing
flag conventions. gzkit has partial coverage in `src/gzkit/cli.py` via
`gz cli audit`, but the comparison must determine whether gzkit's inline
approach matches the depth of opsdev's dedicated audit library.
...
- [ ] REQ-0.26.0-10-01: Given the completed comparison, then the brief records
  one final decision: `Absorb`, `Confirm`, or `Exclude`.
- [ ] REQ-0.26.0-10-02: Given the decision rationale, then it cites concrete
  capability, robustness, or ergonomics differences between opsdev and gzkit.
- [ ] REQ-0.26.0-10-03: Given an `Absorb` outcome, then gzkit contains the
  adapted module/tests needed to carry the pattern safely.
- [ ] REQ-0.26.0-10-04: Given a `Confirm` or `Exclude` outcome, then the brief
  explains why no upstream absorption is warranted.
- [ ] REQ-0.26.0-10-05: Given any operator-visible behavior change, then Gate 4
...
rg -n 'Absorb|Confirm|Exclude' docs/design/adr/pre-release/ADR-0.26.0-governance-library-module-absorption/briefs/OBPI-0.26.0-10-cli-audit-lib.md
...
rg -n 'Gate 4|N/A|behavioral proof' docs/design/adr/pre-release/ADR-0.26.0-governance-library-module-absorption/briefs/OBPI-0.26.0-10-cli-audit-lib.md
