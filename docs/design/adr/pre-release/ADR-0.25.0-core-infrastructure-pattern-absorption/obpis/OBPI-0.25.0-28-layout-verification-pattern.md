...
- [x] REQ-0.25.0-28-01: Given the completed comparison, then the brief records
  one final decision: `Absorb`, `Confirm`, or `Exclude`. — **Exclude** recorded in Decision section.
- [x] REQ-0.25.0-28-02: Given the decision rationale, then it cites concrete
...
- [x] REQ-0.25.0-28-03: Given an `Absorb` outcome, then gzkit contains the
  adapted module/tests needed to carry the pattern safely. — N/A (Exclude decision).
- [x] REQ-0.25.0-28-04: Given a `Confirm` or `Exclude` outcome, then the brief
  explains why no upstream absorption is warranted. — Subtraction test and five-point rationale in Decision section.
- [x] REQ-0.25.0-28-05: Given any operator-visible behavior change, then Gate 4
