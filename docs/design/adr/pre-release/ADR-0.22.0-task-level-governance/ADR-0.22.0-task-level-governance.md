...
- Scope and surface
  - External contract changed (Heavy lane): new `gz task` CLI command, new ledger event types, changes to `gz status` output
- Tests
  - stdlib unittest guards TASK model, ledger events, and lifecycle transitions
  - BDD scenarios for `gz task` CLI
- Docs
  - `gz task` command documentation
  - Four-tier hierarchy concept documentation
- OBPI mapping
...
- Ledger events for each state transition
...
- Define TASK ledger events: `task_started`, `task_completed`, `task_blocked`, `task_escalated` — following the existing event model patterns in `events.py`.
...
- Build `gz task` CLI for lifecycle management: `gz task list`, `gz task start`, `gz task complete`, `gz task block`.
- Integrate TASK status into `gz status` and `gz state` reporting.
- TASK-level tracing is advisory for Lite lane, required for Heavy lane.
...
| **Complex TASK workflow engine** | gzkit is a governance CMS, not a project management tool. TASK lifecycle is simple (5 states, 4 transitions). Over-engineering the state machine adds complexity without value. |
...
| 2 | TASK ledger events | TASKs exist but state changes aren't recorded. No audit trail for execution. |
...
| 4 | gz task CLI | TASK entity exists but operators can't manage lifecycle. No external surface. |
| 5 | Status and state integration | TASKs are managed but invisible in standard reporting. Operators can't see task-level progress. |

## Interfaces

- **CLI (external contract):** `uv run gz task list|start|complete|block` — new commands
...
| 2 | OBPI-0.22.0-02 | TASK ledger events: started, completed, blocked, escalated | Lite | Pending |
| 3 | OBPI-0.22.0-03 | Git commit linkage: TASK ID in commit trailers, parsing for traceability | Lite | Pending |
| 4 | OBPI-0.22.0-04 | `gz task` CLI: list, start, complete, block lifecycle commands | Heavy | Pending |
| 5 | OBPI-0.22.0-05 | Status and state integration: TASK data in `gz status` and `gz state` | Heavy | Pending |
...
**Critical path:** OBPI-01 → OBPI-04 → OBPI-05

**Parallelization:** After OBPI-01, OBPIs 02, 03, and 04 can run concurrently. OBPI-05 needs OBPI-04.
...
- No complex workflow engine or state machine beyond 5 states and 4 transitions.
...
The design is deliberately simple: 5 lifecycle states, 4 ledger events, identifier-based linkage. gzkit is a governance CMS, not a project management tool. TASK captures what happened during execution with enough structure for traceability, not enough to become a workflow engine.
