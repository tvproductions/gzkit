...
**AMENDED 2026-09-01 (operator-ruled, GHI #873) — clauses 8 and 9 below.** Un-retirement now reads PURE `retires` tombstones only. As originally pinned here, Algebra 5 and 6 composed to republish superseded canon: because a `supersedes` row was a tombstone for liveness, superseding one un-retired whatever it had replaced, so `[X, S1(supersedes=X), S2(supersedes=S1)]` folded to `[X, S2]` — two competing statements of one piece of canon. The operator ruled the two roles SPLIT: a `supersedes` row retires its target PERMANENTLY, and neither superseding nor retiring that replacement revives what it replaced. The chain now folds to `[S2]`.

**THE IMPLEMENTATION WAS NEVER WRONG, AND THIS IS NOT A DEFECT REPORT.** `corpus.py` was faithful to the nine clauses as pinned, and clause 3's refusal to choose the algebra at implementation time is what kept it faithful. The ALGEBRA was wrong. This annotation exists because the attested REQs below still bind and must not be left asserting retired doctrine (`.claude/rules/governance-core.md` § an attested REQ whose subject a later ruling retired is repaired at the surface). The parent ADR carries the same amendment in § Tombstone fold algebra (binding); `6072792b` and `d4caa874` carry the implementation and its three-link witness.

**No REQ below is invalidated.** REQ-0.35.0-01-04's fixture `[X, T1(retires=X), T2(retires=T1)]` is a PURE-tombstone chain, so its assertion remains true verbatim under the amended algebra — un-retirement was always intended for exactly that shape. REQ-05's `supersedes` assertion is likewise unchanged. Rejected alternative, recorded because it is the question that returns: forbid a `supersedes` chain at load time, which would make the natural one-row amendment an error and force a two-row retire-then-supersede dance.
...
8. ALGEBRA 5 — LIVENESS BY REVERSE PASS. **(AMENDED 2026-09-01, GHI #873.)** `live(e) = not any(is_replacement(t) or live(t) for t in tombstones targeting e)`. A replacement (`supersedes`) kills its target unconditionally; a pure tombstone (`retires`) kills its target only while the tombstone itself is live. As originally pinned this clause read `live(e) = not any(live(t) for t in tombstones targeting e)`, which made a `supersedes` row's retire role revocable. Because every tombstone strictly follows its target (Algebra 2), evaluating entries from LAST to FIRST resolves every dependency before it is read. This is a single reverse pass — NEVER a fixpoint iteration and NEVER unbounded recursion.
9. ALGEBRA 6 — UN-RETIREMENT IS RETIRING THE TOMBSTONE — **PURE TOMBSTONES ONLY (AMENDED 2026-09-01, GHI #873).** Appending `T2` with `retires = <id of T1>`, where `T1` is a pure `retires` row, makes `live(T1)` false, which by Algebra 5 makes `live(X)` true again. A `supersedes` row is NOT un-retirable this way: its retire role is permanent. As originally pinned this clause was unscoped, which is the composition the amendment closes. All three rows remain in the raw log; retirement history is preserved, not overwritten.
