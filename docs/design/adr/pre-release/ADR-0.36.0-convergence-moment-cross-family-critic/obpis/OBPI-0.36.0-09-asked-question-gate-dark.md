...
**asked-question-gate-dark** — The `PreToolUse` adapter on `AskUserQuestion` — wired, tested, and off by default, lit only by OBPI-08's measured result.
...
1. REQUIREMENT: This OBPI MUST deliver: **asked-question-gate-dark** — The `PreToolUse` adapter on `AskUserQuestion` — wired, tested, and off by default, lit only by OBPI-08's measured result.
...
- [ ] REQ-0.36.0-09-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.36.0-09-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.36.0-09-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
