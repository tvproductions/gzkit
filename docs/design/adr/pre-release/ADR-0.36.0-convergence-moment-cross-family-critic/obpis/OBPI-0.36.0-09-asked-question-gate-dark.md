...
- [ ] REQ-0.36.0-09-06 [SUPPORT]: `.claude/settings.json` carries the adapter as a `PreToolUse` matcher on `AskUserQuestion`, regenerated from `src/gzkit/hooks/claude.py` rather than hand-edited, so the door is genuinely wired while dark — `gz validate --surfaces` + `artifact_edited` event citing `src/gzkit/hooks/claude.py`.
