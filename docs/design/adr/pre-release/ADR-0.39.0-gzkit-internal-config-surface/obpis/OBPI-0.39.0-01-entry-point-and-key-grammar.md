---
id: OBPI-0.39.0-01-entry-point-and-key-grammar
...
verification:
  ...
  - test -f .gzkit/config/settings.json
---

# OBPI-0.39.0-01-entry-point-and-key-grammar: Entry Point And Key Grammar
...
- **Checklist Item:** #1 - "1. Entry point and key grammar: `.gzkit/config/settings.json` as a SINGLE FLAT FILE, `load_registry` addressing logical keys rather than filenames, and the key-naming discipline fixed before any caller adopts it."

**Status:** Draft

## Objective

A single flat `.gzkit/config/settings.json` is the one place gzkit reads its own settings from, `gzkit.registries.load_registry` resolves a logical key such as `thresholds.audit` through it without any caller naming a filename, and the key grammar is written down and enforced before the first caller adopts it. Done means a key resolves, a filename does not, and the grammar rejects a malformed key.
...
- `src/gzkit/registries.py` — the loader that gains logical-key addressing
- `src/gzkit/config.py` — entry-point resolution and merge order
- `.gzkit/` — the surface's home; `.gzkit/config/settings.json` is created here
- `tests/` — coverage for key resolution and grammar rejection
- `docs/governance/` — the key grammar as a written rule
...
1. REQUIREMENT: This OBPI MUST deliver: 1. Entry point and key grammar: `.gzkit/config/settings.json` as a SINGLE FLAT FILE, `load_registry` addressing logical keys rather than filenames, and the key-naming discipline fixed before any caller adopts it.
...
- [ ] Required path exists or is intentionally created in this OBPI: `.gzkit/config/settings.json`
...
- [ ] REQ-0.39.0-01-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.39.0-01-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.39.0-01-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
