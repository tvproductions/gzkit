---
id: OBPI-0.39.0-06-reader-migration
...
# OBPI-0.39.0-06-reader-migration: Reader Migration
...
- **Checklist Item:** #6 - "6. Reader migration: the 49 rostered direct-reach modules move to the entry point, once, after the surface exists."

**Status:** Draft

## Objective

The 49 rostered modules that reach directly into `data/` read through the entry point instead, migrated once, after the surface exists, per the operator's migrate-once ruling. The shrink-only ratchet refuses any new bypass while the migration runs. Done means the roster is empty and the ratchet has nothing left to grandfather.
...
- `src/gzkit/` — the 49 rostered modules named in the direct-reach roster
- `data/direct_data_reach_grandfather.json` — the shrink-only roster being emptied
- `tests/` — the ratchet still refuses a newly introduced bypass
...
1. REQUIREMENT: This OBPI MUST deliver: 6. Reader migration: the 49 rostered direct-reach modules move to the entry point, once, after the surface exists.
...
- [ ] REQ-0.39.0-06-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.39.0-06-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.39.0-06-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
