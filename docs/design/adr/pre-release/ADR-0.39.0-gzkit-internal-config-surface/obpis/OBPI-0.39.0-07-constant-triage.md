---
id: OBPI-0.39.0-07-constant-triage
...
# OBPI-0.39.0-07-constant-triage: Constant Triage
...
- **Checklist Item:** #7 - "7. Constant triage: the 56 rostered module constants are triaged against the shrink-only roster; every timeout becomes a setting."

**Status:** Draft

## Objective

Each of the 56 rostered module constants is triaged against the shrink-only roster and given a disposition: an operator-tunable value becomes a setting, a correctness-critical value stays in code with a comment recording why it is not tunable, and every timeout becomes a setting. Done means each constant carries a recorded disposition rather than sitting undecided, which is the state the roster currently permits indefinitely.
...
- `src/gzkit/` — the modules holding the rostered constants
- `data/module_constant_grandfather.json` — the shrink-only roster being worked down
- `.gzkit/` — settings gaining the values that become tunable
- `tests/` — coverage for constants promoted to settings
...
1. REQUIREMENT: This OBPI MUST deliver: 7. Constant triage: the 56 rostered module constants are triaged against the shrink-only roster; every timeout becomes a setting.
...
- [ ] REQ-0.39.0-07-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.39.0-07-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.39.0-07-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
