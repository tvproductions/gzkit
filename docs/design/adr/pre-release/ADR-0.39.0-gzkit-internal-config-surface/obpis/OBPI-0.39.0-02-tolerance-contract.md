---
id: OBPI-0.39.0-02-tolerance-contract
...
# OBPI-0.39.0-02-tolerance-contract: Tolerance Contract
...
- **Checklist Item:** #2 - "2. Tolerance contract: the loader's behaviour for an ABSENT and for a MALFORMED surface is specified and tested, so a single entry point is not a regression against the 49-reader status quo."

**Status:** Draft

## Objective

The loader has a named, tested contract for two distinct failure states: a settings surface that is ABSENT, and one that is MALFORMED. Absent yields documented defaults, preserving the invariant that 49 readers currently hold only by convention. Malformed fails by the contract rather than by whatever `json.loads` happens to raise. Done means both states have a test, and neither can brick an unrelated command.
...
- `src/gzkit/registries.py` — absent and malformed handling in the loader
- `src/gzkit/config.py` — merge-order behaviour when a layer is missing or invalid
- `tests/` — one test per failure state, plus the adopter-context control
...
1. REQUIREMENT: This OBPI MUST deliver: 2. Tolerance contract: the loader's behaviour for an ABSENT and for a MALFORMED surface is specified and tested, so a single entry point is not a regression against the 49-reader status quo.
...
- [ ] REQ-0.39.0-02-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.39.0-02-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.39.0-02-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
