---
id: OBPI-0.39.0-05-roster-relocation-to-debt
...
# OBPI-0.39.0-05-roster-relocation-to-debt: Roster Relocation To Debt
...
- **Checklist Item:** #5 - "5. Roster relocation: the 29 waiver/grandfather/baseline rosters leave the settings surface for `debt/`, with the exhaustiveness scope moving in the same change."

**Status:** Draft

## Objective

The 29 waiver, grandfather and baseline rosters leave the settings surface for their own home, indexed by the waiver ratchet, so nobody tunes measured debt by editing it as if it were a setting. The exhaustiveness scope moves in the SAME change as the files: today it is a filename glob over top-level `data/`, and files moving without it silently un-fences the boundary this ADR rests on. Done means the rosters have moved and the ownership gate still fails closed on an undeclared roster.
...
- `data/` — the rosters being relocated and the registries indexing them
- `src/gzkit/governance/trust_audits/` — the exhaustiveness scope that moves with them
- `.gzkit/` — the relocation target
- `tests/` — the ownership gate still fails closed after the move
...
1. REQUIREMENT: This OBPI MUST deliver: 5. Roster relocation: the 29 waiver/grandfather/baseline rosters leave the settings surface for `debt/`, with the exhaustiveness scope moving in the same change.
...
- [ ] Required path exists or is intentionally created in this OBPI: `debt/`
...
- [ ] REQ-0.39.0-05-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.39.0-05-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.39.0-05-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
