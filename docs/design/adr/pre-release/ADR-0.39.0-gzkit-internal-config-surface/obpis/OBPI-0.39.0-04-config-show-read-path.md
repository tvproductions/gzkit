---
id: OBPI-0.39.0-04-config-show-read-path
...
# OBPI-0.39.0-04-config-show-read-path: Config Show Read Path
...
- **Checklist Item:** #4 - "4. `gz config show <key>` reports the effective value and the merge layer that supplied it."

**Status:** Draft

## Objective

An operator can ask what a key resolves to and where the value came from, in one command, without reading three files and simulating the merge. The read path reports the effective value and the merge layer that supplied it, and answers the second 2am question by saying plainly when a key is not a knob at all. Done means the layer is named in the output, not inferred by the reader.
...
- `src/gzkit/commands/` — the new read-path subcommand
- `src/gzkit/registries.py` — provenance returned alongside the resolved value
- `tests/` — coverage for each merge layer and for an unknown key
- `docs/user/` — the operator-facing manpage
...
<!-- gz-validate-skip: command-shape -->
1. REQUIREMENT: This OBPI MUST deliver: 4. `gz config show <key>` reports the effective value and the merge layer that supplied it.
...
- [ ] REQ-0.39.0-04-01: Given the parent ADR intent, when the OBPI implementation is complete, then the primary scoped artifacts exist and match the documented contract
- [ ] REQ-0.39.0-04-02: Given the Allowed Paths in this brief, when the OBPI is executed, then changes remain inside scope and denied paths remain untouched
- [ ] REQ-0.39.0-04-03: Given the Verification commands in this brief, when they run, then evidence is recorded before the OBPI is accepted
