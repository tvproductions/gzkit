...
**Movement A — Close the Foundation Sunset** *(forward engine; closes a kind — reductive)* — **HELD as of 2026-08-14 (operator-ratified; § Amendments)**; Movement A is HELD *as a Movement*, while its `ADR-0.35.0` box is **TOPMOST in its own right as of 2026-09-01 (operator-ratified; § Amendments), REAFFIRMED 2026-09-02** — no longer merely *the next work* under ADR order (§ Amendments 2026-08-16 — *ADR ORDER IS ABSOLUTE*). Ascending semver and the TOPMOST label agree rather than pulling apart. The 2026-09-02 amendment placed Movement C's family-closure box **directly beneath** this one; it did not displace it.
...
> **The first pass over-read the ruling and the operator corrected it. The error
> is recorded here rather than rewritten away.** As first written this block moved
> the box to **TOPMOST** and stepped `ADR-0.35.0` down, displacing a position the
> operator had ratified the previous day (§ Amendments 2026-09-01) on the strength
> of a single direction word. *Forward* is a relative move: it does not name the
> top slot, and this plan's TOPMOST is one machine-read marker that another ruling
> already occupied. Landed wrong at `dd2a8185` and corrected in place, because the
> ruling is one ruling — a second amendment block for a clarification of the same
> instruction would be the re-adjudication this plan names as a disease (§
> Amendments 2026-07-18). Recorded as an `improvement` in
> `.gzkit/insights/agent-insights.jsonl` per Behavior Rules — Always #11.

**What changed.** Movement C's **Close the doctrine-declared-without-mechanism
family** box is pulled forward to **NEXT-IN-PRIORITY** — directly beneath
TOPMOST, ahead of Movement B and ahead of every other box in its own Movement.
`ADR-0.35.0-canon-entry-corpus-landing` **remains TOPMOST**, unchanged from
2026-09-01. Movement B steps down one place. Movement A remains HELD *as a
Movement*.

**A box outranking its own Movement is precedented, not novel.** The 2026-09-01
amendment made Movement A's `ADR-0.35.0` box TOPMOST *in its own right* while
Movement A itself stayed HELD. This is the same shape one position down: the box
moves, Movement C does not.
...
order and the receipt"*). Ascending semver is untouched: `ADR-0.35.0` is still
the ADR the order selects and no higher-semver feature ADR may be worked ahead
of it (§ Amendments 2026-08-16 — *ADR ORDER IS ABSOLUTE*).

**Why the box earns a position this high.** It is drawable *without the
operator*. Under the IRON LAW only the operator initiates OBPI work, so TOPMOST
has never been able to cause a brief to be drawn; this box can be worked the
moment a session opens. Placing it directly beneath TOPMOST is therefore the
highest position that changes anything without touching what TOPMOST means.

**The evidence, measured 2026-09-02 rather than asserted.**
