...
> **Topmost (sequenced):** **AMENDED 2026-09-02 (operator-ratified) — Movement C's `Close the doctrine-declared-without-mechanism family` box is TOPMOST. `ADR-0.35.0-canon-entry-corpus-landing` steps down from TOPMOST to next-in-priority and REMAINS the in-flight feature ADR under absolute ADR order; Movement B holds the position the 2026-09-01 amendment gave it; Movement A remains HELD as a Movement.** TOPMOST now names work an agent is authorized to draw: under the IRON LAW only the operator initiates OBPI work, so a TOPMOST label on a feature ADR never drew a brief, while this box discharges through rule-text amendment, scorecard scoring and GHI-shaped direct repair. Measured 2026-09-02: 19 of 32 open GHIs are members of this family, and the open queue climbed 9 → 32 in four weeks while 179 issues closed at a median age of 0.15 days — instances close same-session, the class keeps producing. See § Amendments 2026-09-02. From 2026-08-14 to this amendment the plan declared TOPMOST a Movement that ADR order forbade working, while the work drawn every session sat as a box inside a HELD Movement — two amendments (2026-08-16, 2026-08-23) were spent explaining that split rather than removing it. This removes it. Run `uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing` for its lifecycle and landed count rather than trusting a figure transcribed here. See § Amendments 2026-09-01.
>
...
**Movement A — Close the Foundation Sunset** *(forward engine; closes a kind — reductive)* — **HELD as of 2026-08-14 (operator-ratified; § Amendments)**; Movement A is HELD *as a Movement*, while its `ADR-0.35.0` box was **TOPMOST in its own right from 2026-09-01 and STEPPED DOWN to next-in-priority 2026-09-02 (both operator-ratified; § Amendments)**. It **remains the in-flight feature ADR**: ascending semver still selects it and no higher-semver feature ADR may be worked ahead of it (§ Amendments 2026-08-16 — *ADR ORDER IS ABSOLUTE*). The step-down moves a campaign label, never the ADR order — the box now above it is not ADR work.
...
**Movement B — Put the membrane on the real doors** *(carried by `ADR-0.37.0-airlock-calibration-and-compulsion` — authored 2026-08-14, `kind: feature`, `lane: heavy`, re-homing `ADR-0.33.0`'s disclosed residuals; run `uv run gz adr status ADR-0.37.0-airlock-calibration-and-compulsion` for its lifecycle and landed count rather than transcribing one here)* — **NEXT-IN-PRIORITY as of 2026-09-01 (operator-ratified; § Amendments); was TOPMOST 2026-08-14 → 2026-09-01, throughout which ADR order forbade drawing it.** The step-down is not a demotion of the airlock's importance — the inertness measured at 2026-08-14 is unchanged and unaddressed. It records that TOPMOST should name drawable work, which this Movement was not.
...
**Movement C — Reduce the accretion** *(deferral LIFTED 2026-07-18 — this is pre-1.0; the `doctrine-declared-without-mechanism` box below is **TOPMOST as of 2026-09-02**, operator-ratified — § Amendments)*
...
- [ ] **Close the doctrine-declared-without-mechanism family** *(added 2026-08-07)* — the family's own name, from GHI #537: *"Layer X declares a discipline that Layer X does not mechanically enforce."* Measured by the `failure-class-index` chore over the 333 GHIs closed since 2026-05-09: the **two deepest recurrence chains in the corpus** (depth 12 and depth 7) are both this family, ~19 members, and it holds the two most-cited ancestors on record (#537 cited 3×, #538 cited 4×). Both arms are in scope — **validator-side** (a check whose subject is narrower than its name: #692 *checks section presence, not population*; #693 *verifies a flag is mentioned, never that its description is true*; #770 *an audit named for dispatch attestation whose entire subject is a frontmatter string*) and **agent-side** (a skill mandate with no receipt: #459, #574, #620). **Completion criterion:** a declared discipline either carries a mechanical witness or is demoted to advisory in its own text — no third state. This is the reductive move that stops the `validate()` surface producing scope #95: it closes the family rather than the instance. **Re-scoped 2026-08-08 (operator-ratified) — the six named issues all closed and the box did NOT discharge; see the amendment for why, and the measurable criterion below.** **PULLED TO TOPMOST 2026-09-02 (operator-ratified) — 19 of 32 open GHIs are members of this family and the open queue climbed 9 → 32 in four weeks while instances closed at a median age of 0.15 days; the criterion, the three arms and the doctrine-side residual are UNCHANGED. See § Amendments 2026-09-02.**
  ...
  **THIRD ARM — DOCTRINE-SIDE (added 2026-08-17, operator-directed).** Both arms named above sit *inside the machine*: validator-side is a check whose subject is narrower than its name; agent-side is a skill mandate with no receipt. Neither covers the case where **a foundation-tier doctrine document declares a binding claim and no witness of any kind exists** — not a narrow one, not an absent one. The measured exemplar: `docs/governance/agent-control-surface-rendering-substrate.md:211` has specified `gz content render agent_contract --vendor=root` since authoring — one root `AGENTS.md` contract serving every harness. The string `"root"` appears **zero times** in `src/gzkit/content/` or `compose.py`. The doctrine's own vendor was never built, and a per-consumer shape took its place in three copies: the same file's § Agent Orientation Index (a Layer-3 row 65 lines below the Layer-1 worked example it contradicts), `data/vendor-manifest.json`, and `src/gzkit/content/vendors.py:21` `_FALLBACK_ROUTES`. It then propagated into `REQ-0.35.0-09-02` of a Heavy-lane brief that passed `--brief-reconcile`, `--req-kind-discipline` and its own Discovery Checklist. **This arm is why the criterion cannot be met by counting closed issues.** gzkit's witnesses compare artifact↔artifact and claim↔receipt exhaustively and doctrine↔code not at all, so the drift was unobservable by construction until a human read line 211 aloud. Two sibling boxes in this same Movement are the same wound: *"Render the stability-gradient spine"* (a 06-14 ruling that the `PRD → Constitution` order is backwards, still shipping the old order across ~12 surfaces) and *"Surface mirroring."* **Instance-level redress landed 2026-08-17** under `ADR-0.35.0` / `OBPI-0.35.0-09` (re-scoped: root-only routing, a manifest fence refusing multi-vendor `AgentContract`, and a fail-closed agreement check between `_FALLBACK_ROUTES` and the manifest) plus an `invariant`-tier corpus entry carrying the doctrine into the per-turn surface. **The class remains OPEN:** a general doctrine↔code witness is unbuilt, and the honest statement of the gap is that gzkit does not today know how to mechanize *"this prose binds that code."* Whoever works this box states which arm they are closing.
...
### 2026-09-02 (latest) — the doctrine-declared-without-mechanism box is pulled to TOPMOST; `ADR-0.35.0` steps down (operator-ratified)

**Operator ruling, verbatim:** *"pull the family-closure box forward, amend the campaign"*

**What changed.** Movement C's **Close the doctrine-declared-without-mechanism
family** box is TOPMOST. `ADR-0.35.0-canon-entry-corpus-landing` steps down from
TOPMOST to next-in-priority and **remains the in-flight feature ADR** — ascending
semver still selects it and no higher-semver feature ADR may be worked ahead of
it (§ Amendments 2026-08-16 — *ADR ORDER IS ABSOLUTE*). Movement B holds the
position the 2026-09-01 amendment gave it; Movement A remains HELD *as a
Movement*.

**Nothing is removed and no ADR is resequenced.** This box is not ADR work. Its
three arms discharge through rule-text amendment, scorecard scoring, and
GHI-shaped direct repair — and GHI repair is its own work order under operator
canon (*"GHIs are AUTHORIZED for direct repair, always … the GHI is the work
order and the receipt"*). TOPMOST moving here therefore takes nothing from the
ADR spine: the two labels sit on different axes, and § 8's *"ADR, OBPI, and GHI
repair remain the primary propellants"* covers both.

**Why TOPMOST here does something it could not do at `ADR-0.35.0`.** Under the
IRON LAW only the operator initiates OBPI work, so a TOPMOST label on a feature
ADR never drew a brief — the operator drawing it did. A TOPMOST label on a
GHI-shaped reductive box points at work an agent is authorized to take. This is
the 2026-09-01 defect one axis over: that amendment removed a TOPMOST that named
work ADR order **forbade**; this one removes a TOPMOST that named work the label
**could not cause**.

**Why the box earns the position — measured 2026-09-02 rather than asserted.**

- **Half the open GHI queue is this family.** Of **32** open issues, **16 carry
  the core shape** — a witness whose subject is narrower than the claim it
  carries — verified by reading each body rather than classifying off titles:
  #766, #799, #803, #807, #810, #813, #849, #851, #888, #889, #894, #907, #919,
  #922, #926, #933. **Three more are the same family in a different key** (a
  declared mechanism that never fires or never schedules): #804, #808, #870.
  **19 of 32.** #877 was examined and **excluded** — two witnesses disagreeing is
  a coherence defect, not an unwitnessed discipline. Re-derive with `gh issue
  list --state open`; the classification rule is the box's own criterion, so the
  next reader can check this rather than trust it.
- **The queue's growth is this family being enumerated, not a triage failure.**
  Open stock was **9 on 2026-08-10** and is **32 today** — a monotonic four-week
  climb of roughly 6/week net — while **179 issues closed in the trailing 30 days
  at a median age of 0.15 days**. Instances close same-session; the class keeps
  producing. That is the box's own 2026-08-08 re-scope finding (*"the six named
  issues all closed and the box did NOT discharge"*) reappearing one altitude up,
  and it is why filing discipline is not the remedy: filing is working exactly as
  Prime Directive 6 requires.
- **The third state is rising, and the rise is predicted behaviour rather than
  new drift.** The criterion was ratified 2026-08-08 against **9 Promotable** on a
  91-row scorecard; 2026-08-16 recorded the rules arm regressing **0 → 8** when
  `.gzkit/rules/cli.md` was scored for real. The live fenced Summary table now
  reads **30 Promotable, 0 Ambiguous** across 157 scored rows. Most of that rise
  is surfaces measured for the *first* time landing in the forbidden third state —
  exactly what the 2026-08-16 amendment warned: *"its completion criterion cannot
  be treated as converging while unscored surfaces remain."* Read the live figure
  from `uv run gz validate --advisory-scorecard`, never from this line.

**The criterion is unchanged and no arm is dropped.** All three arms still bind —
**rules** (every `Promotable`/`Ambiguous` row reaches zero by mechanization or by
amending the rule text; re-scoring alone is laundering), **skill** (still
**unmeasured**, which is not the same as passing), **debt** (mechanized, no new
work). The 2026-08-17 doctrine-side third arm stands with its residual stated as
it was: a general doctrine↔code witness is unbuilt, and gzkit does not today know
how to mechanize *"this prose binds that code."* Pulling the box forward
schedules the arms; it does not soften them.

**What this does NOT license.** No ADR is authored for this box — Architectural
Boundary 2 forbids adding pool ADRs to the runtime track, and the box is a
subtraction move, which is what Movement C is for. No OBPI is drawn, started, or
authored; the IRON LAW is untouched and `ADR-0.35.0`'s undrawn briefs stay
undrawn until the operator initiates them. Whoever works this box **states which
arm they are closing**, per the box's own text.

### 2026-09-01 — the CMS / corpora / rendering work is TOPMOST; Movement B steps down (operator-ratified)

**Operator ruling, verbatim:** *"i want this cms/corpora/rendering issue to move to the top of the campaign queue now."*

**What changed.** `ADR-0.35.0-canon-entry-corpus-landing` — the CMS / corpora /
rendering work — is TOPMOST. Movement B steps down from TOPMOST to
next-in-priority. Movement A remains HELD *as a Movement*; its `ADR-0.35.0` box
is now TOPMOST in its own right rather than *the next work under ADR order*.

**This removes a split the plan had been explaining rather than fixing.** From
the 2026-08-14 amendment onward, TOPMOST named a Movement that ADR order
forbade drawing (*"`ADR-0.37.0` is the THIRD of three open feature ADRs, so
Movement B is TOPMOST in priority and NOT pullable"*), while the work actually
drawn every session was a checkbox inside a **HELD** Movement. Two amendments
were spent on that shape rather than on removing it: 2026-08-16 argued a
Movement can be *"TOPMOST **and** gated"*, and 2026-08-23 corrected a § 6 header
that had drifted into calling the HELD Movement TOPMOST — drift that is the
predictable cost of a label pointing away from the work. TOPMOST now names
drawable work, and the label agrees with ascending semver instead of pulling
against it.

**Nothing is resequenced and no box is removed.** ADR order is untouched and
still absolute; `ADR-0.35.0` was already the ADR the order selected, so this
amendment changes what the plan SAYS is topmost, never which ADR is worked.
Movement B's step-down is not a judgement on the airlock: the inertness measured
2026-08-14 and re-measured 2026-08-16 is unchanged and unaddressed, and Movement
B is drawn the moment `ADR-0.35.0` and `ADR-0.36.0` land.

**Why the CMS work earns the position, measured 2026-09-01 rather than
asserted.** The corpus is the declared SOURCE that `AGENTS.md` is generated from
(`ADR-0.35.0` § Decision, SOURCE-OF-TRUTH DIRECTION, operator-ruled at
authoring), and the gap between that claim and the tree is where the open GHI
queue has been silting up:

- **Four open GHIs strand on this ADR and nothing else** — #921 (`.gzkit/rules/**`
  uncorpused, fanning out to every generated nested `AGENTS.md`), #922 (no trust
  audit shape-checks the Layer-1 corpus at all), #815 and #533 (the delivery-cap
  and budget pair).
- **Two of those four were queued behind a design that was CANCELLED.** #533
  states the 5k target unblocks when *"ADR-0.0.37 … is fully landed … remaining
  5 OBPIs deliver the registry-projection path"*, and #815's second remedy names
  the same registry-projection migration. Those briefs are precisely
  `ADR-0.0.37`'s **Abandoned** set — the density-dial / registry-projection
  design retired on 2026-06-03 and replaced by corpus→compress→rendition→playback
  (`OBPI-0.0.37-27` § Objective, Completed and attested). `.gzkit/invariants/`
  still holds four entries. Both issues have been waiting on work that was
  deliberately cancelled, and nothing on either issue said so.
- **The number moved the wrong way while they waited.** #533 measured the
  rendered root contract at 31,387 chars. Measured 2026-09-01: `wc -c AGENTS.md`
  → **46,876**, against a 15,000 target, a 5,000 aspiration, and a 32,768-byte
  Codex delivery cap that `gz validate --instructions-files-budget` reports as
  three advisory warnings. Two must-survive sections are affected.

**Landed into the ADR the same day, ahead of this amendment.** `ADR-0.35.0`
§ Decision's binding fold algebra was amended (operator ruling: split the
`retires` / `supersedes` roles) and implemented — `6072792b`, `d4caa874`,
closing GHI #873, which had been waiting on a ruling since 2026-08-24. The
checklist gained items 11 and 12 for #922 and #921 in `d3ff4ae9`, with the
Decomposition Scorecard moved on this ADR's own recorded rule that *added scope
moves the BASELINE, never a split adder*. Run
`uv run gz adr status ADR-0.35.0-canon-entry-corpus-landing` for the count; it is
deliberately not transcribed here.

**No OBPI work was started and none is authored.** Under the IRON LAW the
operator initiates OBPI work via the `gz-obpi-pipeline` skill. This amendment
and the checklist items give that work a home to be drawn from; they do not
draw it.

### 2026-08-23 — the § 6 Queue header labelled the HELD Movement TOPMOST; corrected
...
**No box removed, nothing resequenced.** Movement B remains TOPMOST, Movement A remains HELD, ADR order stays absolute, and `ADR-0.35.0` keeps the in-flight feature position. One box gains an arm.

**1. The family's two arms did not cover the case that fired.** Movement C's `Close the doctrine-declared-without-mechanism family` box names **validator-side** (a check whose subject is narrower than its name) and **agent-side** (a skill mandate with no receipt). Both live inside the machine. The 2026-08-17 instance is neither: a **foundation-tier doctrine document** declared a binding claim — `agent-control-surface-rendering-substrate.md:211`, `gz content render agent_contract --vendor=root`, one root `AGENTS.md` serving every harness — and **no witness of any kind existed**. Measured the same day: `"root"` appears **zero times** in `src/gzkit/content/` or `compose.py`.

**2. The drift reached executable surfaces and a Heavy-lane brief.** Three copies of the per-consumer shape displaced the doctrine — the same doctrine file's § Agent Orientation Index (Layer-3, 65 lines below the Layer-1 worked example it contradicts), `data/vendor-manifest.json`, and `vendors.py:21` `_FALLBACK_ROUTES` — and then `REQ-0.35.0-09-02` specified playback to a second AGENTS.md at a path no harness loads. That brief passed `--brief-reconcile`, `--req-kind-discipline`, `gz obpi validate --authored`, and its own Discovery Checklist. **Every gate was green and every gate was answering a different question**, because `ADR-0.35.0` § Decision item 8 says only *"wires to a real surface"* and no check asks whether a Decision item is specific enough to constrain its brief.

**3. What this costs the criterion.** The box's completion criterion — *"a declared discipline either carries a mechanical witness or is demoted to advisory in its own text — no third state"* — is now measurably harder than the closed-exemplar list suggested, because gzkit's witnesses compare **artifact↔artifact** and **claim↔receipt** exhaustively and **doctrine↔code** not at all. Two sibling boxes in this same Movement are the identical wound: *"Render the stability-gradient spine"* (the 06-14 ruling that `PRD → Constitution` is backwards, still shipping the superseded order across ~12 surfaces) and *"Surface mirroring."*

**4. Instance closed; class open, and stated as open.** Landed 2026-08-17 under `ADR-0.35.0` / `OBPI-0.35.0-09` (re-scoped in place: root-only routing, a manifest fence refusing a multi-vendor `AgentContract`, a fail-closed agreement check between `_FALLBACK_ROUTES` and the manifest, and min-cap measurement of the single delivered surface), plus an `invariant`-tier corpus entry carrying the root-contract doctrine into the per-turn surface, and a corpus entry replacing the *"90% convinced"* bullet (operator verbatim: **"forget 90%, you have zero basis for any certainty"**). **A general doctrine↔code witness is NOT built and is not scheduled here.** The honest statement of the residual: gzkit does not today know how to mechanize *"this prose binds that code."* Naming it is the amendment; building it is unclaimed.

### 2026-08-17 — the architecture review is dispositioned: §5 is re-cut against a declared date, and Movement D absorbs orientation + findings (operator-ratified)
