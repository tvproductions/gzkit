...
### Lifted 2026-09-13 at version `0.14.0` (rule now at `0.15.0`)
...
### Lifted 2026-09-13 at version `0.3.3` (rule now at `0.4.0`)
...
### Lifted 2026-09-07 at version `0.4.0` (rule now at `0.5.0`)
...
### Lifted 2026-09-23 at version `0.5.2` (rule now at `0.6.0`)
...
Diet pass under GHI #921 (operator: "A" on the full before/after, 11,982 B → 7,083 B). Binding rules unchanged. Lifted verbatim as dated records — the `0.12.0` note, § Five prohibited shapes (a restatement of § Invariant), the § Budget pointer history, the GHI #822 naming paragraph and the attestation-granularity measurement:
...
The transitional window is **open**. The delivered-surface witness (GHI #712, `src/gzkit/governance/trust_audits/surface_delivery_witness.py`) is wired into the default `gz check` scope and reports the byte distance to the Codex delivery cap; run `gz validate --instructions-files-budget` for the live figure. That witness is **advisory and never fail-closed** (operator ruling 2026-07-06, recorded in the budget file's `_doc`) — do not read it as a gate that will stop an overrun. The cap itself is `project_doc_max_bytes`, a Codex **setting gzkit writes** into the `.codex/config.toml` it generates — Codex loads that file in any directory the operator has trusted (its own trust prompt: *"Trusting the directory allows project-local config, hooks, and exec policies to load"*), and it wins over `$CODEX_HOME/config.toml`. So the cap recorded in `data/vendor-manifest.json` is a limit gzkit **established**, not a vendor fact — and it is checked by observation, not by two authored numbers agreeing: `gz validate --instructions-files-budget` carries a codex-delivery witness that reads what Codex actually assembled (GHI #962). Raising the cap is legitimate and is how the contract reaches a Codex session whole; raising it *without* re-observing delivery is what GHI #962 names. This paragraph names no byte figures, per the § Budget instruction four lines above that a duplicated number drifts from what is enforced — the earlier text asserted a `560 B` margin and a fail-close, and by 2026-09-02 both were false while AGENTS.md sat past the cap with Codex silently truncating from § Architectural Boundaries down (GHI #815). Shape enforcement (`gz validate --agents-md-map-conformance`) ships and binds now; the weight target lands with the deferred work. (ADR-0.0.54 closed Completed-Partial on exactly this split.)
...
Lifted 2026-09-14 at version `0.23.0` (rule now at `0.24.0`).

> **Rule version:** `0.23.0` — GHI #977 adds § Isolation *Git fixture isolation*: every `git` a test spawns passes `env=_isolated_git_env()`, git's own repo-local variable set (`local_repo_env`) scrubbed at the fixture boundary. Measured 2026-09-07: a push from a linked worktree ran the pre-push `gz check`, the fixtures inherited the hook's absolute `GIT_DIR`, and `git init` in a temp dir re-initialised the hosting repository as bare while `git config` wrote the fixture identity into its shared config. MECHANICAL from the first landing — `tests/commands/test_common_fixtures.py::TestEveryGitSpawnIsInsideTheBoundary` fences every literal `git` argv under `tests/**`. Prior `0.22.0` — GHI #970 extends § Verification exit-code integrity to the `||` BRANCH: the branch announces a failure in output and replaces it in status, and the gate had honoured the announcement as if it were the report. Names the branch-local remedy (`set -e` is suppressed left of `||`), and tracks the two limits it declares rather than closes — the aggregate status (GHI #969) and the caught `&&` chain (GHI #971). Prior `0.21.0` — GHI #940 extends § Verification exit-code integrity to the SEQUENCE form: the clause was scoped to the pipe character while the shell reports the last *statement* just as it reports the last *stage*, so `verifier > log; tail log` read as compliant. Names the two non-interchangeable remedies (`pipefail` for a pipe, `set -e` for a sequence), the immediacy requirement on a `$?` read, and — declared rather than hidden — that the aggregate status is a floor a shell-level rule cannot lift. Prior `0.20.0` — adds § Mutation-sweep integrity (GHI #963): a failing mutant run is not a kill, sweeps report four outcomes, and every mutant runs with its own bytecode cache. Prior `0.19.0` diet pass under GHI #921 (operator ruling 2026-08-29, *"we are compressing everything and anything that the agent can consume"*). Version history lifted to [Rule Version History](../../docs/governance/rule-version-history.md#testsmd). Binding rules unchanged.

Lifted 2026-09-15 at version `0.24.0` (rule now at `0.25.0`).

> **Rule version:** `0.24.0` — GHI #971 closes the caught `&&` chain in § Verification exit-code integrity: what reports the status is the separator ENDING the verifier's AND-OR list, never its own terminator, and the gate now reads it — and a `$?` read after `&` (the background launch's status) no longer counts as an escape. Prior versions lifted to [Rule Version History](../../docs/governance/rule-version-history.md#testsmd). Binding rules unchanged except as stated.

## `token-block-discipline.md`

### Lifted 2026-09-24 at version `0.8.0` (rule now at `0.9.0`)

The three-subjects ruling was carried into the rule (GHI #1091). Lifted verbatim as a dated record:
...
Diet pass under GHI #921 (operator: "A" on the full before/after, 14,118 B → 7,049 B). Binding rules unchanged. Lifted verbatim as dated records — the `0.7.0` note, the landed staging notes and the two histories:

> **Rule version:** `0.7.0` — diet pass under GHI #921 (operator ruling 2026-08-29, *"we are compressing everything and anything that the agent can consume"*). Version history lifted to [Rule Version History](../../docs/governance/rule-version-history.md#token-block-disciplinemd). Binding rules unchanged.
