...
- [x] `ADR-0.44.0-vendor-alignment` is `IN_PROGRESS` and partly landed (`uv run gz adr status ADR-0.44.0-vendor-alignment` for the count) and was tracked by no campaign edition. **PARKED 2026-08-08 by operator ruling (verbatim: "we DO NOT go out of sequence (0.44.0)").** The item offered "finish it or formally park it to pool"; the ruling forecloses the first arm — finishing a `0.44.0` while `ADR-0.35.0` is the in-flight feature is exactly the out-of-sequence pull the campaign governs against, and it compounds the settled ruling *"only one feature at a time, feature, finish, draw from pool"*. It stays where it is, worked at its sequence position, not now. **The demotion-to-pool arm is NOT executed and needs its own ruling:** moving a partly-landed feature ADR to pool is an artifact mutation with its own ceremony, and the ruling given rules out pulling it, which is not the same as ordering it demoted. The tracking defect the item names — carried by no campaign edition — is closed by this line; the disposition question is the residual.
