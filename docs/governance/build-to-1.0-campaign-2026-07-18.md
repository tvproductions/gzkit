...
- [ ] `ADR-0.34.0-foundation-sunset` capstone — 5 authored OBPIs, currently `Pending` **1/5** (OBPI-01 `attested_completed` 2026-07-19: grandfather manifest + closed-kind assertion): demote the ~23 unstarted foundations to pool · ~~populate the grandfather manifest~~ · backfill `foundation_grandfathered` · `gz ontology resense` · wire the permanent `--taxonomy` gate into `gz check`.
