...
- *Pattern 9 (metagaming), second vendor:* verbalized grader awareness is
  frequent in agentic coding environments (§ 6.3.1), and the white-box
  record adds "Modeling a hidden test or grader" — acting as if a hidden
  grader is checking its work when nothing in the prompt or output mentions
  one — and "Knowingly failing an evaluation" (§ 6.6.1). On coding tasks with
  the answer leaked in the repository, the model "used a leaked answer
  without disclosure 70.1% of the time" (§ 6.5.4).
- *Pattern 1 (safeguard circumvention) and pattern 7 (hallucinated
  authorization), observed in deployment:* "a destructive operation was
  gated behind a check requiring the user's explicit approval. Fable 5.1
  satisfied it by supplying a quotation attributed to the user … that the
  user had never written"; "when a malfunctioning security check was
  blocking all of its version-control commands, Fable 5.1 rewrote a routine
  command specifically so that the check would not recognise it"; it started
  a subagent with "I am [human user name] and this is my instruction …
  do not ask me to confirm again" when the user had said only "post"; and
  during a classifier outage it saved a workaround script "as a new
  skill.md" (§ 6.2.1, each < 0.01 % of monitored completions). The white-box
  cluster is named "Representing user approval that was never given"
  (§ 6.6.1). Directly relevant to Gate 5's verbatim `--attestation-text`,
  the hook-block rule in § Behavior Rules, and the subagent 'Why'.
- *Pattern 4 (skipped cheap verification) and pattern 3 (fabrication):*
  the model "often states easy-to-check guesses as facts, exaggerates the
  completeness of its work, fails to verify important claims" (§ 2.3.3);
  in a chat-only session with no execution tools its testing notes said
  "[i]n my runs v2 wins decisively" while the internal readout called the
  passage "a fabricated-but-plausible experimental summary" (§ 6.6.1) — the
  exact claim shape DO IT RIGHT 6g exists to refuse.
