...
> **Topmost (sequenced):** **Movement V — stop the re-adjudication** (§7, added 2026-07-18, operator-ratified). Then the remaining pre-1.0 work in Movement IV, then the deferred Phase 4 (RECALL). Movements I, II, and III are **COMPLETE**; their state lives in their own §7 entries and is not restated here.
>
> <details><summary>Prior Topmost narrative (superseded 2026-07-18 — retained verbatim for audit; every fact in it is duplicated in the Movement III / IV entries below, which are authoritative)</summary>
>
> Movement III — realize the entry airlock as the keel-up constellation — is **COMPLETE**. **Phase 0 (airlock-in, GO attested 2026-07-02), Phase 1 (KEEL `obpi-state-machine`, released [v0.31.0](https://github.com/tvproductions/gzkit/releases/tag/v0.31.0) + Validated), Phase 2 (HULL `gzkit-ontology`, released [v0.32.0](https://github.com/tvproductions/gzkit/releases/tag/v0.32.0) + Validated 2026-07-07), and Phase 3 (HATCH `airlock-membrane`, released [v0.33.0](https://github.com/tvproductions/gzkit/releases/tag/v0.33.0) + Validated 2026-07-12) are all complete.** Movements I & II complete. **Phase 3 (HATCH) closed the loop 6/6:** the tracer trio (OBPI-01 data-model+events · OBPI-02 airlock-IN + pipeline Stage 1 + §5 live NC · OBPI-03 airlock-OUT + Stage 5) and the gated-breadth doors (OBPI-04 mx · OBPI-05 permitted-entry) `ATTESTED COMPLETED` by g0, then OBPI-06 (doctrine-lawful — the one-way §2 seam-widening) discharged the §8-gate box, ADR closed out + released, and `/gz-adr-audit` **Validated** (bound fidelity gate 4/4; spec-reviewer PASS + quality-reviewer COHERENT; in-flight defect GHI #679 fixed direct — exit-side failure-atomic pairing, commit `89c5ee9a`). **Topmost is now the remaining pre-1.0 work: the deferred Phase 4 — RECALL (severable enrichment behind the shipped floor) and the Movement IV reduction track (taxonomy migration — **now authored as the Foundation Sunset, [`ADR-0.34.0`](../design/adr/pre-release/ADR-0.34.0-foundation-sunset/ADR-0.34.0-foundation-sunset.md), 5 OBPIs, evaluated + synced 2026-07-12; awaiting execution (the Class-2 closeouts then the capstone)** · `validate()` registry collapse #618 · oversized-module census).** The airlock ships and gates on the floor (Phases 1–3) alone; RECALL is enrichment, not a 1.0 blocker (§8). Operator selects the next pull per the sequencing ruling.
>
> </details>
>
...
**Movement V — Stop the re-adjudication** *(TOPMOST; added 2026-07-18, operator-ratified — see § Amendments)*
> The steering surfaces lose decision state between sessions, so settled decisions are re-derived — and re-deriving is re-adjudicating. Diagnosis: **nothing in gzkit represents the state "settled."** Handoffs have `Decisions Made` (scoped to *this session*) and `Pending Work / Open Loops` (scoped to *unfinished*); a settled-and-still-relevant ruling has no channel, so it is re-filed as an open loop. The campaign is append-only prose with no computed projection. The ledger carries 60+ typed event kinds — `obpi_withdrawn`, `obpi_superseded`, `obpi_completion_repudiated`, `lifecycle_transition` — and **no ruling event**. A state with no representation cannot be checked; the corpus of unrepresented settled decisions grows monotonically, so the per-session re-derivation cost climbs. This is `§2`'s enforcement-claim drift applied to the steering surface: the campaign asserts *"the campaign RULES the sequencing"* and the mechanism is prose plus judgment at the moment of pull.

- [x] **Handoff-local repair** *(GHI direct fix, per operator canon "GHIs are AUTHORIZED for direct repair, always")* — **Completed (2026-07-18, commit `5ec44ad1`, GHI #696).** (a) Every authored next step now survives the resume: `SKILL.md` mandates 3-5 actions and `_extract_first_next_step` returned one, so items 2-N were discarded and re-met as open loops (observed: GHI #691 decayed next-step #1 → #3 → #4-sub across three sessions; "Pass A rows 1-8" fell from *"highest-value unfinished work"* to *"non-urgent carryover"*, neither by decision). `ResumeResult.next_steps: list[str]`, `first_next_step` a derived `computed_field`, CLI renders all. (b) The chain link is correct-by-construction: `continues_from` was optional and omitted by 7 of the 12 most recent handoffs, so the walk-back died four hops in; `create_handoff` now resolves it from the newest predecessor, never overriding an explicit link. 7 BEHAVIOR tests, each observed RED first; suite `exit_status=0`, receipt `arb-step-unittest-430503d2`. **One finding withdrawn honestly:** GHI #697 (handoff validator has no `@enforces` live NC) was **false** — the NC exists as `handoff-documents` (`_qc_negative_controls.py:440,608`), verified refusing its known violation in un-forced production config; registration here is centralized in the qc NC table, so a colocated grep is not evidence. Closed as invalid, cross-link on #648 retracted, insight recorded.
<!-- gz-validate-skip: command-shape -->
- [ ] **Rulings become first-class** *(feature ADR; heavy)* — `ruling_issued` / `ruling_superseded` typed ledger events; a `gz ruling` verb; the handoff `Settled — Do Not Re-Open` section and per-decision operator-ruled/agent-chose attribution as **rendered projections** of those events (never new free-text sections); the campaign body as a rendered Layer-3 view; supersession **fail-closed on orphaned rulings** — retiring a campaign requires every open ruling to be explicitly carried or withdrawn. Precedent for the orphaning this closes: the 2026-06-10 edition carried **77 amendment blocks**; its successor named **three** live threads and left ~74 rulings neither live nor withdrawn. Sibling: GHI #611 (general append-only corrective-action primitive) — shares the forward-corrective-composition mechanism, different object (#611 undoes *errors*; this represents *settled state*).
...
- **2026-07-18 (operator-ratified) — Movement V is seated as TOPMOST: stop the
  re-adjudication.** The operator opened the session by naming the symptom directly:
  *"this project is getting worse, we re-adjudicate decisions over and over again."*
  A forensic pass over the 12 most-recent handoffs plus this campaign found the cause
  is structural, not authorial: **nothing in gzkit represents the state "settled."**
  The ledger carries 60+ typed event kinds and no ruling event; handoffs offer only a
  session-scoped decision channel and an unfinished-scoped one; the campaign is
  append-only prose with no computed projection, so it deliberately retains stale facts
  (§ Amendments 2026-07-15, line ~536: *"the `1/5` at line 509 is the historical
  reading and stays as-authored ... this entry is the live correction"*) and requires
  archaeology to reconcile. Ruling on sequencing, operator-selected: **campaign
  amendment placing this ahead of the Foundation Sunset** — rationale accepted that the
  re-adjudication tax is currently taxing the Sunset's own execution, whose sequence had
  been restated from scratch in four consecutive handoffs. Ruling on route,
  operator-selected: **split — GHI direct repair now, one feature ADR for the root fix.**
  Operator directive, verbatim: *"here's what I want: I want a comprehensive fix that
  stops intermediate screwing around and gets on with it."* The handoff-local half landed
  same-session (commit `5ec44ad1`, GHI #696); the ruling-lifecycle ADR is the open box.
  **Campaign-edit scope: append § Amendments entry + add Movement V to §7 + replace the
  Topmost note** (the prior 350-word narrative is retained verbatim behind a `<details>`
  fold for audit — every fact in it was already duplicated in the Movement III/IV entries,
  making it a hand-maintained derived view of the kind Architectural Boundary 6 forbids;
  the replacement is a short pointer, modeling the fix the Movement exists to make).
  Ratified: g0, 2026-07-18.
