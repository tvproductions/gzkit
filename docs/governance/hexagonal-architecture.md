...
|---|---|---|---|
| 1 | **App / System / Hexagon / Core** | All business logic, technology-agnostic — no reference to DB / network / UI | Domain layer: `src/gzkit/core/`, pure `governance/*` logic, and `src/gzkit/ontology/` pure domain (`model`/`purity`/`corpus`, stdlib + Pydantic) — with the one `networkx` dependency confined to the `graph.py` adapter, which returns domain types (rule 3), never `nx` views. |
