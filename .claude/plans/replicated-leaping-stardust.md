...
- Attestation Block: `| 0.0.20 | Validated | g0 | 2026-04-23 | <reason — concrete, grounded> |`
...
3. `uv run gz obpi complete OBPI-0.0.20-05-closeout-and-downstream --attestor "g0" --attestation-text "<operator phrase + enrichment>" --implementation-summary "..." --key-proof "..." --attestor-present`
...
11. `uv run gz adr emit-receipt ADR-0.0.20 --event validated --attestor "g0" --evidence-json '{"scope":"ADR-0.0.20","date":"2026-04-23"}'`
...
2. **Attestor name in receipt:** Brief specifies `g0` (operator name only). Confirmed.
